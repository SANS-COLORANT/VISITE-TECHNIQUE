...
export function MissionSignatureScreen({ navigation, route }) {
  ...
  const save = async () => {
    ...
    const normalized = [...strokes, ...(current.length ? [current] : [])].map((stroke) => stroke.map((p) => ({ x: p.x / width, y: p.y / height })));
    await db.runAsync(
      'INSERT INTO mission_signatures(id,mission_id,visit_id,document_id,role_label,signer_label,signed_at,metadata_json) VALUES(?,?,?,?,?,?,?,?)',
      [id, missionId, visitId, documentId, role.trim() || null, signer.trim(), new Date().toISOString(), JSON.stringify({ format: 'normalized_strokes_v1', strokes: normalized })]
    );
    Alert.alert('Signature enregistrée', 'La signature est rattachée à cette Mission et n’est utilisée que dans les livrables qui la nécessitent.', [
  ...
  return <View style={{ flex: 1, backgroundColor: 'transparent', padding: 16 }}>
    ...
    <TextInput style={[styles.input, missionStyles.input]} value={signer} onChangeText={setSigner} placeholder="Signataire / organisme" />
    <TextInput style={[styles.input, missionStyles.input, { marginTop: 8 }]} value={role} onChangeText={setRole} placeholder="Rôle : exploitant, entreprise, AMO…" />
    ...
    >
      <Svg width="100%" height="100%">
        ...
        {current.length ? <Path d={pathFromPoints(current)} stroke={MISSION_COLORS.accentStrong} strokeWidth={3} fill="none" strokeLinecap="round" strokeLinejoin="round" /> : null}
      </Svg>
      {!strokes.length && !current.length ? <View pointerEvents="none" style={{ position: 'absolute', inset: 0, alignItems: 'center', justifyContent: 'center' }}><Text style={{ color: COLORS.inkFaint, fontSize: 11 }}>Signer ici</Text></View> : null}
    </View>
