...
function AnnotationShape({ row, width, height, onPress }) {
    ...
    return (
      <React.Fragment>
        <Circle
          cx={p.x}
          cy={p.y}
          r={row.annotation_type === 'count' ? 10 : 7}
          fill={MISSION_COLORS.accentLight}
          stroke={color}
          strokeWidth={2.5}
          onPress={onPress}
        />
        {row.annotation_type === 'count' ? (
          ...
          <SvgText x={p.x + 10} y={p.y - 8} fontSize="11" fontWeight="700" fill={MISSION_COLORS.accentStrong}>
            {String(row.text).slice(0, 30)}
          </SvgText>
        ) : null}
      </React.Fragment>
    );
  ...
  if (row.annotation_type === 'text') {
    const p = pts[0];
    return (
      <SvgText x={p.x} y={p.y} fontSize="12" fontWeight="700" fill={MISSION_COLORS.accentStrong} onPress={onPress}>
        {String(row.text || 'Texte').slice(0, 45)}
      </SvgText>
    );
  }
  ...
  if (row.annotation_type === 'signature') {
    ...
    const h = height * Math.max(0.04, Math.min(0.3, Number(props.heightRatio || 0.09)));
    return (
      <React.Fragment>
        <Rect
          x={p.x}
          ...
          height={h}
          rx="5"
          fill="rgba(47,125,88,0.06)"
          stroke={color}
          strokeDasharray="5,4"
          strokeWidth={1.8}
          onPress={onPress}
        />
        <SvgText x={p.x + 6} y={p.y + 16} fontSize="9" fontWeight="700" fill={MISSION_COLORS.accentStrong}>
          {String(row.text || 'Signature').slice(0, 35)}
        </SvgText>
      </React.Fragment>
    );
  }
    ...
    <React.Fragment>
      {pts.slice(0, -1).map((p, i) => (
        <Line
          ...
          y2={pts[i + 1].y}
          stroke={color}
          strokeWidth={row.annotation_type === 'network' ? 4 : 2.5}
          onPress={onPress}
        />
      ))}
      {row.text && pts[0] ? (
        <SvgText x={pts[0].x + 6} y={pts[0].y - 6} fontSize="10" fill={MISSION_COLORS.accentStrong}>
          {String(row.text).slice(0, 35)}
        </SvgText>
      ) : null}
    </React.Fragment>
  );
...
export function MissionPlanScreen({ navigation, route }) {
  ...
  const loadPlanContext = useCallback(async () => {
    ...
    try {
      ...
      setLayers(layerRows || []);
      setLayerId((current) =>
        current && layerRows.some((l) => l.id === current) ? current : layerRows?.[0]?.id || null
      );
  ...
  const signatureTargets = useMemo(() => linkTargets.filter((target) => target.type === 'signature'), [linkTargets]);
  ...
  const filteredLinkTargets = useMemo(() => {
    ...
    return linkTargets
      ...
      .filter(
        (target) =>
          !q ||
          [target.label, target.subtitle].some((value) =>
            String(value || '')
              .toLowerCase()
              .includes(q)
          )
  ...
  const createReserve = async () => {
    ...
    try {
      ...
      await Promise.all([refreshLinkTargets(), listerAnnotationsPlan(selected.id, page).then(setAnnotations)]);
      Alert.alert(
        'Réserve créée',
        'Le point et son action sont localisés sur le plan. Responsable, échéance, coût et photos avant/après restent modifiables dans Actions.'
      );
    } catch (e) {
  ...
  const exportAnnotated = async () => {
    if (!selected) return;
    try {
      await exporterPlanPdfAnnote({ missionId, documentId: selected.id, share: true });
    } catch (e) {
  ...
  const rotate = async () => {
    ...
    try {
      ...
      Alert.alert('Rotation impossible', String(e?.message || e));
    }
  };
  ...
  const movePage = async (delta) => {
    ...
    try {
      ...
      Alert.alert('Déplacement impossible', String(e?.message || e));
    }
  };
    ...
    <View style={{ flex: 1, backgroundColor: MISSION_COLORS.bg }}>
      <ScrollView contentContainerStyle={{ padding: 16, paddingBottom: 120 }}>
        ...
        <Text style={{ color: COLORS.inkSoft, fontSize: 10.5, lineHeight: 15 }}>
          Le plan original reste intact. Les annotations, mesures et objets techniques sont enregistrés en couches
          structurées.
        </Text>

        <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 8, marginTop: 11 }}>
          <TouchableOpacity style={[styles.btnPrimary, missionStyles.primaryButton]} onPress={importPlan}>
            <Text style={[styles.btnPrimaryText, missionStyles.primaryButtonText]}>＋ Importer plan / PDF</Text>
          </TouchableOpacity>
          <TouchableOpacity
            style={[styles.btnSecondary, missionStyles.secondaryButton]}
            onPress={() => navigation.navigate('MissionMap', { missionId })}
          >
            <Text style={[styles.btnSecondaryText, missionStyles.secondaryButtonText]}>Carte SIG</Text>
          </TouchableOpacity>
          <TouchableOpacity
            style={[styles.btnSecondary, missionStyles.secondaryButton]}
            onPress={async () => {
              try {
                await importerGeoJsonMission({ missionId });
                Alert.alert('SIG', 'Couche GeoJSON importée hors ligne.');
              } catch (e) {
                Alert.alert('Import SIG', String(e?.message || e));
              }
            }}
          >
            <Text style={[styles.btnSecondaryText, missionStyles.secondaryButtonText]}>Importer GeoJSON</Text>
          </TouchableOpacity>
          <TouchableOpacity
            style={[styles.btnSecondary, missionStyles.secondaryButton]}
            onPress={async () => {
              try {
                await exporterGeoJsonMission(missionId);
              } catch (e) {
                Alert.alert('Export SIG', String(e?.message || e));
              }
            }}
          >
            <Text style={[styles.btnSecondaryText, missionStyles.secondaryButtonText]}>Exporter GeoJSON</Text>
          </TouchableOpacity>
          <TouchableOpacity
            style={[styles.btnSecondary, missionStyles.secondaryButton]}
            onPress={async () => {
              try {
                await exporterGeoPackageMission(missionId);
              } catch (e) {
                Alert.alert('GeoPackage', String(e?.message || e));
              }
            }}
          >
            <Text style={[styles.btnSecondaryText, missionStyles.secondaryButtonText]}>GeoPackage QGIS</Text>
          </TouchableOpacity>
        </View>
        ...
        {plans.length ? (
          ...
          <View style={[missionStyles.card, { padding: 14, marginTop: 16 }]}>
            <Text style={{ color: COLORS.inkSoft, fontSize: 10.5 }}>Aucun plan importé.</Text>
          </View>
        )}
        ...
        {selected ? (
          <>
            <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 8, marginTop: 14 }}>
              ...
              >
                <Text style={[styles.btnSecondaryText, missionStyles.secondaryButtonText]}>＋ Calque</Text>
              </TouchableOpacity>
              <TouchableOpacity style={[styles.btnSecondary, missionStyles.secondaryButton]} onPress={deleteLast}>
                <Text style={[styles.btnSecondaryText, missionStyles.secondaryButtonText]}>↶ Dernière annotation</Text>
              </TouchableOpacity>
              {String(selected.type).includes('pdf') ? (
                <TouchableOpacity style={[styles.btnSecondary, missionStyles.secondaryButton]} onPress={rotate}>
                  <Text style={[styles.btnSecondaryText, missionStyles.secondaryButtonText]}>↻ Rotation page</Text>
                </TouchableOpacity>
              ) : null}
              {String(selected.type).includes('pdf') ? (
                <TouchableOpacity style={[styles.btnSecondary, missionStyles.secondaryButton]} onPress={duplicatePage}>
                  <Text style={[styles.btnSecondaryText, missionStyles.secondaryButtonText]}>⧉ Dupliquer page</Text>
                </TouchableOpacity>
              ) : null}
              <TouchableOpacity style={[styles.btnPrimary, missionStyles.primaryButton]} onPress={exportAnnotated}>
                <Text style={[styles.btnPrimaryText, missionStyles.primaryButtonText]}>Exporter PDF annoté</Text>
              </TouchableOpacity>
            </View>
            ...
            <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 8 }}>
              <Text style={{ flex: 1, color: COLORS.inkFaint, fontSize: 9.5 }}>
                {calibration
                  ? 'Échelle calibrée : ' +
                    Number(calibration.real_distance || calibration.realDistance).toFixed(2) +
                    ' ' +
                    (calibration.unit || 'm')
                  : 'Échelle non calibrée · utilisez « Calibrer » pour les distances/surfaces réelles.'}
              </Text>
              {tool === 'polygon' && draftPoints.length >= 3 ? (
                <TouchableOpacity style={[styles.btnPrimary, missionStyles.primaryButton]} onPress={finishPolygon}>
                  <Text style={[styles.btnPrimaryText, missionStyles.primaryButtonText]}>Fermer polygone</Text>
                </TouchableOpacity>
              ) : null}
            </View>
              ...
              <>
                ...
                >
                  <TouchableOpacity
                    ...
                    style={{ padding: 8 }}
                  >
                    <Text style={{ color: page <= 1 ? COLORS.inkFaint : MISSION_COLORS.accentDark }}>← Page</Text>
                  </TouchableOpacity>
                  ...
                  <TouchableOpacity
                    ...
                    style={{ padding: 8 }}
                  >
                    <Text style={{ color: page >= pageCount ? COLORS.inkFaint : MISSION_COLORS.accentDark }}>
                      Page →
                    </Text>
                  </TouchableOpacity>
                </View>
                {String(selected.type).includes('pdf') ? (
                  ...
                  >
                    <TouchableOpacity
                      ...
                      onPress={() => movePage(-1)}
                    >
                      <Text style={[styles.btnSecondaryText, missionStyles.secondaryButtonText]}>Déplacer avant</Text>
                    </TouchableOpacity>
                    <TouchableOpacity
                      disabled={page >= pageCount}
                      style={[styles.btnSecondary, missionStyles.secondaryButton]}
                      onPress={() => movePage(1)}
                    >
                      <Text style={[styles.btnSecondaryText, missionStyles.secondaryButtonText]}>Déplacer après</Text>
                    </TouchableOpacity>
                    <TouchableOpacity style={[styles.btnSecondary, missionStyles.secondaryButton]} onPress={deletePage}>
                      <Text style={[styles.btnSecondaryText, missionStyles.secondaryButtonText, { color: '#8B3A3A' }]}>
                        Supprimer page
                      </Text>
                    </TouchableOpacity>
                  </View>
            ...
            <View style={{ alignItems: 'center' }}>
              ...
              >
                ...
                <Svg width={canvas.width} height={canvas.height} style={{ position: 'absolute', left: 0, top: 0 }}>
                  {annotations
                    .filter((a) => Number(a.layer_visible ?? 1) === 1)
                    .map((row) => (
                      <AnnotationShape
                        key={row.id}
                        row={row}
                  ...
                  {draftPx.map((p, i) => (
                    <Circle
                      ...
                      r={5}
                      fill="#FFFFFF"
                      stroke={MISSION_COLORS.accent}
                      strokeWidth={2}
                    />
                  ))}
                  {draftPx.slice(0, -1).map((p, i) => (
                    <Line
                      key={'draft-line-' + i}
                      x1={p.x}
                      y1={p.y}
            ...
            {annotations.length ? (
              <View style={{ marginTop: 12 }}>
                ...
                {annotations.map((a) => {
                  const linked =
                    a.linked_entity_type && a.linked_entity_id
                      ? linkedTargetByKey.get(a.linked_entity_type + ':' + a.linked_entity_id)
                      : null;
                  return (
                    <View key={a.id} style={[missionStyles.card, { padding: 10, marginBottom: 6 }]}>
                      <View style={{ flexDirection: 'row', alignItems: 'flex-start' }}>
                        <View style={{ flex: 1 }}>
                          <Text style={{ color: MISSION_COLORS.accentStrong, fontWeight: '800', fontSize: 10.5 }}>
                            {a.annotation_type}
                            {a.text ? ' · ' + a.text : ''}
                          </Text>
                          <Text style={{ color: COLORS.inkFaint, fontSize: 8.8, marginTop: 2 }}>
                            {a.layer_label || 'Sans calque'}
                          </Text>
                          {a.linked_entity_type ? (
                            <Text style={{ color: MISSION_COLORS.accentDark, fontSize: 8.8, marginTop: 4 }}>
                              Lié : {LINK_TYPE_LABELS[a.linked_entity_type] || a.linked_entity_type} ·{' '}
                              {linked?.label || a.linked_entity_id}
                            </Text>
                          ) : (
                            <Text style={{ color: COLORS.inkFaint, fontSize: 8.5, marginTop: 4 }}>
                              Non lié à un objet METRA
                            </Text>
                          )}
                        </View>
                        <View style={{ alignItems: 'flex-end', gap: 5 }}>
                          ...
                          <TouchableOpacity
                            ...
                            onPress={() => takePhotoAtAnnotation(a)}
                          >
                            <Text style={[styles.btnSecondaryText, missionStyles.secondaryButtonText]}>
                              {mediaBusyId === a.id ? '…' : '📷 Photo'}
                            </Text>
                          </TouchableOpacity>
                          <TouchableOpacity
                            style={[styles.btnSecondary, missionStyles.secondaryButton]}
                            onPress={() => openReserve(a)}
                          >
        ...
        <View style={styles.modalOverlay}>
          ...
          >
            ...
            <TextInput
              ...
              onChangeText={(v) => setNetworkDraft((p) => ({ ...p, label: v }))}
              placeholder="Nom : Départ radiateurs Nord…"
              autoFocus
            />
            <TextInput
              style={[styles.input, missionStyles.input, { marginTop: 8 }]}
              value={networkDraft.type}
              onChangeText={(v) => setNetworkDraft((p) => ({ ...p, type: v }))}
              placeholder="Type : chauffage, ECS, air neuf, fluide…"
            />
            <TextInput
              style={[styles.input, missionStyles.input, { marginTop: 8 }]}
              value={networkDraft.dimension}
              onChangeText={(v) => setNetworkDraft((p) => ({ ...p, dimension: v }))}
              placeholder="Dimension / diamètre / section"
            />
            <TextInput
              style={[styles.input, missionStyles.input, { marginTop: 8 }]}
              value={networkDraft.material}
              onChangeText={(v) => setNetworkDraft((p) => ({ ...p, material: v }))}
              placeholder="Matériau"
            />
            <TextInput
              style={[styles.input, missionStyles.input, { marginTop: 8 }]}
              value={networkDraft.insulation}
              onChangeText={(v) => setNetworkDraft((p) => ({ ...p, insulation: v }))}
              placeholder="Calorifuge / isolation"
            />
            <TextInput
              style={[styles.input, missionStyles.input, { marginTop: 8 }]}
              value={networkDraft.direction}
              onChangeText={(v) => setNetworkDraft((p) => ({ ...p, direction: v }))}
              placeholder="Sens / fonction"
            />
            <View style={styles.modalActions}>
              ...
              <TouchableOpacity style={[styles.btnPrimary, missionStyles.primaryButton]} onPress={confirmNetwork}>
                <Text style={[styles.btnPrimaryText, missionStyles.primaryButtonText]}>Créer le réseau</Text>
              </TouchableOpacity>
            </View>
          </ScrollView>
        </View>
        ...
        <View style={styles.modalOverlay}>
          <View style={[styles.modalSheet, missionStyles.modalSheet]}>
            ...
            <Text style={{ color: COLORS.inkSoft, fontSize: 9.5, lineHeight: 14, marginBottom: 9 }}>
              Le point, la ligne ou le polygone devient la géométrie de l’objet choisi. Une seule saisie pourra ensuite
              alimenter photos, mesures, constats et exports.
            </Text>
            <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{ maxHeight: 44, marginBottom: 7 }}>
              {visibleLinkTypes.map(([key, label]) => (
                <ToolChip
                  key={key}
                  label={label}
            ...
            <ScrollView style={{ maxHeight: 330, marginTop: 7 }}>
              {filteredLinkTargets.map((target) => (
                <TouchableOpacity
                  key={target.type + ':' + target.id}
                  onPress={() => applyLink(target)}
                ...
                >
                  ...
                  {target.subtitle ? (
                    <Text style={{ color: COLORS.inkFaint, fontSize: 8.5, marginTop: 2 }}>{target.subtitle}</Text>
                  ) : null}
                </TouchableOpacity>
              ))}
              {!filteredLinkTargets.length ? (
                <Text style={{ color: COLORS.inkFaint, fontSize: 9.5, paddingVertical: 12 }}>
                  Aucun objet dans cette catégorie.
                </Text>
              ) : null}
            </ScrollView>
            <View style={styles.modalActions}>
              {linkAnnotation?.linked_entity_id ? (
                <TouchableOpacity style={[styles.btnSecondary, missionStyles.secondaryButton]} onPress={clearLink}>
                  <Text style={[styles.btnSecondaryText, missionStyles.secondaryButtonText]}>Délier</Text>
                </TouchableOpacity>
              ) : null}
              <TouchableOpacity
                style={[styles.btnPrimary, missionStyles.primaryButton]}
                onPress={() => setLinkAnnotation(null)}
              >
      ...
      >
        <View style={styles.modalOverlay}>
          <View style={[styles.modalSheet, missionStyles.modalSheet]}>
            ...
            <Text style={{ color: COLORS.inkSoft, fontSize: 9.5, lineHeight: 14, marginBottom: 9 }}>
              Choisis une signature enregistrée dans cette Mission. Elle sera dessinée dans le PDF exporté sans modifier
              le fichier source.
            </Text>
            <ScrollView style={{ maxHeight: 280 }}>
              {signatureTargets.map((target) => (
                <TouchableOpacity
                  key={target.id}
                  onPress={() => placeSignature(target)}
                  style={{ paddingVertical: 10, borderBottomWidth: 1, borderBottomColor: MISSION_COLORS.accentLine }}
                >
                  <Text style={{ color: COLORS.ink, fontSize: 10.5, fontWeight: '900' }}>{target.label}</Text>
                  {target.subtitle ? (
                    <Text style={{ color: COLORS.inkFaint, fontSize: 8.6, marginTop: 2 }}>{target.subtitle}</Text>
                  ) : null}
                </TouchableOpacity>
              ))}
              {!signatureTargets.length ? (
                <View style={{ paddingVertical: 10 }}>
                  <Text style={{ color: COLORS.inkFaint, fontSize: 9.5, lineHeight: 13 }}>
                    Aucune signature enregistrée pour cette Mission.
                  </Text>
                  <TouchableOpacity
                    style={[
                      styles.btnSecondary,
                    ...
                    onPress={() => {
                      setSignatureModal(false);
                      navigation.navigate('MissionSignature', { missionId });
                    }}
                  >
                    <Text style={[styles.btnSecondaryText, missionStyles.secondaryButtonText]}>
                      Créer une signature
            ...
            <View style={styles.modalActions}>
              <TouchableOpacity
                style={[styles.btnSecondary, missionStyles.secondaryButton]}
                onPress={refreshLinkTargets}
              >
                <Text style={[styles.btnSecondaryText, missionStyles.secondaryButtonText]}>Actualiser</Text>
              </TouchableOpacity>
              <TouchableOpacity
                style={[styles.btnPrimary, missionStyles.primaryButton]}
                onPress={() => {
      ...
      <Modal
        visible={!!reserveAnnotation}
        transparent
        animationType="fade"
        onRequestClose={() => setReserveAnnotation(null)}
      >
        <View style={styles.modalOverlay}>
          <ScrollView
            style={[styles.modalSheet, missionStyles.modalSheet]}
          ...
          >
            ...
            <Text style={{ color: COLORS.inkSoft, fontSize: 9.5, lineHeight: 14, marginBottom: 10 }}>
              METRA crée un point et une action liés à cette géométrie. Les photos avant / après pourront ensuite être
              prises depuis Actions sans ressaisie du contexte.
            </Text>
            <TextInput
              style={[styles.input, missionStyles.input]}
              value={reserveDraft.label}
              onChangeText={(v) => setReserveDraft((p) => ({ ...p, label: v }))}
              placeholder="Réserve / action demandée"
              autoFocus
            />
            <TextInput
              style={[styles.input, missionStyles.input, { marginTop: 8, minHeight: 72, textAlignVertical: 'top' }]}
              multiline
              value={reserveDraft.description}
              onChangeText={(v) => setReserveDraft((p) => ({ ...p, description: v }))}
              placeholder="Constat / description"
            />
            <TextInput
              style={[styles.input, missionStyles.input, { marginTop: 8 }]}
              value={reserveDraft.responsible}
              onChangeText={(v) => setReserveDraft((p) => ({ ...p, responsible: v }))}
              placeholder="Responsable / entreprise"
            />
            <TextInput
              style={[styles.input, missionStyles.input, { marginTop: 8 }]}
              value={reserveDraft.dueDate}
              onChangeText={(v) => setReserveDraft((p) => ({ ...p, dueDate: v }))}
              placeholder="Échéance AAAA-MM-JJ"
            />
            <TextInput
              style={[styles.input, missionStyles.input, { marginTop: 8 }]}
              value={reserveDraft.dueText}
              onChangeText={(v) => setReserveDraft((p) => ({ ...p, dueText: v }))}
              placeholder="Échéance libre : prochaine visite…"
            />
            <TextInput
              style={[styles.input, missionStyles.input, { marginTop: 8 }]}
              value={reserveDraft.priority}
              onChangeText={(v) => setReserveDraft((p) => ({ ...p, priority: v }))}
              placeholder="Priorité / criticité"
            />
            <TextInput
              style={[styles.input, missionStyles.input, { marginTop: 8 }]}
              keyboardType="decimal-pad"
              value={reserveDraft.cost}
              onChangeText={(v) => setReserveDraft((p) => ({ ...p, cost: v }))}
              placeholder="Coût estimé €"
            />
            <View style={styles.modalActions}>
              <TouchableOpacity
                style={[styles.btnSecondary, missionStyles.secondaryButton]}
                onPress={() => setReserveAnnotation(null)}
              >
                <Text style={[styles.btnSecondaryText, missionStyles.secondaryButtonText]}>Annuler</Text>
              </TouchableOpacity>
              <TouchableOpacity style={[styles.btnPrimary, missionStyles.primaryButton]} onPress={createReserve}>
                <Text style={[styles.btnPrimaryText, missionStyles.primaryButtonText]}>Créer point + action</Text>
              </TouchableOpacity>
            </View>
          </ScrollView>
        </View>
        ...
        <View style={styles.modalOverlay}>
          <View style={[styles.modalSheet, missionStyles.modalSheet]}>
            ...
            <TextInput
              ...
              placeholder="Libellé / commentaire"
              autoFocus
            />
            <View style={styles.modalActions}>
              <TouchableOpacity
              ...
              <TouchableOpacity style={[styles.btnPrimary, missionStyles.primaryButton]} onPress={confirmText}>
        ...
        <View style={styles.modalOverlay}>
          <View style={[styles.modalSheet, missionStyles.modalSheet]}>
            ...
            <View style={styles.modalActions}>
              ...
              <TouchableOpacity style={[styles.btnPrimary, missionStyles.primaryButton]} onPress={confirmCalibration}>
                <Text style={[styles.btnPrimaryText, missionStyles.primaryButtonText]}>Calibrer</Text>
              </TouchableOpacity>
            </View>
          </View>
        </View>
        ...
        <View style={styles.modalOverlay}>
          <View style={[styles.modalSheet, missionStyles.modalSheet]}>
            ...
            <TextInput
              ...
              placeholder="Réseaux chauffage, réserves, équipements…"
              autoFocus
            />
            <View style={styles.modalActions}>
              <TouchableOpacity
              ...
              <TouchableOpacity style={[styles.btnPrimary, missionStyles.primaryButton]} onPress={createLayer}>
