    ...
    networks: {
      ...
      overflow: {
        ...
        columns: [
          ...
          {
            col: 'F',
            label: 'Consigne / programme horaire',
            importKey: 'programme',
            exportKey: 'consigne_programme_horaire'
          }
        ]
      }
    },
...
const PRE_ALLUMAGE = Object.freeze({
  ...
  version: 1,
  nom: 'Pré-allumage',
  description: 'Visite technique de préparation au lancement de la saison de chauffe',
  actif: true,
  ui: {
    ...
    labels: {
      'p-pa-infos': 'Informations',
      'p-pa-batiments': 'Installations',
      'p-pa-compteurs': 'Compteurs',
      'p-pa-regulation': 'Régulation',
      'p-pa-chaufferie': 'Chaufferie',
      'p-pa-sst': 'Sous-stations',
      'p-pa-conclusion': 'Conclusion',
      'p-equip': 'Équipements',
      'p-remarques': 'Réserves',
      'p-photos': 'Photos'
    }
  ...
  excel: {
    ...
    requiredSheets: ['TRAME PRE-ALLUMAGE'],
    mainSheet: 'TRAME PRE-ALLUMAGE',
    metadata: { client: 'B1', site: 'B2', adresse: 'B3', type: 'B4', dateVisite: 'B5' },
    signature: {
      sheet: 'TRAME PRE-ALLUMAGE',
      cells: [{ ref: 'B4', values: ['PRE-ALLUMAGE v1', 'PRE-ALLUMAGE', 'Pré-allumage'] }]
    },
...
export function detecterTrameDepuisClasseur(wb, lireCellule) {
  ...
  for (const trame of disponibles) {
    ...
    const ok = (signature.cells || []).every(({ ref, values }) => {
      const valeur = String(lireCellule(sheet, ref) || '')
        .trim()
        .toLowerCase();
