...
import {
  exporterPilotageExcel,
  PILOTAGE_DEFAULT_COLUMNS,
  PILOTAGE_EXPORT_COLUMNS,
  PILOTAGE_EXPORT_PRESETS
} from './clientTechnicalMatrixExport.js';
...
function issueKey(issue) {
  return issue?.remarque_id || `${issue?.visit_id || issue?.id}||${issue?.section_code || ''}||${issue?.cle || ''}`;
}
...
function Cell({ cell, onPress }) {
  ...
  return (
    ...
    >
      <Text style={{ fontSize: 18, fontWeight: '900', color: p.text }}>
        {p.symbol} {count || ''}
      </Text>
      <Text style={{ fontSize: 8.5, color: p.text, marginTop: 2 }}>
        {cell.total ? `${cell.s} S · ${cell.ns} N.S` : 'Non concerné'}
      </Text>
    </TouchableOpacity>
  );
}
...
function Stat({ value, label, sub }) {
  return (
    ...
    >
      <Text style={{ fontSize: 24, fontWeight: '900', color: COLORS.ink }}>{value}</Text>
      <Text style={{ fontSize: 11.5, fontWeight: '800', color: COLORS.inkSoft }}>{label}</Text>
      {sub ? <Text style={{ fontSize: 10, color: COLORS.muted, marginTop: 3 }}>{sub}</Text> : null}
    </View>
  );
}
  ...
  return (
    <TouchableOpacity
      ...
      style={{
        ...
        borderRadius: 10,
        borderWidth: 1,
        borderColor: active ? (danger ? '#B42318' : COLORS.orange) : COLORS.line,
        backgroundColor: active ? (danger ? '#FDECEC' : '#FFF3E8') : '#fff',
        alignItems: 'center',
        justifyContent: 'center'
      }}
    >
      <Text
        style={{
          fontSize: 10.5,
          fontWeight: '800',
          color: active ? (danger ? '#B42318' : COLORS.orange) : COLORS.inkSoft
        }}
      >
        {label}
      </Text>
        ...
        <View style={{ width: tableWidth, flex: 1 }}>
          <View style={{ flexDirection: 'row', backgroundColor: '#FFF' }}>
            <View style={{ width: 180, padding: 8 }}>
              <Text style={{ fontWeight: '900' }}>Sites</Text>
            </View>
            {categories.map((category) => (
              <View key={category.key} style={{ width: 98, padding: 6, justifyContent: 'center' }}>
                <Text style={{ textAlign: 'center', fontSize: 10, fontWeight: '800' }}>{category.label}</Text>
              </View>
            ))}
          </View>
          <FlatList
            ...
            renderItem={({ item: site }) => (
              ...
              >
                <TouchableOpacity
                  onPressIn={() => prewarmSiteLocals(site.id).catch(() => {})}
                  onPress={() =>
                    navigation.navigate('SiteLocals', { siteId: site.id, nomSite: site.nom_site, clientId, nomClient })
                  }
                  style={{ width: 180, minHeight: 62, padding: 9, justifyContent: 'center' }}
                >
                  <Text style={{ fontWeight: '800', color: COLORS.primary }}>{site.nom_site}</Text>
                  <Text style={{ fontSize: 9.5, color: COLORS.muted }} numberOfLines={2}>
                    {site.adresse || 'Adresse non renseignée'}
                  </Text>
                </TouchableOpacity>
                {categories.map((category) => (
                  <Cell
                    key={category.key}
                    cell={site.cells[category.key] || { total: 0, state: 'none' }}
                    onPress={() => openCell(site, category, site.cells[category.key])}
                  />
                ))}
              </View>
            )}
...
function recordMatches(record, site, filters) {
  ...
  if (
    Number(filters.minSeverity || 0) > 0 &&
    !(avis === 'N.S' && Number(record.criticite || 0) >= Number(filters.minSeverity))
  )
    return false;
...
export function ClientPilotageScreen({ route, navigation }) {
  ...
  const scopedSites = useMemo(
    () => (matrix?.sites || []).filter((site) => !siteIdsGroupe || siteIdsGroupe.has(site.id)),
    [matrix, siteIdsGroupe]
  );
  ...
  const filteredRecordsBySite = useMemo(() => {
    ...
    for (const site of scopedSites)
      map.set(
        site.id,
        (site.records || []).filter((record) => recordMatches(record, site, filters))
      );
    return map;
  }, [scopedSites, filters]);
  ...
  const sites = useMemo(
    () =>
      scopedSites
        ...
        .filter(
          (site) => site.records.length > 0 || Object.values(filters).every((v) => v === 'all' || v === 0 || v === '')
        ),
    [scopedSites, filteredRecordsBySite, visibleCategories, filters]
  );
  ...
  const technicalSummary = useMemo(() => {
    ...
    for (const r of filteredRecords) {
      ...
      if (avis === 'N.S') {
        result.ns += 1;
        if (Number(r.criticite || 0) >= 4) result.critical += 1;
      }
  ...
  const openCell = async (site, category, cell) => {
    ...
    const entries = await mapAvecConcurrence(cell.issues || [], 4, async (issue) => [
      issueKey(issue),
      await getMatrixCellPhotos(issue)
    ]);
  ...
  const faireExport = async ({ columns = PILOTAGE_DEFAULT_COLUMNS, presetStatuses = [], label = viewLabel } = {}) => {
    ...
    try {
      setExportBusy(true);
      const result = await exporterPilotageExcel({
        clientNom: nomClient,
      ...
      Alert.alert('Extraction impossible', String(e?.message || e));
    } finally {
      setExportBusy(false);
    }
  };
  ...
  const toggleColumn = (key) =>
    setExportColumns((prev) => (prev.includes(key) ? prev.filter((x) => x !== key) : [...prev, key]));

  if (loading || !matrix)
    return (
      <View style={styles.center}>
        <ActivityIndicator size="large" color={COLORS.orange} />
        <Text style={{ color: COLORS.muted, marginTop: 10 }}>Calcul du pilotage client…</Text>
      </View>
    );
    ...
    >
      <View style={{ padding: 14 }}>
        ...
        <Text style={{ color: COLORS.muted, fontSize: 12 }}>
          {nomClient || 'Client'} · calcul local et disponible hors connexion
        </Text>
        {groupes.length ? (
          <ScrollView
            horizontal
          ...
          >
            ...
            {groupes.map((g) => (
              <Chip
                key={g.id}
                label={`${g.nom} · ${g.nb_sites || 0}`}
                active={groupeActif === g.id}
                onPress={() => setGroupeActif(g.id)}
              />
            ))}
          </ScrollView>
        ) : null}
        ...
        <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 7, marginTop: 11 }}>
          <TouchableOpacity style={styles.btnSecondary} onPress={() => setFilterVisible(true)}>
            <Text style={styles.btnSecondaryText}>⚙ Filtres{activeFilterCount ? ` · ${activeFilterCount}` : ''}</Text>
          </TouchableOpacity>
          <TouchableOpacity
            style={[
              styles.btnSecondary,
              filters.status === 'attention' && { borderColor: COLORS.orange, backgroundColor: '#FFF3E8' }
            ]}
            onPress={() => setFilters((f) => ({ ...f, status: f.status === 'attention' ? 'all' : 'attention' }))}
          >
            <Text style={styles.btnSecondaryText}>⚠ Attention</Text>
          </TouchableOpacity>
          <TouchableOpacity
            style={styles.btnPrimary}
            disabled={exportBusy || !filteredRecords.length}
            onPress={() => faireExport({})}
          >
            <Text style={styles.btnPrimaryText}>↧ Exporter cette vue</Text>
          </TouchableOpacity>
          <TouchableOpacity
          ...
          >
            <Text style={styles.btnSecondaryText}>▤ Extraction Excel</Text>
          </TouchableOpacity>
        </View>
        <Text style={{ color: COLORS.muted, fontSize: 10.5, marginTop: 8 }}>
          {viewLabel} · {filteredRecords.length} ligne(s)
        </Text>
      </View>

      <View style={{ borderTopWidth: 1, borderBottomWidth: 1, borderColor: COLORS.line, backgroundColor: '#fff' }}>
        <View style={{ paddingHorizontal: 14, paddingTop: 12 }}>
          <Text style={styles.sectionLabel}>Cartographie technique</Text>
          <Text style={{ color: COLORS.muted, fontSize: 10.5, marginTop: 3 }}>
            Chaque constat est classé dans une seule catégorie. Rouge = criticité 4–5 · orange = N.S · gris = non
            relevé/non visible.
          </Text>
        </View>
        {visibleCategories.length && sites.length ? (
          <VirtualizedTechnicalMatrix
            sites={sites}
          ...
          <View style={{ padding: 20 }}>
            <Text style={{ color: COLORS.muted, textAlign: 'center' }}>Aucun constat ne correspond aux filtres.</Text>
          </View>
        )}
      </View>
      ...
      <View style={{ padding: 14 }}>
        ...
        <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 8 }}>
          <Stat value={resume.sites} label="Sites" />
          <Stat value={resume.reservesOuvertes} label="Réserves ouvertes" sub={`${resume.reservesLevees} levée(s)`} />
      ...
      <Modal visible={filterVisible} transparent animationType="fade" onRequestClose={() => setFilterVisible(false)}>
        <View style={styles.modalOverlay}>
          <View style={[styles.modalSheet, { width: '94%', maxWidth: 720, maxHeight: '90%' }]}>
            <ScrollView keyboardShouldPersistTaps="handled">
              <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                <Text style={[styles.modalTitle, { flex: 1 }]}>Filtres cartographie</Text>
                <TouchableOpacity onPress={() => setFilters(DEFAULT_FILTERS)}>
                  <Text style={{ color: COLORS.primary, fontWeight: '800' }}>Réinitialiser</Text>
                </TouchableOpacity>
              </View>
              <Text style={[styles.fieldLabel, { marginTop: 12 }]}>Recherche</Text>
              <TextInput
                style={styles.input}
                value={filters.search}
                placeholder="Site, réserve, équipement, commentaire…"
                onChangeText={(v) => setFilters((f) => ({ ...f, search: v }))}
              />
              <Text style={[styles.fieldLabel, { marginTop: 12 }]}>Métier</Text>
              <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 6, marginTop: 6 }}>
                <Chip
                  label="Tous"
                  active={filters.trame === 'all'}
                  onPress={() => setFilters((f) => ({ ...f, trame: 'all' }))}
                />
                {(matrix.trames || []).map((t) => (
                  <Chip
                    key={t.id}
                    label={t.label}
                    active={filters.trame === t.id}
                    onPress={() => setFilters((f) => ({ ...f, trame: t.id }))}
                  />
                ))}
              </View>
              <Text style={[styles.fieldLabel, { marginTop: 12 }]}>Avis</Text>
              <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 6, marginTop: 6 }}>
              ...
              <Text style={[styles.fieldLabel, { marginTop: 12 }]}>Criticité minimale</Text>
              <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 6, marginTop: 6 }}>
                {[0, 1, 2, 3, 4, 5].map((v) => (
                  <Chip
                    key={v}
                    label={v === 0 ? 'Toutes' : `≥ ${v}`}
                    active={Number(filters.minSeverity) === v}
                    danger={v >= 4}
                    onPress={() => setFilters((f) => ({ ...f, minSeverity: v }))}
                  />
                ))}
              </View>
              ...
              <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 6, marginTop: 6 }}>
                ...
                {matrix.categories.map((c) => (
                  <Chip
                    ...
                    onPress={() => setFilters((f) => ({ ...f, category: c.key }))}
                  />
                ))}
              </View>
              <TouchableOpacity style={[styles.btnPrimary, { marginTop: 16 }]} onPress={() => setFilterVisible(false)}>
                <Text style={styles.btnPrimaryText}>Afficher la vue</Text>
              </TouchableOpacity>
            </ScrollView>
          </View>
        </View>
      ...
      <Modal visible={exportVisible} transparent animationType="fade" onRequestClose={() => setExportVisible(false)}>
        <View style={styles.modalOverlay}>
          <View style={[styles.modalSheet, { width: '95%', maxWidth: 760, maxHeight: '92%' }]}>
            <ScrollView>
              <Text style={styles.modalTitle}>Extraction Excel</Text>
              <Text style={{ color: COLORS.muted, fontSize: 11, marginTop: 3 }}>
                L’extraction part de la vue filtrée actuelle. Feuille 1 = Synthèse · feuille 2 = Détail.
              </Text>
              <Text style={[styles.fieldLabel, { marginTop: 14 }]}>Préréglage</Text>
              <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 6, marginTop: 6 }}>
                <Chip label="Vue courante" active={exportPreset === 'view'} onPress={() => choisirPreset('view')} />
                {Object.entries(PILOTAGE_EXPORT_PRESETS).map(([key, preset]) => (
                  <Chip
                    key={key}
                    label={preset.label}
                    active={exportPreset === key}
                    onPress={() => choisirPreset(key)}
                  />
                ))}
              </View>
              <Text style={[styles.fieldLabel, { marginTop: 14 }]}>Colonnes</Text>
              <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 6, marginTop: 6 }}>
                {PILOTAGE_EXPORT_COLUMNS.map((column) => (
                  <Chip
                    key={column.key}
                    label={column.label}
                    active={exportColumns.includes(column.key)}
              ...
              <View style={{ flexDirection: 'row', gap: 8, marginTop: 16 }}>
                ...
                <TouchableOpacity
                  ...
                  disabled={exportBusy || !exportColumns.length}
                  onPress={() =>
                    faireExport({
                      columns: exportColumns,
                      presetStatuses:
                        exportPreset === 'view' ? [] : PILOTAGE_EXPORT_PRESETS[exportPreset]?.statuses || [],
                      label: `${viewLabel} · ${exportPreset === 'view' ? 'Vue courante' : PILOTAGE_EXPORT_PRESETS[exportPreset]?.label || 'Extraction'}`
                    })
                  }
                >
                  <Text style={styles.btnPrimaryText}>{exportBusy ? 'Création…' : 'Créer et partager'}</Text>
                </TouchableOpacity>
              </View>
            </ScrollView>
          </View>
      ...
      <Modal visible={!!selected} transparent animationType="fade" onRequestClose={() => setSelected(null)}>
        <View style={styles.modalOverlay}>
          <View style={[styles.modalSheet, { maxWidth: 720, width: '94%', maxHeight: '88%' }]}>
            {selected ? (
              <ScrollView>
                <Text style={styles.modalTitle}>
                  {selected.site.nom_site} · {selected.category.label}
                </Text>
                <Text style={{ color: COLORS.muted, marginTop: 3 }}>
                  {selected.cell.s} S · {selected.cell.ns} N.S · {selected.cell.nr} N.R · {selected.cell.so} S.O ·{' '}
                  {selected.cell.nv} N.V
                </Text>
                {selected.cell.issues.length ? (
                  selected.cell.issues.map((issue, idx) => {
                    const pics = photos[issueKey(issue)] || [];
                    return (
                      <View
                      ...
                      >
                        <Text
                          style={{
                            fontWeight: '900',
                            color: Number(issue.criticite || 0) >= 4 ? COLORS.red : COLORS.orange
                          }}
                        >
                          {issue.reference_libelle || issue.cle} · {reserveSeverityLabel(issue.criticite)}
                        </Text>
                        <Text style={{ marginTop: 5, color: COLORS.ink }}>
                          {issue.prestation || issue.commentaire || 'Anomalie à préciser'}
                        </Text>
                        {pics.length ? (
                          <ScrollView horizontal style={{ marginTop: 8 }}>
                            {pics.map((p) => (
                              <TouchableOpacity key={p.id} onPress={() => setPhotoZoom({ ...p, issue })}>
                                <PhotoVariantImage
                                  uri={p.uri}
                                  variant="thumb"
                                  style={{
                          ...
                          <Text style={{ marginTop: 7, fontSize: 10, color: COLORS.muted }}>
                            Aucune photo liée à ce problème.
                          </Text>
                        )}
                      </View>
                    );
                  ...
                  <Text style={{ marginTop: 12, color: COLORS.muted }}>Aucune réserve N.S dans cette cellule.</Text>
                )}
                <View style={{ flexDirection: 'row', gap: 8, marginTop: 14 }}>
                  <TouchableOpacity
                    ...
                    onPress={() => {
                      ...
                      setSelected(null);
                      navigation.navigate('SiteLocals', { siteId: s.id, nomSite: s.nom_site, clientId, nomClient });
                    }}
                  >
                    <Text style={styles.btnPrimaryText}>Ouvrir le site</Text>
                  </TouchableOpacity>
                  <TouchableOpacity style={[styles.btnSecondary, { flex: 1 }]} onPress={() => setSelected(null)}>
      ...
      <Modal visible={!!photoZoom} transparent animationType="fade" onRequestClose={() => setPhotoZoom(null)}>
        <View style={styles.modalOverlay}>
          <View style={[styles.modalSheet, { width: '96%', maxWidth: 900 }]}>
            {photoZoom ? (
              <>
                <PhotoVariantImage
                  uri={photoZoom.uri}
                  variant="preview"
                  resizeMode="contain"
                  style={{ width: '100%', height: 430, backgroundColor: '#111', borderRadius: 10 }}
                />
                <Text style={{ marginTop: 9, fontWeight: '800' }}>
                  {photoZoom.label || photoZoom.issue?.reference_libelle || photoZoom.issue?.cle}
                </Text>
                <TouchableOpacity style={[styles.btnPrimary, { marginTop: 12 }]} onPress={() => setPhotoZoom(null)}>
                  <Text style={styles.btnPrimaryText}>Revenir au pilotage</Text>
                </TouchableOpacity>
              </>
            ) : null}
          </View>
