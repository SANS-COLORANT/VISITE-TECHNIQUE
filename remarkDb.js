...
async function materialiserReservesVmcManquantes(db, visiteId) {
  ...
  const manquants = await db.getAllAsync(
    `SELECT c.section_code,c.cle,c.commentaire FROM controles_visite c WHERE c.visite_id=? AND c.avis='N.S' AND c.section_code LIKE 'vmc-c%.%' AND NOT EXISTS (SELECT 1 FROM remarques r WHERE r.visite_id=c.visite_id AND r.controle_key=(c.section_code || '||' || c.cle))`,
    [visiteId]
  );
  for (const controle of manquants || []) {
    ...
    await db.runAsync(
      `INSERT INTO remarques(id,visite_id,controle_key,poste,prestation,delai,estimatif,origine,reference_type,reference_id,reference_libelle,criticite,criticite_defaut,criticite_modifiee) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?)`,
      [
        createId(),
        visiteId,
        controleKey,
...
export async function upsertRemarquePrescription(visiteId, controleKey, prescription = {}, origine = null) {
  ...
  const existante = await db.getFirstAsync(
    `SELECT id,criticite,criticite_defaut,criticite_modifiee FROM remarques WHERE visite_id=? AND controle_key=? LIMIT 1`,
  ...
  const criticiteDefaut = clampReserveSeverity(
    prescription.criticiteDefaut ?? prescription.criticite ?? existante?.criticite_defaut ?? 2
  );
  const criticite = existante?.criticite_modifiee ? clampReserveSeverity(existante.criticite) : criticiteDefaut;
  const referenceLibelle = await libelleElementControle(db, visiteId, controleKey);
  if (existante?.id) {
    await db.runAsync(
      `UPDATE remarques SET poste=?,prestation=?,delai=?,estimatif=?,origine=?,criticite=?,criticite_defaut=?,reference_type=COALESCE(reference_type,'controle'),reference_id=COALESCE(reference_id,?),reference_libelle=CASE WHEN reference_libelle IS NULL OR TRIM(reference_libelle)='' THEN ? ELSE reference_libelle END WHERE id=?`,
      [
        poste,
        prestation,
        delai,
...
export async function modifierCriticiteRemarque(id, criticite) {
  ...
  await db.runAsync(
    `UPDATE remarques SET criticite=?,criticite_modifiee=CASE WHEN criticite_defaut=? THEN 0 ELSE 1 END WHERE id=?`,
    [value, value, id]
  );
  return value;
}
...
export async function supprimerRemarqueControle(visiteId, controleKey) {
  const db = await openAppDatabase();
  const remarques = await db.getAllAsync(`SELECT id FROM remarques WHERE visite_id=? AND controle_key=?`, [
    visiteId,
    controleKey
  ]);
...
export async function ajouterRemarqueVisite(visiteId, data = {}) {
  ...
  await db.runAsync(
    `INSERT INTO remarques(id,visite_id,controle_key,poste,prestation,delai,estimatif,origine,criticite,criticite_defaut,criticite_modifiee) VALUES(?,?,?,?,?,?,?,?,?,?,?)`,
    [
      id,
      visiteId,
...
export async function modifierRemarqueVisite(id, patch = {}) {
  ...
  const autorisees = [
    ...
    'estimatif',
    'criticite',
    'intranet_date_reserve',
    'intranet_delai',
    'intranet_etat_avancement'
  ];
  const sets = [];
  const params = [];
  for (const cle of autorisees) {
    ...
    else if (cle.startsWith('intranet_'))
...
export async function rattacherRemarqueVisite(id, cible = {}) {
  const db = await openAppDatabase();
  await db.runAsync(
    `UPDATE remarques SET reference_onglet=?,reference_type=?,reference_id=?,reference_libelle=? WHERE id=?`,
    [cible.onglet || null, cible.type || null, cible.id || null, cible.libelle || null, id]
  );
}
