...
export function MissionWorkflowScreen({ navigation, route }) {
  ...
  const recommendedWorkstreams = React.useMemo(() => getMissionWorkstreamPresets(missionType), [missionType]);
  ...
  const addPhase = async () => {
    ...
    await db.runAsync('INSERT INTO mission_phases(id,mission_id,type,label,status,sort_order) VALUES(?,?,?,?,?,?)', [
      createId('mph'),
      missionId,
      phaseDraft.type || null,
      phaseDraft.label.trim(),
      'planned',
      phases.length
    ]);
    setPhaseModal(false);
  ...
  const movePhase = async (phase, delta) => {
    ...
    await db.withTransactionAsync(async () => {
      await db.runAsync("UPDATE mission_phases SET sort_order=?,updated_at=datetime('now') WHERE id=?", [
        other.sort_order,
        phase.id
      ]);
      await db.runAsync("UPDATE mission_phases SET sort_order=?,updated_at=datetime('now') WHERE id=?", [
  ...
  const prepareRecommendedWorkstreams = async () => {
    ...
    try {
      ...
      Alert.alert(
        ...
        added ? String(added) + ' volet(s) ajoutés. Ils restent entièrement modifiables.' : 'Aucun doublon créé.'
      );
    } catch (e) {
  ...
  const addWorkstream = async () => {
    ...
    await db.runAsync(
      ...
      [
        createId('mw'),
        missionId,
        workstreamDraft.kind || null,
        workstreamDraft.label.trim(),
        workstreamDraft.description || null,
        workstreams.length
      ]
    );
    ...
    <View style={{ flex: 1, backgroundColor: MISSION_COLORS.bg }}>
      <ScrollView contentContainerStyle={{ padding: 16, paddingBottom: 110 }}>
        ...
        <Text style={{ color: COLORS.inkSoft, fontSize: 10.5, lineHeight: 15 }}>
          Une même Mission peut évoluer du diagnostic à l’étude, au chantier puis à la réception sans perdre l’état
          initial. Les volets permettent aussi de gérer un dossier pluriannuel.
        </Text>
        ...
        <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 8, marginTop: 12 }}>
          ...
          >
            <Text style={[styles.btnPrimaryText, missionStyles.primaryButtonText]}>＋ Occurrence / visite</Text>
          </TouchableOpacity>
          <TouchableOpacity
            style={[styles.btnSecondary, missionStyles.secondaryButton]}
            onPress={() => setPhaseModal(true)}
          >
            <Text style={[styles.btnSecondaryText, missionStyles.secondaryButtonText]}>＋ Phase</Text>
          </TouchableOpacity>
          <TouchableOpacity
            style={[styles.btnSecondary, missionStyles.secondaryButton]}
            onPress={() => setWorkstreamModal(true)}
          >
            <Text style={[styles.btnSecondaryText, missionStyles.secondaryButtonText]}>＋ Volet</Text>
          </TouchableOpacity>
          {recommendedWorkstreams.length ? (
            <TouchableOpacity
              style={[styles.btnSecondary, missionStyles.secondaryButton]}
        ...
        {phases.map((phase, index) => (
          <View key={phase.id} style={[missionStyles.card, { padding: 11, marginBottom: 8 }]}>
            <View style={{ flexDirection: 'row', alignItems: 'flex-start' }}>
              <View style={{ flex: 1 }}>
                <Text style={{ color: MISSION_COLORS.accentStrong, fontSize: 11.3, fontWeight: '900' }}>
                  {phase.label}
                </Text>
                <Text style={{ color: COLORS.inkFaint, fontSize: 8.6, marginTop: 2 }}>
                  {phase.type || 'Phase'} · {phase.status}
                </Text>
              </View>
              <TouchableOpacity onPress={() => movePhase(phase, -1)} disabled={index === 0} style={{ padding: 6 }}>
                <Text style={{ color: index === 0 ? COLORS.inkFaint : COLORS.inkSoft }}>↑</Text>
              </TouchableOpacity>
              <TouchableOpacity
                onPress={() => movePhase(phase, 1)}
                disabled={index === phases.length - 1}
                style={{ padding: 6 }}
              >
                <Text style={{ color: index === phases.length - 1 ? COLORS.inkFaint : COLORS.inkSoft }}>↓</Text>
              </TouchableOpacity>
            </View>
            <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{ marginTop: 7 }}>
              {PHASE_STATUS.map(([key, label]) => (
                <Chip
                  key={key}
                  label={label}
                  selected={phase.status === key}
                  onPress={() => setPhaseStatus(phase, key)}
                />
              ))}
            </ScrollView>
          </View>
        ...
        {workstreams.length ? (
          workstreams.map((w) => (
            <View key={w.id} style={[missionStyles.card, { padding: 11, marginBottom: 7 }]}>
              <View style={{ flexDirection: 'row', alignItems: 'flex-start' }}>
                <View style={{ flex: 1 }}>
                  <Text style={{ color: COLORS.ink, fontSize: 10.8, fontWeight: '900' }}>{w.label}</Text>
                  <Text style={{ color: COLORS.inkFaint, fontSize: 8.6, marginTop: 2 }}>
                    {w.kind || 'Volet'} · {w.status}
                  </Text>
                </View>
              ...
              {w.description ? (
                <Text style={{ color: COLORS.inkSoft, fontSize: 9.2, marginTop: 4 }}>{w.description}</Text>
              ) : null}
            </View>
          ))
        ) : (
          <Text style={{ color: COLORS.inkFaint, fontSize: 9.5 }}>
            Aucun volet. Utile pour une AMO pluriannuelle : exploitation, énergie, P3, PPI, réunions, réception…
          </Text>
        )}
        ...
        {visits.map((v) => (
          <TouchableOpacity
            key={v.id}
            onPress={() => navigation.navigate('MissionVisit', { missionId, visitId: v.id })}
            style={[missionStyles.card, { padding: 11, marginBottom: 7 }]}
          >
            <Text style={{ color: COLORS.ink, fontSize: 10.8, fontWeight: '900' }}>
              {v.visit_date || 'Date à préciser'} · {v.visit_type || 'Visite'}
            </Text>
            <Text style={{ color: COLORS.inkFaint, fontSize: 8.7, marginTop: 2 }}>
              {[v.phase_label, v.site_name, v.status].filter(Boolean).join(' · ')}
            </Text>
          </TouchableOpacity>
        ))}
      </ScrollView>
        ...
        <View style={styles.modalOverlay}>
          <View style={[styles.modalSheet, missionStyles.modalSheet]}>
            ...
            <TextInput
              ...
              onChangeText={(v) => setPhaseDraft((p) => ({ ...p, label: v }))}
              placeholder="DIA, AVP, PRO/DCE, DET, OPR…"
            />
            <TextInput
              style={[styles.input, missionStyles.input, { marginTop: 8 }]}
              value={phaseDraft.type}
              onChangeText={(v) => setPhaseDraft((p) => ({ ...p, type: v }))}
              placeholder="Type interne (facultatif)"
            />
            <View style={styles.modalActions}>
              ...
              <TouchableOpacity style={[styles.btnPrimary, missionStyles.primaryButton]} onPress={addPhase}>
      ...
      <Modal
        visible={workstreamModal}
        transparent
        animationType="fade"
        onRequestClose={() => setWorkstreamModal(false)}
      >
        <View style={styles.modalOverlay}>
          <View style={[styles.modalSheet, missionStyles.modalSheet]}>
            ...
            <TextInput
              ...
              onChangeText={(v) => setWorkstreamDraft((p) => ({ ...p, label: v }))}
              placeholder="Exploitation, Énergie, P3, PPI…"
            />
            <TextInput
              style={[styles.input, missionStyles.input, { marginTop: 8 }]}
              value={workstreamDraft.kind}
              onChangeText={(v) => setWorkstreamDraft((p) => ({ ...p, kind: v }))}
              placeholder="Type"
            />
            <TextInput
              style={[styles.input, missionStyles.input, { marginTop: 8, minHeight: 70, textAlignVertical: 'top' }]}
              multiline
              value={workstreamDraft.description}
              onChangeText={(v) => setWorkstreamDraft((p) => ({ ...p, description: v }))}
              placeholder="Périmètre du volet"
            />
            <View style={styles.modalActions}>
              <TouchableOpacity
                style={[styles.btnSecondary, missionStyles.secondaryButton]}
                onPress={() => setWorkstreamModal(false)}
              >
                <Text style={[styles.btnSecondaryText, missionStyles.secondaryButtonText]}>Annuler</Text>
              </TouchableOpacity>
              <TouchableOpacity style={[styles.btnPrimary, missionStyles.primaryButton]} onPress={addWorkstream}>
                <Text style={[styles.btnPrimaryText, missionStyles.primaryButtonText]}>Créer</Text>
              </TouchableOpacity>
            </View>
          </View>
            ...
            <View style={{ flexDirection: 'row', flexWrap: 'wrap', marginBottom: 7 }}>
              {phases.map((p) => (
                <Chip
                  key={p.id}
                  label={p.label}
                  selected={visitDraft.phaseId === p.id}
                  onPress={() => setVisitDraft((v) => ({ ...v, phaseId: v.phaseId === p.id ? '' : p.id }))}
                />
              ))}
            </View>
            ...
            <View style={{ flexDirection: 'row', flexWrap: 'wrap', marginBottom: 7 }}>
              {sites.map((s) => (
                <Chip
                  key={s.id}
                  label={s.name}
                  selected={visitDraft.siteId === s.id}
                  onPress={() => setVisitDraft((v) => ({ ...v, siteId: v.siteId === s.id ? '' : s.id }))}
                />
              ))}
            </View>
            <TextInput
              ...
              onChangeText={(v) => setVisitDraft((p) => ({ ...p, visitType: v }))}
              placeholder="Type : terrain, réunion, OPR, mesures…"
            />
            <TextInput
              style={[styles.input, missionStyles.input, { marginTop: 8 }]}
              value={visitDraft.visitDate}
              onChangeText={(v) => setVisitDraft((p) => ({ ...p, visitDate: v }))}
              placeholder="Date AAAA-MM-JJ (vide = aujourd’hui)"
            />
            <View style={styles.modalActions}>
              <TouchableOpacity
                style={[styles.btnSecondary, missionStyles.secondaryButton]}
                onPress={() => setVisitModal(false)}
              >
                <Text style={[styles.btnSecondaryText, missionStyles.secondaryButtonText]}>Annuler</Text>
              </TouchableOpacity>
              <TouchableOpacity style={[styles.btnPrimary, missionStyles.primaryButton]} onPress={createVisit}>
                <Text style={[styles.btnPrimaryText, missionStyles.primaryButtonText]}>Créer & ouvrir</Text>
              </TouchableOpacity>
            </View>
          </View>
        </View>
