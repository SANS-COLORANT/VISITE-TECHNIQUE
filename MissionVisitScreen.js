...
import {
  enregistrerNoteVocaleMission,
  genererChecklistFinVisite,
  ignorerCheckVisite
} from './missionVisitQualityDb.js';
...
import {
  ajouterNoteVisiteMission,
  chargerVisiteMission,
  compterSaisieVisiteMission,
  enregistrerValeurTrameMission,
  mettreAJourVisiteMission
} from './missionVisitDb.js';
    ...
    <View style={{ flexDirection: 'row', flexWrap: 'wrap', marginTop: 7 }}>
      {(field.options || []).map((option) => {
        ...
        return (
          ...
          >
            <Text
              style={{ color: selected ? MISSION_COLORS.accentDark : COLORS.ink, fontSize: 10.5, fontWeight: '800' }}
            >
              {option}
            </Text>
          </TouchableOpacity>
  ...
  return (
    <View style={{ marginBottom: 14 }}>
      ...
      {autoValue ? (
        ...
        >
          <Text style={{ color: MISSION_COLORS.accentDark, fontSize: 7.8, fontWeight: '900', letterSpacing: 0.45 }}>
      ...
      {previousValue !== '' && previousValue !== null && previousValue !== undefined ? (
        ...
        >
          <View style={{ flexDirection: 'row', alignItems: 'center' }}>
            <View style={{ flex: 1 }}>
              <Text style={{ color: COLORS.inkFaint, fontSize: 7.7, fontWeight: '900', letterSpacing: 0.4 }}>
                VISITE PRÉCÉDENTE{previousLabel ? ' · ' + previousLabel : ''}
              </Text>
              <Text style={{ color: COLORS.inkSoft, fontSize: 9.1, lineHeight: 13, marginTop: 3 }}>
...
export function MissionVisitScreen({ navigation, route }) {
  ...
  const recipe = useMemo(
    () => getMissionVisitRecipe(data?.visit?.family, data?.visit?.mission_type, captureMode),
    [data?.visit?.family, data?.visit?.mission_type, captureMode]
  );
  ...
  const filteredContextEquipment = useMemo(() => {
    ...
    return contextEquipment
      ...
      .filter(
        (row) =>
          !q ||
          [row.type, row.brand, row.model].some((value) =>
            String(value || '')
              .toLowerCase()
              .includes(q)
          )
  ...
  const setContextEquipmentVerification = async (status) => {
    ...
    try {
      await modifierEquipementMission(contextEquipmentId, {
        ...
        changeComment: 'Statut terrain'
      });
      setContextEquipment((rows) =>
        rows.map((row) => (row.id === contextEquipmentId ? { ...row, verification_status: status } : row))
      );
      const next = await chargerContexteAutoVisiteMission(actualMissionId, {
        siteId: data?.visit?.site_id || null,
        locationId: contextLocationId || null,
  ...
  const setContextEquipmentLifecycle = async (status) => {
    ...
    try {
      await modifierEquipementMission(contextEquipmentId, {
        ...
        changeComment: 'Cycle projet depuis visite terrain'
      });
      setContextEquipment((rows) =>
        rows.map((row) => (row.id === contextEquipmentId ? { ...row, lifecycle_status: status } : row))
      );
      const next = await chargerContexteAutoVisiteMission(actualMissionId, {
        siteId: data?.visit?.site_id || null,
        locationId: contextLocationId || null,
  ...
  const addNote = async () => {
    ...
    try {
      await ajouterNoteVisiteMission({
        missionId: actualMissionId,
        visitId,
        siteId: data?.visit?.site_id,
        content: note,
      ...
      Alert.alert('Note non enregistrée', String(e.message || e));
    }
  };
  ...
  const openMeasurePreset = (preset = null) => {
    ...
    setMeasureReference(
      preset?.referenceValue === null || preset?.referenceValue === undefined ? '' : String(preset.referenceValue)
    );
    setMeasureValue('');
    ...
    try {
      ...
      if (result?.anomalyStatus === 'to_check') {
        const description = [
          ...
          submitted.reference
            ? 'Référence : ' + submitted.reference + (submitted.unit ? ' ' + submitted.unit : '')
            : null
        ]
          .filter(Boolean)
          .join(' · ');
  ...
  const addPoint = async () => {
    ...
    try {
      ...
      if (shouldCreateAction) {
        await creerActionMission({
          ...
          label:
            pointRequestedAction.trim() ||
            pointLabel.trim() ||
            (pointType === 'reserve' ? 'Lever la réserve' : 'Traiter le point'),
          description: pointDescription,
          priority: pointPriority,
          responsibleActorId,
          dueText: pointDue,
      ...
      if (
        ...
        setContextEquipment((rows) =>
          rows.map((row) => (row.id === contextEquipmentId ? { ...row, lifecycle_status: 'avec_reserve' } : row))
        );
      }
      setPointModal(false);
      setPointLabel('');
  ...
  if (loading && !data)
    return (
      <View style={[{ flex: 1, alignItems: 'center', justifyContent: 'center' }, missionStyles.screen]}>
  ...
  if (!data?.visit)
    return (
      <View style={styles.center}>
        <Text style={styles.errorTitle}>Visite Mission introuvable</Text>
      </View>
    );
  ...
  return (
    <View style={[{ flex: 1 }, missionStyles.screen]}>
      <ScrollView contentContainerStyle={{ padding: 16, paddingBottom: 120 }} keyboardShouldPersistTaps="handled">
        <View
          style={[
            { backgroundColor: COLORS.white, borderWidth: 1, borderRadius: 16, padding: 14, marginBottom: 14 },
            missionStyles.card
          ]}
        >
          <Text style={[{ fontSize: 17, fontWeight: '900' }, missionStyles.title]}>
            {visit.mission_label || 'Mission'}
          </Text>
          <Text style={{ color: COLORS.inkSoft, fontSize: 10.5, marginTop: 4 }}>
            {[visit.client_name, visit.site_name, visit.visit_date].filter(Boolean).join(' · ')}
          </Text>
          <Text style={{ color: MISSION_COLORS.accentDark, fontSize: 10, fontWeight: '900', marginTop: 9 }}>
            Aucun champ de cette visite n’est obligatoire.
          </Text>
        </View>
        ...
        <View style={[missionStyles.card, { padding: 11, marginBottom: 12 }]}>
          <View style={{ flexDirection: 'row', alignItems: 'center' }}>
            <View style={{ flex: 1 }}>
              <Text style={{ color: COLORS.inkFaint, fontSize: 8.5, fontWeight: '900', letterSpacing: 0.4 }}>
                CONTEXTE DE SAISIE
              </Text>
              <Text style={{ color: COLORS.ink, fontSize: 10.5, fontWeight: '900', marginTop: 3 }} numberOfLines={2}>
                {[
                  ...
                  selectedContextEquipment
                    ? [selectedContextEquipment.type, selectedContextEquipment.brand, selectedContextEquipment.model]
                        .filter(Boolean)
                        .join(' · ')
                    : null
                ]
        ...
        {selectedContextEquipment && (playbook.equipmentVerificationStatuses || []).length ? (
          <View style={[missionStyles.card, { padding: 10, marginBottom: 12 }]}>
            <Text style={{ color: COLORS.inkFaint, fontSize: 8.3, fontWeight: '900', marginBottom: 6 }}>
              STATUT TERRAIN · 1 GESTE
            </Text>
            ...
            <Text style={{ color: COLORS.inkFaint, fontSize: 8.2, lineHeight: 11 }}>
              Le statut est enregistré sur la fiche équipement et sera réutilisé dans l’inventaire, les écarts et le
              rapport.
            </Text>
          </View>
        ) : null}

        {selectedContextEquipment && (playbook.equipmentLifecycleStatuses || []).length ? (
          <View style={[missionStyles.card, { padding: 10, marginBottom: 12 }]}>
            <Text style={{ color: COLORS.inkFaint, fontSize: 8.3, fontWeight: '900', marginBottom: 6 }}>
              CYCLE PROJET · 1 GESTE
            </Text>
            <View style={{ flexDirection: 'row', flexWrap: 'wrap' }}>
            ...
            <Text style={{ color: COLORS.inkFaint, fontSize: 8.2, lineHeight: 11 }}>
              Une même fiche suit l’ouvrage de l’installation au contrôle, aux réserves, à la réception puis à la mise
              en service.
            </Text>
          </View>
        ) : null}

        {previousMemory.previousVisit ? (
          <View style={[missionStyles.card, { padding: 11, marginBottom: 12, borderColor: '#D7DDD9' }]}>
            <View style={{ flexDirection: 'row', alignItems: 'flex-start' }}>
              <View style={{ flex: 1 }}>
                <Text style={{ color: COLORS.inkFaint, fontSize: 8.1, fontWeight: '900', letterSpacing: 0.45 }}>
                  MÉMOIRE · VISITE PRÉCÉDENTE
                </Text>
                <Text style={{ color: COLORS.ink, fontSize: 10.2, fontWeight: '900', marginTop: 3 }}>
                  {previousVisitLabel(previousMemory.previousVisit)}
                </Text>
                <Text style={{ color: COLORS.inkSoft, fontSize: 8.8, lineHeight: 12, marginTop: 4 }}>
                  {previousMemory.summary?.fieldsCount || 0} champ(s) · {previousMemory.summary?.measures?.length || 0}{' '}
                  mesure(s) affichée(s) · {previousMemory.summary?.unresolvedCount || 0} point(s) non soldé(s)
                </Text>
              </View>
              {previousMemory.summary?.openActionsCount ? (
                <TouchableOpacity
                  style={[styles.btnSecondary, missionStyles.secondaryButton, { marginLeft: 8 }]}
                  onPress={() =>
                    navigation.navigate('MissionActions', {
                      missionId: actualMissionId,
                      siteId: data?.visit?.site_id || null
                    })
                  }
                >
                  <Text style={[styles.btnSecondaryText, missionStyles.secondaryButtonText]}>
                    {previousMemory.summary.openActionsCount} action(s) ouverte(s)
                  </Text>
                </TouchableOpacity>
              ) : null}
            </View>
            {(previousMemory.summary?.unresolved || []).length ? (
              <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{ maxHeight: 44, marginTop: 7 }}>
                {previousMemory.summary.unresolved.map((item, index) => (
                  <View
                    key={index}
                    style={{
                      borderWidth: 1,
                  ...
                  >
                    <Text style={{ color: COLORS.ink, fontSize: 8.6, fontWeight: '800' }} numberOfLines={1}>
                      {item.label || 'Point à suivre'}
                    </Text>
                    <Text style={{ color: COLORS.inkFaint, fontSize: 7.7, marginTop: 2 }} numberOfLines={1}>
                      {[item.responsible, item.due, item.priority].filter(Boolean).join(' · ') || item.status}
                    </Text>
                  </View>
                ))}
              </ScrollView>
            ...
            <Text style={{ color: COLORS.inkFaint, fontSize: 8.1, lineHeight: 11, marginTop: 6 }}>
              Les valeurs précédentes restent grisées : METRA ne les recopie jamais automatiquement. « Reprendre »
              confirme explicitement une valeur encore valable.
            </Text>
          </View>
        ) : null}
        ...
        <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 8, marginBottom: 12 }}>
          {[
            ...
            <View
              key={label}
              style={[{ minWidth: 84, flexGrow: 1, borderRadius: 12, padding: 10 }, missionStyles.statBox]}
            >
              <Text style={{ color: MISSION_COLORS.accentStrong, fontWeight: '900', fontSize: 15 }}>{value}</Text>
              <Text style={{ color: COLORS.inkSoft, fontSize: 9 }}>{label}</Text>
            </View>
          ))}
        </View>
        ...
        <View style={[missionStyles.card, { padding: 12, marginBottom: 12 }]}>
          ...
          <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{ maxHeight: 38, marginTop: 9 }}>
            {(playbook.steps || []).map((step, index) => (
              <View key={step} style={{ flexDirection: 'row', alignItems: 'center' }}>
                ...
                >
                  <Text style={{ color: MISSION_COLORS.accentStrong, fontSize: 8.6, fontWeight: '900' }}>
                    {index + 1} · {step}
                  </Text>
                </View>
                {index < playbook.steps.length - 1 ? (
                  <Text style={{ color: MISSION_COLORS.accentLineStrong, marginHorizontal: 4 }}>›</Text>
                ) : null}
              </View>
            ))}
          </ScrollView>
          <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 7, marginTop: 10 }}>
            {(playbook.quickActions || []).slice(0, 8).map((item) => (
              <TouchableOpacity
                key={item.key}
                activeOpacity={0.82}
          ...
          {(playbook.measures || []).length ? (
            <>
              <Text
                style={{ color: COLORS.inkFaint, fontSize: 8.3, fontWeight: '900', marginTop: 10, marginBottom: 5 }}
              >
                MESURES COURANTES
              </Text>
              <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{ maxHeight: 42 }}>
                {playbook.measures.slice(0, 8).map((preset) => (
                  <TouchableOpacity
                  ...
                  >
                    ...
                    <Text style={{ color: COLORS.inkFaint, fontSize: 7.8, marginTop: 1 }}>
                      {preset.unit || 'Valeur'}
                    </Text>
                  </TouchableOpacity>
                ))}
          ...
          {(playbook.pointPresets || []).length ? (
            <>
              ...
              <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{ maxHeight: 42 }}>
                {playbook.pointPresets.slice(0, 7).map((preset) => (
                  <TouchableOpacity
                    key={preset.label}
                  ...
                  >
                    <Text style={{ color: COLORS.ink, fontSize: 8.8, fontWeight: '800' }} numberOfLines={1}>
                      {preset.label}
                    </Text>
                  </TouchableOpacity>
                ))}
        ...
        <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 8, marginBottom: 12 }}>
          <TouchableOpacity
            style={[styles.btnSecondary, missionStyles.secondaryButton, { flexGrow: 1, alignItems: 'center' }]}
            onPress={() => openPointPreset(null)}
          >
            <Text style={[styles.btnSecondaryText, missionStyles.secondaryButtonText]}>＋ Point</Text>
          </TouchableOpacity>
          <TouchableOpacity
            style={[styles.btnSecondary, missionStyles.secondaryButton, { flexGrow: 1, alignItems: 'center' }]}
            onPress={() => openMeasurePreset(null)}
          >
            <Text style={[styles.btnSecondaryText, missionStyles.secondaryButtonText]}>＋ Mesure</Text>
          </TouchableOpacity>
          <TouchableOpacity
            style={[styles.btnSecondary, missionStyles.secondaryButton, { flexGrow: 1, alignItems: 'center' }]}
            disabled={mediaBusy}
            onPress={addPhoto}
          >
            <Text style={[styles.btnSecondaryText, missionStyles.secondaryButtonText]}>📷 Photo</Text>
          </TouchableOpacity>
          <TouchableOpacity
            style={[styles.btnSecondary, missionStyles.secondaryButton, { flexGrow: 1, alignItems: 'center' }]}
            disabled={mediaBusy}
            onPress={addDocument}
          >
            <Text style={[styles.btnSecondaryText, missionStyles.secondaryButtonText]}>＋ Document</Text>
          </TouchableOpacity>
        </View>
        <TouchableOpacity
          style={[styles.btnSecondary, missionStyles.secondaryButton, { marginBottom: 18, alignItems: 'center' }]}
          onPress={complete}
        >
          <Text style={[styles.btnSecondaryText, missionStyles.secondaryButtonText]}>Fin de visite</Text>
        </TouchableOpacity>
          ...
          {MISSION_CAPTURE_MODES.map(([key, label]) => {
            ...
            return (
              ...
              >
                <Text
                  style={{
                    color: selected ? MISSION_COLORS.accentDark : COLORS.inkSoft,
                    fontSize: 10,
                    fontWeight: '900'
                  }}
                >
        ...
        <Text style={{ color: COLORS.inkFaint, fontSize: 9.5, lineHeight: 13.5, marginBottom: 12 }}>
          Rapide = constat essentiel · Standard = mesures/comparaisons · Expert = investigation, calculs et analyse
          approfondie. Le choix reste modifiable à tout moment.
        </Text>
        <Text style={[styles.sectionLabel, missionStyles.sectionLabel]}>Trame proposée</Text>
        {!recipe.sections.length ? (
          <Text style={{ color: COLORS.inkFaint, fontSize: 10.5, marginBottom: 16 }}>
            Mission libre : utilise les Points et Notes, ou complète la Mission plus tard.
          </Text>
        ) : null}
        {recipe.sections.map((section) => (
          <View
            key={section.key}
            style={[
              { backgroundColor: COLORS.white, borderWidth: 1, borderRadius: 15, padding: 14, marginBottom: 12 },
              missionStyles.card
            ]}
          >
            <Text style={{ color: MISSION_COLORS.accentStrong, fontSize: 13.5, fontWeight: '900', marginBottom: 12 }}>
              {section.label}
            </Text>
        ...
        <View
          style={[{ backgroundColor: COLORS.white, borderWidth: 1, borderRadius: 14, padding: 12 }, missionStyles.card]}
        >
          <View>
            <TextInput
              style={[styles.input, missionStyles.input, { minHeight: 76, textAlignVertical: 'top', paddingRight: 46 }]}
              multiline
              value={note}
              onChangeText={setNote}
              placeholder="Note terrain / réunion interne…"
            />
            <TouchableOpacity
              style={{
                position: 'absolute',
          ...
          <TouchableOpacity
            ...
            onPress={addNote}
          >
            <Text style={[styles.btnSecondaryText, missionStyles.secondaryButtonText]}>Ajouter la note</Text>
          </TouchableOpacity>
        </View>
        ...
        {(data.points || []).length ? (
          <>
            <Text style={[styles.sectionLabel, missionStyles.sectionLabel, { marginTop: 18 }]}>
              Points créés pendant cette visite
            </Text>
            {data.points.map((point) => (
              <View
                key={point.id}
                style={[
                  { backgroundColor: COLORS.white, borderWidth: 1, borderRadius: 12, padding: 11, marginBottom: 7 },
                  missionStyles.card
                ]}
              >
                <Text style={{ color: COLORS.ink, fontWeight: '800', fontSize: 11.5 }}>
                  {point.label || point.description || 'Point sans titre'}
                </Text>
                <Text style={{ color: MISSION_COLORS.accentDark, marginTop: 3, fontSize: 9.5 }}>
        ...
        <View style={styles.modalOverlay}>
          <View style={[styles.modalSheet, missionStyles.modalSheet]}>
            ...
            <Text style={{ color: COLORS.inkSoft, fontSize: 9.6, lineHeight: 14, marginBottom: 9 }}>
              Choisis seulement ce que tu connais. METRA reprend ensuite ce contexte dans les nouvelles données sans le
              redemander.
            </Text>
            <TextInput
            ...
            <ScrollView style={{ maxHeight: 390, marginTop: 8 }}>
              <TouchableOpacity
                onPress={() => {
                  clearTechnicalContext();
                  setContextModal(false);
                }}
                style={{ paddingVertical: 9, borderBottomWidth: 1, borderBottomColor: MISSION_COLORS.accentLine }}
              >
                <Text style={{ color: MISSION_COLORS.accentStrong, fontSize: 10.5, fontWeight: '900' }}>
                  Site entier · {visit.site_name || 'Site'}
                </Text>
                <Text style={{ color: COLORS.inkFaint, fontSize: 8.5, marginTop: 2 }}>
                  Aucun local / équipement imposé
                </Text>
              </TouchableOpacity>

              {filteredContextLocations.length ? (
                <Text style={[styles.sectionLabel, missionStyles.sectionLabel, { marginTop: 10 }]}>Locaux / zones</Text>
              ) : null}
              {filteredContextLocations.map((location) => (
                <TouchableOpacity
                  key={location.id}
                  ...
                  style={{
                    ...
                    backgroundColor: contextLocationId === location.id ? MISSION_COLORS.accentSoft : 'transparent'
                  }}
                >
                  <Text
                    style={{
                      color: COLORS.ink,
                      fontSize: 10.1,
                      fontWeight: contextLocationId === location.id ? '900' : '700'
                    }}
                  >
                    {location.label}
                  </Text>
                  <Text style={{ color: COLORS.inkFaint, fontSize: 8.3, marginTop: 2 }}>
                    {location.kind || 'localisation'}
                  </Text>
                </TouchableOpacity>
              ))}

              {filteredContextEquipment.length ? (
                <Text style={[styles.sectionLabel, missionStyles.sectionLabel, { marginTop: 10 }]}>
                  Équipements {contextLocationId ? 'du local sélectionné' : 'du site'}
                </Text>
              ) : null}
              {filteredContextEquipment.map((equipment) => (
                <TouchableOpacity
                  key={equipment.id}
                  ...
                  style={{
                    ...
                    borderBottomWidth: 1,
                    borderBottomColor: MISSION_COLORS.accentLine,
                    backgroundColor: contextEquipmentId === equipment.id ? MISSION_COLORS.accentSoft : 'transparent'
                  }}
                >
                  <Text
                    style={{
                      color: COLORS.ink,
                      fontSize: 10.1,
                      fontWeight: contextEquipmentId === equipment.id ? '900' : '700'
                    }}
                  >
            ...
            <View style={styles.modalActions}>
              <TouchableOpacity
                style={[styles.btnSecondary, missionStyles.secondaryButton]}
                onPress={clearTechnicalContext}
              >
                <Text style={[styles.btnSecondaryText, missionStyles.secondaryButtonText]}>Réinitialiser</Text>
              </TouchableOpacity>
              <TouchableOpacity
              ...
              >
                <Text style={[styles.btnPrimaryText, missionStyles.primaryButtonText]}>Utiliser ce contexte</Text>
              </TouchableOpacity>
            </View>
          </View>
        </View>
        ...
        <View style={styles.modalOverlay}>
          <View style={[styles.modalSheet, missionStyles.modalSheet]}>
            ...
            <Text style={{ color: COLORS.inkSoft, fontSize: 10, lineHeight: 14, marginBottom: 10 }}>
              METRA a détecté quelques points à vérifier. Cette liste est informative : elle ne bloque jamais la fin de
              visite.
            </Text>
            <ScrollView style={{ maxHeight: 340 }}>
              {checklist.map((item) => (
                <View
                  key={item.id}
                  style={{ borderBottomWidth: 1, borderBottomColor: MISSION_COLORS.accentLine, paddingVertical: 9 }}
                >
                  <TouchableOpacity onPress={() => openChecklistItem(item)} activeOpacity={0.78}>
                    <Text
                      style={{
                        color: item.severity === 'warning' ? '#8A5B14' : MISSION_COLORS.accentStrong,
                        fontSize: 10.5,
                        fontWeight: '900'
                      }}
                    >
                    ...
                    {item.message ? (
                      <Text style={{ color: COLORS.inkSoft, fontSize: 9.5, marginTop: 3 }}>{item.message}</Text>
                    ) : null}
                  </TouchableOpacity>
                  <TouchableOpacity
                    onPress={() => ignoreChecklistItem(item)}
                    style={{ alignSelf: 'flex-start', marginTop: 6, paddingVertical: 3 }}
                  >
                    <Text style={{ color: COLORS.inkFaint, fontSize: 8.8, fontWeight: '800' }}>
                      Ignorer pour cette fin de visite
                    </Text>
                  </TouchableOpacity>
                </View>
              ))}
              {!checklist.length ? (
                <Text style={{ color: COLORS.inkSoft, fontSize: 10, lineHeight: 14, paddingVertical: 12 }}>
                  Tous les points de vigilance ont été traités ou ignorés. Tu peux terminer la visite.
                </Text>
              ) : null}
            </ScrollView>
            <View style={styles.modalActions}>
              ...
              >
                <Text style={[styles.btnSecondaryText, missionStyles.secondaryButtonText]}>Revenir à la visite</Text>
              </TouchableOpacity>
              <TouchableOpacity style={[styles.btnPrimary, missionStyles.primaryButton]} onPress={reallyComplete}>
                <Text style={[styles.btnPrimaryText, missionStyles.primaryButtonText]}>Terminer quand même</Text>
              </TouchableOpacity>
            </View>
          </View>
        </View>
      ...
      <Modal visible={measureModal} transparent animationType="fade" onRequestClose={() => setMeasureModal(false)}>
        <View style={styles.modalOverlay}>
          <View style={[styles.modalSheet, missionStyles.modalSheet]}>
            ...
            <Text style={{ color: COLORS.inkSoft, fontSize: 10, lineHeight: 14, marginBottom: 10 }}>
              Le contexte Mission / site / visite est repris automatiquement. La référence est facultative et sert à
              calculer l'écart.
            </Text>
            <TextInput
              style={[styles.input, missionStyles.input]}
              ...
              placeholder="Type : température départ, débit, pression…"
            />
            <View style={{ flexDirection: 'row', gap: 8, marginTop: 9 }}>
              ...
              />
              <TextInput
                style={[styles.input, missionStyles.input, { width: 92 }]}
                value={measureUnit}
            ...
            <TextInput
              ...
              onChangeText={setMeasureReference}
              keyboardType="decimal-pad"
              placeholder="Référence attendue (facultatif)"
            />
            <View style={styles.modalActions}>
              <TouchableOpacity
                style={[styles.btnSecondary, missionStyles.secondaryButton]}
              ...
              <TouchableOpacity style={[styles.btnPrimary, missionStyles.primaryButton]} onPress={addMeasure}>
        ...
        <View style={styles.modalOverlay}>
          <View style={[styles.modalSheet, missionStyles.modalSheet]}>
            <Text style={[styles.modalTitle, missionStyles.title]}>
              {pointLabel ? 'Point · ' + pointLabel : 'Ajouter un point'}
            </Text>
            <View style={{ flexDirection: 'row', flexWrap: 'wrap', marginBottom: 6 }}>
              {POINT_TYPES.map(([key, label]) => (
                <TouchableOpacity
                  key={key}
                  onPress={() => setPointType(key)}
                  style={{
                    borderWidth: 1,
                    borderColor: pointType === key ? MISSION_COLORS.accent : MISSION_COLORS.accentLine,
                    backgroundColor: pointType === key ? MISSION_COLORS.accentLight : COLORS.white,
                    borderRadius: 10,
                    paddingHorizontal: 9,
                    paddingVertical: 7,
                    marginRight: 6,
                ...
                >
                  <Text
                    style={{
                      color: pointType === key ? MISSION_COLORS.accentDark : COLORS.ink,
                      fontSize: 9.5,
                      fontWeight: '800'
                    }}
                  >
            ...
            <TextInput
              ...
              placeholder="Titre / constat (optionnel)"
            />
            <View style={{ marginTop: 9 }}>
              <TextInput
                style={[
                  styles.input,
                  missionStyles.input,
                  { minHeight: 70, textAlignVertical: 'top', paddingRight: 46 }
                ]}
                multiline
            ...
            <View style={{ marginTop: 9 }}>
              <TextInput
                style={[styles.input, missionStyles.input, { paddingRight: 46 }]}
                value={pointRequestedAction}
                onChangeText={setPointRequestedAction}
                placeholder="Action demandée / suite"
              />
              <TouchableOpacity
            ...
            <View style={{ flexDirection: 'row', gap: 8, marginTop: 9 }}>
              <TextInput
                style={[styles.input, missionStyles.input, { flex: 1 }]}
                value={pointResponsible}
                onChangeText={setPointResponsible}
                placeholder="Responsable / entreprise"
              />
              <TextInput
                style={[styles.input, missionStyles.input, { flex: 1 }]}
                value={pointDue}
                onChangeText={setPointDue}
            ...
            <View style={{ flexDirection: 'row', gap: 8, marginTop: 9 }}>
              <TextInput
                style={[styles.input, missionStyles.input, { flex: 1 }]}
                value={pointPriority}
                onChangeText={setPointPriority}
                placeholder="Priorité"
              />
              <TextInput
                style={[styles.input, missionStyles.input, { flex: 1 }]}
                value={pointAllocation}
                onChangeText={setPointAllocation}
                placeholder="Imputation / lot"
              />
              <TextInput
                style={[styles.input, missionStyles.input, { width: 90 }]}
                value={pointCost}
            ...
            <View style={styles.modalActions}>
              <TouchableOpacity
                style={[styles.btnSecondary, missionStyles.secondaryButton]}
                onPress={() => setPointModal(false)}
              >
                <Text style={[styles.btnSecondaryText, missionStyles.secondaryButtonText]}>Annuler</Text>
              </TouchableOpacity>
              <TouchableOpacity style={[styles.btnPrimary, missionStyles.primaryButton]} onPress={addPoint}>
