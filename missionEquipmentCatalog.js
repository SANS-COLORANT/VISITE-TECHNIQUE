...
export const EQUIPMENT_CATEGORIES = Object.freeze([
  { key: 'boiler', label: 'Chaudière', group: 'Chauffage', aliases: ['chaudiere','chaudière','generator','generateur'], fields: [
    f('fuel', 'Énergie / combustible', 'choice', 'rapide', null, ['Gaz','Fioul','Électricité','Biomasse','Autre']),
  ...
  { key: 'burner', label: 'Brûleur', group: 'Chauffage', aliases: ['bruleur','brûleur','burner'], fields: [
  ...
  { key: 'pump', label: 'Pompe / circulateur', group: 'Hydraulique', aliases: ['pompe','circulateur','pump'], fields: [
  ...
  { key: 'heat_exchanger', label: 'Échangeur', group: 'Hydraulique', aliases: ['echangeur','échangeur','exchanger'], fields: [
  ...
  { key: 'valve', label: 'Vanne / organe hydraulique', group: 'Hydraulique', aliases: ['vanne','valve','clapet','robinet'], fields: [
    f('valveType', 'Type', 'choice', 'rapide', null, ['Isolement','Équilibrage','Régulation','Clapet','Mélangeuse','Autre']),
    f('dn', 'DN / dimension', 'text', 'rapide'),
    f('kv', 'Kv / Kvs', 'number', 'expert'),
    f('actuated', 'Motorisée', 'choice', 'standard', null, ['Oui','Non','Non vérifié']),
  ] },
  { key: 'expansion', label: 'Expansion / maintien pression', group: 'Hydraulique', aliases: ['vase','expansion','maintien pression'], fields: [
  ...
  { key: 'hydraulic_accessory', label: 'Organe hydraulique', group: 'Hydraulique', aliases: ['bouteille','separateur','séparateur','pot a boues','pot à boues','degazeur','dégazeur','disconnecteur'], fields: [
    f('accessoryType', 'Type d’organe', 'text', 'rapide'),
    f('dn', 'DN / dimension', 'text', 'standard'),
    f('volumeL', 'Volume', 'number', 'expert', 'L'),
  ] },
  { key: 'ecs_tank', label: 'Ballon / préparateur ECS', group: 'ECS', aliases: ['ballon','preparateur','préparateur','stockage ecs'], fields: [
  ...
  { key: 'mixing_valve', label: 'Mitigeur / régulation ECS', group: 'ECS', aliases: ['mitigeur','melangeur ecs','mélangeur ecs'], fields: [
    f('dn', 'DN', 'text', 'rapide'),
    f('setpointC', 'Consigne mélange', 'number', 'rapide', '°C'),
    f('rangeC', 'Plage de réglage', 'text', 'standard'),
  ] },
  { key: 'vmc_box', label: 'Caisson VMC', group: 'Ventilation', aliases: ['caisson vmc','vmc','extracteur','tourelle'], fields: [
    f('ventilationType', 'Type de ventilation', 'choice', 'rapide', null, ['Simple flux','Double flux','Extraction','Insufflation','Spécifique']),
  ...
  { key: 'cta', label: 'CTA', group: 'Ventilation', aliases: ['cta','centrale traitement air','centrale de traitement d air'], fields: [
    f('ctaType', 'Type CTA', 'choice', 'rapide', null, ['Simple flux','Double flux','Tout air neuf','Recyclage','Autre']),
  ...
  { key: 'fan', label: 'Ventilateur', group: 'Ventilation', aliases: ['ventilateur','fan'], fields: [
    f('airFlowM3h', 'Débit nominal', 'number', 'rapide', 'm³/h'),
    f('pressurePa', 'Pression disponible', 'number', 'standard', 'Pa'),
    f('motorPowerKw', 'Puissance moteur', 'number', 'standard', 'kW'),
    f('frequencyHz', 'Fréquence', 'number', 'expert', 'Hz'),
  ] },
  { key: 'filter', label: 'Filtre', group: 'Ventilation', aliases: ['filtre','filter'], fields: [
  ...
  { key: 'duct', label: 'Réseau aéraulique', group: 'Ventilation', aliases: ['gaine','conduit','duct'], fields: [
  ...
  { key: 'air_terminal', label: 'Bouche / diffuseur', group: 'Ventilation', aliases: ['bouche','diffuseur','grille','terminal air'], fields: [
  ...
  { key: 'heat_pump', label: 'PAC', group: 'Froid / Climatisation', aliases: ['pac','pompe a chaleur','pompe à chaleur'], fields: [
  ...
  { key: 'chiller', label: 'Groupe froid', group: 'Froid / Climatisation', aliases: ['groupe froid','chiller','groupe frigorifique'], fields: [
  ...
  { key: 'outdoor_unit', label: 'Unité extérieure', group: 'Froid / Climatisation', aliases: ['ue','unite exterieure','unité extérieure','outdoor unit'], fields: [
  ...
  { key: 'indoor_unit', label: 'Unité intérieure', group: 'Froid / Climatisation', aliases: ['ui','unite interieure','unité intérieure','cassette','split interieur','split intérieur'], fields: [
    f('indoorType', 'Type UI', 'choice', 'rapide', null, ['Murale','Cassette','Gainable','Console','Plafonnier','Autre']),
    f('coolingPowerKw', 'Puissance froid', 'number', 'standard', 'kW'),
    f('heatingPowerKw', 'Puissance chaud', 'number', 'standard', 'kW'),
    f('servedZone', 'Zone desservie', 'text', 'rapide'),
    f('condensatePump', 'Pompe relevage condensats', 'choice', 'expert', null, ['Oui','Non','Non vérifié']),
  ] },
  { key: 'fan_coil', label: 'Ventilo-convecteur', group: 'Froid / Climatisation', aliases: ['ventilo convecteur','ventilo-convecteur','vc'], fields: [
    f('pipeConfiguration', 'Configuration', 'choice', 'rapide', null, ['2 tubes','4 tubes','Détente directe','Autre']),
    f('heatingPowerKw', 'Puissance chaud', 'number', 'standard', 'kW'),
    f('coolingPowerKw', 'Puissance froid', 'number', 'standard', 'kW'),
    f('servedZone', 'Zone desservie', 'text', 'rapide'),
  ] },
  { key: 'rooftop', label: 'Rooftop', group: 'Froid / Climatisation', aliases: ['rooftop'], fields: [
    f('airFlowM3h', 'Débit air', 'number', 'rapide', 'm³/h'),
    f('heatingPowerKw', 'Puissance chaud', 'number', 'standard', 'kW'),
    f('coolingPowerKw', 'Puissance froid', 'number', 'rapide', 'kW'),
    f('refrigerant', 'Fluide', 'text', 'standard'),
  ] },
  { key: 'dry_cooler', label: 'Dry cooler / aéroréfrigérant', group: 'Froid / Climatisation', aliases: ['dry cooler','aerorefrigerant','aéroréfrigérant'], fields: [
  ...
  { key: 'plc', label: 'Automate / régulateur', group: 'Régulation / GTB', aliases: ['automate','regulateur','régulateur','plc'], fields: [
    f('controllerType', 'Type / fonction', 'text', 'rapide'),
    f('protocol', 'Protocole communication', 'choice', 'standard', null, ['BACnet','Modbus','KNX','Lon','M-Bus','Propriétaire','Autre']),
    f('ipAddress', 'Adresse / repère réseau', 'text', 'expert'),
    f('programAvailable', 'Programme / sauvegarde disponible', 'choice', 'expert', null, ['Oui','Non','À récupérer']),
  ] },
  { key: 'sensor', label: 'Sonde / capteur', group: 'Régulation / GTB', aliases: ['sonde','capteur','sensor'], fields: [
  ...
  { key: 'actuator', label: 'Servomoteur / actionneur', group: 'Régulation / GTB', aliases: ['servomoteur','actionneur','actuator'], fields: [
  ...
  { key: 'gateway', label: 'Passerelle / télégestion', group: 'Régulation / GTB', aliases: ['passerelle','gateway','telegestion','télégestion','modem'], fields: [
  ...
  { key: 'meter', label: 'Compteur', group: 'Comptage', aliases: ['compteur','meter'], fields: [
    f('meterType', 'Énergie / fluide', 'choice', 'rapide', null, ['Gaz','Électricité','Eau','Énergie thermique','Autre']),
  ...
  { key: 'water_treatment', label: 'Traitement d’eau', group: 'Traitement eau', aliases: ['adoucisseur','traitement eau','traitement d eau','filtre eau','doseur'], fields: [
  ...
  { key: 'other', label: 'Autre équipement', group: 'Autre', aliases: [], fields: [
    f('technicalData', 'Caractéristiques principales', 'text', 'standard'),
  ] },
]);
...
const MISSION_OVERLAYS = Object.freeze({
  ...
  opr_reception: [
    ...
    f('installedVsPlanned', 'Prévu ↔ installé', 'choice', 'rapide', null, ['Conforme à l’attendu','Différent','À contrôler']),
  ...
  diagnostic_gtb: [
    f('communicationStatus', 'Communication', 'choice', 'rapide', null, ['Communicant','Non communicant','Non testé']),
    f('accessStatus', 'Accès / supervision', 'choice', 'rapide', null, ['Accessible','Accès inconnu','Indisponible','Non testé']),
  ],
});
...
export function getEquipmentProfile({
  ...
  const fields = uniqueFields([...COMMON_FIELDS, ...(category.fields || []), ...overlay])
    .filter((item) => (LEVEL_RANK[item.level] ?? 1) <= maxRank);
  return {
    category,
    mode,
    fields,
