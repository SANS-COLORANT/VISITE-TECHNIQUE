...
function SectionPreview({ section }) {
  ...
  return <View style={{ marginTop: 7 }}>
    {tableCount ? <Text style={{ color: COLORS.inkFaint, fontSize: 9.5 }}>{tableCount} tableau(x) · {rowCount} ligne(s) issus des données structurées</Text> : null}
    {section.content_text ? <Text style={{ color: COLORS.inkSoft, fontSize: 10.5, lineHeight: 15, marginTop: 5 }} numberOfLines={3}>{section.content_text}</Text> : null}
  </View>;
}
  ...
  return <View style={{ flex: 1, backgroundColor: 'transparent' }}>
    <FlatList
      ...
      ListHeaderComponent={<View>
        <Text style={[styles.sectionTitle, missionStyles.title]}>{report?.profile?.label || 'Rapport Mission'}</Text>
        <Text style={{ color: COLORS.inkSoft, fontSize: 10.5, lineHeight: 15, marginBottom: 12 }}>
          Le rapport est une présentation des données structurées. Vous pouvez modifier les titres, ajouter du texte, masquer ou déplacer les sections sans altérer les constats, mesures, actions et essais d’origine.
        </Text>
      ...
      renderItem={({ item, index }) => {
        ...
        return <View style={[missionStyles.card, { marginBottom: 10, opacity: hidden ? 0.48 : 1 }]}>
          ...
          <View style={{ flexDirection: 'row', flexWrap: 'wrap', marginTop: 9, gap: 7 }}>
            ...
            <TouchableOpacity onPress={() => move(item, 1)} disabled={index === (report?.sections?.length || 1) - 1} style={{ padding: 6 }}><Text style={{ color: COLORS.inkSoft }}>↓</Text></TouchableOpacity>
            <TouchableOpacity onPress={() => toggle(item)} style={{ padding: 6 }}><Text style={{ color: COLORS.inkSoft }}>{hidden ? 'Afficher' : 'Masquer'}</Text></TouchableOpacity>
            <TouchableOpacity onPress={() => remove(item)} style={{ padding: 6 }}><Text style={{ color: '#8B3A3A' }}>Supprimer</Text></TouchableOpacity>
          </View>
        </View>;
      }}
      ListEmptyComponent={<Text style={{ color: COLORS.muted }}>Aucune section. Utilisez « Actualiser les données ».</Text>}
    />
    ...
    <Modal visible={!!editing} transparent animationType="fade" onRequestClose={() => setEditing(null)}>
      <View style={styles.modalOverlay}><View style={[styles.modalSheet, missionStyles.modalSheet]}>
        ...
        <TextInput style={[styles.input, missionStyles.input]} value={draftTitle} onChangeText={setDraftTitle} placeholder="Titre" />
        <TextInput
        ...
        <Text style={{ color: COLORS.inkFaint, fontSize: 9, lineHeight: 13, marginTop: 7 }}>
          Les tableaux issus des mesures, points, actions ou scénarios restent liés aux données structurées. Modifiez ces données à leur source pour conserver une seule vérité.
        </Text>
