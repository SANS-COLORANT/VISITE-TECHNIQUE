  ...
  return (
    <TouchableOpacity
      ...
      style={{
        ...
        backgroundColor: active ? MISSION_COLORS.accentLight : COLORS.white,
    ...
    >
      <Text style={{ color: active ? MISSION_COLORS.accentDark : COLORS.ink, fontSize: 10.5, fontWeight: '800' }}>
        {label}
      </Text>
    </TouchableOpacity>
  );
...
function PhaseRail({ phases = [] }) {
  if (!phases.length)
    return (
      <Text style={{ color: COLORS.inkFaint, fontSize: 10.5 }}>Aucune phase imposée. La Mission reste libre.</Text>
    );
  return (
        ...
        <View key={phase.id} style={{ flexDirection: 'row', alignItems: 'center' }}>
          <View
            style={{
              ...
              borderRadius: 12,
              borderWidth: 1,
              borderColor: phase.status === 'done' ? MISSION_COLORS.accent : MISSION_COLORS.accentLine,
              backgroundColor: phase.status === 'done' ? MISSION_COLORS.accentLight : COLORS.white,
          ...
          >
            <Text
              style={{
                color: phase.status === 'done' ? MISSION_COLORS.accentDark : COLORS.ink,
                fontWeight: '800',
                fontSize: 10.5
              }}
              numberOfLines={2}
          ...
          {index < phases.length - 1 ? (
            <Text style={{ marginHorizontal: 5, color: MISSION_COLORS.accentLineStrong }}>→</Text>
          ) : null}
        </View>
      ))}
    </ScrollView>
...
export function MissionScreen({ navigation, route }) {
  ...
  const reload = useCallback(async () => {
    ...
    try {
      const [dashboard, summary] = await Promise.all([
        getMissionDashboard(missionId),
        getMissionDomainSummary(missionId)
      ]);
      setData(dashboard);
  ...
  const openPoints = useMemo(
    () => (data?.points || []).filter((p) => !['closed', 'no_follow_up'].includes(p.status)),
    [data?.points]
  );
  ...
  const playbook = useMemo(() => getMissionFieldPlaybook(data?.mission?.type), [data?.mission?.type]);
  ...
  const createVisit = async () => {
    try {
      ...
      Alert.alert('Visite non créée', String(e.message || e));
    }
  };
  ...
  const closePoint = async (point) => {
    try {
      ...
      Alert.alert('Point non modifié', String(e.message || e));
    }
  };
  ...
  return (
    <View style={[{ flex: 1 }, missionStyles.screen]}>
      <ScrollView contentContainerStyle={{ padding: 16, paddingBottom: 120 }}>
        <View
          style={[{ backgroundColor: COLORS.white, borderRadius: 18, borderWidth: 1, padding: 16 }, missionStyles.card]}
        >
          <Text style={[{ fontWeight: '900', fontSize: 20 }, missionStyles.title]}>
            {mission.label || 'Mission sans titre'}
          </Text>
          <Text style={{ color: COLORS.inkSoft, marginTop: 4, fontSize: 11.5 }}>
            {[mission.client_name, sites?.[0]?.name].filter(Boolean).join(' · ') || 'Contexte à compléter'}
          </Text>
          <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 8, marginTop: 13 }}>
            <Text style={{ color: MISSION_COLORS.accentDark, fontSize: 10.5, fontWeight: '700' }}>
              {visits.length} visite(s)
            </Text>
            <Text style={{ color: MISSION_COLORS.accentDark, fontSize: 10.5, fontWeight: '700' }}>
              {openPoints.length} point(s) ouvert(s)
            </Text>
            <Text style={{ color: MISSION_COLORS.accentDark, fontSize: 10.5, fontWeight: '700' }}>
        ...
        <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 8 }}>
          {[
            ...
            <View
              key={label}
              style={[{ minWidth: 104, flexGrow: 1, borderRadius: 13, padding: 10 }, missionStyles.statBox]}
            >
        ...
        <View style={[{ marginTop: 16, borderRadius: 14, padding: 13 }, missionStyles.infoBox]}>
          ...
          <Text style={{ color: COLORS.inkSoft, fontSize: 10.5, lineHeight: 15, marginTop: 4 }}>
            Pendant une visite, tu peux laisser n’importe quelle rubrique vide, quitter l’application et reprendre plus
            tard. La progression est informative uniquement.
          </Text>
        </View>

        <Text style={[styles.sectionLabel, missionStyles.sectionLabel, { marginTop: 18 }]}>
          Parcours recommandé · {playbook.label}
        </Text>
        ...
        <View style={{ flexDirection: 'row', gap: 9 }}>
          <TouchableOpacity
            ...
            onPress={createVisit}
          >
            <Text style={[styles.btnPrimaryText, missionStyles.primaryButtonText]}>Démarrer une visite</Text>
          </TouchableOpacity>
          <TouchableOpacity
        ...
        <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 8 }}>
          {[
            ...
            [
              ...
              'Installation · système · réseau · équipement'
            ],
            ...
            [
              ...
              'Cockpit multi-sites',
              'À faire · en cours · terminés · accès'
            ],
            ...
            [
              ...
              'Ouvrages · essais · réserves · documents'
            ],
            [
              'expertiseBoard',
              'MissionExpertise',
              'Expertise / sinistre',
              'Faits → hypothèses → investigations → conclusion'
            ],
            [
              'reserveClearance',
              'MissionReserveClearance',
              'Levée de réserves',
              'Avant / après · levée · maintien · partielle'
            ],
            ...
            .map(([capability, routeName, title, subtitle]) => (
              <TouchableOpacity
                key={routeName}
                activeOpacity={0.82}
        ...
        <View style={{ flexDirection: 'row', gap: 9, marginTop: 9 }}>
          ...
          >
            <Text style={[styles.btnSecondaryText, missionStyles.secondaryButtonText]}>
              {exporting ? 'Export…' : '⇩ Excel complet'}
            </Text>
          </TouchableOpacity>
        </View>
        <TouchableOpacity
        ...
        <Text style={{ color: COLORS.inkFaint, fontSize: 9, lineHeight: 13, marginTop: 6, textAlign: 'center' }}>
          Excel complet = réimportable dans METRA. Excel client = lecture directe : synthèse, actions, réserves,
          inventaire, mesures et documents.
        </Text>
        ...
        {points.length ? (
          points.map((point) => (
            <View
              key={point.id}
              style={[
                { backgroundColor: COLORS.white, borderWidth: 1, borderRadius: 14, padding: 13, marginBottom: 9 },
                missionStyles.card
              ]}
            >
              <View style={{ flexDirection: 'row', alignItems: 'flex-start', gap: 10 }}>
                <View style={{ flex: 1 }}>
                  <Text style={{ color: COLORS.ink, fontWeight: '900', fontSize: 12.5 }}>
                    {point.label || point.description || 'Point sans titre'}
                  </Text>
                  <Text style={{ color: COLORS.inkSoft, fontSize: 10, marginTop: 4 }}>
                    {POINT_STATUS[point.status] || point.status} · {point.type || 'information'}
                    {point.due_text ? ` · ${point.due_text}` : ''}
                  </Text>
                </View>
                {!['closed', 'no_follow_up'].includes(point.status) ? (
                  <TouchableOpacity
                    onPress={() => closePoint(point)}
                    style={{
                      paddingHorizontal: 9,
                      paddingVertical: 7,
                      borderRadius: 9,
                      backgroundColor: MISSION_COLORS.accentLight
                    }}
                  >
                    <Text style={{ color: MISSION_COLORS.accentDark, fontSize: 9.5, fontWeight: '900' }}>Clôturer</Text>
                  </TouchableOpacity>
                ) : null}
              </View>
            </View>
          ...
          <Text style={{ color: COLORS.inkFaint, fontSize: 10.5 }}>
            Aucun point. Tu peux en ajouter à tout moment, même s’il n’était pas prévu dans la trame.
          </Text>
        )}
        ...
        {visits.length ? (
          visits.map((visit) => (
            <TouchableOpacity
              key={visit.id}
              onPress={() => navigation.navigate('MissionVisit', { missionId, visitId: visit.id })}
              style={[
                { backgroundColor: COLORS.white, borderWidth: 1, borderRadius: 13, padding: 12, marginBottom: 8 },
                missionStyles.card
              ]}
            >
              <Text style={{ color: COLORS.ink, fontWeight: '800', fontSize: 11.5 }}>
                {visit.visit_date || 'Date à préciser'} · {visit.visit_type || 'Visite'}
              </Text>
              <Text style={{ color: MISSION_COLORS.accentDark, fontSize: 9.5, marginTop: 3 }}>
                {visit.status === 'draft' ? 'Brouillon — reprise possible' : visit.status}
              </Text>
            </TouchableOpacity>
          ))
        ) : (
          <Text style={{ color: COLORS.inkFaint, fontSize: 10.5 }}>Aucune visite créée.</Text>
        )}
      </ScrollView>
      ...
      <Modal visible={pointModal} transparent animationType="fade" onRequestClose={() => setPointModal(false)}>
        <View style={styles.modalOverlay}>
          <View style={[styles.modalSheet, missionStyles.modalSheet]}>
            ...
            <Text style={{ color: COLORS.inkSoft, fontSize: 10.5, marginBottom: 10 }}>
              Un point ajouté librement a la même valeur qu’un point proposé par une recette.
            </Text>
            <View style={{ flexDirection: 'row', flexWrap: 'wrap', marginBottom: 5 }}>
              {POINT_TYPES.map(([key, labelText]) => (
                <Pill key={key} active={pointType === key} label={labelText} onPress={() => setPointType(key)} />
              ))}
            </View>
            <TextInput
              style={[styles.input, missionStyles.input, { marginTop: 6 }]}
              value={pointLabel}
              onChangeText={setPointLabel}
              placeholder="Titre / constat (peut rester vide)"
            />
            <TextInput
              style={[styles.input, missionStyles.input, { marginTop: 9, minHeight: 72, textAlignVertical: 'top' }]}
              multiline
            ...
            <TextInput
              ...
              value={pointDueText}
              onChangeText={setPointDueText}
              placeholder="Échéance libre : fin octobre, prochaine visite…"
            />
            <View style={styles.modalActions}>
              ...
              <TouchableOpacity style={[styles.btnPrimary, missionStyles.primaryButton]} onPress={createPoint}>
