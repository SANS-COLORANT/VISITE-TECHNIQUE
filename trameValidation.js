...
export function validerDefinitionTrame(trame) {
  ...
  if (!trame || typeof trame !== 'object')
    return { ok: false, erreurs: ['Définition de trame absente'], avertissements };
  if (!String(trame.id || '').trim()) erreurs.push(erreur(trame, 'id requis'));
  if (!String(trame.nom || '').trim()) erreurs.push(erreur(trame, 'nom requis'));
  if (!Number.isInteger(Number(trame.version)) || Number(trame.version) < 1)
    erreurs.push(erreur(trame, 'version entière >= 1 requise'));
  ...
  for (const id of tabs) {
    if (!panels[id] && !specials.has(id))
      erreurs.push(erreur(trame, `onglet « ${id} » absent de ui.panels et ui.specialPanels`));
  ...
  if (!(cfg.requiredSheets || []).includes(cfg.mainSheet))
    erreurs.push(erreur(trame, 'la feuille principale doit être incluse dans requiredSheets'));
  ...
  for (const mapping of cfg.fieldMappings || []) {
    ...
    if (!['champ', 'controle'].includes(mapping.type))
      erreurs.push(erreur(trame, `type invalide pour « ${identite} »`));
    ...
    if (cellules.has(cellKey))
      avertissements.push(
        erreur(trame, `cellule Excel partagée « ${cellKey} » entre « ${cellules.get(cellKey)} » et « ${identite} »`)
      );
    else cellules.set(cellKey, identite);
    if (mapping.type === 'controle' && !estCelluleExcel(mapping.commentCell))
      erreurs.push(erreur(trame, `commentCell invalide pour le contrôle « ${identite} »`));
  }
  ...
  for (const [nom, table] of Object.entries(cfg.tables || {})) {
    ...
    if (nom !== 'note' && !(table.columns || []).length)
      erreurs.push(erreur(trame, `colonnes d’import absentes pour la table « ${nom} »`));
  ...
  if (overflow) {
    ...
    for (const c of overflow.columns || []) {
      ...
      if (!c.importKey || !c.exportKey)
        erreurs.push(erreur(trame, 'mapping import/export incomplet dans overflow réseaux'));
    }
  }
