...
export function MissionTechnicalStructureScreen({ navigation, route }) {
  ...
  const [data, setData] = useState({ locations: [], installations: [], systems: [], networks: [], equipment: [], components: [] });
  const [createVisible, setCreateVisible] = useState(false);
  const [createDraft, setCreateDraft] = useState({ kind: 'installation', label: '', type: '', locationId: '', installationId: '', systemId: '' });
  ...
  const equipmentFor = useCallback((installationId, systemId = null, networkId = null, directOnly = false) => {
    return (data.equipment || []).filter((equipment) => {
      ...
      if (installationId) return equipment.installation_id === installationId && (!directOnly || !equipment.system_id);
      return !equipment.installation_id;
    });
  }, [data.equipment]);

  const componentsCount = useCallback((equipmentId) => (
    (data.components || []).filter((component) => component.equipment_id === equipmentId).length
  ), [data.components]);
  ...
  const openCreateSystem = (installation) => {
    setCreateDraft({ kind: 'system', label: '', type: '', locationId: installation.location_id || '', installationId: installation.id, systemId: '' });
    setCreateVisible(true);
  };
  ...
  const candidateEquipment = useMemo(() => {
    ...
    return (data.equipment || [])
      .filter((equipment) => !q || [equipment.type,equipment.brand,equipment.model,equipment.location_label].some((value) => String(value || '').toLowerCase().includes(q)))
      .slice(0, 250);
  ...
  const EquipmentBadge = ({ equipment, allowUnlink = false }) => <View style={{ flexDirection: 'row', alignItems: 'center', paddingVertical: 5 }}>
    <TouchableOpacity
      style={{ flex: 1 }}
  ...
  return <View style={{ flex: 1, backgroundColor: 'transparent' }}>
    <ScrollView contentContainerStyle={{ padding: 16, paddingBottom: 120 }}>
      ...
      <Text style={{ color: COLORS.inkSoft, fontSize: 10.5, lineHeight: 15 }}>
        Structure fonctionnelle : Installation → Système → Réseau / circuit → Équipement. Elle complète la hiérarchie physique Bâtiment → Niveau → Local sans la dupliquer.
      </Text>

      <Text style={[styles.sectionLabel, missionStyles.sectionLabel, { marginTop: 14 }]}>Site</Text>
      <ScrollView horizontal showsHorizontalScrollIndicator={false}>
        {sites.map((site) => <Chip key={site.id} label={site.name} selected={siteId === site.id} onPress={() => setSiteId(site.id)} />)}
      </ScrollView>
      ...
      {(data.installations || []).map((installation) => {
        ...
        return <View key={installation.id} style={[missionStyles.card, { padding: 12, marginTop: 12 }]}>
          ...
          <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 6, marginTop: 8 }}>
            <TouchableOpacity style={[styles.btnSecondary, missionStyles.secondaryButton]} onPress={() => openCreateSystem(installation)}><Text style={[styles.btnSecondaryText, missionStyles.secondaryButtonText]}>＋ Système</Text></TouchableOpacity>
            <TouchableOpacity style={[styles.btnSecondary, missionStyles.secondaryButton]} onPress={() => openCreateNetwork(installation)}><Text style={[styles.btnSecondaryText, missionStyles.secondaryButtonText]}>＋ Réseau direct</Text></TouchableOpacity>
            <TouchableOpacity style={[styles.btnSecondary, missionStyles.secondaryButton]} onPress={() => openAssign({ installationId: installation.id, label: installation.label })}><Text style={[styles.btnSecondaryText, missionStyles.secondaryButtonText]}>＋ Équipement</Text></TouchableOpacity>
          </View>

          {directEquipment.length ? <View style={{ marginTop: 8 }}>
            <Text style={[styles.sectionLabel, missionStyles.sectionLabel]}>Équipements directs</Text>
            {directEquipment.map((equipment) => <EquipmentBadge key={equipment.id} equipment={equipment} allowUnlink />)}
          </View> : null}
          ...
          {systems.map((system) => {
            ...
            return <View key={system.id} style={{ marginTop: 10, padding: 10, borderWidth: 1, borderColor: MISSION_COLORS.accentLine, borderRadius: 11, backgroundColor: 'rgba(255,255,255,0.82)' }}>
              ...
              <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 6, marginTop: 6 }}>
                <TouchableOpacity style={[styles.btnSecondary, missionStyles.secondaryButton]} onPress={() => openCreateNetwork(installation, system)}><Text style={[styles.btnSecondaryText, missionStyles.secondaryButtonText]}>＋ Réseau</Text></TouchableOpacity>
                <TouchableOpacity style={[styles.btnSecondary, missionStyles.secondaryButton]} onPress={() => openAssign({ installationId: installation.id, systemId: system.id, label: system.label })}><Text style={[styles.btnSecondaryText, missionStyles.secondaryButtonText]}>＋ Équipement</Text></TouchableOpacity>
              </View>

              {systemEquipment.map((equipment) => <EquipmentBadge key={equipment.id} equipment={equipment} allowUnlink />)}
      ...
      {!data.installations?.length && siteId ? <View style={[missionStyles.card, { padding: 13, marginTop: 12 }]}>
        <Text style={{ color: COLORS.inkSoft, fontSize: 9.7 }}>Aucune architecture technique créée. Commence par une installation : Chaufferie, Production ECS, CTA, Climatisation, GTB…</Text>
      </View> : null}

      {unassigned.length ? <View style={{ marginTop: 16 }}>
        <Text style={[styles.sectionLabel, missionStyles.sectionLabel]}>Équipements non rattachés · {unassigned.length}</Text>
        <View style={[missionStyles.card, { padding: 10 }]}>
          {unassigned.slice(0, 40).map((equipment) => <EquipmentBadge key={equipment.id} equipment={equipment} />)}
          {unassigned.length > 40 ? <Text style={{ color: COLORS.inkFaint, fontSize: 8.5, marginTop: 6 }}>+ {unassigned.length - 40} autre(s). Utilise une installation pour les rattacher sans charger toute la liste à l’écran.</Text> : null}
        </View>
      </View> : null}
    </ScrollView>
    ...
    <Modal visible={createVisible} transparent animationType="fade" onRequestClose={() => setCreateVisible(false)}>
      <View style={styles.modalOverlay}><ScrollView style={[styles.modalSheet, missionStyles.modalSheet]} contentContainerStyle={{ paddingBottom: 16 }}>
        <Text style={[styles.modalTitle, missionStyles.title]}>
          {createDraft.kind === 'installation' ? 'Nouvelle installation' : createDraft.kind === 'system' ? 'Nouveau système' : 'Nouveau réseau / circuit'}
        </Text>
        <TextInput style={[styles.input, missionStyles.input]} value={createDraft.label} onChangeText={(v) => setCreateDraft((p) => ({ ...p, label: v }))} placeholder={createDraft.kind === 'installation' ? 'Production chauffage, ECS, CTA…' : createDraft.kind === 'system' ? 'Circuit radiateurs Nord, Bouclage ECS…' : 'Départ Nord, Retour ECS, Air neuf…'} autoFocus />
        <TextInput style={[styles.input, missionStyles.input, { marginTop: 8 }]} value={createDraft.type} onChangeText={(v) => setCreateDraft((p) => ({ ...p, type: v }))} placeholder="Type / fonction (facultatif)" />
        {createDraft.kind === 'installation' ? <>
          <Text style={[styles.sectionLabel, missionStyles.sectionLabel, { marginTop: 10 }]}>Localisation physique (facultatif)</Text>
          <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{ maxHeight: 44 }}>
            {(data.locations || []).map((location) => <Chip key={location.id} compact label={location.label} selected={createDraft.locationId === location.id} onPress={() => setCreateDraft((p) => ({ ...p, locationId: p.locationId === location.id ? '' : location.id }))} />)}
          </ScrollView>
        </> : null}
    ...
    <Modal visible={!!assignTarget} transparent animationType="fade" onRequestClose={() => setAssignTarget(null)}>
      <View style={styles.modalOverlay}><View style={[styles.modalSheet, missionStyles.modalSheet]}>
        ...
        <Text style={{ color: COLORS.inkSoft, fontSize: 9.4, lineHeight: 14, marginBottom: 8 }}>{assignTarget?.label || 'Objet technique'}</Text>
