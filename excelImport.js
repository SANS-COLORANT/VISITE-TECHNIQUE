...
export function analyserClasseur(wb, nomFichier) {
  ...
  const reseauxPrincipaux = reseauxCfg
    ? (reseauxCfg.starts || [])
        .map((row, index) => {
  ...
  const materiel = materielCfg
    ? lireTable(wb.Sheets[materielCfg.sheet], materielCfg).map((m) => ({ ...m, etat: m.etat || 'Bon' }))
    : [];
...
export async function importerAnalyseExcel(analyse) {
  ...
  await db
    .withTransactionAsync(async () => {
      ...
      if (!client) {
        client = { id: uuidv4() };
        await db.runAsync('INSERT INTO clients (id, nom) VALUES (?, ?)', [
          client.id,
          String(analyse.client || 'Client importé').trim()
        ]);
      }
      ...
      if (!site) {
        ...
        await db.runAsync('INSERT INTO sites (id, client_id, nom_site, adresse) VALUES (?, ?, ?, ?)', [
          site.id,
          client.id,
          String(analyse.site || 'Site importé').trim(),
          analyse.adresse || null
        ]);
      }
      ...
      let installation = await db.getFirstAsync(
        'SELECT id FROM installations WHERE site_id = ? AND actif = 1 LIMIT 1',
        [site.id]
      );
      if (!installation) {
      ...
      for (const r of analyse.remarques) {
        await db.runAsync(
          ...
          [
            ...
            r.prestation,
            Number(r.delai) || null,
            Number(String(r.estimatif).replace(',', '.')) || null
          ]
        );
      }
