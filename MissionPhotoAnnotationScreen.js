...
function Annotation({ row, width, height }) {
  ...
  if (row.annotation_type === 'circle') {
    return (
      <Circle
        cx={Number(g.x || 0) * width}
        cy={Number(g.y || 0) * height}
        r={Number(g.r || 0.08) * Math.min(width, height)}
        fill="rgba(47,125,88,0.08)"
        stroke={color}
        strokeWidth={3}
      />
    );
  ...
  if (row.annotation_type === 'arrow') {
    const x1 = Number(g.x1 || 0) * width,
      y1 = Number(g.y1 || 0) * height;
    const x2 = Number(g.x2 || 0) * width,
      y2 = Number(g.y2 || 0) * height;
    ...
    return (
      <React.Fragment>
        ...
        <Line
          x1={x2}
          y1={y2}
          x2={x2 - size * Math.cos(angle - Math.PI / 6)}
          y2={y2 - size * Math.sin(angle - Math.PI / 6)}
          stroke={color}
          strokeWidth={4}
        />
        <Line
  ...
  if (row.annotation_type === 'text') {
    return (
      <SvgText x={Number(g.x || 0) * width} y={Number(g.y || 0) * height} fill={color} fontSize="15" fontWeight="700">
        {String(row.text || 'Texte').slice(0, 50)}
      </SvgText>
    );
  }
  return null;
    ...
    <View style={{ flex: 1, backgroundColor: MISSION_COLORS.bg }}>
      <ScrollView contentContainerStyle={{ padding: 16, paddingBottom: 110 }}>
        ...
        <Text style={{ color: COLORS.inkSoft, fontSize: 10.5, lineHeight: 15 }}>
          L’original reste intact. Les flèches, cercles, zones et textes sont stockés en couche séparée et restent
          modifiables.
        </Text>
        <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 7, marginTop: 11 }}>
          <TouchableOpacity
            style={[styles.btnSecondary, missionStyles.secondaryButton]}
            disabled={exportingAlbum}
            onPress={() => exportAlbum('all')}
          >
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false}>
          {photos.map((p, index) => (
            <TouchableOpacity
              key={p.id}
              onPress={() => choosePhoto(p)}
              style={{
                width: 108,
                marginRight: 7,
                borderWidth: 2,
                borderColor: p.id === selectedId ? MISSION_COLORS.accent : 'transparent',
                borderRadius: 10,
                overflow: 'hidden'
              }}
            >
              <Image
                source={{ uri: p.thumbnail_uri || p.preview_uri || p.file_uri }}
                style={{ width: 104, height: 76 }}
                resizeMode="cover"
              />
              <Text style={{ color: COLORS.inkSoft, fontSize: 8, padding: 4 }} numberOfLines={1}>
                {p.label || 'Photo ' + (index + 1)}
              </Text>
            </TouchableOpacity>
          ))}
        </ScrollView>
          ...
          <>
              ...
              <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                ...
                <TouchableOpacity
                  ...
                  style={{
                    minWidth: 74,
                    borderWidth: 1,
                    borderColor: ['report', 'client'].includes(String(selected.visibility || ''))
                      ? MISSION_COLORS.accent
                      : MISSION_COLORS.accentLineStrong,
                    backgroundColor: ['report', 'client'].includes(String(selected.visibility || ''))
                      ? MISSION_COLORS.accentLight
                      : '#FFFFFF',
                    borderRadius: 10,
                    paddingHorizontal: 9,
                    paddingVertical: 8,
            ...
            <View style={{ flexDirection: 'row', flexWrap: 'wrap' }}>
              ...
              <TouchableOpacity style={[styles.btnSecondary, missionStyles.secondaryButton]} onPress={deleteLast}>
                <Text style={[styles.btnSecondaryText, missionStyles.secondaryButtonText]}>↶ Dernière</Text>
              </TouchableOpacity>
            </View>
            <Text style={{ color: COLORS.inkFaint, fontSize: 9, marginBottom: 7 }}>
              {tool === 'arrow' || tool === 'zone'
                ? 'Place deux points.'
                : tool === 'text'
                  ? 'Touchez la position du texte.'
                  : tool === 'circle'
                    ? 'Touchez la zone à entourer.'
                    : 'Consultez les annotations.'}
            </Text>
            ...
            <View style={{ alignItems: 'center' }}>
              ...
              >
                <Image
                  source={{ uri: selected.file_uri }}
                  style={{ width: size.width, height: size.height }}
                  resizeMode="stretch"
                />
                <Svg width={size.width} height={size.height} style={{ position: 'absolute', left: 0, top: 0 }}>
                  {annotations.map((a) => (
                    <Annotation key={a.id} row={a} width={size.width} height={size.height} />
                  ))}
                  {draft.map((p, i) => (
                    <Circle
                      key={i}
                  ...
                  {draft.length >= 2 ? (
                    <Line
                      x1={draft[0].x * size.width}
                      y1={draft[0].y * size.height}
                      x2={draft[1].x * size.width}
                      y2={draft[1].y * size.height}
                      stroke={MISSION_COLORS.accent}
                      strokeDasharray="5,4"
                      strokeWidth={2}
                    />
            ...
            {annotations.map((a, index) => (
              <View key={a.id} style={[missionStyles.card, { padding: 9, marginBottom: 5 }]}>
                <Text style={{ color: COLORS.ink, fontSize: 9.8, fontWeight: '800' }}>
                  {index + 1}. {a.annotation_type}
                  {a.text ? ' · ' + a.text : ''}
                </Text>
              </View>
            ))}
