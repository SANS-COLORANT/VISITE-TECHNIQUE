...
function ZoomablePhoto({ photo, onPrevious, onNext }) {
  const surface = useRef(null),
    viewport = useRef({ width: 1, height: 1, left: 0, top: 0 });
  ...
  const apply = (next) => {
    const v = viewport.current,
      p = current.current.photo;
    const imageWidth = Number(p.largeurPixels || v.width),
      imageHeight = Number(p.hauteurPixels || v.height);
    const fit = Math.min(v.width / imageWidth, v.height / imageHeight);
    const z = clamp(next.scale, 1, 8);
    const maxX = Math.max(0, (imageWidth * fit * z - v.width) / 2),
      maxY = Math.max(0, (imageHeight * fit * z - v.height) / 2);
    pose.current = { scale: z, x: clamp(next.x || 0, -maxX, maxX), y: clamp(next.y || 0, -maxY, maxY) };
    scale.setValue(z);
    translateX.setValue(pose.current.x);
    translateY.setValue(pose.current.y);
    setZoom(z);
  };
  const reset = () => {
  ...
  const responder = useRef(
    PanResponder.create({
      ...
      onPanResponderGrant: (e) => {
        surface.current?.measureInWindow((left, top) => {
          viewport.current = { ...viewport.current, left, top };
        });
        begin(e);
      },
      onPanResponderMove: (e) => {
        ...
        if (!start || start.mode !== point.mode) {
          const multi = start?.multi || point.mode === 'pinch';
          begin(e);
          if (gesture.current) gesture.current.multi = multi;
          return;
        }
        if (point.mode === 'pinch') {
          const z = clamp((start.pose.scale * point.distance) / start.distance, 1, 8),
            ratio = z / start.pose.scale;
          const v = viewport.current,
            cx = v.left + v.width / 2,
            cy = v.top + v.height / 2;
          apply({
            scale: z,
            x: point.x - cx - (start.x - cx - start.pose.x) * ratio,
            y: point.y - cy - (start.y - cy - start.pose.y) * ratio
          });
        } else if (pose.current.scale > 1)
          apply({ scale: start.pose.scale, x: start.pose.x + point.x - start.x, y: start.pose.y + point.y - start.y });
      },
      onPanResponderRelease: (e, g) => {
        ...
        if (!wasMulti && pose.current.scale === 1 && Math.abs(g.dx) > 70 && Math.abs(g.dx) > Math.abs(g.dy) * 1.5) {
          if (g.dx < 0) current.current.onNext?.();
          else current.current.onPrevious?.();
        } else if (!wasMulti && Math.abs(g.dx) < 6 && Math.abs(g.dy) < 6) {
          const now = Date.now();
          if (now - lastTap.current < 300) {
            apply({ scale: pose.current.scale > 1 ? 1 : 2 });
            lastTap.current = 0;
  ...
  return (
    <>
      <View
        ref={surface}
        onLayout={(e) => {
          viewport.current = { ...viewport.current, ...e.nativeEvent.layout };
          reset();
        }}
      ...
      >
        {failed ? (
          <Text style={{ color: '#FFF', padding: 20 }}>Cette image ne peut pas être affichée.</Text>
        ) : (
          <Animated.Image
      ...
      <View style={{ flexDirection: 'row', justifyContent: 'center', alignItems: 'center', gap: 14 }}>
        <ViewerButton
          ...
          onPress={() => apply({ ...pose.current, scale: pose.current.scale / 1.5 })}
          disabled={zoom <= 1}
        />
...
export function ReferencePhotoViewer({ photos, photoId, onClose, onSelect }) {
  ...
  return (
    <Modal visible={Boolean(photo)} animationType="fade" onRequestClose={onClose}>
      <SafeAreaView style={{ flex: 1, backgroundColor: '#101820' }}>
        <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
          <Text style={{ color: '#FFF', paddingLeft: 16, fontWeight: '700' }}>
            Référence Intranet · {index + 1}/{photos.length}
          </Text>
          <ViewerButton label="Fermer" onPress={onClose} />
        </View>
        {photo ? (
          <ZoomablePhoto key={`${photo.id}-${photo.localUri}`} photo={photo} onPrevious={previous} onNext={next} />
        ) : null}
        <ScrollView style={{ maxHeight: 110 }} contentContainerStyle={{ paddingHorizontal: 18, paddingVertical: 8 }}>
          ...
          <Text style={{ color: '#D0D5DD', marginTop: 5, lineHeight: 19 }}>
            {[photo?.site?.nom, photo?.local?.designation, `Visite du ${formatPhotoDate(photo?.derniereVisite?.date)}`]
              .filter(Boolean)
              .join(' · ')}
          </Text>
        </ScrollView>
        <View style={{ flexDirection: 'row', justifyContent: 'space-between', paddingHorizontal: 12 }}>
          <ViewerButton label="‹ Précédente" disabled={!previous} onPress={previous} />
          <ViewerButton label="Suivante ›" disabled={!next} onPress={next} />
        </View>
      </SafeAreaView>
    </Modal>
  );
