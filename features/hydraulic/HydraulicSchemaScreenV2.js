...
const compactPoints = (points) =>
  points.filter(
    (point, index, array) =>
      point && (index === 0 || point.x !== array[index - 1]?.x || point.y !== array[index - 1]?.y)
  );
      ...
      style={[
        {
          ...
          borderRadius: 10,
          borderWidth: 1,
          borderColor: active ? COLORS.orange : danger ? '#F2B8B8' : COLORS.line,
          backgroundColor: active ? COLORS.orangeLight : danger ? COLORS.redBg : COLORS.white,
    ...
    >
      <Text
        style={{
          fontSize: 11.5,
          fontWeight: '800',
          color: active ? COLORS.orangeDark : danger ? COLORS.red : COLORS.ink
        }}
      >
        {children}
      </Text>
  ...
  return (
    <TouchableOpacity
      ...
      style={{
        ...
        backgroundColor: active ? soft : COLORS.white,
        alignItems: 'center',
        justifyContent: 'center'
      }}
    >
      <Text style={{ fontSize: 11, fontWeight: '800', color: active ? color : COLORS.inkSoft }}>{children}</Text>
    </TouchableOpacity>
  );
}
...
function MovableEquipment({ item, selected, connectMode, pendingPort, phase, zoom, onSelect, onMove, onPortPress }) {
  ...
  useEffect(() => {
    currentRef.current = { x: item.x, y: item.y };
  }, [item.x, item.y]);
  useEffect(() => {
    ...
    () =>
      PanResponder.create({
        ...
        onPanResponderMove: (_evt, gesture) =>
          moveRef.current(
            item.id,
            clamp(startRef.current.x + gesture.dx / zoom, 0, CANVAS_W - ITEM_W),
            clamp(startRef.current.y + gesture.dy / zoom, 0, CANVAS_H - ITEM_H)
          )
      }),
    [item.id, zoom]
  );
    ...
    >
      <View
        ...
        style={{
          ...
          backgroundColor: selected ? COLORS.orangeLight : '#F3F3F0',
          borderWidth: 1,
          borderColor: selected ? '#F3C89B' : COLORS.line
        }}
      >
        <Text numberOfLines={1} style={{ flex: 1, fontSize: 10.5, fontWeight: '800', color: COLORS.ink }}>
          {item.label || hydraulicEquipmentDefinition(item.type)?.label}
        </Text>
      ...
      <TouchableOpacity
        ...
        style={{ position: 'absolute', left: ICON_X, top: ICON_Y, width: ICON_SIZE, height: ICON_SIZE }}
      >
        <View
          style={{
            width: ICON_SIZE,
            height: ICON_SIZE,
            borderRadius: 8,
            backgroundColor: selected ? '#FFF8F2' : 'transparent',
        ...
        >
          <EquipmentIcon
            ...
            secondaryRunning={item.secondaryRunning !== false}
            status={item.status || 'ok'}
            valvePosition={item.valvePosition ?? 50}
          />
        </View>
      </TouchableOpacity>
      {connectMode
        ? hydraulicPorts(item.type).map((port) => {
            const pending = pendingPort?.equipmentId === item.id && pendingPort?.portId === port.id;
            return (
              <TouchableOpacity
                key={port.id}
                onPress={() => onPortPress(item.id, port.id)}
                hitSlop={{ top: 8, bottom: 8, left: 8, right: 8 }}
                style={{
                  position: 'absolute',
                  left: ICON_X + (port.x / 128) * ICON_SIZE - 7,
                  top: ICON_Y + (port.y / 128) * ICON_SIZE - 7,
                  width: 14,
                  height: 14,
                  borderRadius: 7,
                  backgroundColor: pending ? '#FFD54A' : COLORS.white,
                  borderWidth: 3,
                  borderColor: pending ? '#A87400' : COLORS.orange,
                  zIndex: 60
                }}
              />
            );
        ...
        : null}
      {item.source === 'visit-equipment' ? (
        <View style={{ position: 'absolute', left: 8, right: 8, bottom: 0, alignItems: 'center' }}>
          <Text numberOfLines={1} style={{ fontSize: 8.5, color: COLORS.inkFaint }}>
            Équipement visite
          </Text>
        </View>
      ) : null}
...
function FreeEndpointHandle({ endpoint, endpointKey, connectionId, selected, zoom, onMove, onSelect }) {
  ...
  useEffect(() => {
    currentRef.current = { x: endpoint.x, y: endpoint.y };
      ...
      PanResponder.create({
        ...
        onPanResponderMove: (_evt, gesture) =>
          onMove(
            connectionId,
            endpointKey,
            clamp(startRef.current.x + gesture.dx / zoom, 0, CANVAS_W),
            clamp(startRef.current.y + gesture.dy / zoom, 0, CANVAS_H)
          )
      }),
  ...
  return (
    <View
      ...
      style={{
        ...
        borderWidth: 3,
        borderColor: selected ? COLORS.orangeDark : COLORS.inkSoft,
        zIndex: 70,
        alignItems: 'center',
        justifyContent: 'center'
      }}
    >
      <View
        style={{ width: 6, height: 6, borderRadius: 3, backgroundColor: selected ? COLORS.white : COLORS.inkSoft }}
      />
    </View>
  );
}
  ...
  return (
    <View
      ...
      style={{
        ...
        alignItems: 'center',
        justifyContent: 'center',
        backgroundColor: selected ? 'rgba(242,100,38,0.18)' : 'rgba(255,255,255,0.04)'
      }}
    >
      <View
        style={{
          ...
          borderRadius: 8,
          backgroundColor: selected ? COLORS.orange : 'rgba(90,90,90,0.30)',
        ...
        return (
          <React.Fragment key={connection.id}>
            ...
            <Polyline
              ...
              strokeLinejoin="round"
              strokeLinecap="round"
              onPress={() => onSelectConnection(connection.id)}
            />
...
function GridLayer() {
  ...
  for (let x = 0; x <= CANVAS_W; x += 40)
    vertical.push(<Line key={`v${x}`} x1={x} y1="0" x2={x} y2={CANVAS_H} stroke="#EDEDE8" strokeWidth="1" />);
  for (let y = 0; y <= CANVAS_H; y += 40)
    horizontal.push(<Line key={`h${y}`} x1="0" y1={y} x2={CANVAS_W} y2={y} stroke="#EDEDE8" strokeWidth="1" />);
  return (
    <Svg width={CANVAS_W} height={CANVAS_H} style={{ position: 'absolute', left: 0, top: 0 }}>
      <Rect x="0" y="0" width={CANVAS_W} height={CANVAS_H} fill="#FCFCFA" />
      {vertical}
      {horizontal}
    </Svg>
  );
...
function appendVisitEquipment(schemaInput, materials) {
  const schema = {
    ...EMPTY_SCHEMA,
    ...schemaInput,
    equipment: [...(schemaInput?.equipment || [])],
    connections: [...(schemaInput?.connections || [])]
  };
  ...
  const additions = (materials || []).filter(
    (material) =>
      ...
      !(material.equipement_id && existingEquipmentIds.has(material.equipement_id))
  );
...
function HydraulicSchemaScreenV2({ route, onImportJson, importingJson = false }) {
    ...
    (async () => {
      try {
        ...
        if (row?.valeur) {
          ...
          if (parsed && Array.isArray(parsed.equipment) && Array.isArray(parsed.connections))
            base = { ...EMPTY_SCHEMA, ...parsed, equipment: parsed.equipment, connections: parsed.connections };
        }
        const merged = appendVisitEquipment(base, materials).schema;
        if (mounted) setSchema(merged);
      } catch (error) {
        console.warn('Schéma technique non chargé', error);
      } finally {
  ...
  useEffect(() => {
    const timer = setInterval(() => setPhase((value) => (value + 12) % 360), 80);
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      try {
        ...
        console.warn('Schéma technique non sauvegardé', error);
        setSaveState('error');
      }
    }, 450);
  ...
  useEffect(
    () => () => {
      if (visiteId)
        upsertChamp(visiteId, SCHEMA_SECTION, SCHEMA_KEY, JSON.stringify(schemaRef.current)).catch(() => {});
    },
    [visiteId]
  );
  ...
  const selectedConnection = selectedConnectionId
    ? schema.connections.find((item) => item.id === selectedConnectionId)
    : null;
  const libraryItems = useMemo(
    () => searchHydraulicEquipment(libraryQuery, libraryGroup),
  ...
  const patchConnection = useCallback(
    (id, patch) =>
      setSchema((current) => ({
        ...current,
        connections: current.connections.map((item) => (item.id === id ? { ...item, ...patch } : item))
      })),
    []
  );
  ...
  const addEquipment = useCallback(
    (type) => {
      ...
      const item = {
        id: uid('eq'),
        type,
        label: definition?.defaultLabel || definition?.label || type,
        x: clamp(55 + (count % 8) * 155, 0, CANVAS_W - ITEM_W),
        y: clamp(60 + (Math.floor(count / 8) % 6) * 150, 0, CANVAS_H - ITEM_H),
        running: type !== 'water_leak',
        secondaryRunning: true,
        status: type === 'water_leak' ? 'fault' : 'ok',
        valvePosition: 50,
        source: 'manual'
      };
  ...
  const deleteEquipment = useCallback(
    (id) =>
      Alert.alert('Supprimer cet équipement ?', 'Les réseaux raccordés à cet équipement seront également supprimés.', [
          ...
          onPress: () => {
            setSchema((current) => ({
              ...
              connections: current.connections.filter(
                (connection) => connection.from?.equipmentId !== id && connection.to?.equipmentId !== id
              )
            }));
            setSelectedEquipmentId(null);
            setPendingPort((value) => (value?.equipmentId === id ? null : value));
          }
        }
      ]),
    []
  ...
  const onPortPress = useCallback(
    (equipmentId, portId) => {
      ...
      const fromPoint = endpointPoint(schema, pendingPort),
        toPoint = endpointPoint(schema, { equipmentId, portId });
      const connection = {
        id: uid('co'),
        from: pendingPort,
        ...
        medium: activeMedium,
        direction: 1,
        via: fromPoint && toPoint ? { x: (fromPoint.x + toPoint.x) / 2, y: (fromPoint.y + toPoint.y) / 2 } : undefined
      };
  ...
  const finishWithFreeEnd = useCallback(() => {
    ...
    const free =
      start.side === 'left'
        ...
        : start.side === 'top'
          ? { free: true, x: start.x, y: clamp(start.y - distance, 20, CANVAS_H - 20) }
          : start.side === 'bottom'
            ? { free: true, x: start.x, y: clamp(start.y + distance, 20, CANVAS_H - 20) }
            : { free: true, x: clamp(start.x + distance, 20, CANVAS_W - 20), y: start.y };
    const connection = {
      ...
      medium: activeMedium,
      direction: 1,
      via: { x: (start.x + free.x) / 2, y: (start.y + free.y) / 2 }
    };
    setSchema((current) => ({ ...current, connections: [...current.connections, connection] }));
    setPendingPort(null);
    setSelectedConnectionId(connection.id);
  ...
  const moveFreeEndpoint = useCallback(
    (connectionId, endpointKey, x, y) =>
      setSchema((current) => ({
        ...current,
        connections: current.connections.map((connection) =>
          connection.id === connectionId
            ? { ...connection, [endpointKey]: { ...connection[endpointKey], free: true, x, y } }
            : connection
        )
      })),
    []
  ...
  const moveConnection = useCallback(
    (connectionId, snapshot, dx, dy) =>
      setSchema((current) => ({
        ...current,
        connections: current.connections.map((connection) => {
          if (connection.id !== connectionId) return connection;
          const patch = {
            via: { x: clamp(snapshot.via.x + dx, 0, CANVAS_W), y: clamp(snapshot.via.y + dy, 0, CANVAS_H) }
          };
          if (snapshot.from && snapshot.to) {
            patch.from = {
              ...connection.from,
              x: clamp(snapshot.from.x + dx, 0, CANVAS_W),
              y: clamp(snapshot.from.y + dy, 0, CANVAS_H)
            };
            patch.to = {
              ...connection.to,
              x: clamp(snapshot.to.x + dx, 0, CANVAS_W),
              y: clamp(snapshot.to.y + dy, 0, CANVAS_H)
            };
          }
  ...
  const deleteConnection = useCallback((id) => {
    setSchema((current) => ({ ...current, connections: current.connections.filter((item) => item.id !== id) }));
    setSelectedConnectionId(null);
  }, []);
  ...
  const syncVisitEquipment = useCallback(async () => {
    ...
    try {
      ...
      Alert.alert(
        ...
        result.added
          ? `${result.added} équipement(s) de la visite ont été ajoutés au schéma.`
          : 'Tous les équipements de la visite sont déjà présents dans le schéma.'
      );
    } catch (error) {
      Alert.alert('Synchronisation impossible', String(error?.message || error));
    } finally {
      setSyncing(false);
    }
  }, [syncing, visiteId]);
  ...
  const resetSchema = useCallback(
    () =>
      Alert.alert(
        'Effacer le schéma ?',
        'Tous les équipements et réseaux dessinés seront supprimés. Les équipements de la visite pourront être réimportés ensuite.',
        [
          { text: 'Annuler', style: 'cancel' },
          {
            text: 'Effacer',
  ...
  const fitZoom = useCallback(
    () =>
      setZoomSafe(
        wide
          ? Math.min(0.9, Math.max(ZOOM_MIN, (width - 330) / CANVAS_W))
          : Math.min(0.72, Math.max(ZOOM_MIN, (width - 24) / CANVAS_W))
      ),
    [setZoomSafe, wide, width]
  );
  ...
  if (!loaded)
    return (
      <View style={styles.center}>
        ...
        <Text style={{ marginTop: 10, color: COLORS.inkSoft }}>Chargement du schéma…</Text>
      </View>
    );
    ...
    >
      {selectedEquipment ? (
        <ScrollView keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
          <Text style={{ fontSize: 10.5, fontWeight: '800', color: COLORS.inkFaint, textTransform: 'uppercase' }}>
            Équipement sélectionné
          </Text>
          <Text style={{ fontSize: 15, fontWeight: '800', color: COLORS.ink, marginTop: 4 }}>
            {hydraulicEquipmentDefinition(selectedEquipment.type)?.label || selectedEquipment.type}
          </Text>
          <TextInput
            style={[styles.input, { marginTop: 10, minHeight: 40 }]}
            value={selectedEquipment.label || ''}
            onChangeText={(label) => patchEquipment(selectedEquipment.id, { label })}
            placeholder="Nom affiché"
          />
          {selectedEquipment.source === 'visit-equipment' ? (
            <View style={{ marginTop: 8, padding: 8, borderRadius: 8, backgroundColor: COLORS.orangeLight }}>
              <Text style={{ fontSize: 10.5, fontWeight: '800', color: COLORS.orangeDark }}>
                Lié à l’onglet Équipements de la visite
              </Text>
              <Text style={{ marginTop: 2, fontSize: 9.5, color: COLORS.inkSoft }}>
                {[selectedEquipment.metadata?.marque, selectedEquipment.metadata?.modele].filter(Boolean).join(' · ') ||
                  'Référence terrain'}
              </Text>
            </View>
          ) : null}
          ...
          <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 7 }}>
            <ChoiceChip
              active={selectedEquipment.running !== false}
              color={COLORS.green}
              soft={COLORS.greenBg}
              onPress={() => patchEquipment(selectedEquipment.id, { running: true })}
            >
              En marche
            </ChoiceChip>
            <ChoiceChip
              ...
              soft="#F1F1EE"
              onPress={() => patchEquipment(selectedEquipment.id, { running: false })}
            >
              À l'arrêt
            </ChoiceChip>
          </View>
          ...
          <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 7 }}>
            {Object.entries(STATUS).map(([key, meta]) => (
              <ChoiceChip
                key={key}
                active={(selectedEquipment.status || 'ok') === key}
                color={meta.color}
                soft={meta.soft}
                onPress={() => patchEquipment(selectedEquipment.id, { status: key })}
              >
                {meta.label}
              </ChoiceChip>
            ))}
          ...
          {[
            ...
            <>
              <Text style={{ fontSize: 11, color: COLORS.inkSoft, marginTop: 12, marginBottom: 6 }}>Position</Text>
              <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 6 }}>
                {[0, 25, 50, 75, 100].map((value) => (
                  <ChoiceChip
                    key={value}
                    active={(selectedEquipment.valvePosition ?? 50) === value}
                    color={COLORS.orangeDark}
                    soft={COLORS.orangeLight}
                    onPress={() => patchEquipment(selectedEquipment.id, { valvePosition: value })}
                  >
                    {value}%
                  </ChoiceChip>
                ))}
          ...
          <Text style={{ fontSize: 10.5, lineHeight: 16, color: COLORS.inkFaint, marginTop: 12 }}>
            Maintiens la barre grise pour déplacer l’équipement. Le schéma est sauvegardé automatiquement.
          </Text>
          <ToolbarButton danger onPress={() => deleteEquipment(selectedEquipment.id)} style={{ marginTop: 14 }}>
            Supprimer du schéma
          </ToolbarButton>
        </ScrollView>
      ) : selectedConnection ? (
        <ScrollView showsVerticalScrollIndicator={false}>
          ...
          <Text style={{ fontSize: 14, fontWeight: '800', color: COLORS.ink, marginTop: 5 }}>Tracé déplaçable</Text>
          <Text style={{ fontSize: 10.5, lineHeight: 16, color: COLORS.inkSoft, marginTop: 7 }}>
            Maintiens le point central du réseau puis glisse-le. Les extrémités libres peuvent également être déplacées
            séparément.
          </Text>
          <Text style={{ fontSize: 11, color: COLORS.inkSoft, marginTop: 11, marginBottom: 6 }}>Fluide / couleur</Text>
          <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 7 }}>
            {Object.entries(MEDIUMS).map(([key, meta]) => (
              <ChoiceChip
                key={key}
                active={selectedConnection.medium === key}
                color={meta.color}
                soft={meta.soft}
                onPress={() => patchConnection(selectedConnection.id, { medium: key })}
              >
                {meta.label}
              </ChoiceChip>
            ))}
          ...
          <ToolbarButton
            active
            onPress={() =>
              patchConnection(selectedConnection.id, { direction: selectedConnection.direction === -1 ? 1 : -1 })
            }
            style={{ marginTop: 12 }}
          >
            ↔ Inverser le sens
          </ToolbarButton>
          <ToolbarButton danger onPress={() => deleteConnection(selectedConnection.id)} style={{ marginTop: 8 }}>
            Supprimer le réseau
          </ToolbarButton>
        </ScrollView>
      ) : (
        <View>
          <Text style={{ fontSize: 15, fontWeight: '800', color: COLORS.ink }}>Schéma technique</Text>
          <Text style={{ fontSize: 11.5, lineHeight: 18, color: COLORS.inkSoft, marginTop: 7 }}>
            Les équipements saisis dans la visite sont ajoutés automatiquement. Tu peux aussi ajouter des composants de
            bibliothèque et dessiner des réseaux raccordés ou totalement libres.
          </Text>
          <View
            style={{
              marginTop: 11,
          ...
          >
            <Text style={{ fontSize: 10.5, fontWeight: '800', color: '#A74618' }}>
              Réseau libre = deux extrémités déplaçables
            </Text>
            <Text style={{ fontSize: 10, color: COLORS.inkSoft, marginTop: 3 }}>
              Touchez un réseau pour afficher son point de déplacement.
            </Text>
          </View>
        </View>
      )}
  ...
  return (
    <View style={{ flex: 1, backgroundColor: COLORS.bg }}>
      ...
      >
        ...
        >
          <ToolbarButton active onPress={() => setLibraryVisible(true)}>
            ＋ Équipement
          </ToolbarButton>
          <ToolbarButton active={connectMode} onPress={toggleConnectMode}>
            {connectMode ? '✓ Raccorder' : '○ Raccorder'}
          </ToolbarButton>
          <ToolbarButton onPress={addFreeNetwork}>＋ Réseau libre</ToolbarButton>
          {pendingPort ? (
            <ToolbarButton active onPress={finishWithFreeEnd}>
              ↳ Fin libre
            </ToolbarButton>
          ) : null}
          {Object.entries(MEDIUMS).map(([key, meta]) => (
            <ChoiceChip
              key={key}
              active={activeMedium === key}
              color={meta.color}
              soft={meta.soft}
              onPress={() => setActiveMedium(key)}
            >
              {meta.label}
            </ChoiceChip>
          ))}
          <ToolbarButton disabled={syncing} onPress={syncVisitEquipment}>
            {syncing ? 'Synchronisation…' : '↻ Équipements visite'}
          </ToolbarButton>
          {onImportJson ? (
            <ToolbarButton disabled={importingJson} onPress={onImportJson}>
              {importingJson ? 'Import…' : '⇩ JSON METRA'}
            </ToolbarButton>
          ) : null}
          ...
          <View style={{ minWidth: 48, alignItems: 'center' }}>
            <Text style={{ fontSize: 10.5, fontWeight: '800', color: COLORS.ink }}>{Math.round(zoom * 100)}%</Text>
          </View>
          <ToolbarButton onPress={() => setZoomSafe(zoom + ZOOM_STEP)}>＋</ToolbarButton>
          <ToolbarButton danger onPress={resetSchema}>
            Effacer
          </ToolbarButton>
          <View style={{ minWidth: 88, alignItems: 'center' }}>
            <Text
              style={{
                fontSize: 10.5,
                fontWeight: '800',
                color: saveState === 'error' ? COLORS.red : saveState === 'saving' ? COLORS.amber : COLORS.green
              }}
            >
              {saveState === 'saving' ? 'Sauvegarde…' : saveState === 'error' ? 'Erreur' : '✓ Sauvegardé'}
            </Text>
            <Text style={{ fontSize: 9, color: COLORS.inkFaint }}>
              {schema.equipment.length} équip. · {schema.connections.length} réseaux
            </Text>
          </View>
        </ScrollView>
        {connectMode ? (
          <Text
            style={{
              ...
              fontSize: 10.5,
              fontWeight: '700',
              color: pendingPort ? '#9A6A00' : COLORS.orangeDark
            }}
          >
            {pendingPort
              ? 'Premier raccord choisi : touche une autre borne, ou utilise « Fin libre ».'
              : `Raccordement actif — ${MEDIUMS[activeMedium].label}. Touche la borne de départ.`}
          </Text>
        ) : null}
      </View>
            ...
            <ScrollView showsVerticalScrollIndicator contentContainerStyle={{ minHeight: scaledHeight }}>
              <View style={{ width: scaledWidth, height: scaledHeight }}>
                ...
                >
                  ...
                  />
                  {schema.connections.map((connection) => (
                    <React.Fragment key={`handles-${connection.id}`}>
                      <MovableConnectionHandle
                        schema={schema}
      ...
      <Modal visible={libraryVisible} transparent animationType="fade" onRequestClose={() => setLibraryVisible(false)}>
        <View style={styles.modalOverlay}>
          <View style={[styles.modalSheet, { width: wide ? 760 : '94%', maxHeight: '88%' }]}>
            <Text style={styles.modalTitle}>Bibliothèque du schéma technique</Text>
            <Text style={{ fontSize: 10.5, color: COLORS.inkSoft, marginBottom: 9 }}>
              {HYDRAULIC_EQUIPMENT_TYPES.length} composants techniques disponibles. Les composants spécialisés
              réutilisent un symbole métier proche tout en conservant leur type exact dans le schéma.
            </Text>
            <TextInput
              value={libraryQuery}
              onChangeText={setLibraryQuery}
              style={styles.input}
              placeholder="Rechercher : chaudière, vanne, capteur, VMC…"
              autoCorrect={false}
            />
            <ScrollView
              horizontal
            ...
            >
              <ChoiceChip
                ...
                color={COLORS.orangeDark}
                soft={COLORS.orangeLight}
                onPress={() => setLibraryGroup(null)}
              >
                Tous
              </ChoiceChip>
              {HYDRAULIC_GROUPS.map((group) => (
                <ChoiceChip
                  key={group}
                  active={libraryGroup === group}
                  color={COLORS.orangeDark}
                  soft={COLORS.orangeLight}
                  onPress={() => setLibraryGroup(group)}
                >
                  {group}
                </ChoiceChip>
              ))}
            ...
            <ScrollView contentContainerStyle={{ flexDirection: 'row', flexWrap: 'wrap', gap: 9 }}>
              {libraryItems.map((item) => (
                <TouchableOpacity
                  key={item.id}
                  onPress={() => addEquipment(item.id)}
                  style={{
                    width: wide ? '23.5%' : '47.5%',
                    minHeight: 128,
                    borderWidth: 1,
                    borderColor: COLORS.line,
                    borderRadius: 11,
                ...
                >
                  <EquipmentIcon
                    type={hydraulicIconType(item.id)}
                    size={76}
                    phase={phase}
                    running
                    status={item.id === 'water_leak' ? 'fault' : 'ok'}
                  />
                  <Text
                    style={{ fontSize: 11, fontWeight: '800', color: COLORS.ink, textAlign: 'center', marginTop: 4 }}
                  >
                    {item.label}
                  </Text>
                  <Text style={{ fontSize: 8.5, color: COLORS.inkFaint, marginTop: 2 }}>{item.group}</Text>
                </TouchableOpacity>
              ))}
              {!libraryItems.length ? (
                <View style={{ width: '100%', paddingVertical: 26 }}>
                  <Text style={{ textAlign: 'center', color: COLORS.inkSoft }}>Aucun équipement correspondant.</Text>
                </View>
              ) : null}
            </ScrollView>
            <ToolbarButton onPress={() => setLibraryVisible(false)} style={{ marginTop: 13 }}>
              Fermer
            </ToolbarButton>
          </View>
        </View>
