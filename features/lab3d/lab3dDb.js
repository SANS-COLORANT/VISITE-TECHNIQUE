...
function presetByCode(code) {
  return (
    LAB3D_EQUIPMENT_PRESETS.find((item) => item.code === code) ||
...
export function inferLab3dProfile(equipment = {}) {
  const text = normalize(
    [equipment.type_code, equipment.designation, equipment.marque, equipment.modele].filter(Boolean).join(' ')
  );
  let code = 'equipement';
...
async function ensureInstallation(db, siteId) {
  ...
  await db.runAsync(`INSERT INTO installations(id,site_id,type_code,nom,description,actif) VALUES(?,?,?,?,?,1)`, [
    ...
    'Créée automatiquement depuis LAB 3D'
  ]);
  return id;
}
...
export async function ensureLab3dScene(siteId) {
  ...
  const id = createId();
  await db.runAsync(`INSERT INTO lab3d_scenes(id,site_id,nom,grid_step) VALUES(?,?,?,?)`, [
    id,
    siteId,
    'Maquette technique',
    DEFAULT_GRID_STEP
...
export async function placeExistingEquipment(sceneId, equipment, position = {}) {
  ...
  const db = await openAppDatabase();
  const existing = await db.getFirstAsync(`SELECT * FROM lab3d_objects WHERE scene_id=? AND equipment_id=?`, [
    sceneId,
    equipment.id
  ]);
...
export async function setMasonryBase(object) {
  ...
  const support =
    current.support?.type === 'masonry'
      ? null
      : {
          type: 'masonry',
          overhang: 0.05,
          height: 0.1,
          width: Number(object.width || 1) + 0.1,
          depth: Number(object.depth || 0.6) + 0.1
        };
...
export function makeOrthogonalNetworkPoints(start, end) {
  ...
  return [a, { x: a.x, y: a.y, z: routingZ }, { x: b.x, y: a.y, z: routingZ }, { x: b.x, y: b.y, z: routingZ }, b];
}
...
export async function updateLab3dNetwork(networkId, patch = {}) {
  ...
  if (Object.prototype.hasOwnProperty.call(patch, 'diameter_mm')) {
    sets.push('diameter_mm=?');
    params.push(Number(patch.diameter_mm));
  }
  if (Object.prototype.hasOwnProperty.call(patch, 'type_code')) {
    sets.push('type_code=?');
    params.push(patch.type_code);
  }
  if (Object.prototype.hasOwnProperty.call(patch, 'label')) {
    sets.push('label=?');
    params.push(patch.label || null);
  }
  if (Object.prototype.hasOwnProperty.call(patch, 'points')) {
    sets.push('points_json=?');
...
export async function saveLab3dCamera(sceneId, camera = {}) {
  const db = await openAppDatabase();
  await db.runAsync(`UPDATE lab3d_scenes SET camera_json=?,modifie_le=datetime('now') WHERE id=?`, [
