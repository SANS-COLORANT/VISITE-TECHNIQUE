...
function Cuboid({ object, viewport, camera, selected = false, support = false }) {
  ...
  const height = support ? num(object.params?.support?.height, 0.1) : num(object.height, 1);
  ...
  const a = rotate(-hw, -hd);
  const b = rotate(hw, -hd);
  const c = rotate(hw, hd);
  const d = rotate(-hw, hd);
  ...
  const faces = [
    ...
    { points: [bottom[3], bottom[0], top[0], top[3]], opacity: support ? 0.5 : 0.66 },
    ...
    .map((face, index) => ({
      ...
      key: index,
      depth:
        face.points.reduce((sum, point) => sum + cameraPoint(point, viewport, camera).depth, 0) / face.points.length
    }))
  ...
  return (
    <>
      {faces.map((face) => (
        <Polygon
          key={face.key}
          points={polygonPoints(face.points, viewport, camera)}
          fill={color}
          opacity={face.opacity}
          stroke={selected ? CYAN : '#B8CDD8'}
...
function SceneGrid({ viewport, camera, step = 0.25 }) {
  ...
  for (let v = -size; v <= size + 0.001; v += increment) {
    ...
    lines.push(
      <Line key={`gx-${index++}`} x1={p1.x} y1={p1.y} x2={p2.x} y2={p2.y} stroke={stroke} strokeWidth={width} />
    );
    lines.push(
      <Line key={`gy-${index++}`} x1={p3.x} y1={p3.y} x2={p4.x} y2={p4.y} stroke={stroke} strokeWidth={width} />
    );
  }
  ...
  return (
    <>
      {lines}
      <Line x1={origin.x} y1={origin.y} x2={xEnd.x} y2={xEnd.y} stroke={AXIS.X} strokeWidth={2} />
      <Line x1={origin.x} y1={origin.y} x2={yEnd.x} y2={yEnd.y} stroke={AXIS.Y} strokeWidth={2} />
      <Line x1={origin.x} y1={origin.y} x2={zEnd.x} y2={zEnd.y} stroke={AXIS.Z} strokeWidth={2} />
    </>
  );
}
...
function RadialMenu({ menu, object, onAction, onClose }) {
  ...
  return (
    <View
      ...
      style={{ position: 'absolute', left: menu.x - 112, top: menu.y - 112, width: 224, height: 224 }}
    >
      <View
        pointerEvents="none"
        style={{
      ...
      >
        <Text style={{ color: WHITE, fontWeight: '900', fontSize: 11, textAlign: 'center' }}>
          {menu.mode === 'main'
            ? String(object.label || 'Objet').slice(0, 10)
            : menu.mode === 'axis'
              ? 'AXE'
              : menu.mode === 'rotate'
                ? 'ROT.'
...
export function Lab3DScreen({ route, navigation }) {
  ...
  const selectedEquipment = useMemo(
    () => data?.equipments?.find((item) => item.id === selected?.equipment_id) || null,
    [data, selected]
  );
  ...
  const focusObject = useCallback(
    (object) => {
      ...
      setCamera((current) => ({
        ...current,
        targetX: num(object.x),
        targetY: num(object.y),
        targetZ: num(object.z) + num(object.height, 1) * 0.45,
        offsetX: 0,
        offsetY: 0,
        zoom: clamp(Math.max(current.zoom, 68), 36, 92)
      }));
      haptic(10);
    },
    [stopInertia]
  ...
  const scenePanResponder = useMemo(
    () =>
      PanResponder.create({
        ...
        onPanResponderMove: (event, gesture) => {
          ...
          if (touches.length >= 2) {
            ...
            if (state.mode !== 'multi') {
              ...
              state.mode = 'multi';
              state.start = { ...current };
              state.centerX = (a.pageX + b.pageX) / 2;
              state.centerY = (a.pageY + b.pageY) / 2;
            ...
            setCamera((current) => ({
              ...current,
              zoom: clamp(state.start.zoom * ratio, 24, 110),
    ...
    () =>
      PanResponder.create({
        onStartShouldSetPanResponder: (event) =>
          Boolean(selectedId && moveMode && (event.nativeEvent.touches || []).length >= 1),
        onMoveShouldSetPanResponder: (event, gesture) =>
          Boolean(
            selectedId &&
            moveMode &&
            ((event.nativeEvent.touches || []).length >= 2 || Math.abs(gesture.dx) > 2 || Math.abs(gesture.dy) > 2)
          ),
        onPanResponderGrant: (event) => {
          const touches = event.nativeEvent.touches || [];
          if (touches.length >= 2) {
          ...
          if (object) {
            dragStart.current = { ...object };
            lastSnapRef.current = num(object[axis.toLowerCase()]);
          }
        },
        onPanResponderMove: (event, gesture) => {
          const touches = event.nativeEvent.touches || [];
          ...
          const nextValue =
            axis === 'Z' ? Math.max(0, snap(num(start[key]) + delta, step)) : snap(num(start[key]) + delta, step);
          if (lastSnapRef.current !== nextValue) {
            lastSnapRef.current = nextValue;
            haptic(5);
          }
          setObjects((current) => current.map((item) => (item.id === start.id ? { ...item, [key]: nextValue } : item)));
        },
        onPanResponderRelease: async () => {
          if (cameraGestureRef.current?.mode === 'multi') {
            cameraGestureRef.current = null;
  ...
  const moveByStep = async (direction) => {
    ...
    const patch =
      axis === 'X'
        ? { x: num(selected.x) + step }
        : axis === 'Y'
          ? { y: num(selected.y) + step }
  ...
  const openDimensions = () => {
    ...
    setEditForm({
      ...
      supportHeight: selected.params?.support?.height ?? 0.1,
    ...
    if (params.support?.type === 'masonry') {
      ...
      params = {
        ...
        support: {
          ...params.support,
          overhang,
          height: Math.max(0.02, num(editForm.supportHeight, 0.1)),
          width: width + overhang * 2,
          depth: depth + overhang * 2
        }
      };
    }
    await updateLab3dObject(selected.id, {
      width,
      depth,
      height,
      z,
      label: editForm.designation || selected.label,
      params
    });
    if (selected.equipment_id)
      await updateLab3dEquipmentData(selected.equipment_id, {
  ...
  const deleteSelected = () => {
    ...
    Alert.alert('Retirer de la maquette ?', "L'équipement reste enregistré dans le patrimoine du site.", [
      ...
      {
        text: 'Retirer de la 3D',
        style: 'destructive',
        onPress: async () => {
          await removeLab3dObject(selected.id);
          setSelectedId(null);
          setRadialMenu(null);
          await reload();
        }
  ...
  const createEquipment = async (preset) => {
    try {
      ...
      Alert.alert('Nouvel équipement', String(err.message || err));
    }
  };
  ...
  const changeNetworkDn = async (dn) => {
    if (selectedNetwork) {
      ...
      setNetworks((current) =>
        current.map((item) => (item.id === selectedNetwork.id ? { ...item, diameter_mm: dn } : item))
      );
      haptic(7);
    }
  };
  const deleteNetwork = () => {
    if (selectedNetwork)
      Alert.alert(
        'Supprimer ce réseau ?',
        `${networkLabel(selectedNetwork.type_code)} · DN${selectedNetwork.diameter_mm || '-'}`,
        [
          { text: 'Annuler', style: 'cancel' },
          {
            text: 'Supprimer',
            style: 'destructive',
            onPress: async () => {
              await removeLab3dNetwork(selectedNetwork.id);
              setSelectedNetworkId(null);
              await reload();
            }
          }
  ...
  const updateRadialHover = useCallback(
    (object, frame, event) => {
      ...
      if (gesture.hovered !== hovered) {
        ...
        if (hovered) haptic(4);
        setRadialMenu((menu) => (menu ? { ...menu, hovered } : menu));
      }
    },
    [radialMenu]
  );
  ...
  const executeRadialAction = async (key) => {
    ...
    if (key === 'move') {
      setRadialMenu((menu) => (menu ? { ...menu, mode: 'axis', hovered: null } : menu));
      return;
    }
    if (key === 'rotate') {
      setRadialMenu((menu) => (menu ? { ...menu, mode: 'rotate', hovered: null } : menu));
      return;
    }
    if (key === 'more') {
      setRadialMenu((menu) => (menu ? { ...menu, mode: 'more', hovered: null } : menu));
      return;
    }
    ...
    if (key === 'rotateExact') {
      setRotationValue(String(num(object.rotation_deg).toFixed(0)));
  ...
  const handleObjectTap = useCallback(
    (object) => {
      ...
      if (suppressTapRef.current === object.id) {
  ...
  const openExactPosition = () => {
    if (selected) {
      setExactValue(String(num(selected[axis.toLowerCase()]).toFixed(2)));
  ...
  return (
    <View style={{ flex: 1, backgroundColor: BG }}>
      ...
      >
        <View style={{ flex: 1 }}>
          ...
          <Text style={{ color: MUTED, fontSize: 10 }}>
            {requestedVisitId ? 'Maquette du site · couche visite' : 'Maquette permanente'} · {objects.length}{' '}
            équipement(s) · {networks.length} réseau(x)
          </Text>
        </View>
      ...
      <View
        ...
        {...(moveMode ? movePanResponder.panHandlers : scenePanResponder.panHandlers)}
      >
        <Svg width="100%" height="100%" style={{ backgroundColor: BG }} onPress={handleEmptyPress}>
          ...
          {networks.map((network) => (
            <React.Fragment key={network.id}>
              {(network.points || []).slice(0, -1).map((point, index) => {
                const a = project(point, viewport, camera);
                const b = project(network.points[index + 1], viewport, camera);
                return (
                  <Line
                    ...
                    x2={b.x}
                    y2={b.y}
                    stroke={NETWORK_COLORS[network.type_code] || NETWORK_COLORS.autre}
                    strokeWidth={clamp(2 + num(network.diameter_mm, 50) / 35, 2.5, 8)}
                    strokeLinecap="round"
                    opacity={selectedNetworkId === network.id ? 1 : 0.82}
                    onPress={(event) => {
          ...
          {objects.map((object) => (
            <React.Fragment key={object.id}>
              {object.params?.support?.type === 'masonry' ? (
                <Cuboid object={object} viewport={viewport} camera={camera} support />
              ) : null}
              <Cuboid object={object} viewport={viewport} camera={camera} selected={selectedId === object.id} />
              {(() => {
                const labelPoint = project(
                  { x: object.x, y: object.y, z: num(object.z) + num(object.height) + 0.18 },
                  viewport,
                  camera
                );
                return (
                  <SvgText
                    x={labelPoint.x}
                    y={labelPoint.y}
                    fill={selectedId === object.id ? CYAN : '#DCEAF1'}
                    fontSize="11"
                    fontWeight="700"
                    textAnchor="middle"
                  >
                    {String(object.label || object.subtype || 'Équipement').slice(0, 24)}
                  </SvgText>
                );
              })()}
              {(markersByObject.get(object.id) || []).map((remark, index) => {
                const marker = project(
                  {
                    x: num(object.x) + 0.25 + index * 0.12,
                    y: num(object.y) - 0.2,
                    z: num(object.z) + num(object.height) + 0.5 + index * 0.15
                  },
                  viewport,
                  camera
                );
                return (
                  <React.Fragment key={remark.id}>
                    <Circle cx={marker.x} cy={marker.y} r={8} fill={RED} stroke="#FFF" strokeWidth={1.5} />
                    <SvgText
                      x={marker.x}
                      y={marker.y + 3.5}
                      fill="#FFF"
        ...
        {objects.map((object) => {
          const center = project(
            { x: object.x, y: object.y, z: num(object.z) + num(object.height) * 0.5 },
            viewport,
            camera
          );
          ...
          return (
            <Pressable
              key={`hit-${object.id}`}
              onPressIn={() => {
                touchingObjectRef.current = true;
                stopInertia();
              }}
              onPress={() => handleObjectTap(object)}
              onLongPress={() => openRadialMenu(object, center)}
              onTouchMove={(event) => updateRadialHover(object, frame, event)}
              onPressOut={() => {
                touchingObjectRef.current = false;
                releaseRadialGesture();
              }}
              delayLongPress={420}
              style={{ position: 'absolute', ...frame, borderRadius: hit / 2 }}
            />
          );
        })}
        ...
        {moveMode && selected ? (
          ...
          >
            ...
            <TouchableOpacity onPress={openExactPosition} style={{ paddingVertical: 5 }}>
              <Text style={{ color: AXIS[axis], fontSize: 18, fontWeight: '900' }}>
                {axis} : {num(selected[axis.toLowerCase()]).toFixed(2)} m ✎
              </Text>
            </TouchableOpacity>
            <View style={{ flexDirection: 'row', gap: 5 }}>
              <TinyButton label="− pas" onPress={() => moveByStep(-1)} />
              <TinyButton label="+ pas" onPress={() => moveByStep(1)} />
              <TinyButton label="Terminer" onPress={() => setMoveMode(false)} />
            </View>
          </View>
        ) : null}

        {!selected && !selectedNetwork && !radialMenu && !networkDraft ? (
          <View
            pointerEvents="none"
          ...
          >
            ...
            >
              1 doigt sur le vide : orbite · 2 doigts : déplacer · pincer : zoom · double tap : cadrer
            </Text>
          </View>
        ) : null}
      </View>
      ...
      <View style={{ backgroundColor: PANEL, borderTopWidth: 1, borderTopColor: '#284657', padding: 9 }}>
        {!selected && !selectedNetwork ? (
          <View style={{ flexDirection: 'row', gap: 7 }}>
            <TinyButton
              flex
              label={`Équipements du site${unplaced.length ? ` (${unplaced.length})` : ''}`}
          ...
          <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
            <View style={{ flex: 1 }}>
              <Text style={{ color: WHITE, fontWeight: '900' }}>{selected.label}</Text>
              <Text style={{ color: MUTED, fontSize: 10 }}>
                X {num(selected.x).toFixed(2)} · Y {num(selected.y).toFixed(2)} · Z {num(selected.z).toFixed(2)} m ·
                appui long = roue d'actions
              </Text>
            </View>
            <TinyButton
              label="Actions"
              ...
              onPress={() => {
                const center = project(
                  { x: selected.x, y: selected.y, z: num(selected.z) + num(selected.height) * 0.5 },
                  viewport,
                  camera
                );
                openRadialMenu(selected, center);
          ...
          <>
            <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 7 }}>
              <View style={{ flex: 1 }}>
                ...
                <Text style={{ color: MUTED, fontSize: 10 }}>
                  DN{selectedNetwork.diameter_mm || '-'} · tracé orthogonal entre équipements
                </Text>
              </View>
              <TouchableOpacity onPress={() => setSelectedNetworkId(null)}>
                <Text style={{ color: MUTED, fontSize: 22 }}>×</Text>
              </TouchableOpacity>
            </View>
      ...
      <Modal visible={equipmentModal} transparent animationType="slide" onRequestClose={() => setEquipmentModal(false)}>
        <View style={{ flex: 1, backgroundColor: '#0008', justifyContent: 'flex-end' }}>
          ...
          >
            <Text style={{ color: WHITE, fontSize: 18, fontWeight: '900' }}>Équipements du site</Text>
            <Text style={{ color: MUTED, marginTop: 3, marginBottom: 12 }}>
              La maquette utilise le patrimoine permanent du site. Un équipement retiré de la 3D reste dans le
              patrimoine.
            </Text>
            <ScrollView>
              {unplaced.length ? (
                  ...
                  >
                    ...
                    <Text style={{ color: MUTED, fontSize: 11, marginTop: 3 }}>
                      {[equipment.marque, equipment.modele].filter(Boolean).join(' · ') ||
                        'Modèle générique paramétrique'}{' '}
                      · {equipment.visualProfile?.source === 'generic' ? 'fallback 3D générique' : 'profil 3D reconnu'}
                    </Text>
                  </TouchableOpacity>
                ))
              ) : (
            ...
            <TinyButton label="Fermer" onPress={() => setEquipmentModal(false)} />
          </View>
        </View>
      </Modal>

      <Modal
        visible={newEquipmentModal}
        transparent
        animationType="slide"
        onRequestClose={() => setNewEquipmentModal(false)}
      >
        <View style={{ flex: 1, backgroundColor: '#0008', justifyContent: 'flex-end' }}>
          <View
            style={{
          ...
          >
            <Text style={{ color: WHITE, fontSize: 18, fontWeight: '900' }}>Ajouter depuis la 3D</Text>
            <Text style={{ color: MUTED, marginTop: 3, marginBottom: 12 }}>
              L'équipement sera immédiatement ajouté au patrimoine permanent du site puis placé dans la maquette.
            </Text>
            <ScrollView>
              {LAB3D_EQUIPMENT_PRESETS.map((preset) => (
                ...
                >
                  ...
                  <Text style={{ color: MUTED, fontSize: 11, marginTop: 2 }}>
                    Base {preset.width} × {preset.depth} × {preset.height} m · dimensions modifiables
                  </Text>
                </TouchableOpacity>
              ))}
            </ScrollView>
            <TinyButton label="Fermer" onPress={() => setNewEquipmentModal(false)} />
          </View>
        </View>
      </Modal>
      ...
      <Modal visible={dimensionModal} transparent animationType="fade" onRequestClose={() => setDimensionModal(false)}>
        <View style={{ flex: 1, backgroundColor: '#000A', justifyContent: 'center', padding: 18 }}>
          <ScrollView
            style={{ maxHeight: '90%', backgroundColor: PANEL, borderRadius: 18, padding: 16 }}
            keyboardShouldPersistTaps="handled"
          >
            <Text style={{ color: WHITE, fontSize: 18, fontWeight: '900', marginBottom: 12 }}>
              Équipement & dimensions
            </Text>
            <Field
              label="Désignation"
              value={editForm.designation}
              keyboardType="default"
              onChangeText={(value) => setEditForm((f) => ({ ...f, designation: value }))}
            />
            <View style={{ flexDirection: 'row', gap: 8 }}>
              <View style={{ flex: 1 }}>
                <Field
                  ...
                  keyboardType="default"
                  onChangeText={(value) => setEditForm((f) => ({ ...f, marque: value }))}
                />
              </View>
              <View style={{ flex: 1 }}>
                <Field
                  ...
                  keyboardType="default"
                  onChangeText={(value) => setEditForm((f) => ({ ...f, modele: value }))}
                />
              </View>
            </View>
            <View style={{ flexDirection: 'row', gap: 8 }}>
              <View style={{ flex: 1 }}>
                <Field
                  ...
                  onChangeText={(value) => setEditForm((f) => ({ ...f, width: value }))}
                />
              </View>
              <View style={{ flex: 1 }}>
                <Field
                  ...
                  onChangeText={(value) => setEditForm((f) => ({ ...f, depth: value }))}
                />
              </View>
              <View style={{ flex: 1 }}>
                <Field
                  ...
                  onChangeText={(value) => setEditForm((f) => ({ ...f, height: value }))}
                />
              </View>
            </View>
            <Field
              label="Hauteur par rapport au sol Z (m)"
              value={editForm.z}
              onChangeText={(value) => setEditForm((f) => ({ ...f, z: value }))}
            />
            {selected?.params?.support?.type === 'masonry' ? (
              <View style={{ marginTop: 5, padding: 12, backgroundColor: '#182C35', borderRadius: 12 }}>
                <Text style={{ color: WHITE, fontWeight: '900', marginBottom: 8 }}>Socle maçonné</Text>
                <View style={{ flexDirection: 'row', gap: 8 }}>
                  <View style={{ flex: 1 }}>
                    <Field
                      label="Débord par côté (m)"
                      value={editForm.supportOverhang}
                      onChangeText={(value) => setEditForm((f) => ({ ...f, supportOverhang: value }))}
                    />
                  </View>
                  <View style={{ flex: 1 }}>
                    <Field
                      label="Hauteur socle (m)"
                      value={editForm.supportHeight}
                      onChangeText={(value) => setEditForm((f) => ({ ...f, supportHeight: value }))}
                    />
                  </View>
                </View>
              </View>
            ) : null}
            <View style={{ flexDirection: 'row', gap: 8, marginTop: 14, marginBottom: 22 }}>
      ...
      <Modal visible={networkModal} transparent animationType="fade" onRequestClose={() => setNetworkModal(false)}>
        <View style={{ flex: 1, backgroundColor: '#000A', justifyContent: 'center', padding: 18 }}>
          <View style={{ backgroundColor: PANEL, borderRadius: 18, padding: 16, maxHeight: '88%' }}>
            <Text style={{ color: WHITE, fontSize: 18, fontWeight: '900' }}>Créer un réseau</Text>
            <Text style={{ color: MUTED, marginTop: 4, marginBottom: 10 }}>
              Départ : {selected?.label}. Choisis le type et le diamètre, puis touche l'équipement d'arrivée.
            </Text>
            <Text style={{ color: MUTED, fontWeight: '800', fontSize: 11, marginBottom: 6 }}>TYPE DE RÉSEAU</Text>
            ...
            <Text style={{ color: MUTED, fontWeight: '800', fontSize: 11, marginTop: 10, marginBottom: 6 }}>
              DIAMÈTRE
            </Text>
            <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ gap: 6 }}>
              {LAB3D_DN.map((dn) => (
            ...
            <View style={{ flexDirection: 'row', gap: 8, marginTop: 14 }}>
              <TinyButton flex label="Annuler" onPress={() => setNetworkModal(false)} />
      ...
      <Modal visible={positionModal} transparent animationType="fade" onRequestClose={() => setPositionModal(false)}>
        <View style={{ flex: 1, backgroundColor: '#000A', justifyContent: 'center', padding: 24 }}>
          <View style={{ backgroundColor: PANEL, borderRadius: 18, padding: 16 }}>
            <Text style={{ color: WHITE, fontSize: 18, fontWeight: '900', marginBottom: 12 }}>
              Position exacte · axe {axis}
            </Text>
      ...
      <Modal visible={rotationModal} transparent animationType="fade" onRequestClose={() => setRotationModal(false)}>
        <View style={{ flex: 1, backgroundColor: '#000A', justifyContent: 'center', padding: 24 }}>
          <View style={{ backgroundColor: PANEL, borderRadius: 18, padding: 16 }}>
            <Text style={{ color: WHITE, fontSize: 18, fontWeight: '900', marginBottom: 12 }}>Rotation exacte</Text>
