...
export function CuboidSolid({
  ...
  const faces = [
    ...
    .map((face, index) => ({
      ...
      key: index,
      depth:
        face.points.reduce((sum, point) => sum + cameraPoint3d(point, viewport, camera).depth, 0) / face.points.length
    }))
...
function PrismSolid({ object, viewport, camera, fill, selected = false, sides = 10, opacity = 1 }) {
  ...
  for (let i = 0; i < sides; i += 1) {
    ...
    faces.push({
      ...
      points,
      shade: 0.62 + (i % 3) * 0.07,
      depth: points.reduce((sum, p) => sum + cameraPoint3d(p, viewport, camera).depth, 0) / 4
    });
  }
  faces.push({
    key: 'top',
    points: ring1,
    shade: 0.96,
    depth: ring1.reduce((sum, p) => sum + cameraPoint3d(p, viewport, camera).depth, 0) / sides
  });
    ...
    <>
      {faces.map((face) => (
        <Polygon
          ...
          fill={fill}
          opacity={opacity * face.shade}
          stroke={selected ? LAB3D_RENDER_COLORS.cyan : '#D1DFE6'}
          strokeWidth={selected ? 2.1 : 0.6}
        />
      ))}
    </>
  );
...
function BoilerModel({ object, viewport, camera, skin, selected }) {
  const w = n(object.width, 1.4);
  const d = n(object.depth, 0.9);
  const h = n(object.height, 1.55);
  const body = partObject(object, { z: h * 0.05, width: w * 0.94, depth: d * 0.92, height: h * 0.9 });
...
function PumpModel({ object, viewport, camera, skin, selected }) {
  const w = n(object.width, 0.75);
  const d = n(object.depth, 0.45);
  const h = n(object.height, 0.55);
  const body = partObject(object, { z: h * 0.18, width: w * 0.38, depth: d * 0.72, height: h * 0.52 });
  const motor = partObject(object, { x: w * 0.2, z: h * 0.3, width: w * 0.42, depth: d * 0.58, height: h * 0.44 });
  const left = partObject(object, { x: -w * 0.36, z: h * 0.28, width: w * 0.28, depth: d * 0.48, height: h * 0.3 });
  const right = partObject(object, { x: w * 0.36, z: h * 0.28, width: w * 0.28, depth: d * 0.48, height: h * 0.3 });
...
function CylinderEquipment({ object, viewport, camera, skin, selected, controller = false }) {
  const h = n(object.height, 1.2);
  const d = Math.min(n(object.width, 0.7), n(object.depth, 0.7));
  const cylinder = partObject(object, { z: h * 0.03, width: d * 0.84, depth: d * 0.84, height: h * 0.9 });
  const foot = partObject(object, { z: 0, width: d * 0.6, depth: d * 0.6, height: h * 0.05 });
  const top = partObject(object, { z: h * 0.91, width: d * 0.4, depth: d * 0.4, height: h * 0.06 });
...
function ExchangerModel({ object, viewport, camera, skin, selected }) {
  const w = n(object.width, 0.9);
  const d = n(object.depth, 0.5);
  const h = n(object.height, 1.2);
...
function AirBoxModel({ object, viewport, camera, skin, selected }) {
  const w = n(object.width, 1.2);
  const d = n(object.depth, 0.8);
  const h = n(object.height, 0.8);
  const body = partObject(object, { z: h * 0.06, width: w * 0.94, depth: d * 0.9, height: h * 0.84 });
...
function CabinetModel({ object, viewport, camera, skin, selected }) {
  const w = n(object.width, 0.8);
  const d = n(object.depth, 0.35);
  const h = n(object.height, 1.8);
  const body = partObject(object, { width: w * 0.96, depth: d * 0.94, height: h * 0.98 });
  const door = partObject(object, { y: -d * 0.49, z: h * 0.08, width: w * 0.82, depth: d * 0.04, height: h * 0.8 });
  const handle = partObject(object, {
    x: w * 0.33,
    y: -d * 0.53,
    z: h * 0.47,
    width: w * 0.045,
...
export function EquipmentLowPoly({ object, equipment, skin, viewport, camera, selected = false }) {
  ...
  if (subtype === 'chaudiere')
    return <BoilerModel object={object} viewport={viewport} camera={camera} skin={skin} selected={selected} />;
  if (subtype === 'pompe')
    return <PumpModel object={object} viewport={viewport} camera={camera} skin={skin} selected={selected} />;
  if (subtype === 'adoucisseur')
    return (
      <CylinderEquipment
        object={object}
        viewport={viewport}
  ...
  if (subtype === 'ballon' || subtype === 'vase_expansion' || subtype === 'bouteille')
    return <CylinderEquipment object={object} viewport={viewport} camera={camera} skin={skin} selected={selected} />;
  if (subtype === 'echangeur')
    return <ExchangerModel object={object} viewport={viewport} camera={camera} skin={skin} selected={selected} />;
  if (subtype === 'cta' || subtype === 'vmc' || subtype === 'ventilateur')
    return <AirBoxModel object={object} viewport={viewport} camera={camera} skin={skin} selected={selected} />;
  if (subtype === 'armoire')
    return <CabinetModel object={object} viewport={viewport} camera={camera} skin={skin} selected={selected} />;
  if (subtype === 'bac_sel')
    return <CuboidSolid object={object} viewport={viewport} camera={camera} fill={skin.primary} selected={selected} />;
  return (
    <>
      <CuboidSolid object={object} viewport={viewport} camera={camera} fill={skin.primary} selected={selected} />
      <AccentBand object={object} viewport={viewport} camera={camera} skin={skin} />
    </>
  );
}
...
export function MasonryBaseLowPoly({ object, viewport, camera }) {
  ...
  const height = n(support.height, 0.1);
  const base = {
    ...object,
    z: n(object.z) - height,
    width: n(support.width, n(object.width) + overhang * 2),
    depth: n(support.depth, n(object.depth) + overhang * 2),
    height
  };
...
export function ArchitectureLowPoly({ object, viewport, camera, selected = false }) {
  if (object.subtype === 'floor') {
    return (
      <CuboidSolid
        ...
        viewport={viewport}
        camera={camera}
        fill={LAB3D_RENDER_COLORS.floor}
        selected={selected}
        opacity={0.52}
      />
    );
      ...
      <>
        {parts.map((part, index) => (
          <CuboidSolid
            ...
            camera={camera}
            fill="#7C858B"
            selected={selected && index === steps - 1}
            opacity={0.9}
          />
        ))}
      </>
  ...
  return (
    <CuboidSolid object={object} viewport={viewport} camera={camera} fill="#7C8B94" selected={selected} opacity={0.7} />
  );
}
...
export function OpeningLowPoly({ opening, wall, viewport, camera, selected = false }) {
  ...
  const thin = {
    ...
    y: n(opening.y),
    z: n(opening.z),
    width: n(opening.width, 0.9),
    depth: thickness,
    height: n(opening.height, 2.05),
  ...
  return (
    <CuboidSolid
      ...
      fill={kindColor}
      selected={selected}
      opacity={opening.kind === 'door' ? 0.9 : 0.62}
    />
  );
}
...
export function SceneGrid3D({ viewport, camera, step = 0.25, size = 12 }) {
  ...
  return (
    <>
      {lines}
      <Line x1={origin.x} y1={origin.y} x2={x.x} y2={x.y} stroke={LAB3D_RENDER_COLORS.axisX} strokeWidth={2} />
      <Line x1={origin.x} y1={origin.y} x2={y.x} y2={y.y} stroke={LAB3D_RENDER_COLORS.axisY} strokeWidth={2} />
      <Line x1={origin.x} y1={origin.y} x2={z.x} y2={z.y} stroke={LAB3D_RENDER_COLORS.axisZ} strokeWidth={2} />
    </>
  );
}
...
export function NetworkLowPoly({ network, viewport, camera, color = '#B6C8D0', selected = false }) {
  ...
  for (let i = 0; i < points.length - 1; i += 1) {
    const a = project3d(points[i], viewport, camera);
    const b = project3d(points[i + 1], viewport, camera);
  ...
  if (arrow && bestLength > 18) {
    const dx = arrow.b.x - arrow.a.x;
    const dy = arrow.b.y - arrow.a.y;
    const len = Math.max(1, Math.hypot(dx, dy));
    const ux = dx / len;
    const uy = dy / len;
    const px = -uy;
    const py = ux;
    const mx = (arrow.a.x + arrow.b.x) / 2;
    const my = (arrow.a.y + arrow.b.y) / 2;
  ...
  return (
    <>
      {points.slice(0, -1).map((point, index) => {
        ...
        const b = project3d(points[index + 1], viewport, camera);
        return (
          <Line
            key={index}
            x1={a.x}
...
export function ObjectLabel({ object, viewport, camera, selected = false, suffix = '' }) {
  ...
  return (
    <SvgText
      x={p.x}
      y={p.y}
      fill={selected ? LAB3D_RENDER_COLORS.cyan : '#DCEAF1'}
      fontSize="10.5"
      fontWeight="700"
      textAnchor="middle"
    >{`${String(object.label || object.subtype || 'Objet').slice(0, 24)}${suffix}`}</SvgText>
  );
}
...
export function RemarkMarker({ object, index = 0, viewport, camera }) {
  const p = project3d(
    {
      x: n(object.x) + 0.2 + index * 0.12,
      y: n(object.y) - 0.18,
      z: n(object.z) + n(object.height, 1) + 0.48 + index * 0.14
    },
    viewport,
    camera
  );
  return (
    <>
      <Circle cx={p.x} cy={p.y} r={8} fill="#FF5D63" stroke="#FFF" strokeWidth={1.5} />
      <SvgText x={p.x} y={p.y + 3.5} fill="#FFF" fontSize="8" fontWeight="900" textAnchor="middle">
        !
      </SvgText>
    </>
  );
