...
function LongField({ visiteId, sectionCode, label, storageKey, value, onSaved }) {
  ...
  return <View style={{ backgroundColor: 'rgba(255,255,255,0.82)', borderWidth: 1, borderColor: 'rgba(22,21,15,0.1)', borderRadius: 10, padding: 10, marginBottom: 7 }}><Text style={{ color: COLORS.ink, fontSize: 12, fontFamily: FONTS.black, marginBottom: 6 }}>{label}</Text><TextInput multiline value={text} onChangeText={setText} onBlur={() => save().catch(console.warn)} placeholder="Saisir ou compléter…" style={[styles.input, { minHeight: /Conclusion libre/i.test(label) ? 130 : 82, textAlignVertical: 'top', fontSize: 12 }]} /></View>;
}
...
export function PreAllumageConclusionPanel({ visiteId, onSaved }) {
  ...
  return <ScrollView contentContainerStyle={styles.panelContent} keyboardShouldPersistTaps="handled">
    <View style={{ backgroundColor: 'rgba(255,255,255,0.82)', borderWidth: 1, borderColor: 'rgba(22,21,15,0.1)', borderRadius: 13, padding: 12, marginBottom: 12 }}>
      <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}><View style={{ flex: 1 }}><Text style={{ color: COLORS.ink, fontSize: 15, fontFamily: FONTS.black }}>Conclusion proposée depuis les remarques</Text><Text style={{ color: COLORS.inkSoft, fontSize: 10, marginTop: 3 }}>{remarks.length} remarque{remarks.length > 1 ? 's' : ''} · {groupes.length} sujet{groupes.length > 1 ? 's' : ''} synthétisé{groupes.length > 1 ? 's' : ''}{doublons ? ` · ${doublons} répétition${doublons > 1 ? 's' : ''} regroupée${doublons > 1 ? 's' : ''}` : ''}</Text></View><TouchableOpacity onPress={() => load().catch(console.warn)} style={{ minHeight: 36, justifyContent: 'center', paddingHorizontal: 10, borderWidth: 1, borderColor: 'rgba(22,21,15,0.1)', borderRadius: 9 }}><Text style={{ color: COLORS.inkSoft, fontSize: 10, fontFamily: FONTS.black }}>Actualiser</Text></TouchableOpacity></View>
      {groupes.length ? <View style={{ marginTop: 10, gap: 7 }}>{groupes.map((g, i) => <View key={`${normaliserTexte(g.prestation)}-${i}`} style={{ borderTopWidth: i ? 1 : 0, borderTopColor: COLORS.line, paddingTop: i ? 7 : 0 }}><View style={{ flexDirection: 'row', gap: 7 }}><Text style={{ flex: 1, color: COLORS.ink, fontSize: 11, lineHeight: 16 }}>{ligneSynthese(g)}</Text><Text style={{ color: g.criticite >= 4 ? COLORS.red : COLORS.inkSoft, fontSize: 9, fontFamily: FONTS.black }}>C{g.criticite} · {reserveSeverityLabel(g.criticite)}</Text></View></View>)}</View> : <Text style={{ color: COLORS.inkSoft, fontSize: 11, marginTop: 10 }}>Aucune remarque : la conclusion reste entièrement libre.</Text>}
      {groupes.length ? <TouchableOpacity onPress={() => inserer().catch(console.warn)} style={{ alignSelf: 'flex-start', marginTop: 12, minHeight: 40, justifyContent: 'center', paddingHorizontal: 12, borderRadius: 10, backgroundColor: COLORS.orange }}><Text style={{ color: COLORS.white, fontFamily: FONTS.black, fontSize: 11 }}>Insérer cette synthèse dans la conclusion</Text></TouchableOpacity> : null}
    </View>

    {rubriques.map((r) => <View key={r.id} style={{ marginBottom: 10 }}><Text style={{ color: COLORS.ink, fontSize: 14, fontFamily: FONTS.black, marginBottom: 5 }}>{r.nom}</Text>{(r.champs || []).map((c) => { const key = `${r.section_code}||${c.field.cle}`; return <LongField key={c.id} visiteId={visiteId} sectionCode={r.section_code} label={c.libelle || c.field.cle} storageKey={c.field.cle} value={champs[key]} onSaved={(value) => { setChamps((m) => ({ ...m, [key]: value })); onSaved?.(); }} />; })}</View>)}
  </ScrollView>;
}
