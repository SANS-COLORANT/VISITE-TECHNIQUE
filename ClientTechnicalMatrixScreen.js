...
const MATRIX_FAST_CACHE = new Map();
const STATE = {
...
function Cell({ cell, onPress }) { const p = STATE[cell.state] || STATE.none; const count = cell.ns || cell.nr || cell.nv || cell.s || 0; return <TouchableOpacity disabled={!cell.total} onPress={onPress} style={{ width: 92, minHeight: 58, margin: 3, borderRadius: 10, borderWidth: 1.5, borderColor: p.border, backgroundColor: p.bg, alignItems: 'center', justifyContent: 'center' }}><Text style={{ fontSize: 18, fontWeight: '900', color: p.text }}>{p.symbol} {count || ''}</Text><Text style={{ fontSize: 8.5, color: p.text, marginTop: 2 }}>{cell.total ? `${cell.s} S · ${cell.ns} N.S` : 'Non concerné'}</Text></TouchableOpacity>; }

export function ClientTechnicalMatrixScreen({ route, navigation }) {
  const { clientId, nomClient } = route.params || {};
  const key = String(clientId || '');
  const [data, setData] = useState(() => MATRIX_FAST_CACHE.get(key) || null);
  const [selected, setSelected] = useState(null);
  const [photos, setPhotos] = useState({});
  const [photoZoom, setPhotoZoom] = useState(null);
  const load = useCallback(async () => {
    const next = await getClientTechnicalMatrix(clientId);
    MATRIX_FAST_CACHE.set(key, next);
    setData(next);
  }, [clientId, key]);
  ...
  const openCell = async (site, category, cell) => {
    ...
    setPhotos({});
    // Les miniatures ne bloquent plus l'ouverture de la pop-up : toutes les
    // recherches de photos indépendantes démarrent en parallèle.
    const entries = await Promise.all((cell.issues || []).map(async (issue) => [`${issue.id}-${issue.cle}`, await getMatrixCellPhotos(issue)]));
    setPhotos(Object.fromEntries(entries));
  };
  if (!data) return <View style={styles.center}><ActivityIndicator size="large" color={COLORS.orange}/></View>;

  const renderSite = ({ item: site }) => <View style={{ flexDirection: 'row', borderTopWidth: 1, borderTopColor: COLORS.line, alignItems: 'center' }}><TouchableOpacity onPress={() => navigation.navigate('SiteVisites', { siteId: site.id, nomSite: site.nom_site })} style={{ width: 180, padding: 9 }}><Text style={{ fontWeight: '800', color: COLORS.primary }}>{site.nom_site}</Text><Text style={{ fontSize: 9.5, color: COLORS.muted }} numberOfLines={2}>{site.adresse || 'Adresse non renseignée'}</Text></TouchableOpacity>{data.categories.map((c) => <Cell key={c.key} cell={site.cells[c.key]} onPress={() => openCell(site, c, site.cells[c.key])}/>)}</View>;

  return <View style={{ flex: 1, backgroundColor: COLORS.bg }}>
    <View style={{ padding: 12, borderBottomWidth: 1, borderBottomColor: COLORS.line }}><Text style={{ fontWeight: '900', fontSize: 17, color: COLORS.ink }}>Cartographie technique · {nomClient || 'Client'}</Text><Text style={{ color: COLORS.muted, fontSize: 11, marginTop: 3 }}>Vert = OK · orange = à traiter · rouge = critique · gris = information incomplète. Les métiers restent séparés dans les données.</Text></View>
    <ScrollView horizontal style={{ flex: 1 }} contentContainerStyle={{ padding: 8 }}>
      <View style={{ flex: 1 }}>
        <View style={{ flexDirection: 'row' }}><View style={{ width: 180, padding: 8 }}><Text style={{ fontWeight: '900' }}>Sites</Text></View>{data.categories.map((c) => <View key={c.key} style={{ width: 98, padding: 6, justifyContent: 'center' }}><Text style={{ textAlign: 'center', fontSize: 10, fontWeight: '800' }}>{c.label}</Text></View>)}</View>
        <FlatList
          data={data.sites}
