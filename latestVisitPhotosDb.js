...
export function normalizeLatestVisitPhotosManifest(payload = {}, expectedClientId = null) {
  ...
  const sites = list(payload?.sites).map((entry, siteIndex) => {
    ...
    const locaux = list(entry?.locaux)
      .map((localEntry, localIndex) => {
        ...
        const visitSource =
          localEntry?.derniereVisite && typeof localEntry.derniereVisite === 'object'
...
export function flattenLatestVisitPhotos(manifest) {
  return list(manifest?.sites).flatMap((siteEntry) =>
    list(siteEntry?.locaux).flatMap((localEntry) =>
...
export async function cacheLatestVisitPhotosManifest(remoteClientId, payload) {
  ...
  await withPhotoTransaction(database, async (database) => {
    const previousFiles = await database.getAllAsync(
      `SELECT * FROM api_latest_visit_photos WHERE remote_client_id=? AND local_uri IS NOT NULL`,
      [clientId]
    );
    for (const row of previousFiles) {
      ...
      await database.runAsync(
        ...
        [
          clientId,
          row.remote_photo_id,
          photoFileKey(previous),
          row.local_uri,
          Number(row.downloaded_bytes || 0),
    ...
    await database.runAsync(
      ...
      [
        clientId,
        manifest.client.nom,
        manifest.nombreSites,
        manifest.nombrePhotosDisponibles,
        manifest.volumePhotosDisponibles,
        json(manifest)
      ]
    );
    await database.runAsync(`UPDATE api_latest_visit_photos SET manifest_present=0 WHERE remote_client_id=?`, [
      clientId
    ]);
    ...
    for (const photo of photos) {
      await database.runAsync(
        ...
        [
          ...
          photo.site?.nom || null,
          photo.local?.id || null,
          photo.local?.designation || null,
          photo.derniereVisite?.id || null,
          photo.derniereVisite?.date || null,
          photo.derniereVisite?.statut || null,
          photo.description,
          photo.ordre,
          photo.grandFormat ? 1 : 0,
          photo.disponible ? 1 : 0,
          photo.typeMime,
          photo.tailleOctets,
          photo.largeurPixels,
          photo.hauteurPixels,
          photo.cheminTelechargement,
          photo.disponible ? 'pending' : 'unavailable',
          json(photo)
        ]
      );
    }
...
function photoFromCacheRow(row) {
  return {
    id: row.remote_photo_id,
    site: { id: row.remote_site_id },
    local: { id: row.remote_local_id },
    derniereVisite: { id: row.remote_visit_id },
    typeMime: row.mime_type,
    tailleOctets: row.expected_bytes,
    largeurPixels: row.width_pixels,
    hauteurPixels: row.height_pixels
  };
}
...
export async function hydratePhotoMetadata(remoteClientId, manifest) {
  ...
  return mapLatestVisitPhotos(manifest, (photo) => {
    const key = photoFileKey(photo),
      cached = rowsByKey.get(key),
      file = filesByKey.get(key);
    ...
    return {
      ...
      downloadError: cached?.last_error || null,
      downloadedAt: file?.downloaded_at || cached?.downloaded_at || null
    };
  });
}
...
export async function getCachedLatestVisitPhotosManifest(remoteClientId) {
  ...
  const database = await openAppDatabase();
  const row = await database.getFirstAsync(`SELECT * FROM api_latest_visit_photo_manifests WHERE remote_client_id=?`, [
    clientId
  ]);
  if (!row?.payload_json) return null;
  let manifest;
  try {
    manifest = normalizeLatestVisitPhotosManifest(JSON.parse(row.payload_json), clientId);
  } catch {
    return null;
  }
  ...
  await withPhotoTransaction(database, async (database) => {
    ...
    await database.runAsync(
      ...
      [
        ...
        clean(photo.id),
        clean(photo.derniereVisite?.id),
        Number(photo.tailleOctets || 0),
        clean(photo.typeMime),
        clean(photo.site?.id),
        clean(photo.local?.id),
        Number(photo.largeurPixels || 0),
        Number(photo.hauteurPixels || 0)
      ]
    );
  });
}
...
export async function markLatestVisitPhotoError(remoteClientId, photo, error) {
  await (
    ...
    `UPDATE api_latest_visit_photos SET download_status='error',last_error=?
     WHERE remote_client_id=? AND remote_photo_id=? AND COALESCE(remote_visit_id,'')=?`,
    [
      String(error?.message || error || 'Téléchargement impossible'),
      clean(remoteClientId),
      clean(photo.id),
      clean(photo.derniereVisite?.id)
    ]
    ...
    if (scope?.sites.length)
      await database.runAsync(
        ...
        [
          ...
          clean(remoteClientId),
          clean(remoteSiteId),
          json(normalizeLatestVisitPhotosManifest(scope, remoteClientId))
        ]
      );
  }
...
export async function resolvePhotoContexts({
  visiteId = null,
  siteId = null,
  remoteLocalId = null,
  ignoreVisitLocal = false
} = {}) {
  const database = await openAppDatabase();
  const visit = visiteId
    ? await database.getFirstAsync(`SELECT site_id,api_remote_local_id,installation_id FROM visites WHERE id=?`, [
        visiteId
      ])
    : null;
  ...
  if (!localId && !ignoreVisitLocal && visit?.installation_id) {
    const links = await database.getAllAsync(
      `SELECT remote_local_id FROM api_local_links WHERE local_installation_id=?`,
      [visit.installation_id]
    );
  ...
  const rows = await database.getAllAsync(
    `SELECT DISTINCT cs.remote_client_id,cs.remote_site_id,c.nom AS client_name,s.nom AS site_name,
     ...
     WHERE cs.local_site_id=? ORDER BY own_client DESC,c.nom`,
    [localSiteId]
  );
  ...
  for (const row of candidates) {
    if (localId) {
      const room = await database.getFirstAsync(`SELECT remote_site_id FROM api_local_links WHERE remote_local_id=?`, [
        localId
      ]);
      if (room && String(room.remote_site_id) !== String(row.remote_site_id)) continue;
    }
    results.push({
      client: { remote_client_id: row.remote_client_id, nom: row.client_name },
      remoteSiteId: row.remote_site_id,
