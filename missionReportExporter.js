...
async function photoDataUri(uri) {
  if (!uri) return null;
  try {
    const result = await ImageManipulator.manipulateAsync(uri, [{ resize: { width: 1100 } }], {
      compress: 0.72,
...
async function buildHtml(report) {
  ...
  const photos = photoItems.length
    ? '<section class="pageBreak"><h2>Photographies</h2><div class="photos">' +
      photoItems
        .map(
          (p) =>
            '<figure><img src="' +
  ...
  return (
    ...
    '</style></head><body>' +
    '<h1>' +
    esc(report.profile?.label || mission.label || 'Rapport Mission') +
    '</h1>' +
    '<div class="meta"><span class="badge">METRA Missions</span> ' +
    esc(mission.client_name || '') +
    ' · ' +
    esc(mission.reference || '') +
    '</div>' +
    body +
    photos +
    '</body></html>'
...
function wcell(text, bold = false) {
  return (
    '<w:tc><w:tcPr><w:tcMar><w:top w:w="70" w:type="dxa"/><w:left w:w="80" w:type="dxa"/><w:bottom w:w="70" w:type="dxa"/><w:right w:w="80" w:type="dxa"/></w:tcMar></w:tcPr>' +
    wp(text, { bold, size: 17, after: 0 }) +
    '</w:tc>'
  );
}
...
function wtable(headers = [], rows = []) {
  const border =
    '<w:tblBorders><w:top w:val="single" w:sz="4" w:color="B9C8BF"/><w:left w:val="single" w:sz="4" w:color="B9C8BF"/><w:bottom w:val="single" w:sz="4" w:color="B9C8BF"/><w:right w:val="single" w:sz="4" w:color="B9C8BF"/><w:insideH w:val="single" w:sz="4" w:color="D9E2DC"/><w:insideV w:val="single" w:sz="4" w:color="D9E2DC"/></w:tblBorders>';
...
async function buildDocx(report, outputUri) {
  ...
  const body = [];
  body.push(
    wp(report.profile?.label || mission.label || 'Rapport Mission', {
      bold: true,
      size: 38,
      color: '2F7D58',
      after: 150
  ...
  body.push(
    wp((mission.client_name || '') + (mission.reference ? ' · ' + mission.reference : ''), {
      size: 18,
      color: '607168',
      after: 220
    })
  ...
  if ((report.data?.photos || []).length) {
    ...
    body.push(
      wp(
        'Les photographies originales et leurs références sont conservées dans METRA et dans l’export Excel complet.',
        { size: 18, color: '607168' }
      )
    );
    body.push(
      wtable(
        ['Photo', 'Type', 'Date', 'Référence fichier'],
        (report.data.photos || []).map((p, i) => [
          p.label || 'Photo ' + (i + 1),
          p.type || '',
          p.taken_at || '',
          p.file_uri || ''
  ...
  const core =
    ...
    xmlEsc(report.profile?.label || 'Rapport Mission') +
    '</dc:title><dc:creator>METRA</dc:creator><dcterms:created xsi:type="dcterms:W3CDTF">' +
    new Date().toISOString() +
    '</dcterms:created></cp:coreProperties>';
  const app =
    '<?xml version="1.0" encoding="UTF-8" standalone="yes"?><Properties xmlns="http://schemas.openxmlformats.org/officeDocument/2006/extended-properties"><Application>METRA</Application></Properties>';
...
export async function exporterRapportMissionPdf(missionId, { share = true, siteId = null } = {}) {
  ...
  if (share && (await Sharing.isAvailableAsync()))
    await Sharing.shareAsync(uri, { mimeType: PDF_MIME, dialogTitle: 'Rapport METRA Missions' });
  return { uri, name };
}
...
export async function exporterRapportMissionDocx(missionId, { share = true, siteId = null } = {}) {
  ...
  if (share && (await Sharing.isAvailableAsync()))
    await Sharing.shareAsync(uri, { mimeType: DOCX_MIME, dialogTitle: 'Rapport Word METRA Missions' });
  return { uri, name };
}
