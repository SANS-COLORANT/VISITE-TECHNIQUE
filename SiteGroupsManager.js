  ...
  return <FlatList
    ...
    renderItem={({ item: site }) => <View style={{ borderWidth: 1, borderColor: 'rgba(22,21,15,0.1)', backgroundColor: 'rgba(255,255,255,0.82)', borderRadius: 12, padding: 11, marginBottom: 8 }}>
      ...
      {groupes.length ? <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 6, marginTop: 8 }}>{groupes.map((g) => {
        ...
        return <TouchableOpacity key={g.id} onPress={() => onToggle(site, g)} style={{ minHeight: 36, paddingHorizontal: 10, borderRadius: 10, borderWidth: 1, borderColor: active ? COLORS.orange : COLORS.line, backgroundColor: active ? '#FFF3E8' : '#fff', justifyContent: 'center' }}><Text style={{ fontSize: 10.5, fontFamily: FONTS.bold, color: active ? COLORS.orange : COLORS.inkSoft }}>{active ? '✓ ' : ''}{g.nom}</Text></TouchableOpacity>;
      })}</View> : null}
    </View>}
  />;
}
