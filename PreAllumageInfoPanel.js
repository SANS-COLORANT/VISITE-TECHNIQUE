...
function InfoField({ visiteId, sectionCode, field, valeurInitiale, onSaved }) {
  ...
  useEffect(() => {
    setValeur(estDate ? dateAffichee(valeurInitiale) : String(valeurInitiale || ''));
    setErreur(false);
  }, [valeurInitiale, estDate]);
  ...
  return (
    <View style={{ flex: 1, minWidth: 0, paddingVertical: 6 }}>
      <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 5 }}>
        <Text style={{ flex: 1, color: COLORS.ink, fontSize: 12, fontWeight: '800' }}>{label}</Text>
        {estNombre ? <Text style={{ color: COLORS.inkSoft, fontSize: 9, fontWeight: '800' }}>AUTO</Text> : null}
      </View>
      <TextInput
        style={[
          styles.input,
          { minHeight: multiline ? 68 : 42, textAlignVertical: multiline ? 'top' : 'center', fontSize: 12 },
          erreur && { borderColor: COLORS.red }
        ]}
        multiline={multiline}
        value={valeur}
        onChangeText={(v) => {
          setErreur(false);
          setValeur(estDate ? masquerDate(v) : estNombre ? v.replace(/\D/g, '') : v);
        }}
...
function grouper(champs, deuxColonnes) {
  ...
  let tampon = [];
  const vider = () => {
    if (tampon.length) rows.push({ key: tampon.map((x) => x.key).join('|'), items: tampon });
    tampon = [];
  };
...
export function PreAllumageInfoPanel({ visiteId, onSaved }) {
  ...
  useEffect(() => {
    let alive = true;
    Promise.all([chargerPreAllumageModulaire(visiteId), getChampsVisite(visiteId)])
      .then(([m, c]) => {
        if (alive) {
          setModele(m);
    ...
    <SectionList
      ...
      renderItem={({ item, section }) => (
        ...
        >
          <View
            style={{
              flexDirection: item.large || !deuxColonnes ? 'column' : 'row',
              gap: item.large || !deuxColonnes ? 0 : 12
            }}
          >
            {item.items.map((x, i) => (
              <View
                ...
                style={{
                  flex: 1,
                  minWidth: 0,
                  paddingLeft: deuxColonnes && !item.large && i > 0 ? 10 : 0,
                  borderLeftWidth: deuxColonnes && !item.large && i > 0 ? 1 : 0,
