...
function panelId(index) {
  return `p-vmc-c${index}`;
}
function sectionSituation(index) {
  return `vmc-c${index}.situation`;
}
function cleIdentification(index) {
  return `Identification du caisson n°${index}`;
}
function cleActif(index) {
  return `caisson_${index}_actif`;
}
function cleNom(index) {
  return `caisson_${index}_nom`;
}
...
export async function chargerCaissonsVmc(visiteId) {
  ...
  const [configRows, identRows, donneesExistantes] = await Promise.all([
    db.getAllAsync(`SELECT cle,valeur FROM champs_visite WHERE visite_id=? AND section_code=?`, [
      visiteId,
      CONFIG_SECTION
    ]),
    db.getAllAsync(
      `SELECT section_code,cle,valeur FROM champs_visite WHERE visite_id=? AND section_code LIKE 'vmc-c%.situation'`,
      [visiteId]
    ),
    db.getAllAsync(
  ...
  for (const row of identRows || []) {
    ...
    if (index && row.cle === cleIdentification(index) && String(row.valeur || '').trim())
      ident.set(index, String(row.valeur).trim());
  }
...
export async function retirerCaissonVmc(visiteId, index) {
  ...
  const remarques = await db.getAllAsync(`SELECT id FROM remarques WHERE visite_id=? AND controle_key LIKE ?`, [
    visiteId,
    prefixControle
  ]);
  ...
  await db.runAsync(`DELETE FROM controles_visite WHERE visite_id=? AND section_code LIKE ?`, [
    visiteId,
    prefixSection
  ]);
...
export function VmcCaissonManager({ visiteId, caissons = [], onChange, onNavigate }) {
  ...
  const ajouter = async () => {
    try {
      ...
      Alert.alert('Caissons VMC', String(e?.message || e));
    }
  };
    ...
    Alert.alert(
      ...
      [
        { text: 'Annuler', style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
              await retirerCaissonVmc(visiteId, caisson.index);
              await recharger();
            } catch (e) {
              Alert.alert('Caissons VMC', String(e?.message || e));
            }
  ...
  return (
    <View style={{ marginBottom: 8 }}>
      <View style={{ flexDirection: 'row', alignItems: 'center' }}>
        ...
        >
          {actifs.map((c) => (
            ...
            >
              ...
              <TouchableOpacity
                onPress={() => setEdition(c.index)}
                style={{ paddingHorizontal: 8, paddingVertical: 6 }}
              >
                <Text style={{ color: COLORS.orangeDark, fontWeight: '800' }}>✎</Text>
              </TouchableOpacity>
              <TouchableOpacity onPress={() => demanderRetrait(c)} style={{ paddingRight: 9, paddingVertical: 6 }}>
                <Text style={{ color: COLORS.inkFaint, fontWeight: '800' }}>×</Text>
              </TouchableOpacity>
            </View>
          ))}
        </ScrollView>
        <TouchableOpacity
          onPress={ajouter}
          disabled={actifs.length >= MAX_CAISSONS}
          style={{
            marginLeft: 6,
            paddingHorizontal: 10,
            paddingVertical: 6,
            borderRadius: 16,
            backgroundColor: actifs.length >= MAX_CAISSONS ? COLORS.line : COLORS.orangeLight
          }}
        >
          <Text
            style={{
              fontSize: 11,
              fontWeight: '800',
              color: actifs.length >= MAX_CAISSONS ? COLORS.inkFaint : COLORS.orangeDark
            }}
          >
            + Ajouter
          </Text>
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalSheet}>
            ...
            <TextInput
              ...
              value={nom}
              onChangeText={setNom}
              placeholder={`Caisson ${edition || ''}`}
            />
            <View style={styles.modalActions}>
              <TouchableOpacity style={styles.btnSecondary} onPress={() => setEdition(null)}>
                <Text style={styles.btnSecondaryText}>Annuler</Text>
              </TouchableOpacity>
              <TouchableOpacity style={styles.btnPrimary} onPress={enregistrerNom}>
