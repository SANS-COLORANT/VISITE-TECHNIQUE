...
export function SiteOverviewPanel({ siteId, mode }) {
  ...
  if (mode === 'equipements') {
    return <View>
      ...
      <FlatList data={rows} keyExtractor={(item) => item.id} scrollEnabled={false}
        ...
        ListEmptyComponent={<View style={styles.empty}><EmptyIcon name="equipment" /><Text style={styles.emptyText}>{sousMenu === 'historique' ? 'Aucun équipement dans l’historique.' : 'Aucun équipement actif connu pour ce site.'}</Text></View>}
  ...
  return <View>
    ...
    {periode === 'entre' && visites.length ? <View style={{ backgroundColor: 'rgba(255,255,255,0.82)', borderWidth: 1, borderColor: 'rgba(22,21,15,0.1)', borderRadius: 12, padding: 10, marginBottom: 12 }}><Text style={{ fontFamily: FONTS.bold, marginBottom: 8 }}>Comparer deux visites</Text><View style={{ gap: 7 }}><View style={{ flexDirection: 'row', alignItems: 'center' }}><Text style={{ width: 58, color: COLORS.muted, fontSize: 11 }}>Début</Text><TouchableOpacity accessibilityRole="button" accessibilityLabel="Retour" onPress={() => changerVisite('debut', -1)} style={{ padding: 8 }}><CvcIcon name="chevron-left" size={16} color={COLORS.orangeDark} strokeWidth={2.1} /></TouchableOpacity><Text style={{ flex: 1, textAlign: 'center', fontFamily: FONTS.bodyBold }}>{visiteDebut?.date_visite || '—'}</Text><TouchableOpacity accessibilityRole="button" accessibilityLabel="Ouvrir" onPress={() => changerVisite('debut', 1)} style={{ padding: 8 }}><CvcIcon name="chevron-right" size={16} color={COLORS.orangeDark} strokeWidth={2.1} /></TouchableOpacity></View><View style={{ flexDirection: 'row', alignItems: 'center' }}><Text style={{ width: 58, color: COLORS.muted, fontSize: 11 }}>Fin</Text><TouchableOpacity accessibilityRole="button" accessibilityLabel="Retour" onPress={() => changerVisite('fin', -1)} style={{ padding: 8 }}><CvcIcon name="chevron-left" size={16} color={COLORS.orangeDark} strokeWidth={2.1} /></TouchableOpacity><Text style={{ flex: 1, textAlign: 'center', fontFamily: FONTS.bodyBold }}>{visiteFin?.date_visite || '—'}</Text><TouchableOpacity accessibilityRole="button" accessibilityLabel="Ouvrir" onPress={() => changerVisite('fin', 1)} style={{ padding: 8 }}><CvcIcon name="chevron-right" size={16} color={COLORS.orangeDark} strokeWidth={2.1} /></TouchableOpacity></View></View>{datesValides ? <Text style={{ color: COLORS.muted, fontSize: 10.5, marginTop: 7, textAlign: 'center' }}>Période : {datesValides.debut.date_visite} → {datesValides.fin.date_visite}</Text> : null}</View> : null}
