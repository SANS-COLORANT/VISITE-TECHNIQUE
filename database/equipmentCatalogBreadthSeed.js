...
async function ensureModel(db, [category, brand, name, desc, quality]) {
  const categoryId = await getCategoryId(db, category),
    brandId = await getBrandId(db, brand);
  ...
  if (existing) {
    await db.runAsync(
      "UPDATE modeles_equipement SET actif=1, caracteristiques=COALESCE(caracteristiques,?), data_quality=CASE WHEN data_quality IS NULL OR data_quality='catalogue' THEN ? ELSE data_quality END, verified_at=CASE WHEN ? LIKE 'verified%' THEN COALESCE(verified_at,?) ELSE verified_at END WHERE id=?",
      [desc, quality, quality, VERIFIED_AT, existing.id]
    );
    return;
  }
  await db.runAsync(
    `INSERT INTO modeles_equipement(id,categorie_id,marque_id,nom,caracteristiques,mots_cles,data_quality,verified_at) VALUES(?,?,?,?,?,?,?,?)`,
    [
      createId('model'),
      categoryId,
      brandId,
