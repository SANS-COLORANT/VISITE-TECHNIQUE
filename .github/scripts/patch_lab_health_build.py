...
# Keep the panel lazy so opening Settings does not evaluate LAB modules.
# ---------------------------------------------------------------------------
p = Path('ParametresScreen.js')
s = p.read_text(encoding='utf-8')
lazy_lab = "function LabMetraPanelLazy(){const Component=require('./LabMetraPanel.js').LabMetraPanel;return <Component/>;}\n"
if 'LabMetraPanelLazy' not in s and "import { LabMetraPanel } from './LabMetraPanel.js';" not in s:
    marker = "function supportDumpTools(){return require('./supportDump.js');}\n"
    if marker not in s:
        raise SystemExit('parameters lazy LAB marker not found')
    s = s.replace(marker, marker + lazy_lab, 1)
...
if "onglet==='lab'?" not in s:
    render_old = "onglet==='equipements'?contenuEquipements:<GestionDonnees/>"
    render_new = "onglet==='equipements'?contenuEquipements:onglet==='lab'?<LabMetraPanelLazy/>:<GestionDonnees/>"
    if "import { LabMetraPanel } from './LabMetraPanel.js';" in s:
        render_new = "onglet==='equipements'?contenuEquipements:onglet==='lab'?<LabMetraPanel/>:<GestionDonnees/>"
...
# The feature module and health panel stay lazy to avoid adding latency to the
# ordinary Visites / Équipements / Remarques paths.
# ---------------------------------------------------------------------------
p = Path('SiteVisitesScreen.js')
s = p.read_text(encoding='utf-8')
if 'healthLabEnabled' not in s:
    helper_marker = "function chargerBatchExcel(){return require('./batchExcel.js');}\n"
    helper_new = helper_marker + "function chargerLabSettings(){return require('./featureSettings.js');}\nfunction SiteHealthPanelLazy(props){const Component=require('./SiteHealthPanel.js').SiteHealthPanel;return <Component {...props}/>;}\n"
    s = replace_once(s, helper_marker, helper_new, 'site health lazy helpers')
    ...
    effect_marker = "  useEffect(() => { charger().catch(() => {}); }, [charger]);\n"
    effect_line = effect_marker + "  useEffect(() => { chargerLabSettings().getLabFeatureEnabled('health_dashboard').then(setHealthLabEnabled).catch(()=>setHealthLabEnabled(false)); }, []);\n"
    s = replace_once(s, effect_marker, effect_line, 'site health effect')

if "healthLabEnabled ? [...SITE_TABS" not in s:
    s = replace_once(
        s,
        "{SITE_TABS.map((tab) => {",
        "{(healthLabEnabled ? [...SITE_TABS, { id: 'sante', label: 'Santé' }] : SITE_TABS).map((tab) => {",
        'site tabs map',
    )

if "siteTab === 'sante' ? <SiteHealthPanel" not in s and "siteTab === 'sante' ? <SiteHealthPanelLazy" not in s:
    empty_old = "ListEmptyComponent={siteTab === 'visites' ? <View style={styles.empty}><Text style={styles.emptyText}>Aucune visite pour ce site pour l'instant.</Text><Text style={styles.emptySub}>Lance la première avec le bouton ci-dessous.</Text></View> : <SiteOverviewPanel siteId={siteId} mode={siteTab} />}"
    empty_new = "ListEmptyComponent={siteTab === 'visites' ? <View style={styles.empty}><Text style={styles.emptyText}>Aucune visite pour ce site pour l'instant.</Text><Text style={styles.emptySub}>Lance la première avec le bouton ci-dessous.</Text></View> : siteTab === 'sante' ? <SiteHealthPanelLazy siteId={siteId} siteName={nomSite} /> : <SiteOverviewPanel siteId={siteId} mode={siteTab} />}"
...
if 'health:healthLabEnabled&&inclureSante' not in s:
    config_marker = "photos:inclurePhotos,"
    if config_marker not in s:
        raise SystemExit('report health config marker not found')
    s = s.replace(config_marker, config_marker + "health:healthLabEnabled&&inclureSante,", 1)
...
print('METRA LAB health wired compatibly with the low-latency runtime.')