...
function patchLazyActionModules() {
  ...
  let text = read(path);
  text = text.replace(
    /import\s*\{\s*choisirEtAnalyserExcels\s*,\s*importerAnalysesExcel\s*\}\s*from\s*['"]\.\/batchExcel\.js['"];\s*/g,
    ''
  );
  ...
  text = text.replace(
    /\bimporterAnalysesExcel\(importBatch\.analyses\)/g,
    'chargerBatchExcelModule().importerAnalysesExcel(importBatch.analyses)'
  );
  write(path, text);
  ...
  text = text.replace(
    /import\s*\{\s*exporterRapportPreAllumage\s*\}\s*from\s*['"]\.\/preAllumageReportExporter\.js['"];\s*/g,
    ''
  );
  ...
  text = text.replace(
    /\bexporterRapportPreAllumage\(visiteId,\s*format\)/g,
    'chargerPreAllumageReportModule().exporterRapportPreAllumage(visiteId, format)'
  );
  write(path, text);
}
...
function patchDatabaseBootstrap() {
  ...
  text = text.replace(
    /      await syncReferenceCatalog\(db\);\n      await seedEquipmentCatalog\(db\);/g,
    '      await assurerReferentielsBase(db);'
  );
  if (!text.includes('await assurerReferentielsBase(db);'))
    throw new Error('database core catalogue guard not applied');
  write(path, text);
}
...
function patchSiteOverviewReloads() {
  ...
  if (!text.includes('}, [siteId, mode, sousMenu]);'))
    throw new Error('site overview dependency optimization not applied');
  write(path, text);
}
...
function patchCatalogueSearch() {
  ...
  text = text.replace(
    /rechercherCatalogueIntelligent\(\{recherche:search\}\)/g,
...
function patchVisualPackLazyTools() {
  ...
  let text = read(path);
  text = text
    .replace(/import \* as DocumentPicker from ['"]expo-document-picker['"];\s*/g, '')
    .replace(/import \{ unzip \} from ['"]react-native-zip-archive['"];\s*/g, '');
...
console.log(
  'Runtime responsiveness v3 applied: deferred screens/actions, version-gated catalogues, coalesced prefill, bounded map and debounced catalogue work.'
);
