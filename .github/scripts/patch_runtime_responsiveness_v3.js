...
function patchAppDeferredScreens() {
  ...
  if (!/import\s*\{\s*HydraulicSchemaWorkspace\s*\}\s*from\s*['"]\.\/HydraulicSchemaWorkspace\.js['"]/.test(text)) throw new Error('App hydraulic compatibility import missing');
  write(path, text);
}
...
function patchLazyActionModules() {
  ...
  // ClientDocuments garde ici son import historique : le patch Android d'export
  // typé s'appuie dessus. L'écran lui-même est déjà différé depuis App.js.
...
function patchDatabaseBootstrap() {
  ...
  let text = read(path);
  const staticSymbols = [
    ['syncReferenceCatalog', './referenceCatalog.js'], ['seedEquipmentCatalog', './equipmentCatalogSeed.js'],
    ['seedEquipmentCatalogExtra', './equipmentCatalogExtraSeed.js'], ['seedEquipmentCatalogBreadth', './equipmentCatalogBreadthSeed.js'],
    ['seedEquipmentCatalogDeep', './equipmentCatalogDeepSeed.js'], ['seedEquipmentCatalogDeep2', './equipmentCatalogDeepSeed2.js'],
    ['seedEquipmentCatalogDeep3', './equipmentCatalogDeepSeed3.js'], ['seedEquipmentCatalogDeep4', './equipmentCatalogDeepSeed4.js'],
    ['seedEquipmentCatalogAir', './equipmentCatalogAirSeed.js'], ['seedEquipmentCatalogVentilation', './equipmentCatalogVentilationSeed.js'],
    ['seedEquipmentCatalogHydronics', './equipmentCatalogHydronicsSeed.js'], ['seedEquipmentCatalogPeripheral', './equipmentCatalogPeripheralSeed.js'],
    ['seedEquipmentCatalogImages', './equipmentCatalogImageSeed.js'], ['seedEquipmentCatalogVisuals', './equipmentCatalogVisualSeed.js'],
  ];
...
function patchSiteOverviewReloads() {
  const path='SiteOverviewPanel.js';let text=read(path);
...
function patchClientMap() {
  const path='ClientMapScreen.js';let text=read(path);
  if(!text.includes('synchroniserCoordonneesClient(clientId, { max: 12 })')){
    const target='        const r = await synchroniserCoordonneesClient(clientId);';requireAnchor(text,target,'map automatic geocoding');
  ...
  if(!text.includes('tracksViewChanges={false}')){
    const target='                  key={site.id}\n                  coordinate={{ latitude: Number(site.latitude), longitude: Number(site.longitude) }}';requireAnchor(text,target,'map marker rendering');
...
function patchCatalogueSearch() {
  const path='EquipmentCatalogueBrowser.js';let text=read(path);
  if(!text.includes("const [searchDb,setSearchDb]=useState('')")){
    const state="const [tab,setTab]=useState('marques');const [brands,setBrands]=useState([]);const [cats,setCats]=useState([]);const [models,setModels]=useState([]);const [search,setSearch]=useState('');";requireAnchor(text,state,'catalogue search state');
...
function patchVisualPackLazyTools() {
  const path='visual-packs/runtime/visualPackManager.js';let text=read(path);
  text=text.replace(/import \* as DocumentPicker from ['"]expo-document-picker['"];\s*/g,'').replace(/import \{ unzip \} from ['"]react-native-zip-archive['"];\s*/g,'');
  if(!text.includes("const DocumentPicker = require('expo-document-picker');")){
    const signature='export async function importVisualPackZip() {';requireAnchor(text,signature,'visual pack import');
    text=text.replace(signature,`${signature}\n  const DocumentPicker = require('expo-document-picker');\n  const { unzip } = require('react-native-zip-archive');`);
  }
  write(path,text);
}

patchAppDeferredScreens();patchLazyActionModules();patchDatabaseBootstrap();patchVisitPrefillCoalescing();patchSiteOverviewReloads();patchClientMap();patchCatalogueSearch();patchVisualPackLazyTools();
