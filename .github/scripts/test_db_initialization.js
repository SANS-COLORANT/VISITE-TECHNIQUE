...
function makeGetDb(seeds) {
  const db = {};
  const names = ['openAppDatabase', 'seedDemoSiNecessaire', 'seedBibliothequeSiNecessaire', 'seedEquipementsBibliothequeSiNecessaire'];
...
async function main() {
  ...
  const getDb = makeGetDb([
    async () => { seedCalls += 1; await seedGate; },
    async () => { seedCalls += 1; },
    async () => { seedCalls += 1; },
  ]);
  const first = getDb();
  ...
  const second = getDb().then(() => { secondResolved = true; });
  ...
  const retry = makeGetDb([
    async () => { if (++attempts === 1) throw new Error('seed failed'); },
    async () => {},
    async () => {},
  ]);
