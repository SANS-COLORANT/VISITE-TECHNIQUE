...
test('startup uses the new vector spiral and never mounts the legacy full-screen movie', () => {
  ...
  assert.ok(startup.includes('SpiralSvg'));
  assert.ok(startup.includes('SPIRAL_PATH_LENGTH'));
  assert.ok(startup.includes('<VelvetArt mode="dock"'));
  assert.ok(!startup.includes('<VelvetArt mode="intro"'));
  assert.ok(!startup.includes('METRA_Spirale_Intro_V4.webp'));
  assert.ok(startup.includes("finish('completed')"));
  assert.ok(startup.includes("finish('interrupted')"));
  assert.ok(startup.includes('completed.current = true'));
});

test('native media remains available for the exact rotating dock and cleans up correctly', () => {
...
test('rotating dock still uses the complete source-frame artwork', () => {
