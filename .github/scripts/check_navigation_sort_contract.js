...
expect(
  app.includes('gesture.dx < -82') && app.includes('goBack()'),
  'Un swipe franc vers la gauche doit revenir à l’écran précédent.'
);
expect(
  !app.includes("'Visite', 'HydraulicSchema'"),
  'Le swipe retour ne doit pas écraser le pager horizontal d’une visite.'
);
...
expect(
  sites.includes('SITE_SORT_OPTIONS') && sites.includes('sitesTries'),
  'La liste des sites doit utiliser les modes de tri.'
);
expect(
  prewarm.includes('listerAppartenancesClient'),
  'Le tri Lot / groupe doit utiliser les appartenances réelles des sites via le cache de préchauffage.'
);
...
expect(
  memory.includes("PREFIX = 'ui_state::'"),
  'Le contexte UI doit survivre au redémarrage via _meta sans migration de schéma.'
);
