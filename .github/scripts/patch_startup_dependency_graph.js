...
function patchHome() {
  ...
  text = text.replace(
    /await supprimerVisiteComplete\(v\.id\)/g,
    'await chargerEntityManagementModule().supprimerVisiteComplete(v.id)'
  );
  text = text.replace(
    /await getResumeSuppressionClient\(client\.id\)/g,
    'await chargerEntityManagementModule().getResumeSuppressionClient(client.id)'
  );
  text = text.replace(
    /await supprimerClientComplet\(client\.id\)/g,
...
function patchDb() {
  ...
  text = text.replace(
    'async function listerMateriel(id){return listerMaterielPersistant(id);}',
    'async function listerMateriel(id){return chargerMaterielPersistant().listerMaterielPersistant(id);}'
  );
  text = text.replace(
    'async function ajouterMateriel(visiteId){return ajouterMaterielPersistant(visiteId);}',
    'async function ajouterMateriel(visiteId){return chargerMaterielPersistant().ajouterMaterielPersistant(visiteId);}'
  );
  text = text.replace(
    'async function upsertMaterielChamp(id,cle,valeur){return upsertMaterielPersistant(id,cle,valeur);}',
    'async function upsertMaterielChamp(id,cle,valeur){return chargerMaterielPersistant().upsertMaterielPersistant(id,cle,valeur);}'
  );
  text = text.replace(
    'async function supprimerMateriel(id){return retirerMaterielPersistant(id);}',
    'async function supprimerMateriel(id){return chargerMaterielPersistant().retirerMaterielPersistant(id);}'
  );
  ...
  if (text.includes("from './persistentEquipmentDb.js'"))
    throw new Error('db persistent equipment eager import still present');
  if (!text.includes('const {PRESCRIPTIONS}=chargerDonneesLegacy()'))
    throw new Error('db prescriptions lazy load not applied');
  if (!text.includes('const {TRAME_DATA}=chargerDonneesLegacy()')) throw new Error('db trame lazy load not applied');
  if (!text.includes('async function listerHistoriqueEquipement(...args)'))
    throw new Error('db equipment history lazy binding missing');
  write(path, text);
}
...
console.log(
  'Startup dependency graph trimmed and SQLite startup bridge calls collapsed: fewer modules and native round-trips before first interactive screen.'
);
