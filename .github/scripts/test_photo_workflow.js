...
function databaseProcess(filename) {
  ...
  child.on('exit', () => {
    for (const p of pending.values()) p.reject(new Error('SQLite adapter exited'));
    pending.clear();
  });
  function send(method, sql = '', params = []) {
...
const photo = (id, extra = {}) => ({
  ...
  largeurPixels: 1600,
  hauteurPixels: 1200,
  cheminTelechargement: `/api/clients/12/dernieres-visites/photos/${id}`,
  ...extra
});
const payload = () => ({
  client: { id: 12, nom: 'Client test' },
  sites: [
    {
      site: { id: 45, nom: 'Site A' },
      locaux: [
        {
          local: { id: 501, designation: 'Chaufferie', ordre: 1 },
          derniereVisite: { id: 812, date: '2026-08-28', statut: 'Terminee' },
          photos: [photo(9001), photo(9002)]
        },
        {
          local: { id: 502, designation: 'SST', ordre: 2 },
          derniereVisite: { id: 813, date: '2026-08-29' },
          photos: [photo(9003)]
        }
      ]
    },
    {
      site: { id: 46, nom: 'Site B' },
      locaux: [
        {
          local: { id: 503, designation: 'VMC' },
          derniereVisite: { id: 814, date: '2026-08-30' },
          photos: [photo(9004), photo(9005, { disponible: false, cheminTelechargement: null })]
        },
...
async function main() {
  ...
  try {
    ...
    await server.send('migrate', '', [31, 32]);
    check(
      (await server.db.getAllAsync('SELECT id FROM visites')).length === 3,
      'v31 -> v32 upgrade preserves ICPE/VMC/PRE visits; v32 is idempotent'
    );
    ...
    const FileSystem = {
      ...
      makeDirectoryAsync: async () => {
        if (failDirectory) throw new Error('disk unavailable');
      },
    ...
    const transport = async (route, target) => {
      ...
      try {
        ...
        if (id === 9003 && !rateLimited) {
          rateLimited = true;
          throw Object.assign(new Error('rate limited'), { status: 429, retryAfter: '0' });
        }
    ...
    const createStorage = () =>
      load('latestVisitPhotosStorage.js', {
        FileSystem,
        ...repository,
        ...model,
        downloadProtectedPhoto: transport,
        fetchClientLatestVisitPhotosManifest: async () => payload()
      });
    let storage = createStorage();
    let manifest = await repository.cacheLatestVisitPhotosManifest('12', payload());
    check(
      model.photoSummary(manifest).total === 5,
      'normalization preserves site/local/photo structure including unavailable references'
    );
    check(
      model.photoSummary(model.filterLatestVisitPhotos(manifest, { siteIds: [] })).total === 0,
      'empty site selection never downloads the whole client'
    );
    check(
      model.photoSummary(model.filterLatestVisitPhotos(manifest, { siteIds: ['45'], localIds: ['502'] })).total === 1,
      'site and local scopes select exactly the right photos'
    );
    assert.throws(() => repository.normalizeLatestVisitPhotosManifest(payload(), '99'));
    checks++;
    console.log(`OK ${checks}: mismatched client rejected`);
    ...
    check(
      manifest.sites[0].locaux[0].photos[0].localUri === 'file:///private/old.jpg' &&
        manifest.sites[0].locaux[0].photos[0].localAvailable,
      'cached metadata and file existence reach the NESTED gallery photo (clone regression)'
    );
    ...
    manifest = await storage.loadCachedLatestVisitPhotos('12');
    check(
      !manifest.sites[0].locaux[0].photos[0].localUri && model.photoSummary(manifest).saved === 0,
      'missing local file clears availability instead of claiming offline readiness'
    );
    let result = await storage.downloadClientLatestVisitPhotos(
      '12',
      model.filterLatestVisitPhotos(manifest, { siteIds: ['45'] })
    );
    check(
      result.downloaded === 3 && result.failed === 0 && !calls.includes(9004),
      'only selected-site files downloaded; 429 retry succeeds'
    );
    ...
    updated.sites[0].locaux[0].derniereVisite = { id: 999, date: '2026-09-10' };
    updated.sites[0].locaux[0].photos = [];
    await repository.cacheLatestVisitPhotosManifest('12', updated);
    const old = await storage.hydrateLatestVisitPhotosCache(
      '12',
      await repository.getVisitPhotoReference('visit-icpe', '12', '45', updated)
    );
    check(
      old.sites[0].locaux[0].derniereVisite.id === '812' && old.sites[0].locaux[0].photos[0].localAvailable,
      'new latest visit WITHOUT photos does not erase the prepared reference or its files'
    );
    check(
      model.referenceSignature(old) !==
        model.referenceSignature(
          model.filterLatestVisitPhotos(await storage.loadCachedLatestVisitPhotos('12'), { siteIds: ['45'] })
        ),
      'newer reference can be indicated without replacing the pinned version'
    );
    const changed = payload();
    changed.sites[0].locaux[0].photos[0].tailleOctets = 20;
    await repository.cacheLatestVisitPhotosManifest('12', changed);
    const current = await storage.loadCachedLatestVisitPhotos('12');
    check(
      !current.sites[0].locaux[0].photos[0].localAvailable,
      'same photo ID with different metadata does not reuse an older file'
    );
    check(
      (
        await storage.hydrateLatestVisitPhotosCache(
          '12',
          await repository.getVisitPhotoReference('visit-icpe', '12', '45')
        )
      ).sites[0].locaux[0].photos[0].localAvailable,
      'older version remains available for pinned reference'
    );
    await repository.cacheLatestVisitPhotosManifest('12', payload());
    check(
      (await repository.resolvePhotoContexts({ visiteId: 'visit-icpe' }))[0].remoteLocalId === '501',
      'bound visit resolves exact remote local ID'
    );
    check(
      (await repository.resolvePhotoContexts({ visiteId: 'visit-icpe', ignoreVisitLocal: true }))[0].remoteLocalId ===
        null,
      'multi-local PRE context does not silently use another room bound to the visit'
    );
    await repository.savePhotoLocalChoice('visit-pre', 'preallumage-local:one', '12', '45', '502');
    check(
      (await repository.readPhotoLocalChoice('visit-pre', 'preallumage-local:one', '12', '45')) === '502',
      'explicit room choice is persisted per PRE local and visit'
    );
    check(
      (await repository.readPhotoLocalChoice('visit-pre', 'preallumage-local:two', '12', '45')) === null,
      'room choices do not leak to another PRE local'
    );
    await repository.pinPhotoReferencesForVisit('visit-vmc');
    await repository.pinPhotoReferencesForVisit('visit-pre');
    const fieldTables = ['photos', 'remarques', 'controles_visite', 'champs_visite'];
    for (const name of fieldTables)
      check(
        (await server.db.getFirstAsync(`SELECT COUNT(*) n FROM ${name}`)).n === 0,
        `${name} not populated by historical photos`
      );
    server.close();
    server = databaseProcess(path.join(dir, 'test.db'));
    repository = load('latestVisitPhotosDb.js', { openAppDatabase: async () => server.db, ...model });
    ...
    check(
      ...
      'reference and file associations survive database close/reopen'
    );
    ...
    check(
      result.failed === 1 && !model.photoSummary(result.manifest).saved,
      'non-image response is rejected and not exposed as cached'
    );
    failMime = null;
    failSize = 9004;
    ...
    failDirectory = true;
    check(
      (await storage.downloadClientLatestVisitPhotos('12', siteB)).failed === 1,
      'destination failure is contained in file result, not an unhandled worker rejection'
    );
    failDirectory = false;
    const unsafe = structuredClone(siteB);
    unsafe.sites[0].locaux[0].photos[0].cheminTelechargement = '/api/clients/99/dernieres-visites/photos/9004';
    const beforeUnsafe = calls.length;
    check(
      (await storage.downloadClientLatestVisitPhotos('12', unsafe)).failed === 1 && calls.length === beforeUnsafe,
      'cross-client download path rejected before HTTP request'
    );
    diskBytes = 1;
    await assert.rejects(() => storage.downloadClientLatestVisitPhotos('12', siteB), /Espace insuffisant/);
    diskBytes = 10 ** 10;
    checks++;
    console.log(`OK ${checks}: insufficient disk space reported before download`);
    ...
    check(
      model.photoSummary(await storage.hydrateLatestVisitPhotosCache('12', siteB)).saved === 1 && events > 0,
      'closing/unsubscribing UI does not cancel the download'
    );
    // A pending task can be paused while another task owns the global workers.
    const more = payload();
    more.sites[0].locaux[0].photos = Array.from({ length: 8 }, (_, i) => photo(9100 + i));
    await repository.cacheLatestVisitPhotosManifest('12', more);
    const all = model.filterLatestVisitPhotos(await storage.loadCachedLatestVisitPhotos('12'), {
      siteIds: ['45'],
      localIds: ['501']
    });
    delay = 30;
    ...
    tasks.pausePhotoDownload(job.id);
    const stopped = await job.completion;
    check(
      stopped.paused && stopped.downloaded <= 3,
      'pause allows in-flight files to finish and prevents further scheduling'
    );
    const resume = tasks.resumePhotoDownload(job.id);
    await resume.completion;
    check(
      model.photoSummary(await storage.hydrateLatestVisitPhotosCache('12', all)).saved === 8,
      'resume downloads missing files to completion'
    );
    ...
    check(
      (await server.db.getFirstAsync(`SELECT COUNT(*) n FROM api_visit_photo_references WHERE visite_id='visit-icpe'`))
        .n === 0,
      'deleting a visit removes its reference association, not shared cached files'
    );
    check(
      (await server.db.getFirstAsync(`SELECT COUNT(*) n FROM api_photo_files`)).n > 0,
      'shared private cache survives deletion of one visit'
    );
    ...
    try {
      await fresh.send('migrate', '', [0, 33]);
      check(
        (await fresh.db.getAllAsync("SELECT name FROM sqlite_master WHERE name='api_visit_photo_references'"))
          .length === 1,
        'brand-new database migrates through v33'
      );
    } finally {
      fresh.close();
    }
