...
if (schemaVersion < 34) throw new Error(`schema version: expected >= 34, got ${schemaVersion}`);
requireText(migrations, "import { migration034 } from './034_intranet_local_name_sync.js';", 'migration registration');
requireText(migrations, 'migration034', 'migration ordering');
...
requireText(
  ...
  'link/designation refresh trigger'
);
requireText(migration, 'SET nom = NEW.designation', 'Intranet name propagation');

console.log(
  `Intranet LOCAL name sync contract validated: imported LOCAL designation is persisted, migration v34 remains registered under schema v${schemaVersion}, and future link/name refreshes update the METRA installation without replacing it with an empty name.`
);
