...
function forbidText(text, needle, label) {
  if (text.includes(needle)) throw new Error(`${label}: eager dependency regression ${needle}`);
}
...
requireText(
  reference,
  "REFERENCE_CATALOG_META_KEY = 'reference_catalog_icpe_v2'",
  'reference catalogue version marker'
);
const equipment = read('database/equipmentCatalogSeed.js');
requireText(
  equipment,
  "EQUIPMENT_CATALOG_CORE_META_KEY='equipment_catalog_core_v3'",
  'equipment catalogue core marker'
);
...
console.log(
  'Runtime responsiveness v2 contract validated: heavy modules deferred, catalogues version-gated, visit warmup non-blocking, map/search/report work bounded.'
);
