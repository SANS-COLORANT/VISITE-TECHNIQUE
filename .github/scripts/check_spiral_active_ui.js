...
if (manifest.interface?.experimentalBuildingScene !== true)
  throw new Error('Spiral Active building scene flag missing.');
if (manifest.spiralDock?.replaceable !== true) throw new Error('Spiral Active dock appearance must stay replaceable.');
if (Number(manifest.spiralDock?.diameterTablet || 0) < 160)
  throw new Error('Spiral Active tablet dock became too small.');
...
if (!Array.isArray(manifest.homeScene?.layers) || manifest.homeScene.layers.length !== 4)
  throw new Error('Spiral Active home scene must expose exactly four building layers.');
...
console.log(
  'Spiral Active UI contract validated: replaceable startup/dock/home scene, four building layers, pinch opening and spiral gestures are wired.'
);
