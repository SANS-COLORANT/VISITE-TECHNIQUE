...
# ---------------------------------------------------------------------------
# Source serveur analysée : dump MySQL energieetservice du 14/09/2026.
# CLIENT porte logo_client ; SITE porte son adresse et ses métadonnées métier ;
# LOCAL porte type/situation/ordre/périodicité. METRA conserve ces informations
# dans son cache offline au lieu de réduire le serveur à nom/id/designation.
# ---------------------------------------------------------------------------
p = Path('symfonyApiCacheDb.js')
s = p.read_text(encoding='utf-8')

old = """    const local = entry?.local && typeof entry.local === 'object' ? {
      ...entry.local,
...
"""
new = """    const local = entry?.local && typeof entry.local === 'object' ? {
      ...entry.local,
      id: remoteId(entry.local.id),
      designation: nullableString(entry.local.designation),
      type: nullableString(entry.local.type),
      situation: nullableString(entry.local.situation),
      periodiciteVisite: entry.local.periodiciteVisite ?? entry.local.periodicite_visite ?? null,
      prochaineVisite: nullableString(entry.local.prochaineVisite ?? entry.local.prochaine_visite),
      visitePlanifiee: nullableString(entry.local.visitePlanifiee ?? entry.local.visite_planifiee),
      ordre: Number.isFinite(Number(entry.local.ordre)) ? Number(entry.local.ordre) : null,
      typeTrameId: remoteId(entry.local.typeTrameId ?? entry.local.type_trame_id),
    } : null;
    const site = entry?.site && typeof entry.site === 'object' ? {
      ...entry.site,
      id: remoteId(entry.site.id),
      nom: nullableString(entry.site.nom),
      sitePrincipal: nullableString(entry.site.sitePrincipal ?? entry.site.site_principal),
      adresse: nullableString(entry.site.adresse),
      codePostal: nullableString(entry.site.codePostal ?? entry.site.code_postal),
      ville: nullableString(entry.site.ville),
      codeExploitant: nullableString(entry.site.codeExploitant ?? entry.site.code_exploitant),
      surfaceBatiment: entry.site.surfaceBatiment ?? entry.site.surface_batiment ?? null,
      dateBatiment: nullableString(entry.site.dateBatiment ?? entry.site.date_batiment),
      nombreLogements: nullableString(entry.site.nombreLogements ?? entry.site.nombre_logements),
      dateAjout: nullableString(entry.site.dateAjout ?? entry.site.date_ajout),
      dateSupression: nullableString(entry.site.dateSupression ?? entry.site.date_supression),
      code: nullableString(entry.site.code),
      energie: nullableString(entry.site.energie),
      typeBatiment: nullableString(entry.site.typeBatiment ?? entry.site.type_batiment),
      typeMarche: nullableString(entry.site.typeMarche ?? entry.site.type_marche),
      service: nullableString(entry.site.service),
      agence: nullableString(entry.site.agence),
    } : null;
"""
s = replace_once(s, old, new, 'normalize server SITE/LOCAL fields')

old = """    client: payload?.client && typeof payload.client === 'object' ? {
      ...payload.client,
      id: remoteId(payload.client.id),
      nom: nullableString(payload.client.nom),
    } : null,
"""
new = """    client: payload?.client && typeof payload.client === 'object' ? {
      ...payload.client,
      id: remoteId(payload.client.id),
      nom: nullableString(payload.client.nom),
      logoClient: nullableString(payload.client.logoClient ?? payload.client.logo_client),
    } : null,
"""
s = replace_once(s, old, new, 'normalize server client logo path')
...
new = """      await database.runAsync(
        ...
        [siteId, clientId, clean(visite?.site?.nom) || `Site ${siteId}`, 1, json(visite?.site),
          visite?.site?.sitePrincipal ?? null, visite?.site?.adresse ?? null, visite?.site?.codePostal ?? null,
          visite?.site?.ville ?? null, visite?.site?.codeExploitant ?? null,
          Number.isFinite(Number(visite?.site?.surfaceBatiment)) ? Number(visite.site.surfaceBatiment) : null,
          visite?.site?.dateBatiment ?? null, visite?.site?.nombreLogements ?? null, visite?.site?.dateAjout ?? null,
          visite?.site?.dateSupression ?? null, visite?.site?.code ?? null, visite?.site?.energie ?? null,
          visite?.site?.typeBatiment ?? null, visite?.site?.typeMarche ?? null, visite?.site?.service ?? null, visite?.site?.agence ?? null]
      );
"""
...
new = """      await database.runAsync(
        ...
        [localId, siteId, visite?.local?.designation ?? null, visite?.trame?.id ?? null,
          ...
          visite?.derniereVisite?.date ?? null, visite?.derniereVisite?.statut ?? null, json(visite), 1,
          Number(meta.criteriaCount || 0), Number(meta.historicalCriteriaCount || 0), Number(meta.remarkCount || 0), Number(meta.materialCount || 0),
          visite?.local?.type ?? null, visite?.local?.situation ?? null,
          Number.isFinite(Number(visite?.local?.periodiciteVisite)) ? Number(visite.local.periodiciteVisite) : null,
          visite?.local?.prochaineVisite ?? null, visite?.local?.visitePlanifiee ?? null,
          Number.isFinite(Number(visite?.local?.ordre)) ? Number(visite.local.ordre) : null,
          visite?.local?.typeTrameId ?? null]
      );
"""
...
new = """  const adresse = [remote.adresse_postale, remote.ville].filter(Boolean).join(' ');
  if (existing?.id) {
    // Ne jamais écraser une saisie METRA : compléter seulement les champs
    // locaux encore vides avec les valeurs structurelles du serveur.
  ...
  const id = createId();
"""
s = replace_once(s, old, new, 'fill blank local client metadata')
...
# ---------------------------------------------------------------------------
# COMPTEURS : dans le schéma serveur, COMPTEUR est relié aux consommations
# (conso_compteur), pas à visite_technique/local_critere. Un relevé compteur
# absent ne peut donc pas invalider l'envoi d'une visite. On transmet « / ».
# Une vraie ambiguïté (plusieurs compteurs METRA au même libellé) reste signalée.
# ---------------------------------------------------------------------------
p = Path('intranetVisitPayload.js')
s = p.read_text(encoding='utf-8')
old = """function counterValue(counters, criterion, candidate) {
...
"""
new = """function counterValue(counters, criterion, candidate) {
  const keys = new Set([cleanCounterLabel(criterion?.nom), cleanCounterLabel(candidate?.label), cleanCounterLabel(candidate?.cle)].filter(Boolean));
  const exact = counters.filter((counter) => keys.has(cleanCounterLabel(counter.label)));
  if (exact.length === 1) return { value: exact[0].valeur, ambiguous: false };
  // Le serveur ne porte aucune FK compteur -> visite/critère : l'absence de
  // compteur est donc un état métier valide, sérialisé ensuite sous « / ».
  return { value: undefined, ambiguous: exact.length > 1 };
}
"""
s = replace_once(s, old, new, 'schema-aware missing counter behavior')
old = """            value = counterValue(counters, criterion, candidate);
            if (value === undefined) issues.push(`${path} : compteur correspondant introuvable ou ambigu.`);
"""
new = """            const counter = counterValue(counters, criterion, candidate);
            value = counter.value;
            if (counter.ambiguous) issues.push(`${path} : plusieurs compteurs locaux correspondent à ce critère.`);
"""
s = replace_once(s, old, new, 'schema-aware ambiguous counter behavior')
p.write_text(s, encoding='utf-8')


# ---------------------------------------------------------------------------
# Invariants structurants déduits du dump :
# - SITE peut appartenir à plusieurs CLIENT via LOT/SITE_LOT : ne pas rétablir
#   un simple client_id serveur dans api_site_links ;
# - une VISITE_TECHNIQUE serveur cible exactement un LOCAL ;
# - PHOTOGRAPHIE.local_critere_id est nullable ; une photo générale est valide.
# Ces règles sont déjà implémentées ; ce patch vérifie qu'elles le restent.
# ---------------------------------------------------------------------------
...
if 'api_client_site_links' not in cache:
    raise SystemExit('Server LOT/SITE_LOT relation protection missing')
...
if 'compteur correspondant introuvable ou ambigu' in payload:
    raise SystemExit('Missing compteur still incorrectly blocks upload')
if 'ORDER BY COALESCE(ordre,1000000),designation,remote_local_id' not in cache:
    raise SystemExit('Server LOCAL order is not preserved')

print('Intranet SQL schema alignment applied: client/site/local metadata, LOCAL order, site address materialization and non-blocking missing counters.')
