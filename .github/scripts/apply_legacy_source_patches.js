...
function run(args, label) {
  ...
  if (result.error || result.status !== 0) {
    console.error(
      `[METRA legacy] ${label} failed. This recovery path is intended only for an old, unmaterialized checkout.`
    );
...
console.log(
  '[METRA legacy] Explicit recovery mode. This command mutates runtime source and must never run automatically during npm install or Android CI.'
);
