...
function preparationReference({
  trame = remoteTrame,
  materials = [{ id: 'm1' }],
  latest = { id: '812', date: '2026-09-01', statut: 'Terminé' }
} = {}) {
  return {
    local: { id: '501', designation: 'Chaufferie' },
    site: { id: '45', nom: 'Site A' },
    derniereVisite: latest,
    trame,
    materiels: materials,
    remarques: [],
    notes: []
...
async function seedVisit(db, id, reference = preparationReference()) {
  ...
  `);
  await db.runAsync(
    `INSERT OR REPLACE INTO api_local_links(remote_local_id,remote_site_id,designation,remote_trame_id,remote_trame_nom,reference_json,criteria_count,material_count,remote_present)
    VALUES('501','45','Chaufferie','3','ICPE',?,?,?,1)`,
    [JSON.stringify(reference), 1, Array.isArray(reference.materiels) ? reference.materiels.length : 0]
  );
  await db.runAsync(
    ...
    [
      `prov-${id}`,
      id,
      JSON.stringify({ ...reference, schemaVersion: 3, sourceType: 'preparation_visite', remoteLocalId: '501' })
    ]
  );
}
function mapRemoteTrameToLocal(remote) {
  const v = String(remote?.nom || '')
    .normalize('NFD')
    .replace(/[\u0300-\u036f]/g, '')
    .toLowerCase();
...
async function main() {
  ...
  try {
    ...
    const partial = await payloadModule.buildIntranetVisitPayload(
      'partial-material',
      '11111111-1111-4111-8111-111111111111'
    );
    check(
      partial.destructiveMaterialChange &&
        !partial.destructiveMaterialClear &&
        partial.removedSourceMaterialCount === 1,
    ...
    const sender = async (_client, serialized) => {
      ...
      if (mode === '429-long')
        throw Object.assign(new Error('rate limit'), { status: 429, code: 'rate_limit_exceeded', retryAfter: '1800' });
    ...
    await assert.rejects(
      () => outbox.queueVisitUpload('partial-material'),
      (error) => error?.code === 'material_replacement_confirmation_required'
    );
    checks++;
    console.log(`OK ${checks}: partial remote material deletion requires explicit confirmation`);
    ...
    check(
      rateRow.status === 'retry' && retryAt - before429 >= 1_790_000,
    ...
    duplicate.categories[0].sousCategories[0].criteres.push({
      ...duplicate.categories[0].sousCategories[0].criteres[0]
    });
    await server.db.runAsync(`UPDATE provenances SET details_json=? WHERE entite_id='duplicate-branch'`, [
      JSON.stringify({
        ...preparationReference({ trame: duplicate }),
        schemaVersion: 3,
        sourceType: 'preparation_visite'
      })
    ]);
    await assert.rejects(
      () => payloadModule.buildIntranetVisitPayload('duplicate-branch', '33333333-3333-4333-8333-333333333333'),
      (error) => error?.code === 'local_validation_failed' && error.issues.some((issue) => /dupliquée/.test(issue))
    );
    checks++;
    console.log(`OK ${checks}: duplicate remote criterion branch is rejected locally`);

    await seedVisit(server.db, 'corrupt-trame');
    await server.db.runAsync(`UPDATE provenances SET details_json=? WHERE entite_id='corrupt-trame'`, [
      JSON.stringify({
        ...preparationReference({ trame: { id: '3', nom: 'ICPE', categories: {} } }),
        schemaVersion: 3,
        sourceType: 'preparation_visite'
      })
    ]);
    await assert.rejects(
      () => payloadModule.buildIntranetVisitPayload('corrupt-trame', '44444444-4444-4444-8444-444444444444'),
      (error) => error?.code === 'local_validation_failed' && error.issues.some((issue) => /aucun critère/.test(issue))
    );
    checks++;
    console.log(
      `OK ${checks}: malformed cached trame becomes an actionable local validation error, not a runtime crash`
    );
    ...
    await server.db.execAsync(
      `INSERT INTO visites(id,site_id,date_visite,statut,trame_id) VALUES('bind-fallback','site-local','2026-09-10','terminee','icpe_v1');`
    );
    const fallbackReference = preparationReference({ trame: { ...remoteTrame, id: null } });
    await server.db.runAsync(
      `UPDATE api_local_links SET remote_trame_id='3',remote_trame_nom='ICPE',reference_json=? WHERE remote_local_id='501'`,
      [JSON.stringify(fallbackReference)]
    );
    let ids = 0;
    const binding = load('intranetVisitBindingDb.js', {
      getDb: async () => server.db,
      createId: () => `audit-binding-${++ids}`,
      mapRemoteTrameToLocal
    });
    const options = await binding.getVisitIntranetBindingOptions('bind-fallback', {
      remoteClientId: '12',
      remoteSiteId: '45'
    });
    check(
      options.locals[0]?.compatible === true && options.locals[0]?.remoteTrameId === '3',
      'cached remote trame id safely repairs a missing nested trame.id'
    );
    await binding.bindVisitToIntranetTarget('bind-fallback', {
      remoteClientId: '12',
      remoteSiteId: '45',
      remoteLocalId: '501'
    });
    ...
    check(
      bound.api_remote_trame_id === '3',
      'the repaired remote trame id is frozen on the visit instead of an empty id'
    );

    await server.db.execAsync(
      `INSERT INTO visites(id,site_id,date_visite,statut,trame_id) VALUES('bind-invalid','site-local','2026-09-10','terminee','icpe_v1');`
    );
    const badCriterion = preparationReference({
      trame: {
        ...remoteTrame,
        categories: [
          {
            ...remoteTrame.categories[0],
            sousCategories: [
              {
                ...remoteTrame.categories[0].sousCategories[0],
                criteres: [{ id: 'bad', nom: 'Contrôle A', avisApplicable: true }]
              }
            ]
          }
        ]
    ...
    await server.db.runAsync(`UPDATE api_local_links SET reference_json=? WHERE remote_local_id='501'`, [
      JSON.stringify(badCriterion)
    ]);
    const badOptions = await binding.getVisitIntranetBindingOptions('bind-invalid', {
      remoteClientId: '12',
      remoteSiteId: '45'
    });
    check(
      badOptions.locals[0]?.compatible === false &&
        /critère sans identifiant valide/.test(badOptions.locals[0]?.compatibilityReason || ''),
