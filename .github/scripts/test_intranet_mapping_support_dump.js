...
const normaliserSectionCode = (panelId, section) =>
  panelId.replace('p-', '') +
  '.' +
  String(section)
    .toLowerCase()
...
async function main() {
  ...
  const heat = payload.inspectIntranetCriterionCandidate(
    'icpe_v1',
    criterion,
  ...
  check(
    heat.resolved?.panelId === 'p-conf-chauffage',
    'duplicate disconnection criterion resolves to Chauffage from the remote category'
  );
  check(
    heat.candidates[0].score > heat.candidates[1].score,
    'Chauffage category creates a strict score winner instead of a tie'
  );

  const ecs = payload.inspectIntranetCriterionCandidate(
    'icpe_v1',
    criterion,
    'CONFORMITÉ ECS',
    'Disconnection et alimentation eau froide'
  ...
  const ambiguous = payload.inspectIntranetCriterionCandidate(
    'icpe_v1',
    criterion,
    'CONFORMITÉ',
    'Disconnection et alimentation eau froide'
  ...
  check(
    ...
    'mapping still fails safe when the server category has no Chauffage/ECS discriminator'
  );
  ...
  try {
    ...
    const support = load('supportDump.js', {
      ...
      listerTramesDisponibles: () => [localTrame],
      normaliserSectionCode
    });
    const dump = await support.construireSupportDump();
    check(
      dump.format === 'metra-support-dump' && dump.schemaVersion === 34,
      'support dump carries an explicit format and schema version'
    );
    check(
      dump.intranet.preparations[0].payload_json.access_token === '[REDACTED]',
      'access token-like preparation field is redacted recursively'
    );
    check(
      dump.intranet.locals[0].reference_json.private_key === '[REDACTED]',
      'private key-like local reference field is redacted recursively'
    );
    check(
      dump.linkedVisitData.provenance[0].details_json.authorization === '[REDACTED]',
      'Authorization-like provenance field is redacted recursively'
    );
    check(
      dump.intranet.preparations[0].payload_json.client.id === 12,
      'non-sensitive Intranet preparation structure remains available for diagnosis'
    );
    check(
      dump.localTrames[0].panels.some((panel) => panel.panelId === 'p-conf-chauffage'),
      'dump contains the local trame field catalogue used for mapping comparison'
    );
    check(
      dump.privacy.containsPhotos === false && dump.privacy.credentialsIncluded === false,
      'dump explicitly excludes photos and credentials'
    );
  } finally {
    server.close();
  ...
  check(
    settings.includes('onLongPress={deverrouillerSupport}') && settings.includes('Exporter le DUMP support'),
    'support dump is hidden behind a long press in Settings > Data'
  );
  check(
    !dumpSource.includes("from './symfonyApi.js'") && !dumpSource.includes('SecureStore'),
    'dump implementation never imports authentication storage or transport secrets'
  );
