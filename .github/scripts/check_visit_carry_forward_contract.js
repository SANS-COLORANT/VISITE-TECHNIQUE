...
requireText(
  ...
  'pre-allumage durable fields only'
);
...
requireText(
  carry,
  'details_json LIKE \'%\\"sourceType\\":\\"imported_latest_visit\\"%\'',
  'imported historical provenance detection'
);
...
requireText(
  ...
  'historical conformity comments do not seed the next visit'
);
requireText(
  carry,
  'SELECT id,ordre,nom_reseau,t_ext_c,t_dep_c,courbe_de_chauffe,tnc,consigne_programme_horaire,reseau_site_id',
  'network values selected'
);
requireText(
  carry,
  "entite_type='reseau' AND entite_id=? AND origine='api_symfony'",
  'network Intranet provenance carried forward'
);
...
requireText(
  ...
  'selected-local latest visit import'
);
...
requireText(
  ...
  'latest-known criterion semantics'
);
...
requireText(
  ...
  'branch-aware conformity matching'
);
requireText(
  latest,
  'contextOverlap(candidate.panelIdKey, categoryKey)',
  'category context disambiguates duplicate criterion names'
);
...
requireText(
  ...
  'historical conformity comments are hidden'
);
...
forbidText(
  fields,
  'remoteId(criterion?.visiteSourceId) !== remoteVisitId',
  'older preparation fields must not be rejected'
);
forbidText(
  fields,
  'remoteId(criterion?.visiteSourceId) === remoteVisitId',
  'source visit id must not gate fields or networks'
);
...
requireText(
  ...
  'prepared visit history importer wiring'
);
...
if (historicalImport < 0 || referenceImport < 0 || historicalImport > referenceImport) {
  throw new Error(
    'prepared visit must materialize the latest preparation snapshot before importing the current API reference'
  );
}

console.log(
  'Visit preparation contract validated: Intranet history imports latest-known S/N.S/etc. without ordinary conformity comments, keeps technical measurements, leaves Intranet remarks summary-only, carries avis into the next visit without cloning old reserves, and keeps Pré-allumage controls blank.'
);
