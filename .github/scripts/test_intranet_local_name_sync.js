...
async function main() {
  ...
  try {
    ...
    check(
      (await database.first(`SELECT nom FROM installations WHERE id='i1'`))?.nom === 'Chaufferie Intranet',
      'v34 aligns an already-linked installation with the Intranet LOCAL name'
    );

    await database.exec(`UPDATE api_local_links SET designation='Sous-station A' WHERE remote_local_id='501';`);
    check(
      (await database.first(`SELECT nom FROM installations WHERE id='i1'`))?.nom === 'Sous-station A',
      'a refreshed Intranet LOCAL designation updates the linked installation name'
    );

    await database.exec(`UPDATE api_local_links SET designation='' WHERE remote_local_id='501';`);
    check(
      (await database.first(`SELECT nom FROM installations WHERE id='i1'`))?.nom === 'Sous-station A',
      'an empty remote designation never erases the METRA installation name'
    );
    ...
    check(
      (await database.first(`SELECT nom FROM installations WHERE id='i2'`))?.nom === 'Local VMC Intranet',
      'a newly linked remote LOCAL immediately gives its name to the METRA installation'
    );
    check(
      (await database.all('PRAGMA foreign_key_check')).length === 0,
      'v34 LOCAL name synchronization keeps SQLite foreign keys valid'
    );
