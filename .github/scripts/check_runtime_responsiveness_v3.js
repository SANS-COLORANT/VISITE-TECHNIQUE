...
requireRegex(
  app,
  /\bReport\s*:\s*\(\s*\)\s*=>\s*require\(['"]\.\/ReportScreen\.js['"]\)\.ReportScreen/,
  'deferred report'
);
requireRegex(app, /\bLab3D\s*:\s*\(\s*\)\s*=>\s*require\(['"]\.\/Lab3DScreen\.js['"]\)\.Lab3DScreen/, 'deferred LAB3D');
requireRegex(
  app,
  /\bClientDocuments\s*:\s*\(\s*\)\s*=>\s*require\(['"]\.\/ClientDocumentsScreen\.js['"]\)\.ClientDocumentsScreen/,
  'deferred client documents'
);
requireText(
  app,
  "import { HydraulicSchemaWorkspace } from './HydraulicSchemaWorkspace.js';",
  'hydraulic build compatibility'
);
...
if (
  !/import\s*\{\s*exporterDernieresVisitesClient(?:\s*,\s*listerTypesVisitesClient)?\s*\}\s*from\s*['"]\.\/clientBatchExport\.js['"]\s*;/.test(
    docs
  )
) {
...
requireText(
  ...
  'preallumage local swipe separated from tab pager'
);
requireText(
  visit,
  'preAllumageLocalSwipeRef.current?.(direction, false)',
  'preallumage local dry-run before animation'
);
...
requireText(
  ...
  'site export build compatibility'
);
...
console.log(
  'Runtime responsiveness v5 contract validated: three visits stay hot, visit context renders immediately, prefill completion is durable, swipe uses a stable native pager, and startup/catalogue/map work remains bounded.'
);
