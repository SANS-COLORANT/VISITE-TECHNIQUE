...
export function MissionDocumentInboxScreen({ route }) {
  ...
  const accept = async (item) => {
    try {
      ...
      Alert.alert(
        'Élément intégré',
        result?.entityType ? 'Créé dans METRA : ' + result.entityType : 'Information intégrée.'
      );
    } catch (e) {
    ...
    <View style={{ flex: 1, backgroundColor: MISSION_COLORS.bg }}>
      <ScrollView contentContainerStyle={{ padding: 16, paddingBottom: 110 }}>
        ...
        <Text style={{ color: COLORS.inkSoft, fontSize: 10.5, lineHeight: 15 }}>
          PDF, DOCX, images et fichiers texte restent conservés comme sources. METRA peut en extraire localement du
          texte et proposer des éléments à valider avant intégration.
        </Text>

        <TouchableOpacity
          style={[styles.btnPrimary, missionStyles.primaryButton, { alignSelf: 'flex-start', marginTop: 12 }]}
          onPress={importDoc}
        >
        ...
        {(data.documents || []).map((doc) => (
          <View key={doc.id} style={[missionStyles.card, { padding: 11, marginBottom: 7 }]}>
            <View style={{ flexDirection: 'row', alignItems: 'flex-start' }}>
              <View style={{ flex: 1 }}>
                ...
                <Text style={{ color: COLORS.inkFaint, fontSize: 8.6, marginTop: 2 }}>
                  {doc.type || 'source'} · {doc.extraction_count || 0} extraction(s) · {doc.review_count || 0} à revoir
                </Text>
              </View>
              <TouchableOpacity
                disabled={!!busyId}
              ...
              >
                <Text style={[styles.btnSecondaryText, missionStyles.secondaryButtonText]}>
                  {busyId === doc.id ? 'Analyse…' : 'Analyser localement'}
                </Text>
              </TouchableOpacity>
            </View>
          </View>
        ...
        <View style={{ flexDirection: 'row', flexWrap: 'wrap', marginBottom: 9 }}>
          {[
            ...
            <TouchableOpacity
              ...
              style={{
                borderWidth: 1,
                borderColor: filter === key ? MISSION_COLORS.accent : MISSION_COLORS.accentLine,
                backgroundColor: filter === key ? MISSION_COLORS.accentLight : '#FFFFFF',
                borderRadius: 10,
                paddingHorizontal: 9,
                paddingVertical: 7,
                marginRight: 6,
            ...
            >
              <Text
                style={{
                  color: filter === key ? MISSION_COLORS.accentStrong : COLORS.inkSoft,
                  fontSize: 9,
                  fontWeight: '800'
                }}
              >
        ...
        {review.map((item) => (
          <View key={item.id} style={[missionStyles.card, { padding: 11, marginBottom: 8 }]}>
            <View style={{ flexDirection: 'row', alignItems: 'flex-start' }}>
              <View style={{ flex: 1 }}>
                ...
                <Text style={{ color: COLORS.ink, fontSize: 10.2, fontWeight: '800', marginTop: 3 }}>
                  {item.label || item.value_text || 'Information'}
                </Text>
                <Text style={{ color: COLORS.inkFaint, fontSize: 8.4, marginTop: 3 }}>
                  {item.document_name || ''}
              ...
              <Text
                style={{
                  color: item.status === 'accepted' ? MISSION_COLORS.accentDark : COLORS.inkFaint,
                  fontSize: 8.5,
                  fontWeight: '900'
                }}
              >
            ...
            {item.status === 'to_review' ? (
              <View style={{ flexDirection: 'row', gap: 8, marginTop: 9 }}>
                <TouchableOpacity
                  ...
                  onPress={() => ignore(item)}
                >
                  <Text style={[styles.btnSecondaryText, missionStyles.secondaryButtonText]}>Ignorer</Text>
                </TouchableOpacity>
                <TouchableOpacity style={[styles.btnPrimary, missionStyles.primaryButton]} onPress={() => accept(item)}>
                  <Text style={[styles.btnPrimaryText, missionStyles.primaryButtonText]}>Valider & intégrer</Text>
                </TouchableOpacity>
              </View>
            ) : null}
          </View>
        ...
        {!review.length ? (
          <Text style={{ color: COLORS.inkFaint, fontSize: 9.5 }}>Aucun élément dans ce filtre.</Text>
        ) : null}
      </ScrollView>
    </View>
  );
