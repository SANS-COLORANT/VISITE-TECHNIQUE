...
function libelleApplicationAvis(cle, avis) {
  if (avis === 'S') {
    const presence =
      /présence|present|présent|existe|disponible|signalétique|signalisation|coupure|extincteur|détection|detection|alarme|baes|prise|éclairage|eclairage|robinet|soupape|thermomètre|thermometre/i.test(
        String(cle || '')
      );
...
async function chargerBiblioReserve() {
  ...
  if (!biblioReservePromise) {
    biblioReservePromise = listerBibliothequeReserves()
      .then((rows) => {
        biblioReserveCache = rows || [];
        return biblioReserveCache;
      })
      .finally(() => {
        biblioReservePromise = null;
      });
  }
...
async function chargerRemarquesMap(visiteId) {
  ...
  const promise = listerRemarquesVisite(visiteId)
    .then((rows) => {
      const map = new Map();
      (rows || []).forEach((r) => {
        if (r.controle_key) map.set(r.controle_key, r);
      });
      remarquesCache.set(visiteId, { data: map, promise: null });
      return map;
    })
    .catch((e) => {
      remarquesCache.delete(visiteId);
      throw e;
    });
...
function EditionReserve({ remarque, onPatch }) {
  ...
  const [prix, setPrix, flushPrix] = useDurableAutosave(
  ...
  const [delai, setDelai, flushDelai] = useDurableAutosave(
...
export const PersistentControleGenerique = React.memo(function PersistentControleGenerique({
  visiteId,
  sectionCode,
  field,
  etatInitial,
  ...
  const baseOptions = useMemo(
    () => PRESCRIPTIONS_COMPLETES[categorieKey] || PRESCRIPTIONS_COMPLETES[field.cle] || [],
    [categorieKey, field.cle]
  );
  ...
  useEffect(() => {
    let alive = true;
    Promise.all([chargerRemarquesMap(visiteId), chargerBiblioReserve()])
      .then(([map, biblio]) => {
        if (!alive) return;
        const perso = (biblio || [])
        ...
        const merged = [
          ...baseOptions,
          ...perso.filter((o) => !signatures.has(`${o.critere || ''}||${o.prestation || ''}`))
        ];
        setOptions(merged);
  ...
  const notifierEtat = useCallback(
    (patch) => {
      onEtatChange?.(patch);
      onSaved?.();
    },
  ...
  const choisirCritere = useCallback(
    async (opt, idx) => {
      ...
      const next = {
        ...
        poste: opt.poste || 'Observation',
        prestation: opt.prestation || '',
  ...
  const sauverLibre = useCallback(
    async (texte) => {
      ...
      if (v.trim()) {
        const opt = {
          poste: remarque?.poste || 'Observation',
          prestation: v,
  ...
  const sauverCommentaireSimple = useCallback(
    async (texte) => {
      const v = String(texte || '');
      setCommentaire(v);
  ...
  const [commentaireSimple, setCommentaireSimple, flushCommentaireSimple] = useDurableAutosave(
    commentaire,
    sauverCommentaireSimple,
    450
  );
  useEffect(() => {
  ...
  useEffect(() => {
    if (avis && avis !== 'N.S') setCommentaireSimple(commentaire || '');
  }, [avis, commentaire, setCommentaireSimple]);

  const patchReserve = useCallback(
    (patch) => {
      setRemarque((r) => {
        if (!r) return r;
        const next = { ...r, ...patch };
