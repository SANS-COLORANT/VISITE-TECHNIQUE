...
function StatCard({ value, label, sub, accent = null }) {
  return <View style={{ flex: 1, minWidth: 130, backgroundColor: 'rgba(255,255,255,0.82)', borderWidth: 1, borderColor: 'rgba(22,21,15,0.1)', borderRadius: 14, padding: 14 }}><Text style={{ fontSize: 26, fontFamily: FONTS.black, color: accent || COLORS.ink }}>{value}</Text><Text style={{ fontSize: 12, color: COLORS.inkSoft, fontFamily: FONTS.bodyBold, marginTop: 2 }}>{label}</Text>{sub ? <Text style={{ fontSize: 10.5, color: COLORS.muted, marginTop: 4 }}>{sub}</Text> : null}</View>;
}

function Barre({ value, max, label }) {
  const pct = max > 0 ? Math.max(0, Math.min(100, (Number(value || 0) / max) * 100)) : 0;
  return <View style={{ marginTop: 8 }}><View style={{ flexDirection: 'row', justifyContent: 'space-between', marginBottom: 4 }}><Text style={{ fontSize: 11, color: COLORS.muted }}>{label}</Text><Text style={{ fontSize: 11, fontFamily: FONTS.bold, color: COLORS.ink }}>{value || 0}</Text></View><View style={{ height: 7, borderRadius: 4, backgroundColor: '#ECEFF2', overflow: 'hidden' }}><View style={{ width: `${pct}%`, height: '100%', backgroundColor: COLORS.orange }} /></View></View>;
}

function HealthPill({ label, score }) {
  return <View style={{ minWidth: 105, flex: 1, borderRadius: 10, backgroundColor: COLORS.bg, borderWidth: 1, borderColor: 'rgba(22,21,15,0.1)', paddingHorizontal: 9, paddingVertical: 8 }}><Text style={{ fontSize: 9.2, color: COLORS.inkSoft }} numberOfLines={1}>{label}</Text><Text style={{ marginTop: 2, fontSize: 15, fontFamily: FONTS.black, color: scoreColor(score) }}>{scoreText(score)}/100</Text></View>;
}
    ...
    ListHeaderComponent={<View>
      ...
      {healthEnabled && clientHealth ? <>
        <View style={{ borderRadius: 15, backgroundColor: '#FFF7F1', borderWidth: 1, borderColor: '#F6C7AD', padding: 15, marginBottom: 12 }}>
          <Text style={{ fontSize: 10.5, fontFamily: FONTS.black, color: COLORS.orangeDark }}>LAB METRA · SANTÉ CLIENT</Text>
          <View style={{ flexDirection: 'row', alignItems: 'flex-end', marginTop: 5 }}>
            <Text style={{ fontSize: 42, lineHeight: 46, fontFamily: FONTS.black, color: scoreColor(clientHealth.overall) }}>{scoreText(clientHealth.overall)}</Text>
          ...
          {clientHealth.lowest ? <Text style={{ marginTop: 5, fontSize: 10.5, fontFamily: FONTS.bold, color: scoreColor(clientHealth.lowest.overall) }}>Site le plus faible : {clientHealth.lowest.siteName || 'Site'} · {scoreText(clientHealth.lowest.overall)}/100</Text> : null}
        </View>
      ...
      <View style={{ backgroundColor: 'rgba(255,255,255,0.82)', borderWidth: 1, borderColor: 'rgba(22,21,15,0.1)', borderRadius: 14, padding: 14, marginBottom: 16 }}><Text style={{ fontFamily: FONTS.black, color: COLORS.ink }}>{healthEnabled ? 'Lecture de l’indice expérimental' : 'Lecture rapide'}</Text><Text style={{ color: COLORS.muted, fontSize: 11.5, marginTop: 5 }}>{healthEnabled ? 'La note moyenne ne masque jamais les sites faibles : les sites prioritaires et le site le plus dégradé restent affichés séparément. L’indice ne remplace pas un avis technique ou réglementaire.' : 'Les barres comparent les sites entre eux. Aucun fichier Excel, photo ou trame complète n’est chargé pour cet écran : uniquement des agrégats SQLite indexés.'}</Text></View>
