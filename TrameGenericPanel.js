...
import {
  enregistrerAliasPreAllumage,
  fieldAliasKey,
  libelleChamp,
  listerAliasesPreAllumage,
  sectionAliasDescriptor
} from './preAllumageAliases.js';
  ...
  return (
    <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6 }}>
      <TextInput
        style={[
          styles.sectionTitle,
          { flex: 1, borderBottomWidth: 1, borderBottomColor: '#D0D5DD', paddingVertical: 3 }
        ]}
        value={texte}
...
function mapperControles(rows = []) {
  const map = {};
  rows.forEach((row) => {
    if (row?.section_code && row?.cle) map[`${row.section_code}||${row.cle}`] = row;
  });
  return map;
}
...
export function mettreAJourCacheChamp(visiteId, key, valeur) {
  ...
  visiteDataCache.set(visiteId, {
    data: { ...courant.data, champsMap: { ...courant.data.champsMap, [key]: valeur } },
...
export function mettreAJourCacheControle(visiteId, key, patch) {
  ...
  visiteDataCache.set(visiteId, {
    data: { ...courant.data, controlesMap: { ...courant.data.controlesMap, [key]: { ...ancien, ...patch } } },
    promise: courant.promise || null
  });
}
...
function TrameGenericStaticPanel({ visiteId, panelId, sections, onSaved }) {
  ...
  useEffect(() => {
    ...
    listerAliasesPreAllumage(visiteId)
      .then((r) => {
        if (actif) setAliases(r);
      })
      .catch((e) => console.warn('Noms personnalisés non chargés', e));
    return () => {
      actif = false;
    };
  }, [visiteId, panelId]);
      ...
      .map(([sub, fields]) => {
        ...
        return {
          ...
          data: (fields || [])
            ...
            .map((field) => ({ field, sectionCode, key: `${sectionCode}||${field.cle}` }))
        };
      })
  ...
  const sauverAlias = (key, valeur, defaut) => {
    ...
    enregistrerAliasPreAllumage(visiteId, key, valeur, defaut).catch((e) =>
      console.warn('Nom personnalisé non enregistré', e)
    );
  };
  ...
  return (
    <SectionList
      ...
      renderSectionHeader={({ section }) => {
        ...
        return (
          <EditableAlias
            valeur={aliases[d.key] || d.base}
            suffix={d.suffix}
            onSave={(v) => sauverAlias(d.key, v, d.base)}
          />
        );
      }}
      renderItem={({ item }) => (
        <View style={styles.formCard}>
          {item.field.type === 'champ' ? (
            <DurableChampGenerique
              ...
              displayLabel={libelleChamp(item.sectionCode, item.field.cle, aliases)}
              onRename={
                item.field.renamable
                  ? (v) => sauverAlias(fieldAliasKey(item.sectionCode, item.field.cle), v, item.field.cle)
                  : null
              }
              onSaved={(valeur) => {
            ...
            <VmcControleGenerique
              ...
              onEtatChange={(patch) => patchControle(item.key, patch)}
              onSaved={onSaved}
            />
          ) : item.field.presets ? (
