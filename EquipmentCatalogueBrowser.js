...
function SimpleModal({ visible, title, fields, onClose, onSave }) {
  const [values,setValues]=useState({}); useEffect(()=>{if(visible)setValues({});},[visible]);
...
export function EquipmentCatalogueBrowser() {
  ...
  if(sheet) return <View style={{flex:1}}><Header title={title} subtitle={subtitle} onBack={back} action="Caractéristique" onAction={actionPress}/><ScrollView contentContainerStyle={{padding:18,paddingTop:4}}>
    {sheet.description?<Text style={{fontSize:13,color:'#555',marginBottom:12}}>{sheet.description}</Text>:null}
    ...
    <View style={{flexDirection:'row',alignItems:'center',marginTop:22}}><Text style={{fontWeight:'800',fontSize:15,flex:1}}>Courbes</Text><TouchableOpacity onPress={()=>setModal('curve')}><Text style={{color:COLORS.primary,fontWeight:'700'}}>+ Ajouter</Text></TouchableOpacity></View>{sheet.courbes?.map(c=><CurvePreview key={c.id} curve={c}/>)}
