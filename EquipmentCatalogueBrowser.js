...
function QualityBadge({ quality }) {
  ...
  return <View style={{alignSelf:'flex-start',marginTop:5,paddingHorizontal:7,paddingVertical:3,borderRadius:999,backgroundColor:verified?'#EAF7EF':'#F2F3F5'}}><Text style={{fontSize:9,fontWeight:'700',color:verified?'#287A45':'#6B7280'}}>{verified?'✓ Données constructeur':'Catalogue'}</Text></View>;
}
...
function BrandCard({ brand, onPress, tablet }) {
  return <TouchableOpacity onPress={onPress} activeOpacity={.82} style={{flex:tablet?1:undefined,minHeight:92,borderRadius:14,overflow:'hidden',marginBottom:12,borderWidth:1,borderColor:'rgba(0,0,0,.06)'}}>
    <Gradient marque={brand.nom}/><View style={{minHeight:92,flexDirection:'row',alignItems:'center',paddingHorizontal:16,gap:14}}><View style={{width:108,alignItems:'center'}}><BrandMark marque={brand} onColor/></View><View style={{flex:1}}><Text style={{color:'#fff',fontWeight:'900',fontSize:17}}>{brand.nom}</Text><Text style={{color:'rgba(255,255,255,.88)',marginTop:3}}>{brand.nb_modeles} modèle{brand.nb_modeles>1?'s':''}</Text></View><Text style={{fontSize:24,color:'rgba(50,50,50,.6)'}}>›</Text></View>
  </TouchableOpacity>;
}
...
function ModelCard({ model, onPress, tablet }) {
  return <TouchableOpacity onPress={onPress} activeOpacity={.8} style={{flex:tablet?1:undefined,backgroundColor:'#fff',borderRadius:14,padding:12,marginBottom:12,borderWidth:1,borderColor:'#E7E7EB'}}>
    <View style={{flexDirection:'row',alignItems:'center',gap:12}}><ProductImage uri={model.image_uri} brand={model.marque} logoUri={model.logo_uri} size={78}/><View style={{flex:1}}><Text style={{fontSize:16,fontWeight:'800',color:COLORS.text}}>{model.nom}</Text><Text style={{marginTop:2,color:COLORS.muted,fontSize:12}}>{model.categorie} · {model.marque}</Text>{model.caracteristiques?<Text numberOfLines={2} style={{marginTop:5,color:'#555',fontSize:11}}>{model.caracteristiques}</Text>:null}<QualityBadge quality={model.data_quality}/></View><View style={{alignItems:'flex-end'}}><Text style={{fontWeight:'800',color:COLORS.primary}}>{model.nb_variantes||0}</Text><Text style={{fontSize:10,color:COLORS.muted}}>réf.</Text></View></View>
  </TouchableOpacity>;
}

function VariantCard({ item, model, onPress }) {
  return <TouchableOpacity onPress={onPress} style={{backgroundColor:'#fff',borderRadius:14,padding:12,marginBottom:10,borderWidth:1,borderColor:'#E5E7EB'}}><View style={{flexDirection:'row',alignItems:'center',gap:12}}><ProductImage uri={item.image_uri||model?.image_uri} brand={model?.marque} logoUri={model?.logo_uri} size={66}/><View style={{flex:1}}><Text style={{fontWeight:'800',fontSize:16}}>{item.nom}</Text>{item.reference?<Text style={{color:COLORS.muted,marginTop:2}}>{item.reference}</Text>:null}{item.description?<Text numberOfLines={2} style={{color:'#555',fontSize:12,marginTop:5}}>{item.description}</Text>:null}<QualityBadge quality={item.data_quality}/></View><Text style={{fontSize:22,color:'#777'}}>›</Text></View></TouchableOpacity>;
}
...
function CurvePreview({ curve }) {
  ...
  return <View style={{marginTop:8,backgroundColor:'#F7F8FA',borderRadius:12,padding:12}}><Text style={{fontWeight:'700'}}>{curve.nom}</Text><Text style={{fontSize:11,color:COLORS.muted}}>{curve.axe_y} ({curve.unite_y}) / {curve.axe_x} ({curve.unite_x})</Text><View style={{height:90,flexDirection:'row',alignItems:'flex-end',gap:4,marginTop:10,borderBottomWidth:1,borderBottomColor:'#CCC'}}>{points.map((p,i)=><View key={i} style={{flex:1,alignItems:'center',justifyContent:'flex-end'}}><View style={{width:'72%',height:`${Math.max(6,(Number(p.y)||0)/maxY*78)}%`,borderRadius:4,backgroundColor:COLORS.primary}}/><Text style={{fontSize:8,color:'#666',marginTop:2}}>{p.x}</Text></View>)}</View></View>;
}
...
export function EquipmentCatalogueBrowser(){
  ...
  const [tab,setTab]=useState('marques'),[brands,setBrands]=useState([]),[cats,setCats]=useState([]),[models,setModels]=useState([]),[search,setSearch]=useState('');
  const [brand,setBrand]=useState(null),[model,setModel]=useState(null),[variants,setVariants]=useState([]),[variant,setVariant]=useState(null),[sheet,setSheet]=useState(null),[modal,setModal]=useState(null);
  ...
  const saveModal=async v=>{try{if(modal==='brand'){if(!v.nom?.trim())return;await ajouterMarqueEquipement({nom:v.nom.trim(),logoUri:v.logo||null});}if(modal==='category'){if(!v.nom?.trim())return;await ajouterCategorieEquipement({nom:v.nom.trim(),icone:v.icone||'⚙️'});}if(modal==='model'){if(!v.nom?.trim()||!brand)return;const cat=cats.find(c=>c.nom.toLowerCase()===String(v.categorie||'').trim().toLowerCase())||cats[0];if(!cat)return;await ajouterModeleEquipement({categorieId:cat.id,marqueId:brand.id,nom:v.nom.trim(),reference:v.reference,caracteristiques:v.description,motsCles:`${brand.nom} ${v.nom}`});}if(modal==='variant'){if(!v.nom?.trim()||!model)return;await ajouterVarianteEquipement({modeleId:model.id,nom:v.nom.trim(),reference:v.reference,description:v.description});}if(modal==='spec'){if(!v.cle?.trim()||!sheet)return;await ajouterCaracteristiqueEquipement({varianteId:sheet.id,cle:v.cle.trim(),valeur:v.valeur,unite:v.unite});}if(modal==='curve'){if(!v.nom?.trim()||!sheet)return;let serie='[]';try{serie=JSON.stringify(String(v.points||'').split(';').filter(Boolean).map(x=>{const[a,b]=x.split(',');return{x:Number(a),y:Number(b)}}));}catch{}await ajouterCourbeEquipement({varianteId:sheet.id,nom:v.nom,axeX:v.axeX||'Débit',uniteX:v.uniteX||'m³/h',axeY:v.axeY||'HMT',uniteY:v.uniteY||'mCE',serie});}if(modal==='doc'){if(!v.nom?.trim()||!v.uri?.trim()||!sheet)return;await ajouterDocumentEquipement({varianteId:sheet.id,type:v.type||'Document',nom:v.nom,uri:v.uri});}setModal(null);await refresh();if(brand)await openBrand(brand);if(model){setModel(model);setVariants(await listerVariantesEquipement(model.id));}if(sheet){const d=await getFicheVarianteEquipement(sheet.id);setSheet(d?{...d,modele_image_uri:model?.image_uri,modele_logo_uri:model?.logo_uri}:null);}}catch(e){Alert.alert('Erreur',String(e.message||e));}};
