...
async function getContexteVisite(db, visiteId) {
  const visite = await db.getFirstAsync(
    `SELECT id, site_id, installation_id, api_remote_local_id, COALESCE(trame_id,'icpe_v1') AS trame_id FROM visites WHERE id=?`,
    [visiteId]
  );
...
const norm = (v) =>
  String(v || '')
    ...
    .replace(/[\u0300-\u036f]/g, '')
    .toLowerCase();
function compatibleParDefaut(e, trameId) {
  const texte = norm([e.type_code, e.designation, e.marque, e.modele].filter(Boolean).join(' '));
  const vmc = [
    'vmc',
    'caisson',
    'extracteur',
...
function equipementCompatible(e, trameId) {
  const explicites = String(e.trames_explicit || '')
    .split(',')
    .filter(Boolean);
...
async function affecterEquipementTrame(db, equipementId, trameId) {
  ...
  await db.runAsync(
    ...
    [equipementId, trameId]
  );
}
async function ensureInstallation(db, siteId, preferredInstallationId = null) {
  if (preferredInstallationId) {
    const preferred = await db.getFirstAsync(
      `SELECT id FROM installations WHERE id=? AND site_id=? AND actif=1 LIMIT 1`,
      [preferredInstallationId, siteId]
    );
  ...
  await db.runAsync(`INSERT INTO installations(id,site_id,type_code,nom,description,actif) VALUES(?,?,?,?,?,1)`, [
    ...
    'Créée automatiquement depuis une visite'
  ]);
  return id;
}
async function upsertObservation(db, equipementId, visiteId, { etat = null, commentaire = null, present = 1 } = {}) {
  const id = uuidv4();
  await db.runAsync(
    `INSERT INTO observations_equipement(id,equipement_id,visite_id,etat,commentaire,present) VALUES(?,?,?,?,?,?)
    ON CONFLICT(equipement_id,visite_id) DO UPDATE SET etat=COALESCE(excluded.etat,observations_equipement.etat), commentaire=COALESCE(excluded.commentaire,observations_equipement.commentaire), present=excluded.present, observe_le=datetime('now')`,
    [id, equipementId, visiteId, etat, commentaire, present ? 1 : 0]
  );
}
async function convertirMaterielLegacy(db, visiteId, installationId, trameId) {
  ...
  for (const m of lignes) {
    ...
    await db.runAsync(
      `INSERT INTO equipements(id,installation_id,type_code,designation,marque,modele,annee,statut) VALUES(?,?,?,?,?,?,?,'actif')`,
      [
        equipementId,
        installationId,
        m.categorie || 'equipement',
        m.designation || 'Équipement',
        m.marque || null,
        m.modele || null,
        m.annee ? Number(m.annee) || null : null
      ]
    );
    ...
    await db.runAsync(
      `UPDATE materiel SET equipement_id=?,nombre=CASE WHEN TRIM(COALESCE(nombre,''))='' THEN '1' ELSE nombre END WHERE id=?`,
      [equipementId, m.id]
    );
    await upsertObservation(db, equipementId, visiteId, { etat: m.etat || 'Bon', present: 1 });
  }
}
async function injecterEquipementsActifsDuSite(
  db,
  visiteId,
  siteId,
  trameId,
  ...
  const actifsBruts = await db.getAllAsync(
    `SELECT e.*,
    ...
    [visiteId, siteId, installationId, installationId, visiteId]
  );
  ...
  for (const e of actifs) {
    ...
    await db.runAsync(
      `INSERT INTO materiel(id,visite_id,categorie,nombre,designation,numero_materiel,reseau_desservi,marque,modele,caracteristiques,annee,etat,equipement_id) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)`,
      [
        materielId,
        visiteId,
        e.type_code || 'Équipement',
        nombre,
        e.designation || 'Équipement',
        e.numero_materiel_reference || null,
        e.reseau_desservi_reference || null,
        e.marque || null,
        e.modele || null,
        e.caracteristiques_reference || null,
        e.annee ? String(e.annee) : null,
        etat,
        e.id
      ]
    );
...
export async function listerMaterielPersistant(visiteId) {
  ...
  if (!contexte.installation_id) {
    await db.runAsync(`UPDATE visites SET installation_id=?,modifie_le=datetime('now') WHERE id=?`, [
      installationId,
      visiteId
    ]);
  ...
  // doit rester celle reçue de l'Intranet.
  await db.runAsync(
    `UPDATE materiel SET nombre='1' WHERE visite_id=? AND TRIM(COALESCE(nombre,''))=''
    AND equipement_id IS NOT NULL
    AND NOT EXISTS(SELECT 1 FROM provenances p WHERE p.entite_type='equipement' AND p.entite_id=materiel.equipement_id AND p.origine='api_symfony')`,
    [visiteId]
  );
  return db.getAllAsync(
    `SELECT m.*, e.statut AS statut_equipement,
     ...
     WHERE m.visite_id=? ORDER BY m.cree_le,m.id`,
    [apiPrepared ? 1 : 0, visiteId]
  );
}
export async function ajouterMaterielPersistant(visiteId) {
  ...
  await db.runAsync(
    `INSERT INTO materiel(id,visite_id,categorie,nombre,designation,etat,equipement_id) VALUES(?,?,?,?,?,?,?)`,
    [materielId, visiteId, 'Équipement', '1', 'Équipement', 'Bon', equipementId]
  );
...
export async function upsertMaterielPersistant(materielId, cle, valeur) {
  ...
  await db.runAsync(`UPDATE equipements SET ${colonne}=?,modifie_le=datetime('now') WHERE id=?`, [
    persist,
    m.equipement_id
  ]);
}
export async function retirerMaterielPersistant(materielId) {
  ...
  if (!m) return;
  if (m.equipement_id) {
    await db.runAsync(`UPDATE equipements SET statut='retire',modifie_le=datetime('now') WHERE id=?`, [
      m.equipement_id
    ]);
...
export async function listerHistoriqueEquipement(equipementId) {
  ...
  const db = await getDb();
  return db.getAllAsync(
    `SELECT o.id,o.etat,o.commentaire,o.present,o.observe_le, v.id AS visite_id,v.date_visite,v.statut AS statut_visite,v.trame_id,s.nom_site,
      (SELECT COUNT(*) FROM remarques r WHERE r.visite_id=v.id AND r.reference_type='equipement' AND r.reference_id=o.equipement_id) AS nb_remarques,
      (SELECT COUNT(*) FROM photos p WHERE p.entite_key='equipement||' || o.equipement_id) AS nb_photos
     FROM observations_equipement o JOIN visites v ON v.id=o.visite_id JOIN sites s ON s.id=v.site_id
     WHERE o.equipement_id=? ORDER BY COALESCE(v.date_visite,'') DESC,o.observe_le DESC`,
    [equipementId]
  );
}
