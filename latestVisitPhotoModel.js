...
export function mapLatestVisitPhotos(manifest, mapper) {
  ...
  return {
    ...manifest,
    sites: (manifest.sites || []).map((entry) => ({
      ...entry,
      locaux: (entry.locaux || []).map((room) => ({
        ...room,
        photos: (room.photos || []).map((photo) =>
          mapper({
            ...
            site: entry.site,
            local: room.local,
            derniereVisite: room.derniereVisite
          })
        )
      }))
    }))
...
export function filterLatestVisitPhotos(manifest, { siteIds = null, localIds = null, photoIds = null } = {}) {
  ...
  return {
    ...
    sites: (manifest.sites || [])
      .filter((s) => !sites || sites.has(String(s.site?.id)))
      .map((s) => ({
        ...s,
        locaux: (s.locaux || [])
          .filter((l) => !locals || locals.has(String(l.local?.id)))
    ...
    (manifest?.sites || [])
      .flatMap((s) =>
        (s.locaux || []).map((l) => [
          ...
          (l.photos || [])
            .map((p) => photoFileKey({ ...p, site: s.site, local: l.local, derniereVisite: l.derniereVisite }))
            .sort()
        ])
      )
