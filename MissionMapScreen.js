...
function coordinateList(geometry) {
  ...
  if (geometry.type === 'Point')
    return [{ latitude: Number(geometry.coordinates?.[1]), longitude: Number(geometry.coordinates?.[0]) }];
  if (geometry.type === 'LineString')
    return (geometry.coordinates || []).map((c) => ({ latitude: Number(c[1]), longitude: Number(c[0]) }));
  if (geometry.type === 'Polygon')
    return (geometry.coordinates?.[0] || []).map((c) => ({ latitude: Number(c[1]), longitude: Number(c[0]) }));
  return [];
}

function regionFromGeometries(rows) {
  const coords = rows
    .flatMap((row) => coordinateList(parse(row.geojson)))
...
export function MissionMapScreen({ navigation, route }) {
  ...
  const visibleTileLayers = useMemo(
    () =>
      mapLayers.filter(
        (layer) => Number(layer.visible) === 1 && layer.type === 'xyz_tiles' && layer.data?.pathTemplate
      ),
    [mapLayers]
  );
  ...
  const importGeo = async () => {
    try {
      ...
      Alert.alert('Import SIG impossible', String(e?.message || e));
    }
  };

  const exportGeo = async () => {
    try {
      await exporterGeoJsonMission(missionId, { share: true });
    } catch (e) {
      Alert.alert('Export SIG impossible', String(e?.message || e));
    }
  };
  ...
  const importTiles = async () => {
    ...
    try {
      ...
      if (result) {
        await load();
        Alert.alert(
          'Tuiles hors ligne importées',
          'Zoom ' + result.minZoom + ' à ' + result.maxZoom + '. Le fond peut maintenant fonctionner sans réseau.'
        );
      }
    } catch (e) {
    ...
    <View style={{ flex: 1, backgroundColor: MISSION_COLORS.bg }}>
      <View style={{ paddingHorizontal: 14, paddingTop: 10, paddingBottom: 8 }}>
        ...
        <Text style={{ color: COLORS.inkSoft, fontSize: 9.7, lineHeight: 14 }}>
          Maintiens le doigt sur la carte pour positionner le Site sélectionné. Les géométries restent exportables vers
          QGIS/SIG.
        </Text>
        <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 6, marginTop: 8 }}>
          {sites.map((site) => (
            <TouchableOpacity
              ...
              style={{
                borderRadius: 10,
                borderWidth: 1,
                borderColor: selectedSiteId === site.id ? MISSION_COLORS.accent : MISSION_COLORS.accentLine,
                backgroundColor: selectedSiteId === site.id ? MISSION_COLORS.accentLight : '#FFFFFF',
                paddingHorizontal: 9,
                paddingVertical: 6
              }}
            >
              <Text
                style={{
                  color: selectedSiteId === site.id ? MISSION_COLORS.accentStrong : COLORS.inkSoft,
                  fontSize: 8.8,
                  fontWeight: '800'
                }}
              >
          ...
          <TouchableOpacity style={[styles.btnSecondary, missionStyles.secondaryButton]} onPress={importGeo}>
            <Text style={[styles.btnSecondaryText, missionStyles.secondaryButtonText]}>＋ GeoJSON</Text>
          </TouchableOpacity>
          <TouchableOpacity style={[styles.btnSecondary, missionStyles.secondaryButton]} onPress={exportGeo}>
            <Text style={[styles.btnSecondaryText, missionStyles.secondaryButtonText]}>⇩ GeoJSON</Text>
          </TouchableOpacity>
          <TouchableOpacity
          ...
          >
            <Text style={[styles.btnSecondaryText, missionStyles.secondaryButtonText]}>⇩ GeoPackage</Text>
          </TouchableOpacity>
          <TouchableOpacity
            style={[styles.btnSecondary, missionStyles.secondaryButton]}
            disabled={layerBusy}
            onPress={importRaster}
          >
            <Text style={[styles.btnSecondaryText, missionStyles.secondaryButtonText]}>＋ Raster local</Text>
          </TouchableOpacity>
          <TouchableOpacity
            style={[styles.btnSecondary, missionStyles.secondaryButton]}
            disabled={layerBusy}
            onPress={importTiles}
          >
            <Text style={[styles.btnSecondaryText, missionStyles.secondaryButtonText]}>＋ Tuiles XYZ ZIP</Text>
          </TouchableOpacity>
          <TouchableOpacity
            style={[styles.btnSecondary, missionStyles.secondaryButton]}
            onPress={() => setLayersModal(true)}
          >
            <Text style={[styles.btnSecondaryText, missionStyles.secondaryButtonText]}>
              Couches ({mapLayers.length})
            </Text>
          </TouchableOpacity>
        </View>
      </View>
      ...
      >
        ...
        {geometries.map((row) => {
          ...
          if (geometry.type === 'Point') {
            ...
            return (
              <Marker
                ...
                description={
                  siteStat
                    ? String(siteStat.visits_done || 0) +
                      '/' +
                      String(siteStat.visits_total || 0) +
                      ' visite(s) · ' +
                      String(siteStat.open_points || 0) +
                      ' point(s) ouvert(s)' +
                      (Number(siteStat.critical_points || 0) ? ' · ' + siteStat.critical_points + ' critique(s)' : '')
                    : 'Géométrie Mission'
                }
                pinColor={
                  Number(siteStat?.critical_points || 0)
                    ? '#8B3A3A'
                    : Number(siteStat?.open_points || 0)
                      ? '#C78120'
                      : MISSION_COLORS.accent
                }
                onCalloutPress={() =>
                  row.site_id && navigation.navigate('MissionStructure', { missionId, siteId: row.site_id })
                }
              />
            );
          }
          ...
          if (geometry.type === 'Polygon')
            return <Polygon key={row.id} coordinates={coords} strokeWidth={3} fillColor="rgba(47,125,88,0.12)" />;
          return null;
        })}
      </MapView>
        ...
        <View style={styles.modalOverlay}>
          <View style={[styles.modalSheet, missionStyles.modalSheet]}>
            ...
            <Text style={{ color: COLORS.inkSoft, fontSize: 9.5, lineHeight: 14, marginBottom: 8 }}>
              GeoJSON, raster géoréférencé et tuiles XYZ peuvent rester disponibles hors ligne. Le fond web est masqué
              lorsqu’une couche XYZ locale est active.
            </Text>
                  ...
                  <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                    <TouchableOpacity
                      ...
                      style={{
                        ...
                        borderRadius: 8,
                        borderWidth: 1,
                        borderColor: Number(layer.visible) ? MISSION_COLORS.accent : MISSION_COLORS.accentLineStrong,
                        backgroundColor: Number(layer.visible) ? MISSION_COLORS.accentLight : '#FFFFFF',
                        alignItems: 'center',
                    ...
                    <View style={{ flex: 1 }}>
                      ...
                      <Text style={{ color: COLORS.inkFaint, fontSize: 8.5, marginTop: 2 }}>
                        {layer.type} · {Number(layer.offline_available) ? 'hors ligne' : 'métadonnées'}
                      </Text>
                    </View>
                    {layer.type === 'raster' ? (
                      <TouchableOpacity onPress={() => editRaster(layer)} style={{ padding: 6 }}>
                        <Text style={{ color: MISSION_COLORS.accentDark, fontSize: 9, fontWeight: '800' }}>Caler</Text>
                      </TouchableOpacity>
                    ) : null}
                    <TouchableOpacity onPress={() => removeLayer(layer)} style={{ padding: 6 }}>
                      <Text style={{ color: '#8B3A3A', fontSize: 13 }}>×</Text>
                    </TouchableOpacity>
                  </View>
                </View>
              ))}
        ...
        <View style={styles.modalOverlay}>
          ...
          >
            ...
            <TextInput
              ...
              value={rasterDraft.north}
              onChangeText={(v) => setRasterDraft((p) => ({ ...p, north: v }))}
              placeholder="Latitude Nord"
            />
            <TextInput
              style={[styles.input, missionStyles.input, { marginTop: 7 }]}
              keyboardType="decimal-pad"
              value={rasterDraft.south}
              onChangeText={(v) => setRasterDraft((p) => ({ ...p, south: v }))}
              placeholder="Latitude Sud"
            />
            <TextInput
              style={[styles.input, missionStyles.input, { marginTop: 7 }]}
              keyboardType="decimal-pad"
              value={rasterDraft.east}
              onChangeText={(v) => setRasterDraft((p) => ({ ...p, east: v }))}
              placeholder="Longitude Est"
            />
            <TextInput
              style={[styles.input, missionStyles.input, { marginTop: 7 }]}
              keyboardType="decimal-pad"
              value={rasterDraft.west}
              onChangeText={(v) => setRasterDraft((p) => ({ ...p, west: v }))}
              placeholder="Longitude Ouest"
            />
            <TextInput
              style={[styles.input, missionStyles.input, { marginTop: 7 }]}
              keyboardType="decimal-pad"
              value={rasterDraft.opacity}
              onChangeText={(v) => setRasterDraft((p) => ({ ...p, opacity: v }))}
              placeholder="Opacité 0 à 1"
            />
            <View style={styles.modalActions}>
              <TouchableOpacity
                style={[styles.btnSecondary, missionStyles.secondaryButton]}
                onPress={() => setRasterLayer(null)}
              >
                <Text style={[styles.btnSecondaryText, missionStyles.secondaryButtonText]}>Plus tard</Text>
              </TouchableOpacity>
              <TouchableOpacity style={[styles.btnPrimary, missionStyles.primaryButton]} onPress={saveRasterBounds}>
                <Text style={[styles.btnPrimaryText, missionStyles.primaryButtonText]}>Enregistrer le calage</Text>
              </TouchableOpacity>
            </View>
          </ScrollView>
        </View>
      ...
      <View
        style={{
          padding: 10,
          borderTopWidth: 1,
          borderTopColor: MISSION_COLORS.accentLine,
          backgroundColor: '#FFFFFF'
        }}
      >
        <Text style={{ color: MISSION_COLORS.accentStrong, fontSize: 9.5, fontWeight: '900', marginBottom: 4 }}>
          PROGRESSION MULTI-SITES
        </Text>
          ...
          {stats
            .map(
              (s) =>
                ...
                '/' +
                (s.visits_total || 0) +
                (Number(s.open_points || 0) ? ' · ' + s.open_points + ' ouvert(s)' : '') +
                (Number(s.critical_points || 0) ? ' · ' + s.critical_points + ' critique(s)' : '')
            )
            .join('   •   ') || 'Aucune donnée de progression.'}
        </Text>
      </View>
    </View>
  );
