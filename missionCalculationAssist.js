...
const RECOMMENDED_KEYS = Object.freeze({
  audit_energetique: [
    'specific_energy_area',
    'specific_energy_dwelling',
    'dju_corrected',
    'annual_energy_cost',
...
export async function getCalculationAutoValues({ missionId, formulaRow, siteId = null, equipmentId = null } = {}) {
  ...
  const [measures, references, equipment] = await Promise.all([
    db.getAllAsync(
      'SELECT m.*,d.measured_at,d.source_label FROM mission_measures m LEFT JOIN mission_measure_details d ON d.measure_id=m.id WHERE ' +
        measureWhere.join(' AND ') +
        ' ORDER BY COALESCE(d.measured_at,m.created_at) DESC LIMIT 150',
      measureParams
    ),
    db.getAllAsync(
  ...
  for (const input of schema) {
    ...
    const measure = measures.find(
      (row) => matchesMeasure(key, row) && row.value_number !== null && row.value_number !== undefined
    );
    if (measure) {
