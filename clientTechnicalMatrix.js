...
function normAvis(value) { const v = String(value || '').trim().toUpperCase().replace(/\s+/g, ''); if (v === 'NS') return 'N.S'; return v; }
...
export async function getClientTechnicalMatrix(clientId) {
  ...
  for (const visit of latest.values()) {
    const controls = await db.getAllAsync(`SELECT c.cle,c.avis,c.commentaire,r.id remarque_id,r.prestation,r.reference_libelle,COALESCE(r.criticite,2) criticite FROM controles_visite c LEFT JOIN remarques r ON r.visite_id=c.visite_id AND r.controle_key=(c.section_code || '||' || c.cle) WHERE c.visite_id=?`, [visit.id]);
    for (const control of controls) rows.push({ ...control, ...visit });
  }
  const bySite = sites.map((site) => {
    const cells = {};
    for (const category of MATRIX_CATEGORIES) cells[category.key] = { s: 0, ns: 0, nr: 0, nv: 0, so: 0, total: 0, maxSeverity: 0, issues: [], state: 'none' };
    for (const row of rows.filter((r) => r.site_id === site.id)) {
      for (const category of categoryFor(row)) {
        const cell = cells[category.key]; const avis = normAvis(row.avis); if (!avis) continue; cell.total += 1;
        if (avis === 'S') cell.s += 1; else if (avis === 'N.S') { cell.ns += 1; cell.maxSeverity = Math.max(cell.maxSeverity, Number(row.criticite || 2)); cell.issues.push(row); } else if (avis === 'N.R') cell.nr += 1; else if (avis === 'N.V') cell.nv += 1; else if (avis === 'S.O') cell.so += 1;
      }
    }
    for (const cell of Object.values(cells)) cell.state = stateFor(cell);
    return { ...site, cells };
  });
...
export async function getMatrixCellPhotos(issue) {
  if (!issue?.id) return [];
  const db = await getDb();
  const key = issue.remarque_id ? `remarque||${issue.remarque_id}` : null;
  const controlKey = issue.section_code && issue.cle ? `${issue.section_code}||${issue.cle}` : null;
