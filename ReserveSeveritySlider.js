...
export function ReserveSeveritySlider({ value = 2, defaultValue = null, onChange, compact = false }) {
  ...
  return (
    <View style={{ marginTop: compact ? 6 : 9 }}>
      <View style={{ flexDirection: 'row', alignItems: 'baseline', gap: 6, marginBottom: 4 }}>
        <Text style={{ color: COLORS.ink, fontSize: compact ? 10 : 11, fontWeight: '900' }}>
          Criticité · {current}/5 — {reserveSeverityLabel(current)}
        </Text>
        {proposed !== null ? (
          <Text style={{ color: adjusted ? COLORS.orangeDark : COLORS.inkSoft, fontSize: 9, fontWeight: '800' }}>
            {adjusted ? `ajustée · défaut ${proposed}` : 'valeur proposée'}
          </Text>
        ) : null}
      </View>
      ...
      <View
        ...
        accessibilityActions={[
          { name: 'increment', label: 'Augmenter la criticité' },
          { name: 'decrement', label: 'Diminuer la criticité' }
        ]}
      ...
      >
        ...
        >
          <View
            style={{
              width: `${(current / MAX_SEVERITY) * 100}%`,
              height: 8,
              borderRadius: 4,
              ...
              return (
                <View
                  ...
                  style={{
                    ...
                    height: 12,
                    borderRadius: 3,
                    backgroundColor: v <= current ? COLORS.orangeDark : COLORS.inkFaint
                  }}
                />
              );
            })
        ...
        {width ? (
          <View
            ...
            style={{
              ...
              shadowColor: '#000',
              shadowOpacity: dragging ? 0.18 : 0.1,
          ...
          >
            <Text
              style={{
                color: dragging ? COLORS.white : COLORS.orangeDark,
