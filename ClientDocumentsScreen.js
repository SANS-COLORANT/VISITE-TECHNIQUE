...
export function ClientDocumentsScreen({ route, navigation }) {
  ...
  const preparerStockage = async () => {
    try {
      ...
      setRacine(uri);
      if (!uri) Alert.alert('Stockage METRA', "L'autorisation du dossier Documents est nécessaire une seule fois pour classer automatiquement les fichiers.");
      return uri;
    } finally { setBusy(false); }
  };

  const ouvrirRapports = async () => {
    const uri = racine || await preparerStockage();
    if (!uri) return;
  ...
  return <ScrollView style={{ flex: 1, backgroundColor: COLORS.bg }} contentContainerStyle={styles.content}>
    ...
    <View style={{ padding: 12, borderRadius: 12, backgroundColor: racine ? '#EEF8F1' : '#FFF8E7', borderWidth: 1, borderColor: racine ? '#B7DEC2' : '#F0D99B', marginBottom: 14 }}>
      ...
      <Text style={{ marginTop: 4, fontSize: 11, color: COLORS.muted }}>{racine ? 'Photos, PDF, Word et Excel sont rangés dans Documents/METRA selon Client → Site → Visite.' : "Android demandera une seule fois l'accès au dossier Documents. Ensuite METRA range les fichiers sans redemander."}</Text>
