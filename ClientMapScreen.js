    ...
    return (
      <View style={{ minHeight: 280, alignItems: 'center', justifyContent: 'center', padding: 24 }}>
        <Text style={{ fontWeight: '800', textAlign: 'center' }}>
          Aucun point cartographique disponible pour cette sélection.
        </Text>
  ...
  return (
    <ScrollView horizontal showsHorizontalScrollIndicator>
      <Svg width={MAP_W} height={MAP_H}>
        ...
        <SvgText x={MAP_W - 30} y="26" fontSize="13" fontWeight="700" fill="#69757D">
          N
        </SvgText>
        {points.map(({ site, x, y }) => {
          ...
          return (
            <G key={site.id} onPress={() => onSelect(site)}>
              <Circle
                ...
                cy={y}
                r={selected ? 13 : 10}
                fill={selected ? '#10384B' : '#F07E31'}
                stroke="#fff"
                strokeWidth="3"
              />
              <SvgText x={x + 14} y={y + 4} fontSize="11" fontWeight="700" fill="#25313A">
                {String(site.nom_site || 'Site').slice(0, 28)}
              </SvgText>
            </G>
          );
        })}
...
function ClientMapScreen({ route, navigation }) {
  ...
  const synchroniser = async () => {
    ...
    try {
      ...
      setMessage(
        `${r.positionnes} adresse${r.positionnes > 1 ? 's' : ''} positionnée${r.positionnes > 1 ? 's' : ''}${r.indisponibles ? ` · ${r.indisponibles} à reprendre avec Internet` : ''}`
      );
  ...
  const choisirMode = async (mode) => {
    ...
    if (!GOOGLE_MAPS_BUILD_ENABLED) {
      setMessage(
        'Le fond Google nécessite la clé Google Maps du build Android. La Carte METRA hors ligne reste disponible.'
      );
      setModeFond('offline');
      return;
    }
      ...
      <ScrollView contentContainerStyle={{ padding: 14, paddingBottom: 40 }}>
        <View style={{ marginBottom: 12 }}>
          <Text style={{ fontSize: 20, fontWeight: '900', color: COLORS.ink }}>
            Carte METRA · {nomClient || 'Client'}
          </Text>
          <Text style={{ color: COLORS.muted, fontSize: 12, marginTop: 4 }}>
            Google Maps et Satellite sont utilisés quand Internet est disponible. Sans réseau, METRA conserve les
            positions des sites et bascule sur son fond hors ligne sans demander la position de la tablette.
          </Text>
        </View>
          ...
          >
            <Text
              style={{
                fontSize: 14,
                fontWeight: '900',
                color: online === true ? '#18794E' : online === false ? '#A26A00' : COLORS.muted
              }}
            >
        ...
        <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 8, marginTop: 10, alignItems: 'center' }}>
          <ModeButton
            actif={modeFond === 'google'}
            disabled={!GOOGLE_MAPS_BUILD_ENABLED}
            onPress={() => choisirMode('google')}
          >
            Google
          </ModeButton>
          <ModeButton
            actif={modeFond === 'satellite'}
            disabled={!GOOGLE_MAPS_BUILD_ENABLED}
            onPress={() => choisirMode('satellite')}
          >
            Satellite
          </ModeButton>
          <ModeButton actif={modeFond === 'offline'} onPress={() => choisirMode('offline')}>
            METRA hors ligne
          </ModeButton>
          <TouchableOpacity
            style={[styles.btnSecondary, { paddingVertical: 9 }]}
            disabled={syncing}
        ...
        {!GOOGLE_MAPS_BUILD_ENABLED ? (
          <Text style={{ color: '#A26A00', fontSize: 11.5, marginTop: 8 }}>
            Fond Google prêt dans METRA mais non activé sur ce build : une clé Google Maps Android doit être fournie au
            build GitHub.
          </Text>
        ) : null}
        ...
        >
          {modeFond !== 'offline' && googleAffichable ? (
            <MapView
              ...
              loadingEnabled
              onMapLoaded={() =>
                setMessage(modeFond === 'satellite' ? 'Fond satellite Google chargé.' : 'Fond Google Maps chargé.')
              }
            >
        ...
        <Text style={{ color: COLORS.muted, fontSize: 10.5, marginTop: 7 }}>
          Les tuiles Google ne sont jamais enregistrées par METRA. Le mode hors ligne utilise uniquement les positions
          locales déjà calculées, conformément aux règles Google Maps.
        </Text>
        ...
        {selection ? (
          ...
          >
            ...
            {selection.localisation_note ? (
              <Text style={{ color: COLORS.muted, marginTop: 5 }}>{selection.localisation_note}</Text>
            ) : null}
            <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 8, marginTop: 12 }}>
              ...
              <TouchableOpacity style={styles.btnSecondary} onPress={() => ouvrirGoogleMaps(selection)}>
                <Text style={styles.btnSecondaryText}>Google Maps ↗</Text>
              </TouchableOpacity>
              <TouchableOpacity style={styles.btnSecondary} onPress={() => ouvrirGoogleEarth(selection)}>
                <Text style={styles.btnSecondaryText}>Google Earth ↗</Text>
              </TouchableOpacity>
            </View>
          </View>
        ) : null}
