...
function AppContent() {
  ...
  const initialiser = useCallback(async () => {
    ...
    try {
      await getDb();
      const [pack, schemaVisible, lab3dEnabled] = await Promise.all([
        getActiveVisualPack(),
  ...
  useEffect(() => subscribeLabFeatureChanges((key, enabled) => {
    ...
    if (key === 'lab_3d') {
      ...
      if (!enabled) {
        setStack((s) => s.filter((entry, index) => entry.name !== 'Lab3D' || index === 0));
      }
    }
  }), []);
  ...
  return (
    <View key={`visual-${visualRevision}-${visualPack.id}`} style={{ flex: 1, backgroundColor: COLORS.bg }}>
      {current.name === 'Home' ? (
        <>
          <SimpleHeader title="Visite Technique" visualPack={visualPack} />
          <HomeScreen navigation={navigation} route={route} onR1LongPress={() => setR1Visible(true)} />
        </>
      ) : null}
      ...
      {current.name === 'SiteVisites' ? (
        <>
          <SimpleHeader title={current.params?.nomSite || 'Visites'} onBack={goBack} visualPack={visualPack} />
      ...
      {current.name === 'Visite' ? (
        <>
          ...
          {hydraulicVisible ? (
            ...
            >
              <Text style={{ color: COLORS.inkSoft, fontWeight: '800', fontSize: 10.5 }}>⌁ Schéma technique</Text>
            </TouchableOpacity>
          ) : null}
        </>
      ) : null}
