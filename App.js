...
function AppContent() {
  ...
  const initialiser=useCallback(async()=>{setDbReady(false);setDbError(null);setVisualPack(null);try{await getDb();const [pack,schemaVisible,lab3dEnabled]=await Promise.all([getActiveVisualPack(),getHydraulicSchemaVisible(),getLab3DVisible()]);setHydraulicVisible(schemaVisible);setLab3dVisible(lab3dEnabled);setRuntimeVisualPalette(pack?.colors);setVisualPack(pack);await new Promise((resolve)=>setTimeout(resolve,getVisualPackStartupDuration(pack)));setDbReady(true);}catch(err){setDbError(err);}},[]); useEffect(()=>{initialiser();},[initialiser]);
  const navigate=useCallback((name,params={})=>setStack((s)=>{const current=s[s.length-1];return current?.name===name&&JSON.stringify(current.params||{})===JSON.stringify(params||{})?s:[...s,{name,params}];}),[]);
  ...
  useEffect(()=>subscribeLabFeatureChanges((key,enabled)=>{if(key==='hydraulic_schema')setHydraulicVisible(enabled);if(key==='lab_3d'){setLab3dVisible(enabled);if(!enabled)setStack((s)=>s.filter((entry,index)=>entry.name!=='Lab3D'||index===0));}}),[]);
  useEffect(()=>{const subscription=BackHandler.addEventListener('hardwareBackPress',()=>{if(r1Visible)return true;if(stack.length<=1)return false;goBack();return true;});return()=>subscription.remove();},[stack.length,goBack,r1Visible]);
  ...
  if(!visualPack)return <View style={{flex:1,backgroundColor:SPLASH_BG}}/>; if(!dbReady)return <VisualPackLoadingScreen pack={visualPack}/>;
  const current=stack[stack.length-1],navigation={navigate,goBack},route={params:current.params};
  return <View key={`visual-${visualRevision}-${visualPack.id}`} style={{flex:1,backgroundColor:COLORS.bg}}>
    ...
    {current.name==='ClientTechnicalMatrix'&&<><SimpleHeader title="Cartographie technique" onBack={goBack} visualPack={visualPack}/><ClientTechnicalMatrixScreen navigation={navigation} route={route}/></>}
    {current.name==='SiteVisites'&&<><SimpleHeader title={current.params?.nomSite||'Visites'} onBack={goBack} visualPack={visualPack}/><SiteVisitesScreen navigation={navigation} route={route}/>{lab3dVisible?<Lab3DFab onPress={()=>navigate('Lab3D',{siteId:current.params?.siteId,nomSite:current.params?.nomSite})} bottom={82} label="⬡ LAB 3D du site"/>:null}</>}
    ...
    {current.name==='Lab3D'&&lab3dVisible&&<><SimpleHeader title="LAB 3D · Maquette du site" onBack={goBack} visualPack={visualPack}/><Lab3DScreen navigation={navigation} route={route}/></>}
    {current.name==='Report'&&<ReportScreen route={route} onBack={goBack}/>} {current.name==='Parametres'&&<><SimpleHeader title="Paramètres" onBack={goBack} visualPack={visualPack}/><VisualPacksSettingsScreen visualPack={visualPack} onVisualPackChanged={handleVisualPackChanged}/></>}<R1EasterEgg visible={r1Visible} onFinish={()=>setR1Visible(false)}/>
  </View>;
}
