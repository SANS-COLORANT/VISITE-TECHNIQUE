...
export async function chargerContexteAutoVisiteMission(missionId, {
  siteId = null,
  locationId = null,
  equipmentId = null,
} = {}) {
  ...
  if (siteId) { equipmentWhere.push('e.site_id=?'); equipmentParams.push(siteId); }
  if (locationId) { equipmentWhere.push('e.location_id=?'); equipmentParams.push(locationId); }
  if (equipmentId) { equipmentWhere.push('e.id=?'); equipmentParams.push(equipmentId); }
  ...
  const [
    ...
    db.getAllAsync(
      'SELECT label,type,status FROM mission_installations WHERE mission_id=?' + commonSiteWhere + ' ORDER BY label LIMIT 40',
      siteParams
    ),
    db.getAllAsync(
      'SELECT label,type,status FROM mission_networks WHERE mission_id=?' + commonSiteWhere + ' ORDER BY label LIMIT 50',
      siteParams
    ),
    db.getAllAsync(
      ...
      [missionId, ...(siteId ? [siteId] : []), ...(locationId ? [locationId] : []), ...(equipmentId ? [equipmentId] : [])]
    ),
    db.getAllAsync(
  ...
  const activePoints = points.filter((row) => !['closed','no_follow_up'].includes(String(row.status || '').toLowerCase()));
  const activeActions = actions.filter((row) => !['closed','cancelled','done'].includes(String(row.status || '').toLowerCase()));

  const equipmentSummary = compactList(equipment.map((row) => {
    const extras = [row.state,row.verification_status].filter(Boolean).join('/');
    return equipmentLabel(row) + (extras ? ' [' + extras + ']' : '');
  }), 8);

  const installationSummary = compactList(installations.map((row) => [row.label,row.type].filter(Boolean).join(' · ')), 8);
  const networkSummary = compactList(networks.map((row) => [row.label,row.type].filter(Boolean).join(' · ')), 8);

  const measureSummary = compactList(measures.map((row) => {
    const value = valueWithUnit(row.value_number,row.value_text,row.unit);
    return [row.type,value].filter(Boolean).join(' : ');
  }), 8);

  const referenceSummary = compactList(references.map((row) => {
    const value = valueWithUnit(row.value_number,row.value_text,row.unit);
    return [row.measure_type,value,row.source_label].filter(Boolean).join(' : ');
  }), 6);

  const pointSummary = compactList(activePoints.map((row) => [row.label || row.description,row.status,row.priority].filter(Boolean).join(' · ')), 7);
  const actionSummary = compactList(activeActions.map((row) => [row.label,row.status,row.due_date || row.due_text].filter(Boolean).join(' · ')), 7);
  const documentSummary = compactList(documents.map((row) => [row.name,row.document_date].filter(Boolean).join(' · ')), 7);
  const scenarioSummary = compactList(scenarios.map((row) => row.label), 6);
...
export function valeurAutoPourChampMission(field, context = {}) {
  ...
  if (containsAny(token, ['equipement','équipement','generateur','générateur','caisson','automate','ouvrage','production'])) {
    return context.equipmentSummary || '';
  }
  if (containsAny(token, ['reseau','réseau','circuit','distribution','bouclage','architecture','colonne','branche','schema','schéma'])) {
    return context.architectureSummary || context.networkSummary || '';
  }
  if (containsAny(token, ['mesure','température','temperature','pression','debit','débit','frequence','fréquence','consigne','index','puissance'])) {
