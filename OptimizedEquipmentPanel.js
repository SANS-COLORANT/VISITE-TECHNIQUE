...
function fusionnerOptions(base, rows) {
  return unique([...base, ...rows.map((r) => r.nom)]);
}
function egal(a, b) {
  return (
    String(a || '')
    ...
    <View style={{ marginTop: 8 }}>
      ...
      <View style={styles.catalogueChoiceGrid}>
        {valeurs.slice(0, 6).map((v) => (
          <TouchableOpacity
            key={v}
            style={[styles.catalogueChoice, egal(actif, v) && styles.catalogueChoiceActive]}
            onPress={() => onPick(v)}
          >
...
const EquipmentCard = memo(function EquipmentCard({
  ...
  const [designation, setDesignation, blurDesignation, setDesignationImmediate] = useDurableAutosave(
    item.designation,
    (v) => upsertMaterielChamp(item.id, 'designation', v)
  );
  const [modele, setModele, blurModele, setModeleImmediate] = useDurableAutosave(item.modele, (v) =>
  ...
  const marquesContextuelles = useMemo(() => unique(catalogueCategorie.map((e) => e.marque)), [catalogueCategorie]);
  const modelesRecommandes = useMemo(() => {
    const candidats = catalogue.filter(
      (e) => (!categorie || egal(e.categorie, categorie)) && (!marque || egal(e.marque, marque)) && e.modele
    );
    return unique(candidats.map((e) => e.modele));
  }, [catalogue, categorie, marque]);
  const designationsRecommandees = useMemo(
    () =>
      unique([
        ...designationOptions,
        categorie,
  ...
  const sauverMarque = async (v) => {
    setMarque(v);
    await upsertMaterielChamp(item.id, 'marque', v);
  };
  ...
  const choisirModele = async (nomModele) => {
    const ref =
      catalogue.find(
        (e) =>
          egal(e.modele, nomModele) &&
          (!categorie || egal(e.categorie, categorie)) &&
          (!marque || egal(e.marque, marque))
      ) || catalogue.find((e) => egal(e.modele, nomModele));
    if (ref?.categorie && !egal(ref.categorie, categorie)) {
      setCategorie(ref.categorie);
      await upsertMaterielChamp(item.id, 'categorie', ref.categorie);
    }
    if (ref?.marque && !egal(ref.marque, marque)) {
      setMarque(ref.marque);
      await upsertMaterielChamp(item.id, 'marque', ref.marque);
    }
  ...
  const choisirCatalogue = async (e) => {
    ...
    if (!designation || egal(designation, 'Équipement'))
      await setDesignationImmediate(nouvelleCategorie || 'Équipement');
  ...
  return (
    <View style={styles.formCard}>
      ...
      <View style={styles.materielTopRow}>
        <View style={{ flex: 1 }}>
          ...
          <TypeAheadInput
            valeur={categorie}
            options={categories}
            placeholder="Ex. Chaudière, pompe, échangeur…"
      ...
      <TextInput
        style={styles.input}
        placeholder="Ex. Chaudière n°1"
        value={designation}
        onChangeText={setDesignation}
        onBlur={blurDesignation}
      />
      <Suggestions
        titre="Désignations suggérées"
        valeurs={designationsRecommandees}
        actif={designation}
        onPick={(v) => setDesignationImmediate(v).catch(() => {})}
      />
      ...
      <View style={{ flexDirection: 'row', gap: 8 }}>
        <View style={{ flex: 1 }}>
          ...
          <TypeAheadInput
            valeur={marque}
            options={marquesContextuelles.length ? marquesContextuelles : marques}
            placeholder="Marque"
      ...
      <Suggestions
        titre={categorie && marque ? `Modèles ${categorie} · ${marque}` : 'Modèles recommandés'}
        valeurs={modelesRecommandes}
        actif={modele}
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalSheet}>
            ...
            <Text style={styles.importHint}>
              Les références compatibles avec le type et la marque sélectionnés sont affichées en premier.
            </Text>
            <FlatList
              data={[...catalogue].sort((a, b) => {
                const sa =
                  (categorie && egal(a.categorie, categorie) ? 2 : 0) + (marque && egal(a.marque, marque) ? 1 : 0);
                const sb =
                  (categorie && egal(b.categorie, categorie) ? 2 : 0) + (marque && egal(b.marque, marque) ? 1 : 0);
                return (
                  sb - sa ||
                  String(a.categorie || '').localeCompare(String(b.categorie || '')) ||
                  String(a.marque || '').localeCompare(String(b.marque || ''))
                );
              })}
              keyExtractor={(e) => e.id}
              style={{ maxHeight: 380 }}
...
function OptimizedEquipmentPanel({ visiteId }) {
  ...
  return (
    <FlatList
      ...
      ListHeaderComponent={
        <View>
          <Text style={styles.sectionTitle}>Équipements — feuille MATERIEL · {materiel.length}</Text>
          <Text style={styles.importHint}>
            Sélectionne d’abord le type puis la marque : les modèles du catalogue sont proposés automatiquement.
          </Text>
        </View>
      }
      ListFooterComponent={
      ...
      ListEmptyComponent={
        <View style={styles.empty}>
          <Text style={styles.emptyText}>Aucun équipement pour cette visite.</Text>
        </View>
      }
