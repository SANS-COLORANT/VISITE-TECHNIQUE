...
function expandSearch(value = '') {
  ...
  Object.entries(SYNONYMS).forEach(([key, vals]) => {
    if (q.includes(key)) {
      vals.forEach((v) => out.add(q.replace(key, v)));
      out.add(key);
...
export async function rechercherCatalogueIntelligent({
  ...
  if (categorieId) {
    filters.push('m.categorie_id=?');
    params.push(categorieId);
  }
  if (marqueId) {
    filters.push('m.marque_id=?');
    params.push(marqueId);
  }
  if (lifecycle) {
    filters.push('m.lifecycle_status=?');
  ...
  return db.getAllAsync(
    `SELECT m.*,c.nom categorie,c.icone,b.nom marque,b.logo_uri,b.couleur,
    ...
    ORDER BY COALESCE(u.favori,0) DESC, COALESCE(u.dernier_acces,'') DESC, c.ordre,b.nom,m.nom LIMIT ?`,
    params
  );
}

export async function enregistrerOuvertureModele(modeleId) {
  const db = await getCatalogueDb();
  await db.runAsync(
    `INSERT INTO catalogue_usage(modele_id,favori,ouvertures,dernier_acces) VALUES(?,0,1,datetime('now')) ON CONFLICT(modele_id) DO UPDATE SET ouvertures=ouvertures+1,dernier_acces=datetime('now')`,
    [modeleId]
  );
}
export async function basculerFavoriModele(modeleId) {
  const db = await getCatalogueDb();
  await db.runAsync(
    `INSERT INTO catalogue_usage(modele_id,favori,ouvertures,dernier_acces) VALUES(?,1,0,datetime('now')) ON CONFLICT(modele_id) DO UPDATE SET favori=CASE WHEN favori=1 THEN 0 ELSE 1 END,dernier_acces=datetime('now')`,
    [modeleId]
  );
  return db.getFirstAsync('SELECT favori FROM catalogue_usage WHERE modele_id=?', [modeleId]);
}
export async function listerModelesRecents(limit = 12) {
  const db = await getCatalogueDb();
  return db.getAllAsync(
    `SELECT m.*,c.nom categorie,b.nom marque,b.logo_uri,b.couleur,u.favori,u.dernier_acces,u.ouvertures,(SELECT COUNT(*) FROM variantes_equipement v WHERE v.modele_id=m.id AND v.actif=1) nb_variantes,(SELECT COUNT(*) FROM variantes_equipement v JOIN documents_equipement d ON d.variante_id=v.id WHERE v.modele_id=m.id) nb_documents,(SELECT COUNT(*) FROM variantes_equipement v JOIN courbes_equipement q ON q.variante_id=v.id WHERE v.modele_id=m.id) nb_courbes FROM catalogue_usage u JOIN modeles_equipement m ON m.id=u.modele_id JOIN categories_equipement c ON c.id=m.categorie_id JOIN marques_equipement b ON b.id=m.marque_id WHERE m.actif=1 ORDER BY u.dernier_acces DESC LIMIT ?`,
    [limit]
  );
}
...
export function getFamilyPriorityKeys(category = '') {
  ...
  if (c.includes('pompe') || c.includes('circulateur'))
    return ['Débit', 'HMT', 'Vitesse', 'Puissance', 'DN', 'Pression', 'EEI'];
  if (c.includes('pac') || c.includes('groupe eau') || c.includes('froid'))
    return [
      'Puissance chauffage',
      'Puissance froid',
      'COP',
  ...
  if (c.includes('cta') || c.includes('vmc') || c.includes('ventil'))
    return ['Débit d’air', 'Pression', 'Récupération', 'Filtration', 'Puissance moteur', 'Régulation'];
  if (c.includes('chaudi')) return ['Puissance', 'Rendement', 'Pression', 'Température', 'Combustible', 'NOx'];
  if (c.includes('échangeur'))
    return ['Puissance', 'Débit', 'Nombre de plaques', 'Pression', 'Température', 'Matériau'];
  return [];
}
...
export async function obtenirAuditCompletudeCatalogue(limit = 50) {
  ...
  return rows
    .map((r) => ({ ...r, completude: scoreCompletudeModele(r) }))
    .sort((a, b) => a.completude - b.completude || String(a.marque).localeCompare(String(b.marque)))
    .slice(0, limit);
}
