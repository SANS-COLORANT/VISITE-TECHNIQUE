...
export async function listerCouchesCarteMission(missionId) {
  ...
  const rows = await db.getAllAsync('SELECT * FROM mission_map_layers WHERE mission_id=? ORDER BY created_at', [
    missionId
  ]);
...
export async function importerRasterMission({ missionId } = {}) {
  ...
  await db.runAsync(
    ...
    [
      id,
      missionId,
      asset.name || 'Raster local',
      'raster',
      uri,
      JSON.stringify({ bounds: null }),
      1,
      1,
      JSON.stringify({ opacity: 0.75 })
    ]
  );
  return { id, label: asset.name || 'Raster local', sourceUri: uri };
}

export async function configurerRasterMission(layerId, { north, south, east, west, opacity = 0.75 } = {}) {
  const nums = {
  ...
  const db = await getDb();
  const row = await db.getFirstAsync('SELECT data_json,style_json FROM mission_map_layers WHERE id=? AND type=?', [
    layerId,
    'raster'
  ]);
  ...
  await db.runAsync("UPDATE mission_map_layers SET data_json=?,style_json=?,updated_at=datetime('now') WHERE id=?", [
    JSON.stringify(data),
...
export async function mettreAJourCoucheCarteMission(layerId, changes = {}) {
  ...
  if ('label' in changes) {
    setters.push('label=?');
    values.push(clean(changes.label) || current.label);
  }
  if ('visible' in changes) {
    setters.push('visible=?');
    values.push(changes.visible ? 1 : 0);
  }
  if ('data' in changes) {
    setters.push('data_json=?');
    values.push(JSON.stringify(changes.data || {}));
  }
  if ('style' in changes) {
    setters.push('style_json=?');
