...
export function extrairePlaqueDepuisOcr(rawText) {
  ...
  return {
    ...
    refrigerantChargeKg: charge
      ? charge[2].toLowerCase() === 'g'
        ? numberOrNull(charge[1]) / 1000
        : numberOrNull(charge[1])
      : null,
    rawLines: lines
  };
}
...
export async function creerEquipementMission({
  ...
  await db.runAsync(
    ...
    [
      ...
      siteId,
      clean(locationId),
      clean(type) || 'Équipement',
      clean(brand),
      clean(model),
      clean(installationYear),
      finalState,
      ...
      finalVerification,
      finalLifecycle,
      numberOrNull(expectedLifetimeYears),
      numberOrNull(replacementCost),
      replacementYear ? Number(replacementYear) : null,
...
export async function modifierEquipementMission(equipmentId, changes = {}) {
  ...
  const lifecycleChanged =
    'lifecycleStatus' in changes && clean(current.lifecycle_status) !== clean(changes.lifecycleStatus);
  const verificationChanged =
    'verificationStatus' in changes && clean(current.verification_status) !== clean(changes.verificationStatus);
  ...
  for (let i = 0; i < count; i += 1) {
    ...
    await db.runAsync(
      ...
      [
        ...
        source.expected_lifetime_years,
        source.replacement_cost,
        source.replacement_year,
        source.criticality_json
      ]
    );
    ids.push(id);
  }
...
export async function creerComposantMission({
  missionId,
  equipmentId,
  type = null,
  label,
...
export async function listerStructureTechniqueMission(missionId) {
  ...
  const [installations, systems, networks, components] = await Promise.all([
    ...
    db.getAllAsync('SELECT * FROM mission_networks WHERE mission_id=? ORDER BY installation_id,system_id,label', [
      missionId
    ]),
...
export async function lancerOcrPlaqueMission({ missionId, photoId, equipmentId = null, fileUri } = {}) {
  ...
  try {
    ...
    if (result?.unavailable) {
      await db.runAsync('UPDATE mission_ocr_jobs SET status=?,error_text=?,duration_ms=?,completed_at=? WHERE id=?', [
        'unavailable',
        'OCR local indisponible sur cet appareil.',
        Date.now() - started,
        new Date().toISOString(),
        id
      ]);
    ...
    await db.runAsync(
      ...
      [
        ...
        Number(result?.durationMs || Date.now() - started),
        new Date().toISOString(),
        id
      ]
    );
...
export async function getEquipmentDetails(equipmentId) {
  ...
  const [components, photos, measures, points, lifecycle, ocr] = await Promise.all([
    ...
    db.getAllAsync('SELECT * FROM mission_photos WHERE equipment_id=? ORDER BY COALESCE(taken_at,created_at) DESC', [
      equipmentId
    ]),
    db.getAllAsync('SELECT * FROM mission_measures WHERE equipment_id=? ORDER BY created_at DESC LIMIT 100', [
      equipmentId
    ]),
    db.getAllAsync('SELECT * FROM mission_points WHERE equipment_id=? ORDER BY created_at DESC LIMIT 100', [
      equipmentId
    ]),
    db.getAllAsync('SELECT * FROM mission_equipment_lifecycle WHERE equipment_id=? ORDER BY created_at DESC', [
      equipmentId
    ]),
    db.getAllAsync('SELECT * FROM mission_ocr_jobs WHERE equipment_id=? ORDER BY created_at DESC LIMIT 20', [
      equipmentId
    ])
  ]);
  return {
