...
export function SpiralActiveDock({ exploreActions = [], actionActions = [], quickActions = [], disabled = false }) {
  ...
  useEffect(() => {
    Animated.timing(panelProgress, {
      ...
      duration: mode === 'closed' ? 130 : 190,
      useNativeDriver: true
    }).start();
  }, [mode, panelProgress]);
  const settleRotation = (target) =>
    Animated.spring(rotation, { toValue: target, speed: 18, bounciness: 7, useNativeDriver: true }).start();
  const close = () => {
    setMode('closed');
    settleRotation(0);
  };
  const selectMode = (nextMode) => {
    ...
    setMode(nextMode);
    settleRotation(nextMode === 'explore' ? -22 : nextMode === 'actions' ? 22 : 0);
  };
  ...
  return (
    <View pointerEvents="box-none" style={StyleSheet.absoluteFill}>
      {mode !== 'closed' ? (
        <TouchableOpacity
          accessibilityLabel="Fermer le menu de la spirale"
          activeOpacity={1}
      ...
      <Animated.View
        ...
        style={[styles.panel, { opacity: panelProgress, transform: [{ translateY: panelTranslate }] }]}
      >
        <View style={styles.panelHeader}>
          <View>
            <Text style={styles.panelEyebrow}>SPIRALE ACTIVE</Text>
            <Text style={styles.panelTitle}>
              {mode === 'explore' ? 'Explorer' : mode === 'actions' ? 'Agir' : 'Accès rapide'}
            </Text>
          </View>
          <TouchableOpacity
            accessibilityRole="button"
      ...
      <View pointerEvents="none" style={styles.directionHints}>
        <Text style={[styles.directionText, styles.directionLeft]}>‹ EXPLORER</Text>
        <Text style={[styles.directionText, styles.directionRight]}>AGIR ›</Text>
      </View>
      <View style={styles.touchZone} {...panResponder.panHandlers}>
        <View pointerEvents="none" style={styles.dockHalo} />
        <Animated.View pointerEvents="none" style={[styles.spiralWrap, { transform: [{ rotate }] }]}>
          <SpiralSvg size={DOCK_SIZE} strokeWidth={9.2} showCenter={false} />
        </Animated.View>
      </View>
    </View>
  );
...
const styles = StyleSheet.create({
  ...
  panelEyebrow: { color: '#F26426', fontSize: 9, lineHeight: 12, fontWeight: '900', letterSpacing: 1.6 },
  panelTitle: { marginTop: 1, color: '#14202C', fontSize: 17, lineHeight: 21, fontWeight: '900' },
  closeButton: {
    width: 34,
    height: 34,
    borderRadius: 17,
  ...
  closeButtonText: { color: '#14202C', fontSize: 21, lineHeight: 23, fontWeight: '500' },
  ...
  actionIconWrapHot: { backgroundColor: '#FFF0E7' },
  actionIcon: { color: '#14202C', fontSize: 17, fontWeight: '900' },
  actionLabel: { color: '#14202C', fontSize: 10.5, lineHeight: 13, fontWeight: '900', textAlign: 'center' },
  actionCaption: { marginTop: 2, color: '#76808A', fontSize: 8.5, lineHeight: 11, textAlign: 'center' },
  directionHints: { position: 'absolute', left: 0, right: 0, bottom: 59, height: 28, zIndex: 312 },
  directionText: { position: 'absolute', color: '#7B848A', fontSize: 8.5, fontWeight: '900', letterSpacing: 0.9 },
  directionLeft: { right: '59%' },
  directionRight: { left: '59%' },
  touchZone: {
    position: 'absolute',
    left: '50%',
  ...
  dockHalo: {
    ...
    borderWidth: 1,
    borderColor: 'rgba(242,100,38,0.16)'
  },
  spiralWrap: { position: 'absolute', top: 8, width: DOCK_SIZE, height: DOCK_SIZE }
});
