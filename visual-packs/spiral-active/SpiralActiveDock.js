...
export function SpiralActiveDock({ exploreActions = [], actionActions = [], quickActions = [], disabled = false }) {
  ...
  const panResponder = useMemo(() => PanResponder.create({
    ...
    onPanResponderMove: (_, gesture) => {
      const angle = clamp(gesture.dx * 0.36, -MAX_ANGLE, MAX_ANGLE);
  ...
  return (
    <View pointerEvents="box-none" style={StyleSheet.absoluteFill}>
      {mode !== 'closed' ? (
        <TouchableOpacity accessibilityLabel="Fermer le menu de la spirale" activeOpacity={1} style={styles.dismissLayer} onPress={close} />
      ) : null}
      ...
      <View style={styles.touchZone} {...panResponder.panHandlers}>
        ...
        <Animated.View pointerEvents="none" style={[styles.spiralWrap, { transform: [{ rotate }] }]}>
          <SpiralSvg size={DOCK_SIZE} strokeWidth={8.2} showCenter={false} />
        </Animated.View>
      </View>
    </View>
  );
...
const styles = StyleSheet.create({
  dismissLayer: {
    ...
    backgroundColor: 'rgba(20,32,44,0.06)',
    zIndex: 305,
  },
  panel: {
    position: 'absolute',
  ...
  directionLeft: { right: '57%' },
  directionRight: { left: '57%' },
  touchZone: {
    position: 'absolute',
    left: '50%',
    marginLeft: -78,
