...
const PALETTE = {
  ...
  cardStrong: 'rgba(244,246,244,0.97)',
  cardDisabled: 'rgba(235,238,236,0.94)',
  line: 'rgba(18,27,34,0.12)',
  muted: '#66727C',
};

const BUILDING_SECTORS = Object.freeze({
  haussmann: 'Copro',
  collectif: 'Bailleur',
...
function GlassAction({ title, subtitle, primary = false, onPress, disabled = false, testID, style }) {
  return (
    <TouchableOpacity testID={testID} activeOpacity={0.84} accessibilityRole="button"
...
export function SpiralActiveHome({
  ...
  const layout = getHomeLayout({
    ...
    canvas: MANIFEST.homeScene.canvas,
  });
  ...
  const openBuilding = buildingId => {
    const sector = BUILDING_SECTORS[buildingId];
    ...
    <View style={styles.root} onLayout={measure} testID="premium-home-root">
      ...
      <Modal visible={modalVisible} transparent animationType="fade" onRequestClose={() => setModalVisible(false)}>
        <View style={styles.modalOverlay}><View style={styles.modalSheet}>
          ...
          <TextInput style={styles.modalInput} placeholder="Nom du client" placeholderTextColor="#8A929A"
            value={nouveauNom} onChangeText={setNouveauNom} />
          <TextInput style={styles.modalInput} placeholder="Code exploitant (optionnel)" placeholderTextColor="#8A929A"
            value={nouveauCode} onChangeText={setNouveauCode} />
...
const styles = StyleSheet.create({
  ...
  header: {
    ...
    flexDirection: 'row', justifyContent: 'flex-end', alignItems: 'center', flexWrap: 'wrap', gap: 10,
    paddingVertical: 12, backgroundColor: 'transparent',
  },
  connectivityPill: {
    minWidth: 112, minHeight: 44, paddingHorizontal: 15, borderRadius: 24, borderWidth: 1,
    borderColor: PALETTE.line, backgroundColor: 'rgba(255,255,253,0.94)',
    flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8, flexShrink: 1,
  },
  statusDot: { width: 8, height: 8, borderRadius: 4, flexShrink: 0 },
  connectivityText: { color: PALETTE.ink, fontSize: 12, fontWeight: '800', letterSpacing: 0.5, flexShrink: 1 },
  settingsButton: {
    width: 44, height: 44, borderRadius: 22, borderWidth: 1, borderColor: PALETTE.line,
    backgroundColor: 'rgba(255,255,253,0.94)', alignItems: 'center', justifyContent: 'center',
  },
  settingsIcon: { color: PALETTE.ink, fontSize: 22 },
  body: { flex: 1 },
  content: { alignSelf: 'center', zIndex: 20 },
  glassAction: {
    minHeight: 82, borderRadius: 20, borderWidth: 1, borderColor: PALETTE.line,
    backgroundColor: PALETTE.card, paddingHorizontal: 18, paddingVertical: 15,
    flexDirection: 'row', alignItems: 'center', shadowColor: '#111820', shadowOpacity: 0.07,
    shadowRadius: 14, shadowOffset: { width: 0, height: 5 }, elevation: 3,
  },
  glassActionPrimary: { minHeight: 90, backgroundColor: PALETTE.cardStrong, borderColor: 'rgba(18,27,34,0.15)' },
  glassActionDisabled: { backgroundColor: PALETTE.cardDisabled, opacity: 0.72 },
  actionTextWrap: { flex: 1, minWidth: 0, backgroundColor: 'transparent' },
  actionTitle: { color: PALETTE.ink, fontSize: 17, fontWeight: '700', backgroundColor: 'transparent' },
  actionTitlePrimary: { fontSize: 19, fontWeight: '800' },
  actionSubtitle: { marginTop: 5, color: PALETTE.muted, fontSize: 13, backgroundColor: 'transparent' },
  actionArrow: { marginLeft: 10, color: '#35414A', fontSize: 25, flexShrink: 0 },
  secondaryRow: { gap: 12, marginTop: 12 },
  secondaryAction: { flex: 1, minWidth: 0 },
  sceneNotice: {
    minHeight: 44, justifyContent: 'center', paddingHorizontal: 12, paddingVertical: 8, marginBottom: 8,
    borderRadius: 12, borderWidth: 1, borderColor: PALETTE.line, backgroundColor: PALETTE.card,
  },
  sceneNoticeText: { color: PALETTE.ink, fontSize: 13 },
  modalOverlay: { flex: 1, backgroundColor: 'rgba(9,13,17,0.32)', alignItems: 'center', justifyContent: 'center', padding: 20 },
  modalSheet: {
    width: '100%', maxWidth: 520, borderRadius: 24, backgroundColor: '#FFFDF8', padding: 22,
    borderWidth: 1, borderColor: '#DDE1E3', elevation: 18,
  },
  ...
  modalInput: {
    minHeight: 50, marginBottom: 10, borderRadius: 15, borderWidth: 1, borderColor: '#DDE1E3',
    backgroundColor: '#F5F5F1', paddingHorizontal: 14, color: PALETTE.ink, fontSize: 14,
  },
  modalActions: { flexDirection: 'row', gap: 10, marginTop: 8 },
  modalCancel: {
    flex: 1, minHeight: 48, borderRadius: 15, borderWidth: 1, borderColor: '#D7DCE0',
    backgroundColor: '#F6F7F5', alignItems: 'center', justifyContent: 'center',
  },
