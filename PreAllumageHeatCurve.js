...
function PointEditor({ point, onSave, onDelete }) {
  ...
  useEffect(() => {
    setOutdoor(formatNombre(point.outdoor));
  }, [point.outdoor]);
  useEffect(() => {
    setWater(point.water === null ? '' : formatNombre(point.water));
  }, [point.water]);

  const save = () => {
    const x = numeric(outdoor);
    const y = numeric(water);
    if (x === null) {
      setOutdoor(formatNombre(point.outdoor));
      return;
    }
  ...
  return (
    ...
    >
      ...
      <View style={{ flex: 1, paddingTop: 12 }}>
        <Text style={{ color: point.base ? COLORS.inkSoft : COLORS.orangeDark, fontSize: 9, fontWeight: '800' }}>
          {point.base ? 'Point historique' : 'Point ajouté'}
        </Text>
      </View>
      {!point.base ? (
        <TouchableOpacity
...
export function PreAllumageHeatCurve({
  visiteId,
  sectionCode,
  fields = [],
  champsMap = {},
  ...
  useEffect(() => {
    recharger().catch((e) => console.warn('Courbe de chauffe', e));
  }, [visiteId, sectionCode, fields.length]);
  useEffect(() => {
    setPoints((current) => {
  ...
  const outdoorFromX = (x) =>
    arrondiDemi(X_MIN + ((clamp(x, LEFT, LEFT + plotWidth) - LEFT) / plotWidth) * (X_MAX - X_MIN));
  const waterFromY = (y) =>
    arrondiDemi(Y_MIN + (1 - (clamp(y, TOP, TOP + plotHeight) - TOP) / plotHeight) * (Y_MAX - Y_MIN));
  ...
  const updateFromTouch = (x, y) => {
    ...
    const next = pointsRef.current
      .map((p) => (p.id === id ? { ...p, outdoor: outdoorFromX(x), water: waterFromY(y) } : p))
  ...
  const persistSelected = async () => {
    ...
    const saved = await mettreAJourPointCourbePreAllumage(visiteId, sectionCode, id, {
    ...
    () =>
      PanResponder.create({
        ...
        onStartShouldSetPanResponderCapture: (evt) =>
          Boolean(nearestPoint(evt.nativeEvent.locationX, evt.nativeEvent.locationY)),
        onMoveShouldSetPanResponderCapture: () => Boolean(selectedIdRef.current),
        onStartShouldSetPanResponder: (evt) =>
          Boolean(nearestPoint(evt.nativeEvent.locationX, evt.nativeEvent.locationY)),
        onMoveShouldSetPanResponder: (evt) =>
          Boolean(selectedIdRef.current || nearestPoint(evt.nativeEvent.locationX, evt.nativeEvent.locationY)),
  ...
  const sauvegarderPoint = async (point, patch) => {
    try {
      ...
      Alert.alert('Courbe de chauffe', e.message);
    }
  };
  const ajouter = async () => {
    try {
  ...
  const supprimer = (point) =>
    Alert.alert('Supprimer ce point ?', `${formatNombre(point.outdoor)} °C extérieur`, [
      { text: 'Annuler', style: 'cancel' },
      {
        ...
        onPress: async () => {
          try {
            await supprimerPointCourbePreAllumage(visiteId, sectionCode, point.id);
            await recharger();
            onStructureChanged?.();
          } catch (e) {
            Alert.alert('Courbe de chauffe', e.message);
  ...
  const validPoints = points
    .filter((p) => p.water !== null)
    .map((p) => `${xPx(p.outdoor)},${yPx(p.water)}`)
    .join(' ');
  ...
  return (
    ...
    >
      <View style={{ flexDirection: 'row', alignItems: 'center', gap: 7 }}>
        <View style={{ flex: 1 }}>
          <Text style={{ color: COLORS.ink, fontSize: 12, fontWeight: '900' }}>Courbe de chauffe</Text>
          <Text style={{ color: COLORS.inkSoft, fontSize: 9, marginTop: 2 }}>
            Glissez un point horizontalement (T° extérieure) et verticalement (T° eau).
          </Text>
        </View>
        <TouchableOpacity
          onPress={ajouter}
        ...
        >
          <Text style={{ color: COLORS.white, fontWeight: '900', fontSize: 10 }}>+ Point</Text>
        </TouchableOpacity>
      </View>
      <View
        ...
        {width ? (
          <Svg width={width} height={H} pointerEvents="none">
            <G>
              {Y_TICKS.map((y) => (
                <G key={`y-${y}`}>
                  <Line x1={LEFT} y1={yPx(y)} x2={width - RIGHT} y2={yPx(y)} stroke="#E4E7EC" strokeWidth="1" />
                  <SvgText x={LEFT - 7} y={yPx(y) + 4} textAnchor="end" fontSize="9" fill="#667085">
                    {y}°
                  </SvgText>
                </G>
              ))}
              {X_TICKS.map((x) => (
                <G key={`x-${x}`}>
                  <Line x1={xPx(x)} y1={TOP} x2={xPx(x)} y2={TOP + plotHeight} stroke="#EAECF0" strokeWidth="1" />
                  <SvgText x={xPx(x)} y={TOP + plotHeight + 19} textAnchor="middle" fontSize="9" fill="#667085">
                    {x}
                  </SvgText>
                </G>
              ))}
              ...
              <SvgText x={10} y={TOP + 5} textAnchor="start" fontSize="8" fontWeight="700" fill="#667085">
                Eau °C
              </SvgText>
              {tncVisible ? (
                <G>
                  <Line
                    ...
                    strokeDasharray="5 4"
                  />
                  <SvgText x={xPx(tnc)} y={TOP + 10} textAnchor="middle" fontSize="8" fill="#B54708">
                    TNC {formatNombre(tnc)}°
                  </SvgText>
                </G>
              ) : null}
              {validPoints ? (
                ...
                return (
                  <G key={p.id}>
                    <Circle
                      ...
                      cy={cy}
                      r="10"
                      fill={p.water === null ? '#FFFFFF' : '#F97316'}
                      stroke="#F97316"
                      strokeWidth="3"
                    />
                    <SvgText
                      ...
                      fill="#344054"
                    >
                      {formatNombre(p.outdoor)}° ext.
                    </SvgText>
                    <SvgText
                      x={xPx(p.outdoor)}
                      y={Math.min(TOP + plotHeight - 4, cy + 4)}
                      textAnchor="middle"
                      fontSize="8"
                      fontWeight="800"
                      fill={p.water === null ? '#667085' : '#FFFFFF'}
                    >
                      {p.water === null ? '—' : formatNombre(p.water)}
                    </SvgText>
                  </G>
                );
              })}
      ...
      <View style={{ marginTop: 4 }}>
        <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 3 }}>
          <Text style={{ flex: 1, color: COLORS.ink, fontSize: 10, fontWeight: '900' }}>Points de la courbe</Text>
          <Text style={{ color: COLORS.inkSoft, fontSize: 8 }}>
        ...
        {points.map((p) => (
          <PointEditor
            key={p.id}
            point={p}
            onSave={(patch) => sauvegarderPoint(p, patch)}
            onDelete={() => supprimer(p)}
          />
        ))}
      </View>
      <Text style={{ color: COLORS.inkSoft, fontSize: 9, marginTop: 5 }}>
        Les trois points historiques restent présents. Toute modification ou ajout est repris dans les champs, l’Excel
        et les rapports.
      </Text>
    </View>
  );
