...
const BASE = Object.freeze({
  diagnostic: {
    ...
    objective:
      'Observer l’existant, vérifier les documents, mesurer ce qui est utile et tracer les écarts sans ressaisie.',
    ...
    quickActions: [
      ...
      action('difference', 'Écart constaté', 'point', {
        preset: point('Écart document ↔ terrain', 'control', { priority: 'À contrôler' })
      }),
  ...
  study: {
    ...
    objective:
      'Partir de l’existant réel pour construire des scénarios, dimensionnements, contraintes et livrables sans recréer les données.',
    steps: ['Existant', 'Contraintes', 'Mesures', 'Scénarios', 'Solution'],
    quickActions: [
      action('equipment', 'Existant', 'navigate', { route: 'MissionEquipment' }),
      action('constraints', 'Contrainte', 'point', {
        preset: point('Contrainte de projet / remplacement', 'information')
      }),
      action('measure', 'Mesure', 'measure'),
  ...
  works: {
    ...
    objective:
      'Tracer ce qui est réellement constaté sur chantier, qui doit agir et ce qui devra être recontrôlé à la visite suivante.',
    steps: ['Avancement', 'Constat', 'Réserve', 'Responsable', 'Suivi'],
    quickActions: [
      action('reserve', 'Réserve', 'point', {
        preset: point('Réserve chantier', 'reserve', { priority: 'À traiter' })
      }),
  ...
  reception: {
    ...
    objective:
      'Comparer le prévu et le réalisé, créer les réserves en quelques gestes et préparer immédiatement OPR, réception ou levée.',
    steps: ['Ouvrage', 'Contrôle', 'Réserve', 'Preuve', 'Statut'],
    quickActions: [
      action('reserve', '＋ Réserve', 'point', {
        preset: point('Réserve OPR / réception', 'reserve', { priority: 'À lever' })
      }),
  ...
  passation: {
    ...
    objective:
      'Comparer l’inventaire attendu au terrain, conserver les différences et produire une passation contradictoire sans ressaisie.',
    steps: ['Attendu', 'Trouvé', 'Différence', 'Photo', 'Validation'],
    quickActions: [
      action('equipment', 'Inventaire', 'navigate', { route: 'MissionEquipment' }),
      action('difference', 'Différence', 'point', {
        preset: point('Différence inventaire / terrain', 'control', { priority: 'À confirmer' })
      }),
      action('photo', 'Photo', 'photo'),
  ...
  campaign: {
    ...
    objective:
      'Préparer en masse, saisir très vite site par site et consolider automatiquement la progression et les écarts.',
  ...
  measurementCampaign: {
    ...
    objective:
      'Préparer les points puis utiliser une saisie terrain valeur → suivant avec statuts d’accès et comparaison avant / après.',
    ...
    quickActions: [
      ...
      action('photo', 'Photo', 'photo'),
      action('point', 'Point à traiter', 'point', {
        preset: point('Valeur / situation à contrôler', 'control', { priority: 'À contrôler' })
      }),
  ...
  control: {
    ...
    objective:
      'Contrôler un point précis, rattacher la référence et la preuve, créer l’action utile et prévoir le recontrôle sans transformer METRA en organisme certificateur.',
    ...
    quickActions: [
      ...
      action('control', 'Écart rapide', 'point', {
        preset: point('Écart / contrôle à traiter', 'control', { priority: 'À traiter' })
      }),
...
const TYPES = Object.freeze({
  ...
  audit_technique: {
    ...
    measures: [
      measure('Température départ', '°C'),
      measure('Température retour', '°C'),
      measure('Pression', 'bar'),
      measure('Débit', 'm³/h'),
      measure('Puissance', 'kW')
    ],
    quickActions: [
      action('technical', 'Architecture', 'navigate', { route: 'MissionTechnicalStructure' }),
      action('calculation', 'Calculs 🧮', 'navigate', { route: 'MissionCalculation' })
  ...
  diagnostic_climatisation_pac: {
    ...
    measures: [
      measure('Température soufflage', '°C'),
      measure('Température reprise', '°C'),
      measure('Intensité', 'A'),
      measure('Puissance électrique', 'kW'),
      measure('Fréquence variateur', 'Hz'),
      measure('Niveau sonore', 'dB(A)')
    ],
    quickActions: [
  ...
  diagnostic_gtb: {
    ...
    measures: [
      measure('Consigne', '°C'),
      measure('Valeur sonde', '°C'),
      measure('Position vanne', '%'),
      measure('Fréquence variateur', 'Hz')
    ],
    quickActions: [
  ...
  diagnostic_cible: {
    ...
    quickActions: [
      ...
      action('hypothesis', 'Hypothèse à tester', 'point', {
        preset: point('Hypothèse à tester', 'control', { priority: 'À vérifier' })
      }),
      action('tests', 'Essais', 'navigate', { route: 'MissionTests' })
    ]
  },
  ...
  etude_cvc: {
    base: 'study',
    label: 'Étude CVC / thermique',
    measures: [
      measure('Température', '°C'),
      measure('Débit', 'm³/h'),
      measure('Pression', 'bar'),
      measure('Puissance', 'kW')
    ],
  ...
  etude_renovation: {
    base: 'study',
    label: 'Étude rénovation / remplacement',
    quickActions: [
      action('constraint', 'Contrainte remplacement', 'point', {
        preset: point('Contrainte de remplacement', 'information')
      }),
  ...
  det_chantier: {
    ...
    quickActions: [
      ...
      action('reserve', 'Réserve / observation', 'point', {
        preset: point('Observation / réserve chantier', 'reserve', { priority: 'À traiter' })
      }),
  ...
  commissioning: {
    ...
    objective:
      'Tracer les essais, réglages finaux, écarts et valeurs obtenues sans ressaisir l’inventaire ni les caractéristiques déjà connues.',
  ...
  opr_reception: {
    ...
    label: 'OPR / réception',
    objective:
      'Comparer rapidement le prévu, l’installé et le fonctionnement constaté puis créer les réserves au fil du contrôle.',
    ...
    quickActions: [
      ...
      action('reserve', '＋ Réserve OPR', 'point', {
        preset: point('Réserve OPR / réception', 'reserve', { priority: 'À lever' })
      }),
      action('actions', 'Réserves / actions', 'navigate', { route: 'MissionActions' }),
  ...
  levee_reserves: {
    ...
    objective:
      'Reprendre les réserves existantes, constater seulement leur évolution et produire la preuve avant / après.',
    ...
    quickActions: [
      ...
      action('point', 'Nouvelle réserve', 'point', {
        preset: point('Nouvelle réserve constatée au recontrôle', 'reserve', { priority: 'À traiter' })
      })
    ]
  },
  passation_travaux_exploitant: {
    ...
    objective:
      'Passer de l’ouvrage réceptionné à une prise en main exploitable : inventaire, démonstration, réglages, accès, documents et éléments restant à remettre.',
  ...
  assistance_p2_p3: {
    ...
    objective:
      'Croiser inventaire, état, historique, obligations et renouvellements pour préparer une lecture contractuelle et patrimoniale exploitable.',
    ...
    quickActions: [
      ...
      action('sensitive', 'Équipement sensible', 'point', {
        preset: point('Équipement sensible / renouvellement à anticiper', 'control', { priority: 'À analyser' })
      }),
  ...
  preallumage_reprise_saison: {
    ...
    objective:
      'Vérifier ponctuellement la disponibilité de l’installation pour la reprise de saison sans réutiliser la trame de Visite technique récurrente.',
    ...
    quickActions: [
      ...
      action('point', 'Réserve reprise', 'point', {
        preset: point('Point à traiter avant / pendant reprise de saison', 'control', { priority: 'À traiter' })
      })
    ]
  },
  expertise_sinistre: {
    ...
    objective:
      'Séparer strictement les faits, preuves, hypothèses, investigations et conclusions pour conserver une chronologie exploitable et opposable.',
