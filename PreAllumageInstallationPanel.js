...
function titreRubrique(rubrique, local) {
  ...
  if (rubrique.panel_id === 'p-pa-compteurs')
    return /g[ée]n[ée]raux/i.test(rubrique.nom) ? 'Compteurs généraux' : 'Compteurs';
  if (rubrique.panel_id === 'p-pa-regulation') return 'Régulation & températures';
  if (rubrique.panel_id === 'p-pa-chaufferie')
    return String(rubrique.nom || '').replace(/^Chaufferie\s*[—-]?\s*/i, '') || 'Essais chaufferie';
...
function estEcs(texte) {
  return /\bECS\b|primaire ECS|traitement d.?eau|bouclage/i.test(String(texte || ''));
}
function estChauffage(texte) {
...
function analyserCoherence(rubriques, champsMap) {
  ...
  const verifierPair = (titre, depRx, retRx) => {
    const dep = trouver(depRx);
    const ret = trouver(retRx);
    const a = nombre(dep?.valeur);
    const b = nombre(ret?.valeur);
  ...
  values.forEach((v) => {
    ...
    if (n !== null && (n < -30 || n > 120))
      warnings.push(`${String(v.label).split('(')[0].trim()} : ${n} °C paraît inhabituel.`);
  });
...
function rubriqueCorrespondRecherche(r, q) {
  ...
  const texte = normaliserRecherche(
    `${r.nom} ${(r.champs || []).map((c) => `${c.libelle} ${c.field?.cle}`).join(' ')}`
  );
...
function Toggle({ label, value, onPress }) {
  return (
    ...
    >
      <Text style={{ textAlign: 'center', color: value ? COLORS.orangeDark : COLORS.inkSoft, fontWeight: '800' }}>
        {value ? '✓ ' : ''}
        {label}
      </Text>
    </TouchableOpacity>
      ...
      <View style={styles.modalOverlay}>
        <View style={styles.modalSheet}>
          ...
          <Text style={[styles.importHint, { marginBottom: 12 }]}>
            Ajoute le local dans l’ordre réel de la visite. Les blocs non applicables seront masqués.
          </Text>
          <TextInput
            style={styles.input}
            value={nom}
            onChangeText={setNom}
            placeholder="Ex. SST 11 — Entrée 82"
            autoFocus
          />
          ...
          <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 7 }}>
            {PREALLUMAGE_TYPES_LOCAUX.map((t) => {
              ...
              return (
                ...
                >
                  <Text style={{ color: selected ? COLORS.white : COLORS.inkSoft, fontWeight: '800', fontSize: 12 }}>
                    {t.label}
                  </Text>
                </TouchableOpacity>
              );
          ...
          <View style={styles.modalActions}>
            ...
            <TouchableOpacity style={styles.btnPrimary} onPress={() => onSubmit({ nom, typeCode, chauffage, ecs })}>
    ...
    <Modal visible={visible} transparent animationType="fade" onRequestClose={onClose}>
      <View style={styles.modalOverlay}>
        <View style={styles.modalSheet}>
          ...
          <Text style={[styles.importHint, { marginBottom: 12 }]}>
            Les rubriques qui ne s’appliquent pas disparaissent immédiatement, sans supprimer les anciennes saisies.
          </Text>
          ...
          <View style={styles.modalActions}>
            ...
            <TouchableOpacity style={styles.btnPrimary} onPress={() => onSave({ chauffage, ecs })}>
...
function ActionsModal({ visible, local, peutMonter, peutDescendre, onClose, onAction }) {
  ...
  const items = [
    ['rename', 'Renommer'],
    ['config', 'Configurer Chauffage / ECS'],
      ...
      <View style={styles.modalOverlay}>
        <View style={styles.modalSheet}>
          ...
          {items.map(([id, label]) => (
            ...
            >
              <Text style={{ fontSize: 13, fontWeight: '800', color: id === 'delete' ? COLORS.red : COLORS.ink }}>
                {label}
              </Text>
            </TouchableOpacity>
          ))}
          <TouchableOpacity style={[styles.btnSecondary, { marginTop: 14 }]} onPress={onClose}>
...
export function PreAllumageInstallationPanel({ visiteId, onSaved }) {
  ...
  useEffect(() => {
    recharger().catch((e) => Alert.alert('Pré-allumage', e.message));
  }, [recharger]);
  useEffect(() => {
    activeLocalRef.current = activeLocalId;
  }, [activeLocalId]);

  const rubriquesOrphelines = useMemo(() => (modele?.rubriques || []).filter((r) => !r.local_id), [modele]);
  const chaufferiePhysique = useMemo(
    () => (modele?.locaux || []).find((l) => l.type_code === 'chaufferie') || null,
    [modele]
  );
  const rubriquesChaufferieOrphelines = useMemo(
    () => rubriquesOrphelines.filter((r) => r.panel_id === 'p-pa-chaufferie'),
    [rubriquesOrphelines]
  );
  const rubriquesSite = useMemo(
    () => rubriquesOrphelines.filter((r) => r.panel_id === 'p-pa-compteurs' && /g[ée]n[ée]raux/i.test(r.nom)),
    [rubriquesOrphelines]
  );
  const rubriquesOrphelinesPhysiques = useMemo(
    () =>
      rubriquesOrphelines.filter(
        (r) => ['p-pa-compteurs', 'p-pa-regulation', 'p-pa-sst'].includes(r.panel_id) && !/g[ée]n[ée]raux/i.test(r.nom)
      ),
    [rubriquesOrphelines]
  );
  ...
  const locaux = useMemo(() => {
    ...
    const result = [];
    if (rubriquesSite.length)
      result.push({ id: VIRTUAL_SITE, nom: 'Site / général', type_code: 'site', virtual: true, ordre: -200 });
    if (rubriquesChaufferieOrphelines.length && !chaufferiePhysique)
      result.push({ id: VIRTUAL_CHAUFFERIE, nom: 'Chaufferie', type_code: 'chaufferie', virtual: true, ordre: -100 });
    ...
    [...new Set(rubriquesOrphelinesPhysiques.map((r) => baseNomRubrique(r.nom)).filter(Boolean))].forEach((nom) => {
      if (!existants.has(nom.toLowerCase()))
        result.push({ id: `${VIRTUAL_PREFIX}${nom}`, nom, type_code: 'autre', virtual: true, ordre: 10000 });
    });
    return result;
  ...
  const localActif = useMemo(
    () => locaux.find((l) => l.id === activeLocalId) || locaux[0] || null,
    [locaux, activeLocalId]
  );
  useEffect(() => {
    setNomLocal(localActif?.nom || '');
    setEditionNom(false);
  }, [localActif?.id, localActif?.nom]);

  const rubriquesPourLocal = useCallback(
    (local) => {
      if (!local) return [];
      let base;
      if (local.id === VIRTUAL_SITE) base = rubriquesSite;
        ...
        if (local.type_code === 'chaufferie' && chaufferiePhysique?.id === local.id)
          base = [...base, ...rubriquesChaufferieOrphelines];
      }
      return (base || [])
        .map((r) => filtrerRubriqueSelonConfig(r, local))
        .filter((r) => r && (r.champs || []).length > 0);
    },
  ...
  const rubriquesActives = useMemo(
    () =>
      [...rubriquesPourLocal(localActif)].sort((a, b) => {
  ...
  const sections = useMemo(
    () =>
      rubriquesActives.map((r) => {
        ...
        return {
          ...r,
          title: titreRubrique(r, localActif),
          sectionCode: r.section_code,
          rawData: raw,
          data: replie ? [] : grouperItems(raw, deuxColonnes),
          replie
        };
      }),
  ...
  const stats = useMemo(
    () => statsRubriques(rubriquesActives, champsMap, controlesMap),
    [rubriquesActives, champsMap, controlesMap]
  );
  const warnings = useMemo(() => analyserCoherence(rubriquesActives, champsMap), [rubriquesActives, champsMap]);
  const idx = Math.max(
    0,
    locaux.findIndex((l) => l.id === localActif?.id)
  );
  ...
  const changerLocal = useCallback(
    (id, depuisRecherche = false) => {
  ...
  const soumettreAjout = async ({ nom, typeCode, chauffage, ecs }) => {
    try {
      ...
      Alert.alert('Impossible d’ajouter', e.message);
    }
  };
  const sauverNom = async () => {
    if (!localActif || localActif.virtual) {
      setEditionNom(false);
      return;
    }
    const propre = String(nomLocal || '').trim();
    if (!propre || propre === localActif.nom) {
      setNomLocal(localActif.nom);
      setEditionNom(false);
      return;
    }
  ...
  const supprimer = () => {
    ...
    Alert.alert(
      ...
      `Les saisies liées à « ${localActif.nom} » seront supprimées de cette visite.`,
      [
        { text: 'Annuler', style: 'cancel' },
        {
          ...
          style: 'destructive',
          onPress: async () => {
            const selection = suivant?.id || precedent?.id || null;
            await supprimerLocalPreAllumage(localActif.id);
            await recharger(selection);
            onSaved?.();
          }
        }
      ]
  ...
  const actionLocal = async (action) => {
    ...
    if (action === 'duplicate') {
      try {
        const r = await dupliquerLocalPreAllumage(localActif.id);
        await recharger(r.id);
        onSaved?.();
        Alert.alert('Installation dupliquée', `${r.nom} a été créée sans les relevés ni les avis du jour.`);
      } catch (e) {
    ...
    if (action === 'copy') {
      Alert.alert(
        'Copier les réglages ?',
        'Les courbes de chauffe, TNC, réduit de jour et horaires de cette installation seront appliqués aux autres installations disposant d’une régulation.',
        [
          { text: 'Annuler', style: 'cancel' },
          {
            text: 'Copier',
            onPress: async () => {
              try {
                const n = await copierReglagesPreAllumage(localActif.id);
                await recharger(localActif.id);
                onSaved?.();
                Alert.alert('Réglages copiés', `${n} installation(s) mise(s) à jour.`);
              } catch (e) {
                Alert.alert('Copie impossible', e.message);
              }
            }
          }
        ]
    ...
    if (action === 'up' || action === 'down') {
      await deplacerLocalPreAllumage(localActif.id, action === 'up' ? -1 : 1);
      await recharger(localActif.id);
      onSaved?.();
    }
  };
  const sauverConfig = async ({ chauffage, ecs }) => {
    try {
      await mettreAJourConfigurationLocalPreAllumage(localActif.id, { chauffage, ecs });
  ...
  const allerProchainIncomplet = () => {
    const sectionIndex = rubriquesActives.findIndex((r) => {
      const s = statsRubriques([r], champsMap, controlesMap);
      return s.faits < s.total;
    });
  ...
  const header = (
    <View>
      ...
      >
        <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
          <View style={{ flex: 1 }}>
            ...
            <Text style={{ color: COLORS.inkSoft, fontSize: 12, marginTop: 2 }}>
              Une installation = toutes les informations, relevés et conformités au même endroit.
            </Text>
          </View>
        ...
        >
          ...
          <TextInput
            ...
            placeholder="Rechercher SST 7, ECS, pompe, compteur…"
            style={{ flex: 1, minHeight: 40, fontSize: 12, color: COLORS.ink }}
          />
          {recherche ? (
            <TouchableOpacity onPress={() => setRecherche('')}>
              <Text style={{ color: COLORS.inkSoft, fontWeight: '900' }}>✕</Text>
            </TouchableOpacity>
          ) : null}
        </View>
        {q && locauxAffiches.length === 0 ? (
          <Text style={{ color: COLORS.inkSoft, fontSize: 11, marginTop: 8 }}>
            Aucune installation ne correspond à « {recherche} ».
          </Text>
        ) : null}
        <ScrollView
          horizontal
          ...
          contentContainerStyle={{ gap: 7, paddingTop: 10, paddingRight: 8 }}
        >
          {locauxAffiches.map((local) => {
            ...
            return (
              <TouchableOpacity
                key={local.id}
                onPress={() => changerLocal(local.id, Boolean(q))}
                style={{
                  minHeight: 45,
                  justifyContent: 'center',
                  paddingHorizontal: 12,
                  borderRadius: 11,
                  borderWidth: 1,
                  borderColor: selected ? COLORS.orange : alert ? COLORS.red : COLORS.line,
                  backgroundColor: selected ? COLORS.orangeLight : COLORS.white
                }}
              >
                <Text style={{ color: selected ? COLORS.orangeDark : COLORS.ink, fontWeight: '800', fontSize: 12 }}>
                  {alert ? '⚠ ' : s.pct === 100 && s.total ? '✓ ' : ''}
                  {local.nom}
                </Text>
                <Text style={{ color: alert ? COLORS.red : COLORS.inkSoft, fontSize: 10, marginTop: 1 }}>
                  {s.pct}%{s.aCompleter ? ` · ${s.aCompleter} à compléter` : ''}
                  {alert ? ` · ${s.ns} N.S` : ''}
                </Text>
              </TouchableOpacity>
            );
          ...
          <View style={{ flexDirection: 'row', alignItems: 'center', gap: 9 }}>
            <View style={{ flex: 1 }}>
              {editionNom ? (
                <TextInput
                  value={nomLocal}
                  onChangeText={setNomLocal}
                  onBlur={sauverNom}
                  onSubmitEditing={sauverNom}
                  autoFocus
                  style={{
                    fontSize: 18,
                    fontWeight: '900',
                ...
                <Text style={{ fontSize: 18, fontWeight: '900', color: COLORS.ink }}>{localActif.nom}</Text>
              )}
              <Text style={{ color: COLORS.inkSoft, marginTop: 3, fontSize: 12 }}>
                {typeLabel(localActif)} · {stats.faits}/{stats.total} renseignés · {stats.pct}%
                {stats.ns ? ` · ${stats.ns} N.S` : ''}
                {stats.aCompleter ? ` · ${stats.aCompleter} à compléter` : ''}
              </Text>
            </View>
            <PhotoButton
              visiteId={visiteId}
              entiteKey={`preallumage_local||${localActif.id}`}
              label={localActif.nom}
              style={{ minHeight: 38, paddingHorizontal: 9 }}
            />
            {!localActif.virtual ? <PetitBouton label="⋯" onPress={() => setActionsVisible(true)} /> : null}
          </View>
          <View style={{ height: 5, backgroundColor: COLORS.line, borderRadius: 3, marginTop: 11, overflow: 'hidden' }}>
            <View
              style={{
                height: 5,
                width: `${stats.pct}%`,
                backgroundColor: stats.ns ? COLORS.red : COLORS.orange,
                borderRadius: 3
              }}
            />
          </View>
          {stats.manquants || stats.aCompleter ? (
            <TouchableOpacity onPress={allerProchainIncomplet} style={{ alignSelf: 'flex-start', marginTop: 9 }}>
      ...
      {warnings.length ? (
        ...
        >
          <Text style={{ color: '#93370D', fontWeight: '900', fontSize: 11 }}>
            ⚠ Valeurs à vérifier — la saisie reste autorisée
          </Text>
          {warnings.map((w, i) => (
            <Text key={`${w}-${i}`} style={{ color: '#93370D', fontSize: 10, marginTop: 3 }}>
              • {w}
            </Text>
          ))}
        </View>
  ...
  const footer = (
    <View style={{ paddingTop: 10, paddingBottom: 24 }}>
      ...
      >
        <Text style={{ color: COLORS.ink, fontWeight: '900', fontSize: 12 }}>Synthèse {localActif?.nom}</Text>
        <Text style={{ color: COLORS.inkSoft, fontSize: 11, marginTop: 4 }}>
          {stats.faits} renseignés · {stats.ns} N.S · {stats.aCompleter} à compléter · {stats.manquants} non renseignés
        </Text>
      </View>
      <View style={{ flexDirection: 'row', gap: 9 }}>
        <View style={{ flex: 1 }}>
          {precedent ? <PetitBouton label={`‹ ${precedent.nom}`} onPress={() => changerLocal(precedent.id)} /> : null}
        </View>
        <View style={{ flex: 1, alignItems: 'flex-end' }}>
          {suivant ? (
            <PetitBouton primary label={`${suivant.nom} ›`} onPress={() => changerLocal(suivant.id)} />
          ) : (
            <PetitBouton primary label="+ Installation" onPress={() => setAjoutVisible(true)} />
          )}
        </View>
      </View>
    </View>
    ...
    <>
      <SectionList
        ...
        renderSectionHeader={({ section }) => {
          ...
          return (
            <TouchableOpacity
              onPress={() => setReplies((m) => ({ ...m, [section.id]: !section.replie }))}
              style={{
                marginTop: 5,
            ...
            >
              <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
                ...
                <Text
                  style={{
                    color: s.ns ? COLORS.red : s.aCompleter ? '#B54708' : COLORS.inkSoft,
                    fontWeight: '800',
                    fontSize: 10
                  }}
                >
                  {s.faits}/{s.total}
                  {s.aCompleter ? ` · ${s.aCompleter} à compléter` : ''}
                  {s.ns ? ` · ⚠ ${s.ns}` : ''}
                </Text>
        ...
        renderItem={({ item, section, index }) => (
          <View
            style={{
              ...
              paddingVertical: 4,
              marginTop: -1,
              borderBottomLeftRadius: index === section.data.length - 1 ? 11 : 0,
              borderBottomRightRadius: index === section.data.length - 1 ? 11 : 0
            }}
          >
            {item.kind === 'fields' ? (
              <View style={{ flexDirection: deuxColonnes ? 'row' : 'column', gap: deuxColonnes ? 12 : 0 }}>
                {item.items.map((champ, champIndex) => (
                  <View
                    key={champ.key}
                    style={{
                      flex: 1,
                      minWidth: 0,
                      paddingLeft: deuxColonnes && champIndex > 0 ? 10 : 0,
                      borderLeftWidth: deuxColonnes && champIndex > 0 ? 1 : 0,
                  ...
                  >
                    <PreAllumageCompactField
                      ...
                      field={champ.field}
                      valeurInitiale={champsMap[champ.key]}
                      localName={localActif?.nom}
                      showPhoto={section.panel_id === 'p-pa-compteurs'}
                      onSaved={(valeur) => {
                        setChampsMap((m) => ({ ...m, [champ.key]: valeur }));
                        onSaved?.();
                      }}
                    />
                  </View>
              ...
              <PreAllumageCompactControl
                ...
                etatInitial={controlesMap[item.item.key]}
                localName={localActif?.nom}
                onEtatChange={(patch) =>
                  setControlesMap((m) => ({ ...m, [item.item.key]: { ...(m[item.item.key] || {}), ...patch } }))
                }
                onSaved={onSaved}
              />
            )}
        ...
        ListEmptyComponent={
          ...
          >
            <Text style={{ color: COLORS.inkSoft, textAlign: 'center' }}>
              Aucune donnée applicable à cette installation.
            </Text>
          </View>
        }
      ...
      />
      <AjouterInstallationModal
        visible={ajoutVisible}
        onClose={() => setAjoutVisible(false)}
        onSubmit={soumettreAjout}
      />
      <ConfigurationModal
      ...
      <ActionsModal
        ...
        local={localActif}
        peutMonter={idxPhysique > 0}
        peutDescendre={idxPhysique >= 0 && idxPhysique < physiques.length - 1}
        onClose={() => setActionsVisible(false)}
        onAction={(id) => actionLocal(id).catch((e) => Alert.alert('Action impossible', e.message))}
      />
    </>
  );
}
