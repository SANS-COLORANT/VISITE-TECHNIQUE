...
const EquipmentCard=memo(function EquipmentCard({item,visiteId,onChange,types,marques,catalogue}){
 ...
 const choisirType=async v=>{const t=String(v||'').trim();setCategorie(t);await upsertMaterielChamp(item.id,'categorie',t);await setDesignationNow(t||'Équipement');const refsNouveauType=catalogue.filter(e=>!t||eq(e.categorie,t));const compat=!marque||refsNouveauType.some(e=>eq(e.marque,marque));if(!compat){setMarque('');await upsertMaterielChamp(item.id,'marque','');await setModeleNow('')}else if(modele&&!refsNouveauType.some(e=>eq(e.marque,marque)&&eq(e.modele,modele)))await setModeleNow('')};
