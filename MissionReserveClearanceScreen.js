...
function StatusChip({ label, selected, onPress }) {
  return (
    ...
    >
      <Text
        style={{ color: selected ? MISSION_COLORS.accentStrong : COLORS.inkSoft, fontSize: 8.7, fontWeight: '900' }}
      >
        {label}
      </Text>
    </TouchableOpacity>
...
export function MissionReserveClearanceScreen({ route }) {
  ...
  const initialPhotoFor = useCallback(
    (reserve) => {
    ...
    <View style={{ flex: 1, backgroundColor: MISSION_COLORS.bg }}>
      <ScrollView contentContainerStyle={{ padding: 16, paddingBottom: 120 }}>
        ...
        <Text style={{ color: COLORS.inkSoft, fontSize: 10.5, lineHeight: 15 }}>
          Retrouve la réserve initiale, ajoute la preuve après intervention et tranche en un geste : levée, maintenue,
          partielle, inaccessible ou non vérifiable.
        </Text>
        ...
        <View style={{ flexDirection: 'row', gap: 7, marginTop: 12 }}>
          {[
            ...
            <View key={label} style={[missionStyles.statBox, { flex: 1, borderRadius: 11, padding: 9 }]}>
        ...
        {visible.map((reserve, index) => {
          ...
          return (
            <View key={reserve.id} style={[missionStyles.card, { padding: 12, marginTop: 9 }]}>
              <View style={{ flexDirection: 'row', alignItems: 'flex-start' }}>
                <View style={{ flex: 1 }}>
                  <Text style={{ color: COLORS.inkFaint, fontSize: 8.2, fontWeight: '900' }}>
                    RÉSERVE {String(index + 1).padStart(2, '0')}
                  </Text>
                  <Text style={{ color: COLORS.ink, fontSize: 11.5, fontWeight: '900', marginTop: 3 }}>
                    {reserve.label || 'Réserve'}
                  </Text>
                  <Text style={{ color: COLORS.inkFaint, fontSize: 8.6, marginTop: 3 }}>
                    {[reserve.site_name, reserve.location_label, equipmentLabel(reserve)].filter(Boolean).join(' · ') ||
                ...
                <View
                  style={{
                    borderRadius: 9,
                    backgroundColor: reserve.status === 'closed' ? MISSION_COLORS.accentLight : '#FFFFFF',
                    borderWidth: 1,
                ...
                >
                  <Text
                    style={{
                      color: reserve.status === 'closed' ? MISSION_COLORS.accentStrong : COLORS.inkSoft,
                      fontSize: 8.2,
                      fontWeight: '900'
                    }}
                  >
                    {QUALIFICATION_LABEL[reserve.qualification] ||
                      (reserve.status === 'closed' ? 'Levée' : 'À recontrôler')}
                  </Text>
                </View>
              </View>

              {reserve.requested_action ||
              reserve.responsible_company ||
              reserve.responsible_name ||
              reserve.due_date ||
              reserve.due_text ? (
                <View style={{ marginTop: 8, borderRadius: 9, backgroundColor: MISSION_COLORS.accentSoft, padding: 8 }}>
                  {reserve.requested_action ? (
                    <Text style={{ color: COLORS.ink, fontSize: 8.8 }}>Action : {reserve.requested_action}</Text>
                  ) : null}
                  <Text style={{ color: COLORS.inkFaint, fontSize: 8.3, marginTop: 2 }}>
                    {[reserve.responsible_company || reserve.responsible_name, reserve.due_date || reserve.due_text]
                      .filter(Boolean)
                      .join(' · ')}
                  </Text>
                </View>
              ...
              <View style={{ flexDirection: 'row', gap: 8, marginTop: 9 }}>
                <View style={{ flex: 1 }}>
                  ...
                  {before ? (
                    <Image
                      source={{ uri: photoUri(before) }}
                      style={{ width: '100%', height: 105, borderRadius: 9 }}
                      resizeMode="cover"
                    />
                  ) : (
                    <View
                    ...
                    >
                      <Text style={{ color: COLORS.inkFaint, fontSize: 8.5 }}>Pas de photo initiale</Text>
                    </View>
                  )}
                  <TouchableOpacity
                    disabled={busyId === reserve.id}
                    onPress={() => capture(reserve, 'before')}
                    style={{ paddingVertical: 7, alignItems: 'center' }}
                  >
                    <Text style={{ color: MISSION_COLORS.accentDark, fontSize: 8.5, fontWeight: '900' }}>
                      📷 {before ? 'Remplacer / ajouter' : 'Photo initiale'}
                    </Text>
                  </TouchableOpacity>
                </View>
                <View style={{ flex: 1 }}>
                  ...
                  {after ? (
                    ...
                    >
                      <Text style={{ color: COLORS.inkFaint, fontSize: 8.5 }}>À photographier si utile</Text>
                    </View>
                  )}
                  <TouchableOpacity
                    disabled={busyId === reserve.id}
                    onPress={() => capture(reserve, 'after')}
                    style={{ paddingVertical: 7, alignItems: 'center' }}
                  >
                    <Text style={{ color: MISSION_COLORS.accentDark, fontSize: 8.5, fontWeight: '900' }}>
                      📷 Photo après
              ...
              <Text style={{ color: COLORS.inkFaint, fontSize: 8.2, fontWeight: '900', marginTop: 7, marginBottom: 5 }}>
                RÉSULTAT DU RECONTRÔLE
              </Text>
              <View style={{ flexDirection: 'row', flexWrap: 'wrap' }}>
                {CLEARANCE.map(([key, label, status]) => (
                  <StatusChip
                    key={key}
                    label={label}
        ...
        {!visible.length ? (
          <View style={[missionStyles.card, { padding: 14, marginTop: 12 }]}>
            <Text style={{ color: COLORS.inkSoft, fontSize: 9.7 }}>
              {filter === 'open'
                ? 'Aucune réserve restant à recontrôler dans cette Mission.'
                : 'Aucune réserve dans cette sélection.'}
            </Text>
          </View>
        ) : null}
      </ScrollView>
