...
import {
  getVisitPhotoUploadSummary,
  processVisitPhotoOutbox,
  queueMissingSyncedVisitPhotos,
  retryVisitPhotoUploadsNow,
  subscribeVisitPhotoOutbox,
  syncVisitPhotosNow
} from './intranetVisitPhotoOutboxDb.js';
import {
  bindVisitToImportedClientTarget,
  getVisitIntranetBindingOptions,
  resolveFirstVisitRemoteTrame
} from './intranetVisitBindingDb.js';
...
const EMPTY_PHOTO_SUMMARY = Object.freeze({
  total: 0,
  queued: 0,
  unscheduled: 0,
  pending: 0,
...
function serverFeedback(row) {
  ...
  if (http === 404)
    return `Intranet HTTP 404 · client ou ressource introuvable/non autorisée${fallback ? ` · ${fallback}` : ''}`;
...
export function IntranetVisitSyncBanner() {
  ...
  useEffect(() => {
    refresh();
    return subscribeVisitOutbox(refresh);
  }, [refresh]);
  ...
  return (
    <View
      ...
      style={{
        ...
        paddingVertical: 7,
        backgroundColor: blocked ? '#FFF1F0' : '#FFF8ED',
        borderBottomWidth: 1,
    ...
    >
      <Text style={{ flex: 1, color: COLORS.ink, fontSize: 12, fontWeight: '800' }}>
        Intranet · {sending ? `${sending} envoi en cours` : `${waiting} en attente`}
        {blocked ? ` · ${blocked} à corriger` : ''}
      </Text>
      {waiting ? (
        <TouchableOpacity
          accessibilityRole="button"
          onPress={() =>
            processVisitOutbox({ limit: 3 })
              .then(() => queueMissingSyncedVisitPhotos({ limitVisits: 40 }))
              .then(() => processVisitPhotoOutbox({ limit: 10 }))
              .catch(() => {})
          }
          style={{ minHeight: 40, justifyContent: 'center', paddingHorizontal: 10 }}
        >
          <Text style={{ color: COLORS.primary, fontWeight: '900', fontSize: 12 }}>Synchroniser</Text>
        </TouchableOpacity>
      ) : null}
    </View>
  );
...
export function useVisitUploadState(visiteId) {
  ...
  const refresh = useCallback(async () => {
    if (!visiteId) return;
    try {
      setRow(await getVisitUploadState(visiteId));
    } finally {
      setLoading(false);
    }
  }, [visiteId]);
...
export function useVisitPhotoUploadSummary(visiteId) {
  ...
  const refresh = useCallback(async () => {
    if (!visiteId) return;
    try {
      setSummary(await getVisitPhotoUploadSummary(visiteId));
    } finally {
      setLoading(false);
    }
  }, [visiteId]);
  useEffect(() => {
    setLoading(true);
    refresh();
    return subscribeVisitPhotoOutbox(refresh);
  }, [refresh]);
  ...
  try {
    ...
    if (!referential) {
      const wrapped = new Error(
        `Impossible de charger le référentiel Intranet nécessaire à cette première visite : ${String(error?.message || error)}`
      );
...
async function bindSameImportedClient(visiteId) {
  try {
    ...
    const refreshable =
      error?.remoteClientId &&
      ['imported_site_missing', 'intranet_reference_refresh_required', 'wrong_imported_site'].includes(error?.code);
    ...
    try {
      ...
      const wrapped = new Error(
        `Impossible d’actualiser le client Intranet avant l’envoi : ${String(refreshError?.message || refreshError)}`
      );
...
async function assertQueuedClientStillMatchesImportedClient(visiteId, row) {
  ...
  if (!expected) {
    const error = new Error(
      'Le client Intranet d’origine de cette visite n’est plus identifiable. Cet ancien envoi reste bloqué pour éviter de l’expédier vers un autre client.'
    );
  ...
  if (String(expected) !== String(row.remote_client_id)) {
    const error = new Error(
      'Cet envoi en attente a été préparé avec un autre client Intranet par une ancienne version de METRA. Il ne sera pas rejoué automatiquement. La visite doit rester liée à son client importé d’origine.'
    );
...
export function IntranetVisitSyncControl({ visite, onVisitChanged = null, compact = false }) {
  ...
  const syncPhotosAfterVisit = async () => {
    ...
    if (result.failed > 0) {
      Alert.alert(
        'Visite envoyée · photos à corriger',
        `${result.synced}/${result.total} photo(s) sont confirmées sur l’Intranet. ${result.failed} photo(s) ont été refusées et restent signalées Offline.`
      );
    }
  };
  ...
  const finishQueueAndSend = async (confirmMaterialReplacement = false) => {
    ...
    if (finalRow && ['validation_error', 'rejected', 'conflict', 'auth_error'].includes(finalRow.status)) {
      Alert.alert(
        'Envoi Intranet non validé',
        serverFeedback(finalRow) || 'L’Intranet a refusé ou interrompu l’envoi. La visite reste Offline.'
      );
      return;
    }
    ...
    if (finalRow?.status === 'synced' && finalPhotos.failed > 0) {
      Alert.alert(
        'Visite envoyée · photos à corriger',
        `${finalPhotos.synced}/${finalPhotos.total} photo(s) sont confirmées sur l’Intranet. ${finalPhotos.failed} photo(s) ont été refusées.`
      );
    }
  };
  ...
  const queueWithMaterialSafety = async () => {
    try {
      ...
      if (
        ['material_replacement_confirmation_required', 'material_clear_confirmation_required'].includes(error?.code)
      ) {
        ...
        Alert.alert(
          ...
          [
            ...
            {
              text: destructiveClear ? 'Confirmer le listing vide' : 'Confirmer le remplacement',
              style: 'destructive',
              onPress: () => {
                setBusy(true);
                finishQueueAndSend(true)
    ...
    try {
      ...
      if (row && ['validation_error', 'rejected', 'conflict'].includes(row.status)) {
        if (row.status === 'conflict' && row.remote_client_id) {
          try {
            await syncClientPreparation(row.remote_client_id);
          } catch (e) {
            throw new Error(
              `Impossible d’actualiser le même client Intranet après le conflit : ${String(e?.message || e)}`
            );
          }
        }
  ...
  const remainingPhotos = Math.max(
    0,
    Number(photoSummary.total || 0) - Number(photoSummary.synced || 0) - Number(photoSummary.failed || 0)
  );
  const photoDetail =
    visitSynced && photoSummary.total > 0
  ...
  const detail = online
    ? historical
      ? 'Déjà présente sur l’Intranet (visite importée).'
      : `Export Intranet confirmé${row?.remote_visit_id ? ` · visite n°${row.remote_visit_id}` : ''}${photoSummary.total ? ` · ${photoSummary.synced} photo(s)` : ''}.`
    : visitSynced
      ? `Visite n°${row?.remote_visit_id || ''} créée sur l’Intranet${photoDetail}. Les lots suivants reprennent automatiquement.`
      : row?.status === 'sending'
        ? 'Envoi vers le même client Intranet en cours…'
        : row?.status === 'pending'
          ? 'Envoi en attente.'
          : row?.status === 'retry'
            ? 'Non exportée · nouvelle tentative dès que la connexion le permet.'
            : row?.status === 'auth_error'
              ? 'Non exportée · connexion Intranet à réactiver.'
              : serverFeedback(row) ||
                'Non exportée sur l’Intranet. Appuie sur Offline pour l’envoyer au client importé.';
  const detailIsError =
    !online &&
    ((row && ['validation_error', 'rejected', 'conflict', 'auth_error'].includes(row.status)) ||
      photoSummary.failed > 0);
  ...
  return (
    ...
    >
      <View style={{ flexDirection: 'row', alignItems: 'center', gap: 9 }}>
        {!compact ? (
          <Text style={{ flex: 1, color: COLORS.ink, fontSize: 12.5, fontWeight: '900' }}>Intranet</Text>
        ) : null}
        {busy || row?.status === 'sending' || photoSummary.sending > 0 ? (
          <ActivityIndicator size="small" color={online ? ONLINE : OFFLINE} />
        ) : null}
        <TouchableOpacity
          accessibilityRole="button"
          accessibilityLabel={
            online
              ? 'Online, visite et photos présentes sur l’Intranet'
              : 'Offline, appuyer pour envoyer ou reprendre la visite et ses photos sur l’Intranet'
          }
          disabled={loading || photoLoading || busy || online || row?.status === 'sending'}
          onPress={(event) => {
            event?.stopPropagation?.();
            prepareAndSend().catch(() => {});
          }}
          style={{
            minWidth: 82,
            minHeight: 36,
            ...
            opacity: loading || photoLoading ? 0.55 : 1
          }}
        >
      ...
      {!compact ? (
        <Text
          accessibilityLiveRegion="polite"
          style={{ color: detailIsError ? ERROR : COLORS.muted, fontSize: 10.5, lineHeight: 15, marginTop: 6 }}
        >
          {loading || photoLoading ? 'Lecture de l’état Intranet…' : detail}
        </Text>
      ) : null}
    </View>
  );
