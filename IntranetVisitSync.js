...
export function useVisitUploadState(visiteId, { initialRow = null, passive = false } = {}) {
  const [row, setRow] = useState(initialRow);
  const [loading, setLoading] = useState(!passive && !initialRow);
  const refresh = useCallback(async () => {
    if (!visiteId) return null;
    try { const next = await getVisitUploadState(visiteId); setRow(next); return next; } finally { setLoading(false); }
  }, [visiteId]);
  useEffect(() => {
    if (passive) { setRow(initialRow); setLoading(false); return undefined; }
    setLoading(true); refresh(); return subscribeVisitOutbox(refresh);
  }, [refresh, passive, initialRow?.status, initialRow?.remote_visit_id, initialRow?.error_code, initialRow?.error_message]);
...
export function IntranetVisitSyncControl({ visite, onVisitChanged = null, compact = false, passive = false }) {
  ...
  const { row, loading, refresh } = useVisitUploadState(visiteId, { initialRow, passive });
  ...
  const finishQueueAndSend = async (confirmMaterialReplacement = false) => {
    ...
    if (finalRow && ['validation_error', 'rejected', 'conflict', 'auth_error'].includes(finalRow.status)) Alert.alert('Envoi Intranet non validé', serverFeedback(finalRow) || 'L’Intranet a refusé ou interrompu l’envoi. La visite reste Offline.');
  };
  ...
  const prepareAndSend = async () => {
    ...
    try {
      ...
      if (row && ['pending', 'retry', 'auth_error'].includes(row.status)) {
        await assertQueuedClientStillMatchesImportedClient(visiteId, row); await retryVisitUploadNow(visiteId); await refresh(); await onVisitChanged?.(); return;
      }
  ...
  return <View style={compact ? { alignItems: 'flex-end' } : { borderWidth: 1, borderColor: COLORS.line, borderRadius: 11, backgroundColor: '#F8FAFC', padding: 10, marginVertical: 7 }}>
    <View style={{ flexDirection: 'row', alignItems: 'center', gap: 9 }}>
      ...
      <TouchableOpacity accessibilityRole="button" accessibilityLabel={online ? 'Online, visite présente sur l’Intranet' : 'Offline, appuyer pour envoyer la visite sur l’Intranet'} disabled={loading || busy || online || row?.status === 'sending'} onPress={(event) => { event?.stopPropagation?.(); prepareAndSend().catch(() => {}); }} style={{ minWidth: 82, minHeight: 36, paddingHorizontal: 14, borderRadius: 18, alignItems: 'center', justifyContent: 'center', backgroundColor: online ? ONLINE : OFFLINE, opacity: loading ? 0.55 : 1 }}><Text style={{ color: '#FFFFFF', fontSize: 12, fontWeight: '900', letterSpacing: 0.3 }}>{online ? 'Online' : 'Offline'}</Text></TouchableOpacity>
    </View>
