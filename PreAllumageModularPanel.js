...
function GestionModal({ visible, mode, panelId, onClose, onSubmit }) {
  ...
  const titre = estLocal
    ? 'Ajouter un local / site'
    : mode === 'rubrique'
      ...
      <View style={styles.modalOverlay}>
        <View style={styles.modalSheet}>
          ...
          <TextInput
            ...
            value={nom}
            onChangeText={setNom}
            placeholder={estLocal ? 'Ex. SST 12, Chaufferie Nord…' : 'Nom'}
            autoFocus
          />
          {estLocal ? (
            <>
              ...
              <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 7 }}>
                {PREALLUMAGE_TYPES_LOCAUX.map((t) => {
                  ...
                  return (
                    ...
                    >
                      <Text style={{ color: selected ? COLORS.white : COLORS.inkSoft, fontWeight: '700' }}>
                        {t.label}
                      </Text>
                    </TouchableOpacity>
                  );
              ...
              {typeCode !== 'chaufferie' ? (
                <View style={{ flexDirection: 'row', gap: 8, marginTop: 14 }}>
                  <TouchableOpacity
                    onPress={() => setChauffage(!chauffage)}
                    style={{
                      flex: 1,
                      padding: 11,
                      borderRadius: 10,
                      borderWidth: 1,
                      borderColor: chauffage ? COLORS.orange : COLORS.line,
                      backgroundColor: chauffage ? COLORS.orangeLight : COLORS.white
                    }}
                  >
                    <Text
                      style={{
                        textAlign: 'center',
                        fontWeight: '700',
                        color: chauffage ? COLORS.orangeDark : COLORS.inkSoft
                      }}
                    >
                      {chauffage ? '✓ ' : ''}Chauffage
                    </Text>
                  ...
                  <TouchableOpacity
                    ...
                    style={{
                      ...
                      borderRadius: 10,
                      borderWidth: 1,
                      borderColor: ecs ? COLORS.orange : COLORS.line,
                      backgroundColor: ecs ? COLORS.orangeLight : COLORS.white
                    }}
                  >
                    <Text
                      style={{
                        textAlign: 'center',
                        fontWeight: '700',
                        color: ecs ? COLORS.orangeDark : COLORS.inkSoft
                      }}
                    >
                      {ecs ? '✓ ' : ''}ECS
                    </Text>
...
export function PreAllumageModularPanel({ visiteId, panelId, onSaved }) {
  ...
  const sections = useMemo(
    () =>
      rubriquesVersSections(modele?.rubriques || [], panelId).map((r) => ({
        ...r,
  ...
  const ouvrirChamp = (rubrique) => {
    setRubriqueCible(rubrique);
    setModal('champ');
  };
  const confirmerSuppressionRubrique = (rubrique) =>
    Alert.alert('Supprimer cette rubrique ?', `Les saisies de « ${rubrique.nom} » seront supprimées.`, [
      { text: 'Annuler', style: 'cancel' },
      {
        text: 'Supprimer',
        style: 'destructive',
        onPress: async () => {
          await supprimerRubriquePreAllumage(rubrique.id);
          await recharger();
          onSaved?.();
        }
      }
    ]);
  const confirmerSuppressionLocal = (local) =>
    Alert.alert(
      'Supprimer ce local ?',
      `Tous les compteurs, réglages et contrôles liés à « ${local.nom} » seront supprimés.`,
      [
        { text: 'Annuler', style: 'cancel' },
        {
          text: 'Supprimer',
          style: 'destructive',
          onPress: async () => {
            await supprimerLocalPreAllumage(local.id);
            await recharger();
            onSaved?.();
          }
        }
  ...
  const soumettre = async ({ nom, typeCode, chauffage, ecs }) => {
    try {
      ...
      else if (modal === 'champ' && rubriqueCible)
        await ajouterChampPreAllumage(rubriqueCible.id, {
          libelle: nom,
          type: ['p-pa-chaufferie', 'p-pa-sst'].includes(panelId) ? 'controle' : 'champ',
          numericIndex: panelId === 'p-pa-compteurs'
        });
      setModal(null);
      setRubriqueCible(null);
      await recharger();
      onSaved?.();
    } catch (e) {
    ...
    <>
      <SectionList
          ...
          <View>
            ...
            <View
              style={{
                ...
                borderColor: editionStructure ? COLORS.orange : COLORS.line,
                borderWidth: 1,
                borderRadius: 12,
                padding: 12,
                marginBottom: 14
            ...
            >
              <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
                <View style={{ flex: 1 }}>
                  ...
                  <Text style={{ color: COLORS.inkSoft, fontSize: 12, marginTop: 3 }}>
                    Les locaux alimentent automatiquement les compteurs, la régulation et les contrôles associés.
                  </Text>
                </View>
                <PetitBouton
                  label={editionStructure ? 'Terminer' : 'Modifier'}
                  primary={editionStructure}
                  onPress={() => setEditionStructure((v) => !v)}
                />
              </View>
              {editionStructure ? (
        ...
        renderSectionHeader={({ section }) => {
          ...
          return (
            <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8, marginTop: 5, marginBottom: 9 }}>
              {editionStructure ? (
                <SaisieNom
                  value={section.nom}
                  onSave={async (nom) => {
                    if (local) await renommerLocalPreAllumage(local.id, nom);
                    else await renommerRubriquePreAllumage(section.id, nom);
                    await recharger();
                    onSaved?.();
                  }}
                  style={[
                ...
                <Text style={[styles.sectionTitle, { flex: 1, marginBottom: 0 }]}>{section.nom}</Text>
              )}
              {editionStructure ? <PetitBouton label="+ Champ" onPress={() => ouvrirChamp(section)} /> : null}
              {editionStructure && section.supprimable ? (
                <PetitBouton
                  label="Supprimer"
                  danger
                  onPress={() => (local ? confirmerSuppressionLocal(local) : confirmerSuppressionRubrique(section))}
                />
              ) : null}
            </View>
          );
        ...
        renderItem={({ item, section }) => (
          <View style={styles.formCard}>
            {editionStructure ? (
              <View style={{ alignItems: 'flex-end', marginBottom: 3 }}>
                <PetitBouton
                  ...
                  danger
                  onPress={() =>
                    Alert.alert('Retirer ce champ ?', `La saisie « ${item.field.displayLabel} » sera supprimée.`, [
                      { text: 'Annuler', style: 'cancel' },
                      {
                        text: 'Retirer',
                        style: 'destructive',
                        onPress: async () => {
                          await supprimerChampPreAllumage(item.field.modularFieldId);
                          await recharger();
                          onSaved?.();
                        }
                      }
            ...
            {item.field.type === 'controle' ? (
              editionStructure ? (
                <SaisieNom
                  value={item.field.displayLabel}
                  onSave={async (nom) => {
                    await renommerChampPreAllumage(item.field.modularFieldId, nom);
                    await recharger();
                    onSaved?.();
                  }}
                  style={[
                    styles.fieldLabel,
                    { borderBottomWidth: 1, borderBottomColor: COLORS.line, marginBottom: 8, paddingVertical: 4 }
                  ]}
                />
              ) : null
            ) : null}
            {item.field.type === 'champ' ? (
              <DurableChampGenerique
                ...
                field={{ ...item.field, renamable: editionStructure }}
                valeurInitiale={champsMap[item.key]}
                displayLabel={item.field.displayLabel}
                onRename={
                  editionStructure
                    ? async (nom) => {
                        await renommerChampPreAllumage(item.field.modularFieldId, nom);
                ...
                onSaved={(valeur) => {
                  setChampsMap((m) => ({ ...m, [item.key]: valeur }));
                  onSaved?.();
                }}
              />
            ) : item.field.presets ? (
              <PresetControleGenerique
                visiteId={visiteId}
                sectionCode={section.sectionCode}
                field={item.field}
                displayLabel={item.field.displayLabel}
                ...
                onEtatChange={(patch) =>
                  setControlesMap((m) => ({ ...m, [item.key]: { ...(m[item.key] || {}), ...patch } }))
                }
                onSaved={onSaved}
              />
            ) : (
        ...
        ListEmptyComponent={
          <Text style={{ color: COLORS.inkSoft, textAlign: 'center', padding: 24 }}>
            Aucune rubrique. Activez « Modifier » pour ajouter un local ou une rubrique.
          </Text>
        }
