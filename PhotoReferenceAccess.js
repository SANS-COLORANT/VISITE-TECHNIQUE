...
export function PhotoReferenceAccess({
  siteId = null,
  visiteId = null,
  remoteLocalId = null,
  contextKey = 'visit',
  ...
  const readContexts = useCallback(
    async () =>
      remoteClientId && remoteSiteId
        ? [
            {
              client: { remote_client_id: String(remoteClientId), nom: clientName },
              remoteSiteId: String(remoteSiteId),
              remoteLocalId
            }
          ]
        : resolvePhotoContexts({ visiteId, siteId, remoteLocalId, ignoreVisitLocal: isPreLocal }),
    [siteId, visiteId, remoteLocalId, remoteClientId, remoteSiteId, clientName, isPreLocal]
  );
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const ctx = rows[0],
        clientId = ctx.client.remote_client_id;
      ...
      const choice =
        !ctx.remoteLocalId && visiteId
          ? await readPhotoLocalChoice(visiteId, contextKey, clientId, ctx.remoteSiteId)
          : null;
      const localId = ctx.remoteLocalId || choice;
      const scoped = filterLatestVisitPhotos(manifest, {
        siteIds: [ctx.remoteSiteId],
        localIds: localId ? [localId] : null
      });
      if (alive)
        setStatus(visiteId && !localId ? 'Choisir le local de référence' : photoStatusLabel(photoSummary(scoped)));
    })().catch(() => {
      if (alive) setError('Photos de référence : réessayer');
    });
    return () => {
      alive = false;
    };
  ...
  const siteCover =
    siteId && !visiteId ? (
      <PatrimoineImageCard
        entityType="site"
        entityId={siteId}
        title={contextTitle || 'Site'}
        subtitle="Image du site · disponible hors connexion"
      />
    ) : null;
      ...
      <View style={{ marginVertical: 6 }}>
        ...
        >
          <Text style={{ color: COLORS.ink, fontSize: 13, fontWeight: '700' }}>
            {contextTitle ? `Photos de référence · ${contextTitle}` : 'Photos de référence Intranet'}
          </Text>
          <Text style={{ color: COLORS.muted, fontSize: 12, marginTop: 4 }}>
            {error ||
              status ||
              (contexts.length > 1 ? 'Choisir le client Intranet' : 'Consulter sans quitter la visite')}
          </Text>
        </TouchableOpacity>
        <Modal visible={picker} transparent animationType="fade" onRequestClose={() => setPicker(false)}>
          <View style={styles.modalOverlay}>
            <View style={styles.modalSheet}>
              <Text style={styles.modalTitle}>Choisir le client de référence</Text>
              {contexts.map((ctx) => (
                <TouchableOpacity
                  key={`${ctx.client.remote_client_id}-${ctx.remoteSiteId}`}
                  onPress={() => {
                    setPicker(false);
                    setSelected(ctx);
                  }}
                ...
                >
                  <Text>{ctx.client.nom || ctx.client.remote_client_id}</Text>
                </TouchableOpacity>
              ))}
              <TouchableOpacity onPress={() => setPicker(false)} style={[styles.btnSecondary, { minHeight: 48 }]}>
        ...
        {Gallery ? (
          <Gallery
            ...
            activated={activated}
            siteIds={[selected.remoteSiteId]}
            localIds={selected.remoteLocalId ? [selected.remoteLocalId] : null}
            visiteId={visiteId}
            contextKey={contextKey}
            contextTitle={contextTitle || selected.siteName}
            requireLocalChoice={Boolean(visiteId)}
            onClose={() => {
              setSelected(null);
              setRevision((n) => n + 1);
            }}
