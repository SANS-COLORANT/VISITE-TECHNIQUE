...
export function MissionP3DashboardScreen({ navigation, route }) {
  ...
  const stats = useMemo(() => {
    ...
    const next3 = equipment.filter((row) => Number(row.replacement_year) > year && Number(row.replacement_year) <= year + 3);
    const missing = equipment.filter((row) => !row.replacement_year || !row.replacement_cost || !row.expected_lifetime_years);
    return {
  ...
  const rows = useMemo(() => {
    if (filter === 'due') return equipment.filter((row) => Number(row.replacement_year) > 0 && Number(row.replacement_year) <= year);
    if (filter === 'next3') return equipment.filter((row) => Number(row.replacement_year) > year && Number(row.replacement_year) <= year + 3);
    if (filter === 'missing') return equipment.filter((row) => !row.replacement_year || !row.replacement_cost || !row.expected_lifetime_years);
    return equipment;
  ...
  return <View style={{ flex: 1, backgroundColor: 'transparent' }}>
    <ScrollView contentContainerStyle={{ padding: 16, paddingBottom: 120 }}>
      ...
      <Text style={{ color: COLORS.inkSoft, fontSize: 10.5, lineHeight: 15 }}>
        Cette vue consolide uniquement les données déjà renseignées dans les fiches équipements : état, durée de vie indicative, coût et année de renouvellement. METRA ne transforme pas cette projection en décision contractuelle automatique.
      </Text>
      ...
      {rows.map((row) => {
        ...
        return <TouchableOpacity
          key={row.id}
          activeOpacity={0.82}
          onPress={() => navigation.navigate('MissionEquipment', { missionId, siteId: row.site_id, equipmentId: row.id })}
        ...
        >
          ...
          <Text style={{ color: COLORS.inkFaint, fontSize: 8.2, marginTop: 5 }}>
            Durée de vie : {row.expected_lifetime_years || '?'} ans{axes.length ? ' · Axes : ' + axes.join(', ') : ''}
          </Text>
          {missing.length ? <Text style={{ color: MISSION_COLORS.accentDark, fontSize: 8.2, marginTop: 4 }}>À compléter : {missing.join(', ')}</Text> : null}
        </TouchableOpacity>;
      })}

      {!rows.length ? <View style={[missionStyles.card, { padding: 14 }]}>
