...
export const PreAllumageCompactControl = React.memo(function PreAllumageCompactControl({ visiteId, sectionCode, field, etatInitial, localName, localType, contextLabel, onEtatChange, onSaved }) {
  ...
  const sauvegarderCommentaire = useCallback(async (raw) => {
  ...
  }, [visiteId, sectionCode, field, controleKey, avis, remarque, label, localName, notifier, rechargerRemarque]);

  const [commentaire, setCommentaire, flushCommentaire, , replaceCommentairePersisted] = useDurableAutosave(
  ...
  const appliquer = useCallback(async (nextAvis, preset = null, idx = null) => {
    ...
    const texte = nextAvis === 'S' && texteBrut
      ...
      : texteBrut;
    setAvis(nextAvis);
    replaceCommentairePersisted(texte);
    setPresetChoisi(idx);
  ...
  }, [visiteId, sectionCode, field, label, localName, localType, contextLabel, controleKey, notifier, rechargerRemarque, replaceCommentairePersisted]);
  ...
  const ciblePhotoS = () => ({ entiteKey: controleKey, label: `${localName || ''} · ${label}` });
  ...
  return <View style={{ paddingVertical: 7 }}>
    ...
    {afficherDetail ? <View style={{ marginTop: 8, padding: 9, borderRadius: 10, borderWidth: 1, borderColor: avis === 'N.S' ? '#F4C7C7' : avis === 'S' ? '#B7E2C0' : COLORS.line, backgroundColor: avis === 'N.S' ? COLORS.redBg : avis === 'S' ? COLORS.greenBg : '#F9FAFB' }}>
      ...
      <TextInput style={[styles.input, { minHeight: 52, textAlignVertical: 'top', marginTop: 7, fontSize: 12 }]} multiline value={commentaire} onChangeText={(v) => { setCommentaire(v); setPresetChoisi(null); }} onBlur={() => flushCommentaire().catch(console.warn)} placeholder={avis === 'S' ? 'Commentaire de conformité…' : 'Commentaire technique…'} />
      ...
      {avis === 'S' ? <View style={{ marginTop: 7 }}><PreAllumagePhotoButton visiteId={visiteId} entiteKey={controleKey} label={`${localName || ''} · ${label}`} beforeCapture={ciblePhotoS} style={{ minHeight: 38, paddingHorizontal: 9 }} /></View> : null}
    </View> : null}

    {avis && avis !== 'N.S' && !detailOuvert ? <View style={{ marginTop: 5, flexDirection: 'row', alignItems: 'center', gap: 8 }}><TouchableOpacity onPress={() => setDetailOuvert(true)}><Text style={{ color: avis === 'S' ? COLORS.green : COLORS.orangeDark, fontSize: 10, fontWeight: '800' }}>{avis === 'S' ? 'Modifier le commentaire S' : 'Modifier le motif'}</Text></TouchableOpacity>{avis === 'S' ? <PreAllumagePhotoButton visiteId={visiteId} entiteKey={controleKey} label={`${localName || ''} · ${label}`} beforeCapture={ciblePhotoS} style={{ minHeight: 32, paddingHorizontal: 8, paddingVertical: 4 }} /> : null}</View> : null}
  </View>;
});
