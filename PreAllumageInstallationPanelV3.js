...
import {
  ajouterEquipementChaufferiePreAllumage,
  estRubriqueEquipementChaufferie,
  normaliserLocauxPreAllumage,
  PREALLUMAGE_CHAUFFERIE_EQUIPMENT_TYPES,
  supprimerEquipementChaufferiePreAllumage,
...
function groupItems(items, twoCols, addCurve = false) {
  const rows = []; if (addCurve) rows.push({ kind: 'curve', key: 'heat-curve' }); let buf = [];
...
function AddLocalModal({ visible, onClose, onSubmit }) {
  const [nom, setNom] = useState(''); const [typeCode, setTypeCode] = useState('sous_station'); const [chauffage, setChauffage] = useState(true); const [ecs, setEcs] = useState(true);
  ...
  return <Modal visible={visible} transparent animationType="fade" onRequestClose={onClose}><View style={styles.modalOverlay}><View style={styles.modalSheet}>
    ...
    <Text style={{ color: COLORS.ink, fontWeight: '900', marginTop: 12, marginBottom: 7 }}>Type de local</Text><View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 6 }}>{PREALLUMAGE_TYPES_LOCAUX.map((t) => <TouchableOpacity key={t.code} onPress={() => setTypeCode(t.code)} style={{ minHeight: 38, justifyContent: 'center', paddingHorizontal: 11, borderRadius: 19, borderWidth: 1, borderColor: typeCode === t.code ? COLORS.orange : COLORS.line, backgroundColor: typeCode === t.code ? COLORS.orangeLight : COLORS.white }}><Text style={{ color: typeCode === t.code ? COLORS.orangeDark : COLORS.inkSoft, fontWeight: '900', fontSize: 11 }}>{t.label}</Text></TouchableOpacity>)}</View>
    {typeCode !== 'chaufferie' ? <View style={{ flexDirection: 'row', gap: 8, marginTop: 12 }}><Toggle label="Chauffage" value={chauffage} onPress={() => setChauffage((v) => !v)} /><Toggle label="ECS" value={ecs} onPress={() => setEcs((v) => !v)} /></View> : null}
...
function AddEquipmentModal({ visible, onClose, onAdd }) {
  const [name, setName] = useState(''); const [type, setType] = useState('chaudiere');
  useEffect(() => { if (visible) { setName(''); setType('chaudiere'); } }, [visible]);
  return <Modal visible={visible} transparent animationType="fade" onRequestClose={onClose}><View style={styles.modalOverlay}><View style={styles.modalSheet}>
    <Text style={styles.modalTitle}>Ajouter un équipement</Text><Text style={[styles.importHint, { marginBottom: 10 }]}>Ajoutez autant de chaudières, brûleurs, pompes ou autres équipements que nécessaire.</Text>
    <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 6 }}>{PREALLUMAGE_CHAUFFERIE_EQUIPMENT_TYPES.map((x) => <TouchableOpacity key={x.code} onPress={() => setType(x.code)} style={{ minHeight: 38, justifyContent: 'center', paddingHorizontal: 10, borderRadius: 19, borderWidth: 1, borderColor: type === x.code ? COLORS.orange : COLORS.line, backgroundColor: type === x.code ? COLORS.orangeLight : COLORS.white }}><Text style={{ color: type === x.code ? COLORS.orangeDark : COLORS.inkSoft, fontSize: 11, fontWeight: '900' }}>{x.label}</Text></TouchableOpacity>)}</View>
    <TextInput style={[styles.input, { marginTop: 12 }]} value={name} onChangeText={setName} placeholder="Nom facultatif (sinon numérotation automatique)" />
    <View style={styles.modalActions}><TouchableOpacity style={styles.btnSecondary} onPress={onClose}><Text style={styles.btnSecondaryText}>Annuler</Text></TouchableOpacity><TouchableOpacity style={styles.btnPrimary} onPress={() => onAdd(type, name)}><Text style={styles.btnPrimaryText}>Ajouter</Text></TouchableOpacity></View>
  </View></View></Modal>;
}
function ConfigModal({ visible, local, onClose, onSave }) {
  const [chauffage, setChauffage] = useState(true); const [ecs, setEcs] = useState(true);
  useEffect(() => { if (visible && local) { setChauffage(Number(local.chauffage) !== 0); setEcs(Number(local.ecs) !== 0); } }, [visible, local]);
  return <Modal visible={visible} transparent animationType="fade" onRequestClose={onClose}><View style={styles.modalOverlay}><View style={styles.modalSheet}><Text style={styles.modalTitle}>Configurer {local?.nom}</Text><View style={{ flexDirection: 'row', gap: 8, marginTop: 10 }}><Toggle label="Chauffage" value={chauffage} onPress={() => setChauffage((v) => !v)} /><Toggle label="ECS" value={ecs} onPress={() => setEcs((v) => !v)} /></View><View style={styles.modalActions}><TouchableOpacity style={styles.btnSecondary} onPress={onClose}><Text style={styles.btnSecondaryText}>Annuler</Text></TouchableOpacity><TouchableOpacity style={styles.btnPrimary} onPress={() => onSave({ chauffage, ecs })}><Text style={styles.btnPrimaryText}>Appliquer</Text></TouchableOpacity></View></View></View></Modal>;
}
function ActionsModal({ visible, local, canUp, canDown, onClose, onAction }) {
  if (!local) return null;
  const entries = [['rename', 'Renommer'], ...(local.type_code !== 'chaufferie' ? [['config', 'Configurer Chauffage / ECS']] : []), ['duplicate', 'Dupliquer sans les relevés du jour'], ['copy', 'Copier les réglages de régulation aux autres locaux'], ...(canUp ? [['up', 'Déplacer plus tôt']] : []), ...(canDown ? [['down', 'Déplacer plus tard']] : []), ['delete', 'Supprimer le local']];
...
export function PreAllumageInstallationPanelV3({ visiteId, onSaved }) {
  ...
  const sections = useMemo(() => rubriques.map((r) => {
    ...
    return { ...r, title: titreRubrique(r), raw, data: collapsed[r.id] ? [] : groupItems(raw, twoCols, r.panel_id === 'p-pa-regulation'), collapsed: Boolean(collapsed[r.id]) };
  ...
  const submitLocal = async ({ nom, typeCode, chauffage, ecs }) => {
    try { const id = await ajouterLocalPreAllumage(visiteId, { nom, typeCode, chauffage, ecs }); if (typeCode === 'chaufferie') await preparerChaufferieDynamiquePreAllumage(visiteId, id); await synchroniserNombreLocauxPreAllumage(visiteId); setAddLocal(false); await reload(id); onSaved?.(); } catch (e) { Alert.alert('Ajout impossible', e.message); }
  };
  const submitEquipment = async (type, customName) => {
    try { await ajouterEquipementChaufferiePreAllumage(visiteId, active.id, type, customName); setAddEquipment(false); await reload(active.id); onSaved?.(); } catch (e) { Alert.alert('Ajout impossible', e.message); }
  };
  ...
  const removeEquipment = (section) => Alert.alert('Supprimer cet équipement ?', section.nom, [{ text: 'Annuler', style: 'cancel' }, { text: 'Supprimer', style: 'destructive', onPress: async () => { await supprimerEquipementChaufferiePreAllumage(section.id); await reload(active.id); onSaved?.(); } }]);
  ...
  const header = <View>
    <View style={{ backgroundColor: COLORS.white, borderWidth: 1, borderColor: COLORS.line, borderRadius: 14, padding: 12, marginBottom: 10 }}><View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}><View style={{ flex: 1 }}><Text style={{ color: COLORS.ink, fontSize: 16, fontWeight: '900' }}>Locaux de la visite</Text><Text style={{ color: COLORS.inkSoft, fontSize: 11, marginTop: 2 }}>Aucun local par défaut. Ajoutez Chaufferie, SST, Église, Piscine, Centre commercial… selon le site réel.</Text></View><PetitBouton primary label="+ Local" onPress={() => setAddLocal(true)} /></View>
    ...
    {!active ? <View style={{ backgroundColor: COLORS.white, borderWidth: 1, borderColor: COLORS.line, borderRadius: 14, padding: 22, alignItems: 'center' }}><Text style={{ color: COLORS.ink, fontSize: 16, fontWeight: '900' }}>Aucun local</Text><Text style={{ color: COLORS.inkSoft, textAlign: 'center', fontSize: 11, marginTop: 5, marginBottom: 12 }}>Commencez par ajouter le premier local rencontré sur le site et choisissez son type.</Text><PetitBouton primary label="+ Ajouter le premier local" onPress={() => setAddLocal(true)} /></View> : <View style={{ backgroundColor: '#FFFDFC', borderWidth: 1, borderColor: stats.ns ? '#F4C7C7' : COLORS.line, borderRadius: 14, padding: 12, marginBottom: 10 }}><View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}><View style={{ flex: 1 }}>{rename ? <TextInput value={name} onChangeText={setName} autoFocus onBlur={saveName} onSubmitEditing={saveName} style={{ fontSize: 18, fontWeight: '900', borderBottomWidth: 1, borderBottomColor: COLORS.orange }} /> : <Text style={{ color: COLORS.ink, fontSize: 18, fontWeight: '900' }}>{active.nom}</Text>}<Text style={{ color: COLORS.inkSoft, fontSize: 11, marginTop: 3 }}>{typeLabel(active)} · {stats.done}/{stats.total} renseignés · {stats.pct}%{stats.ns ? ` · ${stats.ns} N.S` : ''}{stats.later ? ` · ${stats.later} à compléter` : ''}</Text></View><PhotoButton visiteId={visiteId} entiteKey={`preallumage_local||${active.id}`} label={active.nom} style={{ minHeight: 38, paddingHorizontal: 8 }} />{active.type_code === 'chaufferie' ? <PetitBouton primary label="+ Équipement" onPress={() => setAddEquipment(true)} /> : null}<PetitBouton label="⋯" onPress={() => setActions(true)} /></View>{(stats.missing || stats.later) ? <TouchableOpacity onPress={nextIncomplete} style={{ marginTop: 8 }}><Text style={{ color: COLORS.orangeDark, fontSize: 10, fontWeight: '900' }}>Aller au prochain champ incomplet →</Text></TouchableOpacity> : null}{active.type_code === 'chaufferie' && !rubriques.some(estRubriqueEquipementChaufferie) ? <View style={{ marginTop: 9, backgroundColor: '#F9FAFB', borderRadius: 9, padding: 9 }}><Text style={{ color: COLORS.inkSoft, fontSize: 10 }}>Ajoutez les chaudières, brûleurs, pompes et autres équipements réellement présents. Aucune quantité n’est imposée.</Text></View> : null}</View>}
  </View>;
  ...
  return <>
    <SectionList ref={listRef} sections={active ? sections : []} keyExtractor={(item) => item.key} ListHeaderComponent={header} ListFooterComponent={footer} renderSectionHeader={({ section }) => { const s = statsRubriques([section], champs, controls); const dyn = estRubriqueEquipementChaufferie(section); return <View style={{ marginTop: 5, flexDirection: 'row', alignItems: 'center', borderWidth: 1, borderColor: s.ns ? '#F4C7C7' : COLORS.line, borderRadius: 11, backgroundColor: COLORS.white }}><TouchableOpacity onPress={() => setCollapsed((m) => ({ ...m, [section.id]: !section.collapsed }))} style={{ flex: 1, minHeight: 43, justifyContent: 'center', paddingHorizontal: 12 }}><View style={{ flexDirection: 'row', alignItems: 'center', gap: 7 }}><Text style={{ flex: 1, color: COLORS.ink, fontSize: 13, fontWeight: '900' }}>{section.title}</Text><Text style={{ color: s.ns ? COLORS.red : COLORS.inkSoft, fontSize: 9, fontWeight: '800' }}>{s.done}/{s.total}{s.ns ? ` · ⚠ ${s.ns}` : ''}</Text><Text style={{ color: COLORS.inkSoft }}>{section.collapsed ? '⌄' : '⌃'}</Text></View></TouchableOpacity>{dyn ? <TouchableOpacity onPress={() => removeEquipment(section)} style={{ minWidth: 42, minHeight: 42, alignItems: 'center', justifyContent: 'center', borderLeftWidth: 1, borderLeftColor: COLORS.line }}><Text style={{ color: COLORS.red, fontWeight: '900' }}>✕</Text></TouchableOpacity> : null}</View>; }} renderItem={({ item, section, index }) => <View style={{ backgroundColor: COLORS.white, borderLeftWidth: 1, borderRightWidth: 1, borderBottomWidth: index === section.data.length - 1 ? 1 : 0, borderColor: COLORS.line, paddingHorizontal: 10, paddingVertical: 4 }}>{item.kind === 'curve' ? <PreAllumageHeatCurve visiteId={visiteId} sectionCode={section.section_code} fields={section.champs} champsMap={champs} onSaved={(key, value) => { setChamps((m) => ({ ...m, [key]: value })); onSaved?.(); }} /> : item.kind === 'fields' ? <View style={{ flexDirection: twoCols ? 'row' : 'column', gap: twoCols ? 12 : 0 }}>{item.items.map((x, i) => <View key={x.key} style={{ flex: 1, minWidth: 0, paddingLeft: twoCols && i > 0 ? 10 : 0, borderLeftWidth: twoCols && i > 0 ? 1 : 0, borderLeftColor: COLORS.line }}><PreAllumageCompactField visiteId={visiteId} sectionCode={section.section_code} field={x.field} valeurInitiale={champs[x.key]} localName={active?.nom} showPhoto={section.panel_id === 'p-pa-compteurs'} onSaved={(value) => { setChamps((m) => ({ ...m, [x.key]: value })); onSaved?.(); }} /></View>)}</View> : <PreAllumageCompactControl visiteId={visiteId} sectionCode={section.section_code} field={item.item.field} etatInitial={controls[item.item.key]} localName={active?.nom} onEtatChange={(patch) => setControls((m) => ({ ...m, [item.item.key]: { ...(m[item.item.key] || {}), ...patch } }))} onSaved={onSaved} />}</View>} contentContainerStyle={styles.panelContent} keyboardShouldPersistTaps="handled" stickySectionHeadersEnabled={false} initialNumToRender={12} maxToRenderPerBatch={12} windowSize={7} />
    <AddLocalModal visible={addLocal} onClose={() => setAddLocal(false)} onSubmit={submitLocal} /><AddEquipmentModal visible={addEquipment} onClose={() => setAddEquipment(false)} onAdd={submitEquipment} /><ConfigModal visible={config} local={active} onClose={() => setConfig(false)} onSave={saveConfig} /><ActionsModal visible={actions} local={active} canUp={idx > 0} canDown={idx >= 0 && idx < locals.length - 1} onClose={() => setActions(false)} onAction={(a) => localAction(a).catch((e) => Alert.alert('Action impossible', e.message))} />
  </>;
}
