...
function AddEquipmentModal({ visible, onClose, onAdd, local }) {
  ...
  return <Modal visible={visible} transparent animationType="fade" onRequestClose={onClose}><View style={styles.modalOverlay}><View style={styles.modalSheet}>
    ...
    <View style={styles.modalActions}><TouchableOpacity style={styles.btnSecondary} onPress={onClose}><Text style={styles.btnSecondaryText}>Annuler</Text></TouchableOpacity><TouchableOpacity style={styles.btnPrimary} onPress={() => onAdd(type, name, controle)}><ButtonGlow /><Text style={styles.btnPrimaryText}>Ajouter</Text></TouchableOpacity></View>
  </View></View></Modal>;
}
...
export function PreAllumageInstallationPanelV3({ visiteId, onSaved }) {
  ...
  const [model, setModel] = useState(null); const [champs, setChamps] = useState({}); const [controls, setControls] = useState({});
  const [activeId, setActiveId] = useState(uiInitial.activeLocalId || null); const [search, setSearch] = useState(''); const [collapsed, setCollapsed] = useState(uiInitial.collapsed || {});
  ...
  useEffect(() => {
    let alive = true;
    hydrateNavigationState(uiKey).then((state) => {
      if (!alive) return;
    ...
    }).catch(() => {}).finally(() => { if (alive) uiHydratedRef.current = true; });
    return () => { alive = false; };
  }, [uiKey]);
  ...
  const { listRef, onScroll } = useListScrollMemory(`visit-preallumage-local:${visiteId}:${active?.id || 'none'}`, active?.id ? 1 : 0);
  ...
  const header = <View>
    <View style={{ backgroundColor: '#FDFCFA', borderWidth: 1, borderColor: 'rgba(22,21,15,0.08)', borderRadius: 18, padding: 14, marginBottom: 10, shadowColor: '#000', shadowOpacity: 0.08, shadowRadius: 16, shadowOffset: { width: 0, height: 8 }, elevation: 3 }}><View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}><View style={{ flex: 1 }}><Text style={{ color: COLORS.ink, fontSize: 16, fontFamily: FONTS.black }}>Locaux de la visite</Text><Text style={{ color: COLORS.inkSoft, fontSize: 11, marginTop: 2 }}>Aucun local par défaut. Ajoutez Chaufferie, Sous-station, Église, Piscine, Centre commercial… selon le site réel.</Text></View><PetitBouton primary label="+ Local" onPress={() => setAddLocal(true)} /></View>
    {locals.length ? <><View style={{ flexDirection: 'row', alignItems: 'center', marginTop: 12, borderWidth: 1, borderColor: 'rgba(22,21,15,0.1)', borderRadius: 14, paddingHorizontal: 11, backgroundColor: 'rgba(255,255,255,0.82)' }}><View style={{ marginRight: 7 }}><CvcIcon name="search" size={16} color={COLORS.inkFaint} strokeWidth={2.1} /></View><TextInput value={search} onChangeText={setSearch} placeholder="Rechercher un local, ECS, pompe…" style={{ flex: 1, minHeight: 40, fontSize: 12 }} />{search ? <TouchableOpacity accessibilityRole="button" accessibilityLabel="Fermer" onPress={() => setSearch('')}><CvcIcon name="close" size={16} color={COLORS.orangeDark} strokeWidth={2.1} /></TouchableOpacity> : null}</View><ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ gap: 7, paddingTop: 10 }}>{visibleLocals.map((l) => { const s = statsRubriques(rubriquesFor(l), champs, controls); const on = l.id === active?.id; const etat = s.ns ? '#C23B2E' : s.total && s.pct === 100 ? '#2E9D5B' : s.pct > 0 ? COLORS.orange : '#D6D1C6'; return <TouchableOpacity key={l.id} onPress={() => setActiveId(l.id)} activeOpacity={0.85} style={{ minWidth: 112, minHeight: 52, justifyContent: 'center', paddingHorizontal: 12, paddingVertical: 8, borderWidth: on ? 1.5 : 1, borderColor: on ? COLORS.orange : 'rgba(22,21,15,0.1)', backgroundColor: on ? COLORS.white : 'rgba(255,255,255,0.72)', borderRadius: 15, shadowColor: on ? COLORS.orange : '#000', shadowOpacity: on ? 0.3 : 0, shadowRadius: 8, shadowOffset: { width: 0, height: 4 }, elevation: on ? 3 : 0 }}><View style={{ flexDirection: 'row', alignItems: 'center', gap: 6 }}><View style={{ width: 8, height: 8, borderRadius: 4, backgroundColor: etat }} /><Text numberOfLines={1} style={{ color: on ? COLORS.orangeDark : COLORS.ink, fontSize: 12, fontFamily: FONTS.bodyBold, maxWidth: 150 }}>{l.nom}</Text></View><View style={{ height: 4, borderRadius: 2, backgroundColor: 'rgba(22,21,15,0.08)', marginTop: 6, overflow: 'hidden' }}><View style={{ width: `${Math.max(0, Math.min(100, s.pct || 0))}%`, height: 4, backgroundColor: etat }} /></View><Text style={{ color: s.ns ? '#C23B2E' : COLORS.inkSoft, fontSize: 10, fontFamily: FONTS.bodySemi, marginTop: 3 }}>{s.pct}%{s.ns ? ` · ${s.ns} N.S` : ''}</Text></TouchableOpacity>; })}</ScrollView></> : null}</View>
    {!active ? <View style={{ backgroundColor: 'rgba(255,255,255,0.8)', borderWidth: 1, borderColor: 'rgba(22,21,15,0.08)', borderRadius: 18, padding: 22, alignItems: 'center' }}><Text style={{ color: COLORS.ink, fontSize: 16, fontWeight: '900' }}>Aucun local</Text><Text style={{ color: COLORS.inkSoft, textAlign: 'center', fontSize: 11, marginTop: 5, marginBottom: 12 }}>Commencez par ajouter le premier local rencontré sur le site et choisissez son type.</Text><PetitBouton primary label="+ Ajouter le premier local" onPress={() => setAddLocal(true)} /></View> : <View style={{ backgroundColor: '#FDFCFA', borderWidth: 1, borderColor: stats.ns ? 'rgba(194,59,46,0.35)' : 'rgba(22,21,15,0.08)', borderRadius: 18, padding: 14, marginBottom: 10, shadowColor: '#000', shadowOpacity: 0.08, shadowRadius: 16, shadowOffset: { width: 0, height: 8 }, elevation: 3 }}><View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}><View style={{ flex: 1 }}>{rename ? <TextInput value={name} onChangeText={setName} autoFocus onBlur={saveName} onSubmitEditing={saveName} style={{ fontSize: 18, fontWeight: '900', borderBottomWidth: 1, borderBottomColor: COLORS.orange }} /> : <Text style={{ color: COLORS.ink, fontSize: 19, fontFamily: FONTS.black }}>{active.nom}</Text>}<Text style={{ color: COLORS.inkSoft, fontSize: 11, marginTop: 3 }}>{typeLabel(active)} · {usagesLabel(active)} · {stats.done}/{stats.total} renseignés · {stats.pct}%{stats.ns ? ` · ${stats.ns} N.S` : ''}{stats.later ? ` · ${stats.later} à compléter` : ''}</Text></View><PhotoButton visiteId={visiteId} entiteKey={`preallumage_local||${active.id}`} label={active.nom} style={{ minHeight: 38, paddingHorizontal: 8 }} /><PetitBouton primary label="+ Équipement" onPress={() => setAddEquipment(true)} /><PetitBouton label="⋯" onPress={() => setActions(true)} /></View>{(stats.missing || stats.later) ? <TouchableOpacity onPress={nextIncomplete} style={{ marginTop: 8 }}><Text style={{ color: COLORS.orangeDark, fontSize: 10, fontWeight: '900' }}>Aller au prochain champ incomplet →</Text></TouchableOpacity> : null}{!rubriques.some(estRubriqueEquipementPreAllumage) ? <View style={{ marginTop: 9, backgroundColor: 'rgba(255,255,255,0.66)', borderRadius: 9, padding: 9 }}><Text style={{ color: COLORS.inkSoft, fontSize: 10 }}>Ajoutez ici les pompes, vannes trois voies, régulations, échangeurs ou autres équipements réellement présents dans ce local. Ils pourront ensuite être dupliqués ou supprimés indépendamment.</Text></View> : null}</View>}
  </View>;
  const footer = active ? <View style={{ paddingTop: 10, paddingBottom: 24 }}><View style={{ backgroundColor: 'rgba(255,255,255,0.82)', borderWidth: 1, borderColor: 'rgba(22,21,15,0.1)', borderRadius: 11, padding: 10, marginBottom: 9 }}><Text style={{ color: COLORS.ink, fontWeight: '900', fontSize: 11 }}>Synthèse {active.nom}</Text><Text style={{ color: COLORS.inkSoft, fontSize: 10, marginTop: 3 }}>{stats.done} renseignés · {stats.ns} N.S · {stats.later} à compléter · {stats.missing} non renseignés</Text></View><View style={{ flexDirection: 'row', gap: 8 }}><View style={{ flex: 1 }}>{prev ? <PetitBouton label={`‹ ${prev.nom}`} onPress={() => setActiveId(prev.id)} /> : null}</View><View style={{ flex: 1, alignItems: 'flex-end' }}>{next ? <PetitBouton primary label={`${next.nom} ›`} onPress={() => setActiveId(next.id)} /> : <PetitBouton primary label="+ Local" onPress={() => setAddLocal(true)} />}</View></View></View> : null;
  ...
  return <>
    {active ? <PhotoReferenceAccess visiteId={visiteId} contextKey={`preallumage-local:${active.id}`} contextTitle={active.nom} /> : null}
    <SectionList ref={listRef} sections={active ? sections : []} onScroll={onScroll} scrollEventThrottle={100} keyExtractor={(item) => item.key} ListHeaderComponent={header} ListFooterComponent={footer} renderSectionHeader={({ section }) => { const s = statsRubriques([section], champs, controls); const dyn = estRubriqueEquipementPreAllumage(section); return <View style={{ marginTop: 10, flexDirection: 'row', alignItems: 'center', borderWidth: 1, borderColor: s.ns ? 'rgba(194,59,46,0.35)' : 'rgba(22,21,15,0.08)', borderRadius: 15, borderBottomLeftRadius: section.collapsed || !section.data.length ? 15 : 0, borderBottomRightRadius: section.collapsed || !section.data.length ? 15 : 0, backgroundColor: 'rgba(255,255,255,0.9)', overflow: 'hidden' }}><TouchableOpacity onPress={() => setCollapsed((m) => ({ ...m, [section.id]: !section.collapsed }))} style={{ flex: 1, minHeight: 43, justifyContent: 'center', paddingHorizontal: 12 }}><View style={{ flexDirection: 'row', alignItems: 'center', gap: 7 }}><View style={{ width: 8, height: 8, borderRadius: 4, backgroundColor: s.ns ? '#C23B2E' : s.total && s.done >= s.total ? '#2E9D5B' : s.done ? COLORS.orange : '#D6D1C6' }} /><Text style={{ flex: 1, color: COLORS.ink, fontSize: 13.5, fontFamily: FONTS.bold }}>{section.title}</Text><Text style={{ color: s.ns ? '#C23B2E' : COLORS.inkFaint, fontSize: 11, fontFamily: FONTS.bodyBold }}>{s.done}/{s.total}{s.ns ? ` · ${s.ns} N.S` : ''}</Text><Text style={{ color: COLORS.inkSoft }}>{section.collapsed ? '⌄' : '⌃'}</Text></View></TouchableOpacity>{dyn ? <><TouchableOpacity accessibilityRole="button" accessibilityLabel="Dupliquer" onPress={() => duplicateEquipment(section)} style={{ minWidth: 42, minHeight: 42, alignItems: 'center', justifyContent: 'center', borderLeftWidth: 1, borderLeftColor: COLORS.line }}><CvcIcon name="copy" size={17} color={COLORS.orangeDark} strokeWidth={2.1} /></TouchableOpacity><TouchableOpacity accessibilityRole="button" accessibilityLabel="Fermer" onPress={() => removeEquipment(section)} style={{ minWidth: 42, minHeight: 42, alignItems: 'center', justifyContent: 'center', borderLeftWidth: 1, borderLeftColor: COLORS.line }}><CvcIcon name="close" size={16} color={COLORS.red} strokeWidth={2.1} /></TouchableOpacity></> : null}</View>; }} renderItem={({ item, section, index }) => <View style={{ backgroundColor: 'rgba(255,255,255,0.82)', borderLeftWidth: 1, borderRightWidth: 1, borderBottomWidth: index === section.data.length - 1 ? 1 : 0, borderColor: 'rgba(22,21,15,0.08)', borderBottomLeftRadius: index === section.data.length - 1 ? 15 : 0, borderBottomRightRadius: index === section.data.length - 1 ? 15 : 0, paddingHorizontal: 12, paddingVertical: 5 }}>{item.kind === 'curve' ? <PreAllumageHeatCurve visiteId={visiteId} sectionCode={section.section_code} fields={section.champs} champsMap={champs} onSaved={(key, value) => { setChamps((m) => ({ ...m, [key]: value })); onSaved?.(); }} onStructureChanged={() => reload(active?.id).catch(() => {})} /> : item.kind === 'regulationGroup' ? <View style={{ marginVertical: 5, padding: 9, borderRadius: 11, borderWidth: 1, borderColor: 'rgba(22,21,15,0.1)', backgroundColor: item.title === 'Chauffage' ? '#FFF9F2' : item.title === 'ECS / primaire' ? '#F5FAFF' : '#F9FAFB' }}><View style={{ flexDirection: 'row', alignItems: 'center', gap: 6, marginBottom: 5 }}><Text style={{ fontSize: 16 }}>{item.icon}</Text><Text style={{ color: COLORS.ink, fontSize: 12, fontWeight: '900' }}>{item.title}</Text></View><View style={{ flexDirection: twoCols ? 'row' : 'column', flexWrap: twoCols ? 'wrap' : 'nowrap', gap: twoCols ? 10 : 0 }}>{item.items.map((x) => <View key={x.key} style={{ width: twoCols ? '48.8%' : '100%', minWidth: 0 }}>{renderCompactField(x, section, { showThermalBadge: false })}</View>)}</View></View> : item.kind === 'fields' ? <View style={{ flexDirection: twoCols ? 'row' : 'column', gap: twoCols ? 12 : 0 }}>{item.items.map((x, i) => <View key={x.key} style={{ flex: 1, minWidth: 0, paddingLeft: twoCols && i > 0 ? 10 : 0, borderLeftWidth: twoCols && i > 0 ? 1 : 0, borderLeftColor: COLORS.line }}>{renderCompactField(x, section)}</View>)}</View> : <PreAllumageCompactControl visiteId={visiteId} sectionCode={section.section_code} field={item.item.field} etatInitial={controls[item.item.key]} localName={active?.nom} localType={active?.type_code} contextLabel={section.title} onEtatChange={(patch) => setControls((m) => ({ ...m, [item.item.key]: { ...(m[item.item.key] || {}), ...patch } }))} onSaved={onSaved} />}</View>} contentContainerStyle={styles.panelContent} keyboardShouldPersistTaps="handled" stickySectionHeadersEnabled={false} initialNumToRender={12} maxToRenderPerBatch={12} windowSize={7} />
