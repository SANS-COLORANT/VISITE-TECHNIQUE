...
export function defaultSeverityForControl(field = {}, preset = null) {
  ...
  const text =
    `${field?.cle || ''} ${preset?.label || ''} ${preset?.commentaire || ''} ${preset?.reserve || ''}`.toLowerCase();

  // 5 - critique : risque immédiat pour les personnes, sécurité incendie/gaz/électricité.
  if (
    /absence de garde-corps|garde-corps absent|ligne de vie absente|risque de chute|chute de hauteur|danger immédiat|électrocution|electrocution|fuite de gaz|incendie|monoxyde|co\b/.test(
      text
    )
  )
    return 5;
  ...
  if (
    /accès dangereux|acces dangereux|accès difficile|acces difficile|sécur|secur|garde-corps|ligne de vie|trappe dangereuse|skydome|échelle non adaptée|echelle non adaptee/.test(
      text
    )
  )
    return 4;
  ...
  if (
    /panne|ne fonctionne pas|hors service|fuite|défaut de fonctionnement|defaut de fonctionnement|fortement dégrad|fortement degrad|corrosion importante|vibration importante/.test(
      text
    )
  )
    return 3;
