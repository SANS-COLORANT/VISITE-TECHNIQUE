...
async function preremplirVisiteDepuisContexteInterne(db, visiteId) {
  let contexte = await db.getFirstAsync(
    `SELECT v.id,v.date_visite,v.technicien,v.mode_visite,v.statut,v.trame_id,v.installation_id,v.api_remote_local_id,
  ...
  if (trame.id !== 'pre_allumage') {
    const equipements = contexte.installation_id
      ? await db.getAllAsync(
          `SELECT e.* FROM equipements e JOIN installations i ON i.id=e.installation_id WHERE i.site_id=? AND i.id=? AND i.actif=1 AND e.statut='actif'`,
          [contexte.site_id, contexte.installation_id]
        )
      : await db.getAllAsync(
          `SELECT e.* FROM equipements e JOIN installations i ON i.id=e.installation_id WHERE i.site_id=? AND i.actif=1 AND e.statut='actif'`,
          [contexte.site_id]
        );
