...
export async function listerProtocolesEssaisMission(missionId) {
  ...
  const protocols = await db.getAllAsync(
    'SELECT * FROM mission_test_protocols WHERE mission_id=? ORDER BY created_at DESC',
    [missionId]
  );
...
export async function creerProtocoleEssaiComplet({
  missionId,
  label,
  type = null,
  description = null,
  ...
  await db.withTransactionAsync(async () => {
    await db.runAsync('INSERT INTO mission_test_protocols(id,mission_id,type,label,description) VALUES(?,?,?,?,?)', [
      protocolId,
      missionId,
      clean(type),
      clean(label) || 'Protocole',
      clean(description)
    ]);
      ...
      if (clean(step.referenceValue) !== null || clean(step.referenceText) !== null) {
        ...
        await db.runAsync(
          ...
          [
            referenceId,
            missionId,
            clean(step.measureType) || clean(step.label) || 'essai',
            numberOrNull(step.referenceValue),
            clean(step.referenceText),
            clean(step.unit),
            'test_protocol',
            clean(label) || 'Protocole',
            numberOrNull(step.toleranceAbs),
            numberOrNull(step.tolerancePct)
          ]
        );
      }
      await db.runAsync(
        ...
        [
          ...
          protocolId,
          i,
          clean(step.label) || 'Étape ' + (i + 1),
          clean(step.expectedText),
          referenceId
        ]
      );
    }
...
export async function demarrerExecutionEssai({
  missionId,
  protocolId,
  visitId = null,
  siteId = null,
...
function evaluateStatus(step, value) {
  ...
  if (step.tolerance_abs !== null && step.tolerance_abs !== undefined && Math.abs(delta) > Number(step.tolerance_abs))
    return 'deviation';
  if (
    step.tolerance_pct !== null &&
    step.tolerance_pct !== undefined &&
    pct !== null &&
    Math.abs(pct) > Number(step.tolerance_pct)
  )
    return 'deviation';
  return 'ok';
}
...
export async function enregistrerEtapeEssai({
  ...
  const existing = await db.getFirstAsync(
    'SELECT id FROM mission_test_results WHERE test_run_id=? AND test_step_id=?',
    [runId, step.id]
  );
  if (existing?.id) {
    await db.runAsync(
