...
function ChoiceChips({ values, value, onChange, getKey = (v) => String(v), getLabel = (v) => String(v) }) {
  return <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 7 }}>
    {(values || []).map((item) => {
      const key = getKey(item); const selected = String(value ?? '') === String(key);
      return <TouchableOpacity key={String(key)} onPress={() => onChange(key)} style={{ minHeight: 36, justifyContent: 'center', paddingHorizontal: 11, borderRadius: 18, borderWidth: 1, borderColor: selected ? COLORS.orange : COLORS.line, backgroundColor: selected ? COLORS.orangeLight : '#FFF' }}>
        <Text style={{ color: selected ? COLORS.orangeDark : COLORS.inkSoft, fontSize: 11.5, fontWeight: selected ? '900' : '700' }}>{getLabel(item)}</Text>
      </TouchableOpacity>;
    })}
  </View>;
}

function SectionLabel({ children }) { return <Text style={[styles.fieldLabel, { marginTop: 11, marginBottom: 7 }]}>{children}</Text>; }
...
export function IntranetSiteCreationModal({ visible, clientId, onClose, onCreated }) {
  ...
  const load = useCallback(async () => {
    ...
    } catch (e) { Alert.alert('Création Intranet', String(e?.message || e)); }
  ...
  const refreshReferential = async () => {
    ...
    try {
      ...
      Alert.alert('Référentiel indisponible', `${String(e?.message || e)}\n\nLe dernier référentiel enregistré reste utilisable hors connexion.`);
    } finally { setSyncing(false); }
  };

  const reset = () => {
    setNom(''); setSitePrincipal('Non'); setNumero(''); setVoie(''); setComplement(''); setCodePostal(''); setVille(''); setEnergie('GAZ'); setTypeBatiment('Habitation');
  };
  ...
  const canCreate = Boolean(context?.remoteClientId && referential && lotId && nom.trim().length >= 2 && energie.trim() && typeBatiment.trim() && (!codePostal || codePostal.length === 5));

  return <Modal visible={visible} transparent animationType="fade" onRequestClose={() => { if (!creating) onClose?.(); }}>
    <View style={styles.modalOverlay}><View style={[styles.modalSheet, { maxHeight: '92%' }]}>
      <ScrollView keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
        ...
        {referential ? <>
          ...
          <ChoiceChips values={referential.lots || []} value={lotId} onChange={setLotId} getKey={(lot) => String(lot.id)} getLabel={(lot) => lot.nom || `Lot ${lot.id}`} />
          <SectionLabel>Nom du site</SectionLabel>
          <TextInput style={styles.input} value={nom} onChangeText={setNom} placeholder="Résidence / bâtiment / ensemble" maxLength={100} />
          <SectionLabel>Site principal</SectionLabel>
          <ChoiceChips values={referential?.valeurs?.sitePrincipal || ['Oui', 'Non']} value={sitePrincipal} onChange={setSitePrincipal} />
          <SectionLabel>Adresse</SectionLabel>
          <View style={{ flexDirection: 'row', gap: 8 }}><TextInput style={[styles.input, { width: 78 }]} value={numero} onChangeText={setNumero} placeholder="N°" /><TextInput style={[styles.input, { flex: 1 }]} value={voie} onChangeText={setVoie} placeholder="Rue / avenue / voie" /></View>
          <TextInput style={[styles.input, { marginTop: 8 }]} value={complement} onChangeText={setComplement} placeholder="Complément (facultatif)" maxLength={255} />
          <View style={{ flexDirection: 'row', gap: 8, marginTop: 8 }}><TextInput style={[styles.input, { width: 120 }]} value={codePostal} onChangeText={(v) => setCodePostal(v.replace(/\D/g, '').slice(0, 5))} keyboardType="number-pad" maxLength={5} placeholder="Code postal" /><TextInput style={[styles.input, { flex: 1 }]} value={ville} onChangeText={setVille} placeholder="Ville" maxLength={50} /></View>
          <SectionLabel>Énergie</SectionLabel>
          <TextInput style={styles.input} value={energie} onChangeText={setEnergie} placeholder="Ex. GAZ, RCU, ÉLECTRICITÉ" maxLength={255} autoCapitalize="characters" />
...
export function IntranetSiteLocalsPanel({ siteId, onStartVisit }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setData(next);
      if (!trameId && next?.context?.referential?.trames?.[0]?.id != null) setTrameId(String(next.context.referential.trames[0].id));
    } catch (e) { console.warn('Chargement locaux Intranet impossible', e); }
  }, [siteId, trameId]);

  useEffect(() => { load(); return subscribeStructureOutbox(load); }, [load]);

  const refreshReferential = async () => {
    const remoteClientId = data.context?.remoteClientId;
    if (!remoteClientId || syncing) return;
    setSyncing(true);
    try { await syncStructureReferential(remoteClientId); await load(); }
    catch (e) { Alert.alert('Référentiel indisponible', `${String(e?.message || e)}\n\nLes données déjà enregistrées restent utilisables hors connexion.`); }
    finally { setSyncing(false); }
  };
  ...
  const createLocal = async () => {
    ...
    } catch (e) { Alert.alert('Création du local impossible', String(e?.message || e)); }
    finally { setCreating(false); }
  };
  ...
  return <View>
    ...
    {data.locals.length ? data.locals.map((local, index) => {
      ...
      return <View key={local.local_installation_id || local.remoteLocalId || `${local.designation}-${index}`} style={[styles.card, { alignItems: 'flex-start' }]}>
        <View style={{ flex: 1 }}><Text style={styles.cardTitle}>{local.designation}</Text><Text style={styles.cardSub}>{[local.type, local.situation, local.remoteTrameNom].filter(Boolean).join(' · ') || 'Local technique'}</Text>{local.error_message ? <Text style={{ color: '#B42318', fontSize: 10.5, marginTop: 5 }}>{local.error_message}</Text> : null}</View>
        <View style={{ alignItems: 'flex-end', gap: 6 }}><StatusPill status={local.syncStatus} />{canStart ? <TouchableOpacity onPress={() => onStartVisit?.({
    ...
    <Modal visible={modalVisible} transparent animationType="fade" onRequestClose={() => { if (!creating) setModalVisible(false); }}>
      <View style={styles.modalOverlay}><View style={[styles.modalSheet, { maxHeight: '90%' }]}><ScrollView keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
        ...
        <Text style={{ color: COLORS.muted, fontSize: 11.5, lineHeight: 16 }}>Le local est créé localement immédiatement. S’il n’y a pas de réseau, METRA conserve le même creationId jusqu’à l’accusé serveur.</Text>
        <SectionLabel>Désignation</SectionLabel><TextInput style={styles.input} value={designation} onChangeText={setDesignation} maxLength={100} placeholder="Chaufferie" />
        <SectionLabel>Type</SectionLabel><TextInput style={styles.input} value={type} onChangeText={setType} maxLength={255} placeholder="Local technique" />
