...
import {
  creerGroupeSite,
  definirSiteDansGroupe,
  dupliquerSite,
  listerAppartenancesClient,
  listerGroupesClient,
  supprimerGroupeSite
} from './siteOrganizationDb.js';
...
function SiteAddressManager({ visible, clientId, sites = [], onClose, onChanged }) {
  ...
  useEffect(() => {
    if (visible) chargerGroupes().catch(() => {});
  }, [visible, chargerGroupes]);
  ...
  const draft = (site) =>
    edits[site.id] || {
      ...decomposerAdresse(site.adresse),
      nomSite: site.nom_site || '',
      note: site.localisation_note || ''
    };
  ...
  const sauver = async (site) => {
    try {
      ...
      if (adresseChangee && adresse)
        synchroniserCoordonneesSite(site.id, adresse)
          .then(() => onChanged?.())
          .catch(() => {});
    } catch (e) {
  ...
  const demanderSuppressionGroupe = (groupe) =>
    Alert.alert(
      'Supprimer ce groupe ?',
      `Le groupe « ${groupe.nom} » sera supprimé. Les sites et les visites ne seront pas supprimés.`,
      [
        { text: 'Annuler', style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
              await supprimerGroupeSite(groupe.id);
              await chargerGroupes();
            } catch (e) {
              Alert.alert('Groupe', String(e?.message || e));
            }
  ...
  const importerExcel = async () => {
    try {
      ...
      setMessage(
        `${preview.length} lignes · ${nbCreate} nouveaux · ${nbUpdate} modifications${nbErr ? ` · ${nbErr} erreurs` : ''}`
      );
    } catch (e) {
  ...
  const appliquer = async () => {
    ...
    try {
      ...
      synchroniserCoordonneesClient(clientId)
        .then(() => onChanged?.())
        .catch(() => {});
    } catch (e) {
      ...
      <View style={{ flex: 1, backgroundColor: COLORS.bg }}>
        ...
        >
          <View style={{ flexDirection: 'row', alignItems: 'center', gap: 10 }}>
            <View style={{ flex: 1 }}>
              ...
              <Text style={{ color: COLORS.muted, marginTop: 3 }}>
                {sites.length} sites · {stats.avecAdresse} adresses renseignées · {groupes.length} groupe(s)
              </Text>
            </View>
          ...
          <Text style={{ color: COLORS.muted, fontSize: 11.5, marginTop: 8 }}>
            Renommage, adresses, groupes et duplication du patrimoine. Les visites restent toujours liées à leur site
            d'origine.
          </Text>
          <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 8, marginTop: 12 }}>
            <TouchableOpacity style={styles.btnSecondary} disabled={busy} onPress={importerExcel}>
              <Text style={styles.btnSecondaryText}>📄 Importer Excel</Text>
            </TouchableOpacity>
          </View>
          {busy ? (
            <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8, marginTop: 10 }}>
              <ActivityIndicator />
              <Text style={{ color: COLORS.muted }}>{message || 'Traitement…'}</Text>
            </View>
          ) : message ? (
            <Text style={{ color: COLORS.muted, marginTop: 10 }}>{message}</Text>
          ) : null}
        </View>
        ...
        {apercu.length ? (
          <ScrollView contentContainerStyle={{ padding: 16 }}>
            ...
            <Text style={{ color: COLORS.muted, fontSize: 11.5, marginBottom: 10 }}>
              Colonnes reconnues : Site, Numéro, Rue/Voie, Complément, Code postal, Ville, Note d’accès — ou Adresse
              complète.
            </Text>
            {apercu.map((x, i) => (
              <View
                key={`${x.nomSite || 'ligne'}-${i}`}
                style={[styles.card, { marginBottom: 8, alignItems: 'flex-start' }]}
              >
                <View style={{ flex: 1 }}>
                  <Text style={styles.cardTitle}>{x.nomSite || `Ligne ${i + 1}`}</Text>
                  {x.adresse ? (
                    <Text style={styles.cardSub}>{x.adresse}</Text>
                  ) : (
                    <Text style={{ color: '#A26A00', fontSize: 12 }}>Adresse manquante</Text>
                  )}
                  <Text
                    style={{
                      marginTop: 5,
                      fontSize: 12,
                      fontWeight: '700',
                      color: x.action === 'erreur' ? '#B42318' : x.action === 'creer' ? '#18794E' : COLORS.primary
                    }}
                  >
                    {x.action === 'creer'
                      ? 'NOUVEAU SITE'
                      : x.action === 'modifier'
                        ? `MODIFIER ${x.changements?.join(', ') || ''}`
                        : x.action === 'identique'
                          ? 'AUCUN CHANGEMENT'
                          : `ERREUR · ${x.erreur}`}
                  </Text>
            ...
            <View style={{ flexDirection: 'row', gap: 10, marginTop: 8 }}>
              ...
              <TouchableOpacity style={[styles.btnPrimary, { flex: 1 }]} disabled={busy} onPress={appliquer}>
          ...
          <ScrollView keyboardShouldPersistTaps="handled" contentContainerStyle={{ padding: 16 }}>
            ...
            <View style={{ flexDirection: 'row', gap: 8, marginTop: 8, marginBottom: 9 }}>
              <TextInput
                style={[styles.input, { flex: 1 }]}
                value={nouveauGroupe}
                placeholder="Ex. Secteur Nord, Lot 1, VMC…"
                onChangeText={setNouveauGroupe}
              />
              <TouchableOpacity
                style={styles.btnPrimary}
                disabled={busy || !texte(nouveauGroupe)}
                onPress={creerGroupe}
              >
                <Text style={styles.btnPrimaryText}>+ Groupe</Text>
              </TouchableOpacity>
            </View>
            {groupes.length ? (
              <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 7, marginBottom: 16 }}>
                {groupes.map((g) => (
                  ...
                  >
                    <Text style={{ paddingHorizontal: 9, paddingVertical: 7, fontSize: 11, fontWeight: '800' }}>
                      {g.nom} · {g.nb_sites || 0}
                    </Text>
                    <TouchableOpacity
                      onPress={() => demanderSuppressionGroupe(g)}
                      style={{ paddingHorizontal: 8, paddingVertical: 7 }}
                    >
                      <Text style={{ color: COLORS.red || '#B42318', fontWeight: '900' }}>×</Text>
                    </TouchableOpacity>
                  </View>
                ))}
              </View>
              ...
              <Text style={{ color: COLORS.muted, fontSize: 11, marginBottom: 16 }}>
                Aucun groupe. Un site peut appartenir à plusieurs groupes.
              </Text>
            )}
            ...
            {sites.map((site) => {
              ...
              return (
                <View
                  key={site.id}
                  style={[styles.card, { marginBottom: 10, alignItems: 'stretch', flexDirection: 'column' }]}
                >
                  ...
                  <TextInput
                    ...
                    onChangeText={(v) => changer(site, { nomSite: v })}
                  />
                  <View style={{ flexDirection: 'row', gap: 8, marginTop: 10 }}>
                    <TextInput
                      ...
                      onChangeText={(v) => changer(site, { numero: v })}
                    />
                    <TextInput
                      style={[styles.input, { flex: 1 }]}
                      value={d.voie}
                      placeholder="Rue / avenue / voie"
                      onChangeText={(v) => changer(site, { voie: v })}
                    />
                  </View>
                  <TextInput
                    ...
                    onChangeText={(v) => changer(site, { complement: v })}
                  />
                  <View style={{ flexDirection: 'row', gap: 8, marginTop: 8 }}>
                    <TextInput
                      ...
                      maxLength={5}
                      onChangeText={(v) => changer(site, { codePostal: v.replace(/\D/g, '').slice(0, 5) })}
                    />
                    <TextInput
                      style={[styles.input, { flex: 1 }]}
                      value={d.ville}
                      placeholder="Ville"
                      onChangeText={(v) => changer(site, { ville: v })}
                    />
                  </View>
                  <TextInput
                    style={[styles.input, { marginTop: 8 }]}
                    value={d.note}
                    placeholder="Note d’accès / bâtiment / digicode…"
                    onChangeText={(v) => changer(site, { note: v })}
                  />
                  {groupes.length ? (
                    <>
                      <Text style={[styles.fieldLabel, { marginTop: 10 }]}>Groupes</Text>
                            ...
                            >
                              <Text
                                style={{
                                  fontSize: 10.5,
                                  fontWeight: '800',
                                  color: active ? COLORS.orange : COLORS.inkSoft
                                }}
                              >
                                {active ? '✓ ' : ''}
                                {g.nom}
                  ...
                  <View style={{ flexDirection: 'row', gap: 8, marginTop: 10 }}>
                    ...
                    <TouchableOpacity
                      style={[styles.btnSecondary, { flex: 1 }]}
                      disabled={busy}
                      onPress={() => demanderDuplication(site)}
                    >
                      <Text style={styles.btnSecondaryText}>⧉ Dupliquer</Text>
                    </TouchableOpacity>
                  </View>
                </View>
              );
