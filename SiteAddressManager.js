...
function SiteAddressManager({ visible, clientId, sites = [], onClose, onChanged }) {
  ...
  return <Modal visible={visible} animationType="slide" onRequestClose={onClose}>
    <View style={{ flex: 1, backgroundColor: COLORS.bg }}>
      <View style={{ paddingHorizontal: 16, paddingTop: 18, paddingBottom: 10, backgroundColor: 'rgba(255,255,255,0.55)', borderBottomWidth: 1, borderBottomColor: '#E3E5E8' }}>
        <View style={{ flexDirection: 'row', alignItems: 'center', gap: 10 }}><View style={{ flex: 1 }}><Text style={{ fontSize: 20, fontFamily: FONTS.bold }}>Gérer les sites</Text><Text style={{ color: COLORS.muted, marginTop: 3 }}>{sites.length} sites · {stats.avecAdresse} adresses renseignées · {groupes.length} groupe(s)</Text></View><TouchableOpacity accessibilityRole="button" accessibilityLabel="Fermer" onPress={onClose} style={{ padding: 10 }}><CvcIcon name="close" size={19} color={COLORS.orangeDark} strokeWidth={2.1} /></TouchableOpacity></View>
      ...
      {apercu.length ? <ScrollView contentContainerStyle={{ padding: 16 }}><Text style={styles.sectionLabel}>Aperçu avant import</Text><Text style={{ color: COLORS.muted, fontSize: 11.5, marginBottom: 10 }}>Colonnes reconnues : Site, Numéro, Rue/Voie, Complément, Code postal, Ville, Note d’accès — ou Adresse complète.</Text>{apercu.map((x, i) => <View key={`${x.nomSite || 'ligne'}-${i}`} style={[styles.card, { marginBottom: 8, alignItems: 'flex-start' }]}><View style={{ flex: 1 }}><Text style={styles.cardTitle}>{x.nomSite || `Ligne ${i + 1}`}</Text>{x.adresse ? <Text style={styles.cardSub}>{x.adresse}</Text> : <Text style={{ color: '#A26A00', fontSize: 12 }}>Adresse manquante</Text>}<Text style={{ marginTop: 5, fontSize: 12, fontFamily: FONTS.bodyBold, color: x.action === 'erreur' ? '#B42318' : x.action === 'creer' ? '#18794E' : COLORS.primary }}>{x.action === 'creer' ? 'NOUVEAU SITE' : x.action === 'modifier' ? `MODIFIER ${x.changements?.join(', ') || ''}` : x.action === 'identique' ? 'AUCUN CHANGEMENT' : `ERREUR · ${x.erreur}`}</Text></View></View>)}<View style={{ flexDirection: 'row', gap: 10, marginTop: 8 }}><TouchableOpacity style={[styles.btnSecondary, { flex: 1 }]} disabled={busy} onPress={() => setApercu([])}><Text style={styles.btnSecondaryText}>Annuler</Text></TouchableOpacity><TouchableOpacity style={[styles.btnPrimary, { flex: 1 }]} disabled={busy} onPress={appliquer}><ButtonGlow /><Text style={styles.btnPrimaryText}>Appliquer</Text></TouchableOpacity></View></ScrollView> : <ScrollView keyboardShouldPersistTaps="handled" contentContainerStyle={{ padding: 16 }}>
        <Text style={styles.sectionLabel}>Groupes de sites</Text>
        <View style={{ flexDirection: 'row', gap: 8, marginTop: 8, marginBottom: 9 }}><TextInput style={[styles.input, { flex: 1 }]} value={nouveauGroupe} placeholder="Ex. Secteur Nord, Lot 1, VMC…" onChangeText={setNouveauGroupe}/><TouchableOpacity style={styles.btnPrimary} disabled={busy || !texte(nouveauGroupe)} onPress={creerGroupe}><ButtonGlow /><Text style={styles.btnPrimaryText}>+ Groupe</Text></TouchableOpacity></View>
        {groupes.length ? <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 7, marginBottom: 16 }}>{groupes.map((g) => <View key={g.id} style={{ flexDirection: 'row', alignItems: 'center', borderRadius: 10, borderWidth: 1, borderColor: 'rgba(22,21,15,0.1)', backgroundColor: 'rgba(255,255,255,0.82)' }}><Text style={{ paddingHorizontal: 9, paddingVertical: 7, fontSize: 11, fontFamily: FONTS.bold }}>{g.nom} · {g.nb_sites || 0}</Text><TouchableOpacity accessibilityRole="button" accessibilityLabel="Fermer" onPress={() => demanderSuppressionGroupe(g)} style={{ paddingHorizontal: 8, paddingVertical: 7 }}><CvcIcon name="close" size={16} color={COLORS.red || '#B42318'} strokeWidth={2.1} /></TouchableOpacity></View>)}</View> : <Text style={{ color: COLORS.muted, fontSize: 11, marginBottom: 16 }}>Aucun groupe. Un site peut appartenir à plusieurs groupes.</Text>}
        ...
        {sites.map((site) => { const d = draft(site); return <View key={site.id} style={[styles.card, { marginBottom: 10, alignItems: 'stretch', flexDirection: 'column' }]}>
          ...
          {groupes.length ? <><Text style={[styles.fieldLabel, { marginTop: 10 }]}>Groupes</Text><View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 6 }}>{groupes.map((g) => { const active = membershipSet.has(`${site.id}||${g.id}`); return <TouchableOpacity key={g.id} onPress={() => basculerGroupe(site, g)} style={{ paddingHorizontal: 9, paddingVertical: 6, borderRadius: 9, borderWidth: 1, borderColor: active ? COLORS.orange : COLORS.line, backgroundColor: active ? '#FFF3E8' : '#fff' }}><Text style={{ fontSize: 10.5, fontFamily: FONTS.bold, color: active ? COLORS.orange : COLORS.inkSoft }}>{active ? '✓ ' : ''}{g.nom}</Text></TouchableOpacity>; })}</View></> : null}
          <View style={{ flexDirection: 'row', gap: 8, marginTop: 10 }}><TouchableOpacity style={[styles.btnPrimary, { flex: 1 }]} disabled={busy} onPress={() => sauver(site)}><ButtonGlow /><Text style={styles.btnPrimaryText}>Enregistrer</Text></TouchableOpacity><TouchableOpacity style={[styles.btnSecondary, { flex: 1 }]} disabled={busy} onPress={() => demanderDuplication(site)}><Text style={styles.btnSecondaryText}>⧉ Dupliquer</Text></TouchableOpacity></View>
        </View>; })}
      </ScrollView>}
    </View>
  </Modal>;
