...
async function sectionHasData(db, visiteId, code) {
  ...
  if (c?.ok) return true;
  const ctrl = await db.getFirstAsync(
...
function canonicalName(localName) {
  ...
  const sst = raw.match(/^SST\s*(\d+)/i);
  if (sst) return `SST ${Number(sst[1])}`;
...
async function materialiserLocauxImportes(db, visiteId) {
  ...
  const maxRow = await db.getFirstAsync(`SELECT COALESCE(MAX(ordre),-1) n FROM pre_allumage_locaux WHERE visite_id=?`, [
    visiteId
  ]);
  ...
  for (const name of candidates) {
    ...
    for (const r of physiques.filter((x) => officialMatches(x, name))) {
      await db.runAsync(`UPDATE pre_allumage_rubriques SET local_id=?,modifie_le=datetime('now') WHERE id=?`, [
        id,
        r.id
      ]);
    }
...
export async function preparerStructurePreAllumage(visiteId) {
  ...
  const existingCh = await db.getFirstAsync(
    `SELECT id FROM pre_allumage_locaux WHERE visite_id=? AND type_code='chaufferie' LIMIT 1`,
    [visiteId]
  );
  if (!existingCh) {
    const orphan = await db.getAllAsync(
...
export async function remapperLocalVersRubriquesOfficielles(visiteId, localId) {
  ...
  for (const panelId of panels) {
    ...
    for (const r of dynamic || []) {
      ...
      await db.runAsync(`DELETE FROM controles_visite WHERE visite_id=? AND section_code=?`, [
        visiteId,
        r.section_code
      ]);
