  ...
  return (
    <View style={styles.compteurRow}>
      <View style={styles.compteurRowTop}>
        ...
        />
        <TouchableOpacity onPress={retirer}>
        ...
        <View style={styles.uniteRow}>
          {UNITES.map((u) => (
            <TouchableOpacity
              ...
              onPress={() => {
                setUnite(u);
                upsertCompteurChamp(compteur.id, 'unite', u).catch((e) =>
                  console.warn('Unité compteur non sauvegardée', e)
                );
              }}
            >
...
export function OptimizedRelevesPanel({ visiteId, onSaved }) {
  ...
  const rows = useMemo(() => {
    const result = [];
    champsPression.forEach((f) =>
      result.push({ type: 'champ', id: `pression-${f.cle}`, section: 'releves.compteurs', field: f })
    );
    ...
    champsTemp.forEach((f) =>
      result.push({ type: 'champ', id: `temp-${f.cle}`, section: 'releves.temperatures', field: f })
    );
    return result;
    ...
    <>
      <FlatList
        ...
        renderItem={({ item }) => {
          ...
          if (item.type === 'compteur')
            return <CompteurCard compteur={item.compteur} visiteId={visiteId} onRemove={retirerLocalement} />;
          if (item.type === 'ajout')
            return (
          ...
          <View style={styles.modalSheet}>
            ...
            <View style={styles.modalActions}>
              <TouchableOpacity
                ...
                onPress={() => setAjoutCompteurVisible(false)}
              ...
              <TouchableOpacity
                style={[
                  styles.btnPrimary,
                  (!nomCompteurChoisi && !nomCompteurLibre.trim()) || creationEnCours ? { opacity: 0.45 } : null
                ]}
