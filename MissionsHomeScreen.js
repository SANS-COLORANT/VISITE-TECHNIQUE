  ...
  return (
    ...
    >
      ...
      <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 6, marginTop: 12 }}>
        <View style={{ borderRadius: 9, backgroundColor: MISSION_COLORS.accentSoft, paddingHorizontal: 8, paddingVertical: 5 }}><Text style={{ color: COLORS.inkSoft, fontSize: 9.5 }}>{mission.site_count || 0} site(s)</Text></View>
        <View style={{ borderRadius: 9, backgroundColor: MISSION_COLORS.accentSoft, paddingHorizontal: 8, paddingVertical: 5 }}><Text style={{ color: COLORS.inkSoft, fontSize: 9.5 }}>{mission.visit_count || 0} visite(s)</Text></View>
        <View style={{ borderRadius: 9, backgroundColor: (mission.open_point_count || 0) > 0 ? MISSION_COLORS.accentLight : MISSION_COLORS.accentSoft, paddingHorizontal: 8, paddingVertical: 5 }}><Text style={{ color: (mission.open_point_count || 0) > 0 ? MISSION_COLORS.accentDark : COLORS.inkSoft, fontSize: 9.5, fontWeight: (mission.open_point_count || 0) > 0 ? '800' : '500' }}>{mission.open_point_count || 0} point(s) ouvert(s)</Text></View>
      </View>
    </TouchableOpacity>
  );
}
...
export function MissionsHomeScreen({ navigation }) {
  ...
  const reload = useCallback(async () => {
    setLoading(true);
    try { setMissions(await listerMissions({ limit: 80 })); }
  ...
  const resume = useMemo(() => missions
    .filter((m) => ['active', 'waiting', 'draft'].includes(m.status))
    .slice(0, 3), [missions]);
  ...
  const visible = useMemo(() => {
    ...
    return missions.filter((m) => {
      ...
      if (!q) return true;
      return `${m.label || ''} ${m.client_name || ''} ${m.family || ''} ${m.type || ''} ${m.reference || ''}`.toLocaleLowerCase('fr').includes(q);
    });
  }, [missions, query, filter]);

  const technicalVisitSwipeResponder = useMemo(() => PanResponder.create({
    ...
    <>
      ...
      <View style={{ marginTop: 6, marginBottom: 10 }}>
        ...
        <View style={{ flexDirection: 'row', flexWrap: 'wrap' }}>
          {FILTERS.map(([key, label]) => <FilterChip key={key} active={filter === key} label={label} onPress={() => setFilter(key)} />)}
        </View>
      </View>
