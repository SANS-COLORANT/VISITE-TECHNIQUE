...
export function SitePhotoPreparationOption({ clientId, siteIds, enabled, onEnabledChange, onPlanChange, activated, disabled }) {
  ...
  return <View style={{ padding: 10, borderWidth: 1, borderColor: 'rgba(22,21,15,0.1)', borderRadius: 10, marginBottom: 8 }}>
    <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}><Text style={{ flex: 1, color: COLORS.ink, fontSize: 13, fontFamily: FONTS.bodyBold }}>Inclure les photos des dernières visites</Text><Switch accessibilityLabel="Inclure les photos des sites sélectionnés" value={enabled} onValueChange={onEnabledChange} disabled={disabled} /></View>
    {enabled ? <>
      ...
      {!loading && (!plan.ready || error) ? <TouchableOpacity disabled={disabled} onPress={() => setAttempt((n) => n + 1)} style={{ minHeight: 48, justifyContent: 'center' }}><Text style={{ color: COLORS.primary, fontFamily: FONTS.bodyBold }}>Réessayer les photos</Text></TouchableOpacity> : null}
