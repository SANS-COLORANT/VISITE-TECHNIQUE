...
function SeriesChart({ series, width }) {
  ...
  if (points.length < 2)
    return <Text style={{ color: COLORS.inkFaint, fontSize: 9 }}>Pas assez de points pour afficher le graphe.</Text>;
  ...
  return (
    <Svg width={w} height={h}>
      ...
      <SvgText x="12" y="16" fontSize="9" fill={COLORS.inkSoft}>
        {max.toFixed(2)} {series.unit || ''}
      </SvgText>
      <SvgText x="12" y={h - 5} fontSize="9" fill={COLORS.inkSoft}>
        {min.toFixed(2)} {series.unit || ''}
      </SvgText>
    </Svg>
  );
}
...
export function MissionMeasurementsScreen({ navigation, route }) {
  ...
  const load = useCallback(async () => {
    ...
    const [m, t, i, s, e] = await Promise.all([
      ...
      db.getAllAsync(
        'SELECT e.*,s.name AS site_name FROM mission_equipment e JOIN mission_site_links l ON l.site_id=e.site_id LEFT JOIN mission_sites s ON s.id=e.site_id WHERE l.mission_id=? ORDER BY s.name,e.type,e.brand,e.model LIMIT 2000',
        [missionId]
      )
    ]);
    setData(m);
  ...
  const filteredEquipment = useMemo(
    () => equipment.filter((e) => !draft.siteId || e.site_id === draft.siteId),
    [equipment, draft.siteId]
  );
  ...
  const saveMeasure = async () => {
    ...
    try {
      ...
      if (result?.anomalyStatus === 'to_check')
        Alert.alert(
          'À contrôler',
          'La valeur dépasse la tolérance définie. METRA conserve l’écart sans conclure automatiquement à un défaut.'
        );
    } catch (e) {
      Alert.alert('Mesure impossible', String(e?.message || e));
    }
  };

  const createType = async () => {
    if (!customType.label.trim()) return;
    const id = await creerTypeMesureMission({
      missionId,
      label: customType.label,
      unit: customType.unit,
      expectedMin: customType.min,
      expectedMax: customType.max
    });
  ...
  const importSeries = async () => {
    try {
      ...
      Alert.alert('Import de série impossible', String(e?.message || e));
    }
  };
    ...
    <View style={{ flex: 1, backgroundColor: MISSION_COLORS.bg }}>
      <ScrollView contentContainerStyle={{ padding: 16, paddingBottom: 110 }}>
        ...
        <Text style={{ color: COLORS.inkSoft, fontSize: 10.5, lineHeight: 15 }}>
          Mesuré, documentaire, GTC, compteur, simulé ou estimé : la source reste visible. Une référence est séparée de
          la mesure pour comparer sans ambiguïté.
        </Text>
        ...
        <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 8, marginTop: 12 }}>
          ...
          >
            <Text style={[styles.btnPrimaryText, missionStyles.primaryButtonText]}>＋ Mesure</Text>
          </TouchableOpacity>
          <TouchableOpacity
            style={[styles.btnSecondary, missionStyles.secondaryButton]}
            onPress={() => setTypeVisible(true)}
          >
            <Text style={[styles.btnSecondaryText, missionStyles.secondaryButtonText]}>＋ Type personnalisé</Text>
          </TouchableOpacity>
          <TouchableOpacity
            style={[styles.btnSecondary, missionStyles.secondaryButton]}
            onPress={() => setInstrumentVisible(true)}
          >
            <Text style={[styles.btnSecondaryText, missionStyles.secondaryButtonText]}>＋ Instrument</Text>
          </TouchableOpacity>
          <TouchableOpacity
            style={[styles.btnSecondary, missionStyles.secondaryButton]}
          ...
          >
            <Text style={[styles.btnSecondaryText, missionStyles.secondaryButtonText]}>
              Campagnes terrain · valeur → suivant
            </Text>
          </TouchableOpacity>
        </View>

        <View style={[missionStyles.card, { padding: 12, marginTop: 12 }]}>
          <Text style={{ color: MISSION_COLORS.accentStrong, fontSize: 11, fontWeight: '900' }}>
            Importer une série / campagne
          </Text>
          <Text style={{ color: COLORS.inkFaint, fontSize: 8.7, marginTop: 3 }}>
            CSV / XLSX : METRA conserve le fichier brut, calcule min/max/moyenne et mémorise un échantillon graphique
            léger.
          </Text>
          <View style={{ flexDirection: 'row', gap: 8, marginTop: 9 }}>
            <TextInput
              ...
              value={seriesType}
              onChangeText={setSeriesType}
              placeholder="Type série"
            />
            <TextInput
              style={[styles.input, missionStyles.input, { width: 90 }]}
              value={seriesUnit}
              onChangeText={setSeriesUnit}
              placeholder="Unité"
            />
            <TouchableOpacity style={[styles.btnSecondary, missionStyles.secondaryButton]} onPress={importSeries}>
              <Text style={[styles.btnSecondaryText, missionStyles.secondaryButtonText]}>Importer</Text>
            </TouchableOpacity>
          </View>
        </View>
        ...
        {(data.measures || []).map((m) => (
          <View key={m.id} style={[missionStyles.card, { padding: 11, marginBottom: 7 }]}>
            <View style={{ flexDirection: 'row', alignItems: 'flex-start' }}>
              <View style={{ flex: 1 }}>
                ...
                <Text style={{ color: COLORS.inkFaint, fontSize: 8.5, marginTop: 2 }}>
                  {[m.source_label || m.source_type, m.instrument_label].filter(Boolean).join(' · ') ||
                    'Source non précisée'}
                </Text>
                {m.reference_number !== null && m.reference_number !== undefined ? (
                  <Text style={{ color: COLORS.inkSoft, fontSize: 8.8, marginTop: 3 }}>
                    Référence {m.reference_number} {m.unit || ''} ·{' '}
                    {m.reference_source_label || m.reference_source_type || ''}
                  </Text>
                ) : null}
              </View>
              <View style={{ alignItems: 'flex-end' }}>
                <Text style={{ color: MISSION_COLORS.accentStrong, fontSize: 13, fontWeight: '900' }}>
                  {m.value_number ?? m.value_text ?? '/'} {m.unit || ''}
                </Text>
                {m.delta_percent !== null && m.delta_percent !== undefined ? (
                  <Text
                    style={{
                      color: m.anomaly_status === 'to_check' ? '#8A5B14' : COLORS.inkFaint,
                      fontSize: 8.7,
                      marginTop: 3
                    }}
                  >
                    {Number(m.delta_percent).toFixed(1)} % {m.anomaly_status === 'to_check' ? '· À contrôler' : ''}
                  </Text>
                ) : null}
              </View>
            </View>
        ...
        {(data.series || []).map((s) => (
          <TouchableOpacity
            key={s.id}
            onPress={() => setSeriesVisible(s)}
            style={[missionStyles.card, { padding: 11, marginBottom: 7 }]}
          >
            <Text style={{ color: COLORS.ink, fontSize: 10.5, fontWeight: '900' }}>
              {s.type} · {s.sample_count} point(s)
            </Text>
            <Text style={{ color: COLORS.inkFaint, fontSize: 8.7, marginTop: 3 }}>
              Min {s.min_value} · Moy {Number(s.avg_value || 0).toFixed(2)} · Max {s.max_value} {s.unit || ''}
            </Text>
          </TouchableOpacity>
        ))}
      </ScrollView>
      ...
      <Modal visible={measureVisible} animationType="slide" onRequestClose={() => setMeasureVisible(false)}>
        <View style={{ flex: 1, backgroundColor: MISSION_COLORS.bg }}>
          <ScrollView contentContainerStyle={{ padding: 16, paddingTop: 52, paddingBottom: 90 }}>
            ...
            <View style={{ flexDirection: 'row', flexWrap: 'wrap', marginBottom: 8 }}>
              {sites.map((s) => (
                <Chip
                  ...
                  onPress={() => setDraft((p) => ({ ...p, siteId: p.siteId === s.id ? '' : s.id, equipmentId: '' }))}
                />
              ))}
            </View>
            ...
            <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{ maxHeight: 46, marginBottom: 8 }}>
              {filteredEquipment.map((e) => (
                <Chip
                  ...
                  selected={draft.equipmentId === e.id}
                  onPress={() => setDraft((p) => ({ ...p, equipmentId: p.equipmentId === e.id ? '' : e.id }))}
                />
              ))}
            </ScrollView>
            ...
            <View
              style={{ flexDirection: 'row', flexWrap: 'wrap', maxHeight: 190, overflow: 'hidden', marginBottom: 8 }}
            >
              {types.slice(0, 26).map((t) => (
                <Chip key={t.id} label={t.label} selected={draft.typeKey === t.key} onPress={() => selectType(t)} />
              ))}
            </View>
            <Field
              label="Type libre si nécessaire"
              value={draft.typeLabel}
              onChangeText={(v) => setDraft((p) => ({ ...p, typeLabel: v, typeKey: p.typeKey || 'free' }))}
            />
            <View style={{ flexDirection: 'row', gap: 8 }}>
              <View style={{ flex: 1 }}>
                <Field
                  ...
                  value={draft.value}
                  onChangeText={(v) => setDraft((p) => ({ ...p, value: v }))}
                  keyboardType="decimal-pad"
                />
              </View>
              <View style={{ width: 100 }}>
                <Field label="Unité" value={draft.unit} onChangeText={(v) => setDraft((p) => ({ ...p, unit: v }))} />
              </View>
            </View>
            <Field
              label="Valeur texte si non numérique"
              value={draft.valueText}
              onChangeText={(v) => setDraft((p) => ({ ...p, valueText: v }))}
            />
            ...
            <View style={{ flexDirection: 'row', flexWrap: 'wrap', marginBottom: 5 }}>
              {SOURCES.map(([k, l]) => (
                <Chip
                  key={k}
                  label={l}
                  selected={draft.sourceType === k}
                  onPress={() => setDraft((p) => ({ ...p, sourceType: k }))}
                />
              ))}
            </View>
            <Field
              label="Source / libellé"
              value={draft.sourceLabel}
              onChangeText={(v) => setDraft((p) => ({ ...p, sourceLabel: v }))}
              placeholder="Écran GTC, compteur, rapport exploitant…"
            />
            ...
            <View style={{ flexDirection: 'row', gap: 8 }}>
              <View style={{ flex: 1 }}>
                <Field
                  ...
                  onChangeText={(v) => setDraft((p) => ({ ...p, referenceValue: v }))}
                  keyboardType="decimal-pad"
                />
              </View>
              <View style={{ flex: 1 }}>
                <Field
                  ...
                  onChangeText={(v) => setDraft((p) => ({ ...p, referenceText: v }))}
                />
              </View>
            </View>
            <View style={{ flexDirection: 'row', flexWrap: 'wrap', marginBottom: 5 }}>
              {REFERENCE_SOURCES.map(([k, l]) => (
                <Chip
                  key={k}
                  label={l}
                  selected={draft.referenceSourceType === k}
                  onPress={() => setDraft((p) => ({ ...p, referenceSourceType: k }))}
                />
              ))}
            </View>
            <Field
            ...
            <View style={{ flexDirection: 'row', flexWrap: 'wrap', marginBottom: 6 }}>
              {instruments.map((i) => (
                <Chip
                  key={i.id}
                  label={i.label}
                  selected={draft.instrumentId === i.id}
                  onPress={() => setDraft((p) => ({ ...p, instrumentId: p.instrumentId === i.id ? '' : i.id }))}
                />
              ))}
            </View>
            <Field
            ...
            <View style={{ flexDirection: 'row', gap: 8, marginTop: 10 }}>
              <TouchableOpacity
                style={[styles.btnSecondary, missionStyles.secondaryButton]}
                onPress={() => setMeasureVisible(false)}
              >
        ...
        <View style={styles.modalOverlay}>
          <View style={[styles.modalSheet, missionStyles.modalSheet]}>
            ...
            <Field
              ...
              onChangeText={(v) => setCustomType((p) => ({ ...p, label: v }))}
            />
            <Field
              label="Unité"
              value={customType.unit}
              onChangeText={(v) => setCustomType((p) => ({ ...p, unit: v }))}
            />
            <View style={{ flexDirection: 'row', gap: 8 }}>
              <View style={{ flex: 1 }}>
                <Field
                  label="Min attendu"
                  value={customType.min}
                  onChangeText={(v) => setCustomType((p) => ({ ...p, min: v }))}
                  keyboardType="decimal-pad"
                />
              </View>
              <View style={{ flex: 1 }}>
                <Field
                  label="Max attendu"
                  value={customType.max}
                  onChangeText={(v) => setCustomType((p) => ({ ...p, max: v }))}
                  keyboardType="decimal-pad"
                />
              </View>
            </View>
            <View style={styles.modalActions}>
              <TouchableOpacity
                style={[styles.btnSecondary, missionStyles.secondaryButton]}
                onPress={() => setTypeVisible(false)}
              >
                <Text style={[styles.btnSecondaryText, missionStyles.secondaryButtonText]}>Annuler</Text>
              </TouchableOpacity>
              <TouchableOpacity style={[styles.btnPrimary, missionStyles.primaryButton]} onPress={createType}>
      ...
      >
        <View style={styles.modalOverlay}>
          ...
          >
            ...
            <Field
              label="Nom"
              value={instrumentDraft.label}
              onChangeText={(v) => setInstrumentDraft((p) => ({ ...p, label: v }))}
            />
            <Field
              label="Marque"
              value={instrumentDraft.brand}
              onChangeText={(v) => setInstrumentDraft((p) => ({ ...p, brand: v }))}
            />
            <Field
              label="Modèle"
              value={instrumentDraft.model}
              onChangeText={(v) => setInstrumentDraft((p) => ({ ...p, model: v }))}
            />
            <Field
              label="N° série"
              value={instrumentDraft.serial}
              onChangeText={(v) => setInstrumentDraft((p) => ({ ...p, serial: v }))}
            />
            <Field
              label="Étalonnage AAAA-MM-JJ"
              value={instrumentDraft.calibration}
              onChangeText={(v) => setInstrumentDraft((p) => ({ ...p, calibration: v }))}
            />
            <Field
              label="Prochain étalonnage"
              value={instrumentDraft.due}
              onChangeText={(v) => setInstrumentDraft((p) => ({ ...p, due: v }))}
            />
            <View style={styles.modalActions}>
              <TouchableOpacity
                style={[styles.btnSecondary, missionStyles.secondaryButton]}
              ...
              <TouchableOpacity style={[styles.btnPrimary, missionStyles.primaryButton]} onPress={createInstrument}>
                <Text style={[styles.btnPrimaryText, missionStyles.primaryButtonText]}>Enregistrer</Text>
              </TouchableOpacity>
            </View>
          </ScrollView>
        ...
        <View style={styles.modalOverlay}>
          <View style={[styles.modalSheet, missionStyles.modalSheet]}>
            ...
            <Text style={{ color: COLORS.inkSoft, fontSize: 9.5, marginTop: 5 }}>
              {seriesVisible?.sample_count || 0} point(s) · min {seriesVisible?.min_value} · moy{' '}
              {Number(seriesVisible?.avg_value || 0).toFixed(2)} · max {seriesVisible?.max_value}{' '}
              {seriesVisible?.unit || ''}
            </Text>
            <View style={styles.modalActions}>
              <TouchableOpacity
                style={[styles.btnPrimary, missionStyles.primaryButton]}
                onPress={() => setSeriesVisible(null)}
              >
