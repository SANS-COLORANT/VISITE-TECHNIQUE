...
export function MetraLoadingScreen() {
  ...
  const animationStyles = useMemo(() => {
    const wingOpacity = progress.interpolate({
      inputRange: [0, 0.22, 1],
      outputRange: [0, 1, 1],
    });
    const wingX = progress.interpolate({
      inputRange: [0, 0.45, 1],
      outputRange: [65, 0, 0],
    });
    const wingY = progress.interpolate({
      inputRange: [0, 0.45, 1],
      outputRange: [-35, 0, 0],
    });
    const wingRotation = progress.interpolate({
      inputRange: [0, 0.45, 1],
      outputRange: ['8deg', '0deg', '0deg'],
    });
    const wingRotationLeft = progress.interpolate({
      inputRange: [0, 0.45, 1],
      outputRange: ['-8deg', '0deg', '0deg'],
    });
    const headOpacity = progress.interpolate({
      inputRange: [0, 0.237, 0.487, 1],
      outputRange: [0, 0, 1, 1],
    });
    const headScale = progress.interpolate({
      inputRange: [0, 0.237, 0.487, 0.637, 1],
      outputRange: [0.3, 0.3, 1.15, 1, 1],
    });
    const bodyOpacity = progress.interpolate({
      inputRange: [0, 0.295, 0.545, 1],
      outputRange: [0, 0, 1, 1],
    });
    const bodyScale = progress.interpolate({
      inputRange: [0, 0.295, 0.545, 0.695, 1],
      outputRange: [0.3, 0.3, 1.15, 1, 1],
    });
    const wordOpacity = progress.interpolate({
      inputRange: [0, 0.531, 0.691, 1],
      outputRange: [0, 0, 1, 1],
    });
    ...
    return {
      wingLeft: {
        opacity: wingOpacity,
        transform: [
          { translateX: Animated.multiply(wingX, -1) },
          { translateY: wingY },
      ...
      wingRight: {
        opacity: wingOpacity,
        transform: [{ translateX: wingX }, { translateY: wingY }, { rotate: wingRotation }],
      },
  ...
  return (
    <View style={styles.screen} accessibilityLabel="Chargement de METRA">
      <View style={styles.canvas}>
        <Animated.Image
          source={ASSETS.wingLeft}
          resizeMode="contain"
          style={layerStyle(styles.wingLeft, animationStyles.wingLeft)}
        />
        <Animated.Image
          source={ASSETS.wingRight}
          resizeMode="contain"
          style={layerStyle(styles.wingRight, animationStyles.wingRight)}
        />
        <Animated.Image
          source={ASSETS.head}
          resizeMode="contain"
          style={layerStyle(styles.head, animationStyles.head)}
        />
        <Animated.Image
          source={ASSETS.body}
          resizeMode="contain"
          style={layerStyle(styles.body, animationStyles.body)}
        />
