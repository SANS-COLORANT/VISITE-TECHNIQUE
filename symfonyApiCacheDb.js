...
function invalidateDirectorySnapshot() {
...
function normalizeSubCategory(subCategory, categoryId, index) {
  ...
  return {
    ...
    criteres: ordered(subCategory?.criteres, (criterion, criterionIndex) =>
      normalizeCriterion(criterion, categoryId, id, criterionIndex)
    )
  };
}
...
function normalizeCategory(category, index) {
  ...
  return {
    ...
    sousCategories: ordered(category?.sousCategories ?? category?.sous_categories, (subCategory, subIndex) =>
      normalizeSubCategory(subCategory, id, subIndex)
    )
  };
}
...
function countCriteria(trame) {
  return list(trame?.categories).reduce(
    (total, category) =>
      total +
      list(category?.sousCategories).reduce(
        (subtotal, subCategory) => subtotal + list(subCategory?.criteres).length,
        0
      ),
    0
  );
...
export function normalizePreparationPayload(payload = {}) {
  const normalizedVisits = list(payload?.visites)
    .map((entry) => {
      const local =
        entry?.local && typeof entry.local === 'object'
          ? {
              ...entry.local,
          ...
          : null;
      const site =
        entry?.site && typeof entry.site === 'object'
          ? {
              ...entry.site,
...
export async function getApiSyncState() {
  ...
  return (
    (await database.getFirstAsync(`SELECT * FROM api_sync_state WHERE id=1`)) || { id: 1, base_url: DEFAULT_BASE_URL }
  );
}
    ...
    for (const client of clients || []) {
      ...
      await database.runAsync(
        ...
        [
          id,
          null,
          clean(client?.nom) || `Client ${id}`,
          client?.categorie ?? null,
          client?.code_everwin ?? null,
          client?.adresse_postale ?? null,
          client?.ville ?? null,
          agence?.id != null ? clean(agence.id) : null,
          agence?.libelle ?? null,
          json(client)
        ]
      );
...
export async function cachePreparation(remoteClientId, payload, { partial = false } = {}) {
  ...
  if (!clientId) throw new Error('Client API requis');
  const database = await db();
  ...
  await database.withTransactionAsync(async () => {
    ...
    for (const visite of visites) {
      const siteId = remoteId(visite?.site?.id);
      if (!siteId) continue;
      await database.runAsync(
      ...
      const localId = remoteId(visite?.local?.id);
      if (!localId) continue;
      ...
      await database.runAsync(
        ...
        [
          localId,
          siteId,
          visite?.local?.designation ?? null,
          visite?.trame?.id ?? null,
          visite?.trame?.nom ?? null,
          visite?.derniereVisite?.id ?? null,
          visite?.derniereVisite?.date ?? null,
          visite?.derniereVisite?.statut ?? null,
          json(visite),
          1,
          Number(meta.criteriaCount || 0),
          Number(meta.historicalCriteriaCount || 0),
          Number(meta.remarkCount || 0),
          Number(meta.materialCount || 0)
        ]
      );
    }
  });
...
export async function searchCachedDirectory(query = '') {
  ...
  return {
    clients: snapshot.clients.filter((c) =>
      normalize([c.nom, c.categorie, c.code_everwin, c.ville, c.agence_libelle, c.adresse_postale].join(' ')).includes(
        q
      )
    ),
    sites: snapshot.sites.filter((s) =>
      normalize(
        [s.nom, s.client_nom, s.client_ville, s.client_code_everwin, s.trames, s.local_designations].join(' ')
      ).includes(q)
    )
  };
}
...
export async function listCachedSites(remoteClientId) {
  return (await db()).getAllAsync(
    ...
    GROUP BY cs.remote_client_id,s.remote_site_id
    ORDER BY s.nom`,
    [clean(remoteClientId)]
  );
}

export async function listCachedLocals(remoteSiteId) {
  return (await db()).getAllAsync(
    `SELECT * FROM api_local_links WHERE remote_site_id=? AND remote_present=1 ORDER BY designation`,
    [clean(remoteSiteId)]
  );
}
...
export async function getCachedLocalReference(remoteLocalId) {
  const row = await (
    await db()
  ).getFirstAsync(`SELECT reference_json FROM api_local_links WHERE remote_local_id=? AND remote_present=1`, [
    clean(remoteLocalId)
  ]);
  if (!row?.reference_json) return null;
  try {
    return JSON.parse(row.reference_json);
  } catch {
    return null;
  }
}
...
export async function setCachedLocalInstallation(remoteLocalId, localInstallationId) {
  ...
  await (
    await db()
  ).runAsync(`UPDATE api_local_links SET local_installation_id=?,synced_at=synced_at WHERE remote_local_id=?`, [
    installationId,
    localId
  ]);
}
...
export async function materializeCachedClient(remoteClientId) {
  const database = await db();
  const remote = await database.getFirstAsync(`SELECT * FROM api_client_links WHERE remote_client_id=?`, [
    clean(remoteClientId)
  ]);
  ...
  if (clean(remote.code_everwin)) {
    existing = await database.getFirstAsync(
      `SELECT id FROM clients WHERE lower(trim(COALESCE(code_exploitant,'')))=lower(trim(?)) LIMIT 1`,
      [clean(remote.code_everwin)]
    );
  }
  if (!existing && clean(remote.nom)) {
    existing = await database.getFirstAsync(`SELECT id FROM clients WHERE lower(trim(nom))=lower(trim(?)) LIMIT 1`, [
      clean(remote.nom)
    ]);
  }
  if (existing?.id) {
    await database.runAsync(
      `UPDATE api_client_links SET local_client_id=?,cree_localement=0 WHERE remote_client_id=?`,
      [existing.id, remote.remote_client_id]
    );
    return existing.id;
  }
  ...
  await database.runAsync(`INSERT INTO clients(id,nom,code_exploitant,adresse) VALUES(?,?,?,?)`, [
    id,
    remote.nom,
    remote.code_everwin || null,
    adresse || null
  ]);
  await database.runAsync(`UPDATE api_client_links SET local_client_id=?,cree_localement=1 WHERE remote_client_id=?`, [
    id,
    remote.remote_client_id
  ]);
  return id;
}
...
export async function materializeCachedSite(remoteSiteId, remoteClientId = null) {
  ...
  if (remote.local_site_id) {
    await database.runAsync(`UPDATE api_client_site_links SET local_site_id=? WHERE remote_site_id=?`, [
      remote.local_site_id,
      siteRemoteId
    ]);
  ...
  if (relationLocalSiteId) {
    ...
    if (mapped?.id) {
      await database.runAsync(`UPDATE api_site_links SET local_site_id=? WHERE remote_site_id=?`, [
        relationLocalSiteId,
        siteRemoteId
      ]);
      await database.runAsync(`UPDATE api_client_site_links SET local_site_id=? WHERE remote_site_id=?`, [
  ...
  await database.runAsync(`INSERT INTO sites(id,client_id,nom_site,statut) VALUES(?,?,?,'Actif')`, [
    localSiteId,
    clientId,
    remote.nom
  ]);

  await database.runAsync(`UPDATE api_site_links SET local_site_id=?,cree_localement=1 WHERE remote_site_id=?`, [
    localSiteId,
    siteRemoteId
  ]);
  await database.runAsync(`UPDATE api_client_site_links SET local_site_id=?,cree_localement=1 WHERE remote_site_id=?`, [
    localSiteId,
    siteRemoteId
  ]);
  return localSiteId;
