...
function FormulaCard({ item, selected, recommended, onPress }) {
  return (
    <TouchableOpacity
      ...
      style={[
        missionStyles.card,
        { padding: 11, marginBottom: 7, borderColor: selected ? MISSION_COLORS.accent : MISSION_COLORS.accentLine }
      ]}
    >
      <View style={{ flexDirection: 'row', alignItems: 'center' }}>
        <Text style={{ flex: 1, color: MISSION_COLORS.accentStrong, fontSize: 11, fontWeight: '900' }}>
          {item.label}
        </Text>
        {recommended ? (
          <Text
            style={{
              color: MISSION_COLORS.accentDark,
      ...
      <Text style={{ color: COLORS.inkFaint, fontSize: 8.8, marginTop: 2 }}>
        {item.family || 'Personnalisée'} · {item.unit || 'sans unité'}
      </Text>
...
export function MissionCalculationScreen({ route }) {
  ...
  const saveCalculation = async () => {
    ...
    try {
      ...
      Alert.alert(
        'Calcul enregistré',
        'Le résultat, la formule et les valeurs d’entrée restent traçables dans la Mission.'
      );
    } catch (e) {
      Alert.alert('Calcul impossible', String(e?.message || e));
    }
  ...
  const saveCustom = async () => {
    const inputKeys = String(custom.inputs || '')
      .split(',')
      .map((v) => v.trim())
      .filter(Boolean);
    ...
    <View style={{ flex: 1, backgroundColor: MISSION_COLORS.bg }}>
      <ScrollView contentContainerStyle={{ padding: 16, paddingBottom: 110 }}>
        ...
        <Text style={{ color: COLORS.inkSoft, fontSize: 10.5, lineHeight: 15, marginBottom: 12 }}>
          Les calculs restent explicables : formule, entrées, hypothèses et résultat sont conservés. Dès que toutes les
          entrées sont disponibles, le résultat se recalcule automatiquement.
        </Text>
        ...
        <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 7 }}>
          <Text style={[styles.sectionLabel, missionStyles.sectionLabel, { flex: 1, marginBottom: 0 }]}>
            Bibliothèque
          </Text>
          {recommendedCount ? (
            <TouchableOpacity
              ...
              style={{
                ...
                paddingHorizontal: 8,
                paddingVertical: 6,
                backgroundColor: recommendedOnly ? MISSION_COLORS.accentSoft : '#FFFFFF'
              }}
            >
              <Text
                style={{
                  color: recommendedOnly ? MISSION_COLORS.accentStrong : COLORS.inkSoft,
                  fontSize: 8.4,
                  fontWeight: '900'
                }}
              >
        ...
        {recommendedCount ? (
          <Text style={{ color: COLORS.inkFaint, fontSize: 8.6, lineHeight: 12, marginBottom: 7 }}>
            METRA place en premier les calculs cohérents avec cette Mission. Tu peux toujours afficher toute la
            bibliothèque.
          </Text>
        ) : null}
        ...
        {selected ? (
          <View style={[missionStyles.card, { padding: 13, marginTop: 12 }]}>
            ...
            <Text style={{ color: COLORS.inkSoft, fontSize: 9.5, marginTop: 3 }}>
              {selected.formula} {selected.unit ? '→ ' + selected.unit : ''}
            </Text>
            <View style={{ marginTop: 12 }}>
              {schema.map((input) => (
                <View key={input.key} style={{ marginBottom: 9 }}>
                  <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 4 }}>
                    <Text style={{ flex: 1, color: COLORS.inkFaint, fontSize: 8.5, fontWeight: '800' }}>
                      {input.label || input.key}
                      {input.unit ? ' · ' + input.unit : ''}
                    </Text>
                    {autoSources[input.key] ? (
                      <Text style={{ color: MISSION_COLORS.accentDark, fontSize: 7.6, fontWeight: '900' }}>
                        PRÉREMPLI
                      </Text>
                    ) : null}
                  </View>
                  {autoSources[input.key] ? (
                    <Text style={{ color: COLORS.inkFaint, fontSize: 7.8, lineHeight: 11, marginBottom: 4 }}>
                      {autoSources[input.key]}
                    </Text>
                  ) : null}
                  <TextInput
            ...
            {preview !== null ? (
              <View style={[missionStyles.statBox, { padding: 12, borderRadius: 12, marginBottom: 10 }]}>
                <Text style={{ color: COLORS.inkFaint, fontSize: 8.5 }}>RÉSULTAT · RECALCUL AUTOMATIQUE</Text>
                <Text style={{ color: MISSION_COLORS.accentStrong, fontSize: 22, fontWeight: '900', marginTop: 2 }}>
                  {Number(preview).toLocaleString('fr-FR', { maximumFractionDigits: 3 })} {selected.unit || ''}
                </Text>
              </View>
            ) : null}
            <View style={{ flexDirection: 'row', gap: 8 }}>
              <TouchableOpacity style={[styles.btnSecondary, missionStyles.secondaryButton]} onPress={calculate}>
                <Text style={[styles.btnSecondaryText, missionStyles.secondaryButtonText]}>Calculer</Text>
              </TouchableOpacity>
              <TouchableOpacity
                style={[styles.btnPrimary, missionStyles.primaryButton, { flex: 1, alignItems: 'center' }]}
                onPress={saveCalculation}
              >
                <Text style={[styles.btnPrimaryText, missionStyles.primaryButtonText]}>Calculer & enregistrer</Text>
              </TouchableOpacity>
            </View>
          </View>
        ) : null}
        ...
        {calculations.map((c) => (
          <View key={c.id} style={[missionStyles.card, { padding: 11, marginBottom: 7 }]}>
            <View style={{ flexDirection: 'row', alignItems: 'flex-start' }}>
              ...
              <Text style={{ color: MISSION_COLORS.accentStrong, fontWeight: '900', fontSize: 12 }}>
                {c.result_number ?? c.result_text ?? '/'} {c.unit || ''}
              </Text>
            </View>
          </View>
        ))}
        ...
        <View style={styles.modalOverlay}>
          <View style={[styles.modalSheet, missionStyles.modalSheet]}>
            ...
            <TextInput
              style={[styles.input, missionStyles.input]}
              value={custom.label}
              onChangeText={(v) => setCustom((p) => ({ ...p, label: v }))}
              placeholder="Nom de la formule"
            />
            <TextInput
              style={[styles.input, missionStyles.input, { marginTop: 8 }]}
              value={custom.family}
              onChangeText={(v) => setCustom((p) => ({ ...p, family: v }))}
              placeholder="Famille"
            />
            <TextInput
              style={[styles.input, missionStyles.input, { marginTop: 8 }]}
              value={custom.inputs}
              onChangeText={(v) => setCustom((p) => ({ ...p, inputs: v }))}
              placeholder="Variables : debit, deltaT, puissance"
            />
            <TextInput
              style={[styles.input, missionStyles.input, { marginTop: 8 }]}
              value={custom.formula}
              onChangeText={(v) => setCustom((p) => ({ ...p, formula: v }))}
              placeholder="Ex : debit * deltaT * 1.163"
            />
            <TextInput
              style={[styles.input, missionStyles.input, { marginTop: 8 }]}
              value={custom.unit}
              onChangeText={(v) => setCustom((p) => ({ ...p, unit: v }))}
              placeholder="Unité résultat"
            />
            <View style={styles.modalActions}>
              ...
              <TouchableOpacity style={[styles.btnPrimary, missionStyles.primaryButton]} onPress={saveCustom}>
