...
export function PhotoTaskList({ clientId = null }) {
  ...
  return <View>{state.tasks.filter((t) => clientId == null || t.clientId === String(clientId)).map((task) =>
    <View key={task.id} style={{ paddingVertical: 9, borderBottomWidth: 1, borderBottomColor: COLORS.line }}>
      <Text style={{ color: COLORS.ink, fontSize: 13, fontFamily: FONTS.bodyBold }}>{task.label}</Text>
      ...
      <View style={{ flexDirection: 'row', gap: 8 }}>
        {['queued', 'running'].includes(task.status) ? <TouchableOpacity accessibilityRole="button" onPress={() => pausePhotoDownload(task.id)} style={{ minHeight: 48, justifyContent: 'center', paddingHorizontal: 12 }}><Text style={{ color: COLORS.primary, fontFamily: FONTS.bodyBold }}>Mettre en pause</Text></TouchableOpacity> : null}
        {['paused', 'partial', 'error'].includes(task.status) ? <TouchableOpacity accessibilityRole="button" onPress={() => resumePhotoDownload(task.id)} style={{ minHeight: 48, justifyContent: 'center', paddingHorizontal: 12 }}><Text style={{ color: COLORS.primary, fontFamily: FONTS.bodyBold }}>Reprendre les manquantes</Text></TouchableOpacity> : null}
...
export function PhotoDownloadBanner() {
  ...
  return <>
    <TouchableOpacity accessibilityRole="button" accessibilityLabel="Afficher les téléchargements de photos" onPress={() => setVisible(true)} style={{ minHeight: 48, paddingHorizontal: 14, paddingVertical: 8, backgroundColor: '#EFF4F8', borderBottomWidth: 1, borderBottomColor: COLORS.line, flexDirection: 'row', alignItems: 'center' }}>
      <Text numberOfLines={1} style={{ flex: 1, color: COLORS.ink, fontSize: 12.5, fontFamily: FONTS.bodyBold }}>{task.label} · {labels[task.status]}{task.progress ? ` ${task.progress.completed}/${task.progress.total}` : ''}</Text>
      <Text style={{ color: COLORS.primary, fontFamily: FONTS.bodyBold, paddingLeft: 12 }}>Suivi</Text>
    </TouchableOpacity>
