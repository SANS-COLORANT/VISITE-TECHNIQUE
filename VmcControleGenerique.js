...
export const VmcControleGenerique = React.memo(function VmcControleGenerique({ visiteId, sectionCode, field, etatInitial, onSaved, onEtatChange }) {
  ...
  const sauvegarderCommentaire = useCallback(async (raw) => {
    ...
    if (avis === 'N.S' && texte) {
      await upsertRemarquePrescription(visiteId, controleKey, {
  ...
  }, [avis, visiteId, sectionCode, field, controleKey, notifier, rechargerRemarque, replaceCommentairePersisted]);
  ...
  const choisirPreset = useCallback(async (opt, idx) => {
    ...
    notifier({ avis, commentaire: texte });
  }, [visiteId, sectionCode, field, controleKey, avis, notifier, rechargerRemarque, replaceCommentairePersisted]);
  ...
  return <View style={styles.controlRow}><View style={styles.controlTop}><Text style={styles.controlLabel}>{field.cle}</Text><View style={styles.avisGroup}>{AVIS_OPTIONS.map((opt) => { const c = avisChipColor(opt); const selected = avis === opt; return <TouchableOpacity key={opt} style={[styles.avisChip, selected && { backgroundColor: c.bg, borderColor: c.border }]} onPress={() => choisirAvis(opt)}><Text style={[styles.avisChipText, selected && { color: c.text }]}>{opt}</Text></TouchableOpacity>; })}</View></View>
    {avis && <View style={[styles.criterePanel, { backgroundColor: palette.bg, borderColor: palette.border }]}>
      ...
      <TextInput style={[styles.input, { marginTop: 8, minHeight: 64, textAlignVertical: 'top', backgroundColor: '#fff' }]} multiline value={commentaire} onChangeText={(v) => { setCommentaire(v); setPresetChoisi(null); }} onBlur={() => flushCommentaire().catch(console.warn)} placeholder="Commentaire technique…" />
      ...
      <PhotoButton visiteId={visiteId} entiteKey={controleKey} label={field.cle} beforeCapture={avis === 'S' ? () => ({ entiteKey: controleKey, label: field.cle }) : undefined} style={avis === 'N.S' ? styles.photoRequiredBox : undefined} />
    </View>}
  </View>;
});
