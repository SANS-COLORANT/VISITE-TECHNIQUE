...
export function PatrimoineThumbnail({ uri, size = 54, radius = 10, style = null }) {
  ...
  return (
    <PhotoVariantImage
      ...
      variant="thumb"
      resizeMode="cover"
      style={[{ width: size, height: size, borderRadius: radius, backgroundColor: '#F1F2F4', marginRight: 12 }, style]}
    />
  );
}
...
export function PatrimoineImageCard({ entityType, entityId, title, subtitle = null, onChanged = null }) {
  ...
  return (
    ...
    >
      <TouchableOpacity
        activeOpacity={uri ? 0.86 : 1}
        onPress={() => {
          if (uri) setViewer(true);
        }}
      ...
      >
        {uri ? (
          <PhotoVariantImage
            uri={uri}
            variant={busy ? 'original' : 'thumb'}
            resizeMode="cover"
            style={{ width: '100%', height: 152, backgroundColor: '#F2F3F5' }}
          />
        ) : (
          <View
            style={{
      ...
      <View style={{ padding: 12 }}>
        <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 10 }}>
          <View style={{ flex: 1, paddingRight: 10 }}>
            ...
            <Text style={{ color: COLORS.muted, fontSize: 11.5, marginTop: 2 }}>
              {subtitle || `${libelleType} · stockée hors connexion`}
            </Text>
          </View>
        ...
        <View style={{ flexDirection: 'row', gap: 8, flexWrap: 'wrap' }}>
          <TouchableOpacity
            ...
            style={[styles.btnSecondary, { flex: 1, minWidth: 145, minHeight: 44 }]}
          >
          ...
          <TouchableOpacity
            ...
            style={[styles.btnSecondary, { flex: 1, minWidth: 130, minHeight: 44 }]}
          >
          ...
          >
            {uri ? (
              <PhotoVariantImage
                ...
                variant="preview"
                resizeMode="contain"
                style={{ width: '100%', height: 520, maxHeight: '75%', backgroundColor: '#111827' }}
              />
            ) : null}
            <View style={{ flexDirection: 'row', gap: 8, padding: 12 }}>
              <TouchableOpacity style={[styles.btnSecondary, { flex: 1 }]} onPress={() => setViewer(false)}>
              ...
              >
                <Text style={styles.btnPrimaryText}>Remplacer</Text>
              </TouchableOpacity>
            </View>
          </View>
        </View>
