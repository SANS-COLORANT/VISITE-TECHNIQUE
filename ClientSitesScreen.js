/** Écran Sites d'un client + accès synthèses et cartographie technique. */
...
function ClientSitesScreen({ route, navigation }) {
  ...
  // Important : ne pas utiliser useEffect(useCallback(...)) sans tableau de
  // dépendances. Cette forme relançait le chargement après chaque rendu et
  // pouvait provoquer une boucle de rendus jusqu'au crash de l'écran.
  useEffect(() => {
    let actif = true;
    (async () => {
      try {
            ...
            {(sansAdresse > 0 || sansGps > 0) ? (
              <View style={{ backgroundColor: '#FFF8E7', borderWidth: 1, borderColor: '#F0D99B', borderRadius: 12, padding: 10, marginBottom: 14 }}>
                <Text style={{ color: '#7A5700', fontSize: 12, fontWeight: '700' }}>
                  {sansAdresse ? `${sansAdresse} site(s) sans adresse` : ''}{sansAdresse && sansGps ? ' · ' : ''}{sansGps ? `${sansGps} sans point GPS` : ''}
                </Text>
              </View>
            ) : null}
              ...
              <View style={{ flex: 1 }}>
                ...
                {hasGps ? (
                  <TouchableOpacity onPress={(e) => { e?.stopPropagation?.(); ouvrirGoogleMaps(item); }}>
                    <Text style={{ color: COLORS.primary, fontSize: 11, fontWeight: '800', marginTop: 5 }}>📍 Google Maps ↗</Text>
                  </TouchableOpacity>
                ) : null}
              </View>
