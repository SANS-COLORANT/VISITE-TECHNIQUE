...
function ClientSitesScreen({ route, navigation }) {
  ...
  const ajouterSiteFn = async () => {
    ...
    try {
      ...
      await charger();
      if (adresse)
        synchroniserCoordonneesSite(siteId, adresse)
          .then(charger)
          .catch(() => {});
    } catch (e) {
  ...
  const demanderDuplication = (site) =>
    Alert.alert(
      'Dupliquer ce site ?',
  ...
  return (
    <View style={{ flex: 1, backgroundColor: COLORS.bg }}>
      <FlatList
        ...
        ListHeaderComponent={
          <View>
            <PatrimoineImageCard
              entityType="client"
              entityId={clientId}
              title={nomClient || 'Client'}
              subtitle="Image du client · disponible hors connexion"
            />
            ...
            >
              <View style={{ flexDirection: 'row', gap: 10 }}>
                <View style={{ flex: 1 }}>
                  <Text style={{ fontSize: 22, fontWeight: '800' }}>{sites.length}</Text>
                  <Text style={{ color: COLORS.muted, fontSize: 12 }}>sites</Text>
                </View>
                <View style={{ flex: 1 }}>
                ...
                <View style={{ flex: 1 }}>
                  ...
                  <Text style={{ color: COLORS.muted, fontSize: 12 }}>à compléter</Text>
                </View>
              </View>
            </View>
            ...
            <TouchableOpacity
              ...
              onPress={() => navigation.navigate('ClientPilotage', { clientId, nomClient })}
              disabled={!sites.length}
            >
              <Text style={styles.btnPrimaryText}>▦ Pilotage patrimoine</Text>
            </TouchableOpacity>
            <View style={{ flexDirection: 'row', gap: 8, marginBottom: 8 }}>
              <TouchableOpacity
                ...
                disabled={!sites.length}
              >
                <Text style={styles.btnSecondaryText}>🗺 Carte</Text>
              </TouchableOpacity>
              <TouchableOpacity
                style={[styles.btnSecondary, { flex: 1 }]}
                onPress={() => setGroupesVisible(true)}
                disabled={!sites.length}
              >
                <Text style={styles.btnSecondaryText}>▦ Groupes</Text>
              </TouchableOpacity>
            </View>
            <View style={{ flexDirection: 'row', gap: 8, marginBottom: 8 }}>
              <TouchableOpacity style={[styles.btnPrimary, { flex: 1 }]} onPress={() => setModalVisible(true)}>
                <Text style={styles.btnPrimaryText}>+ Site local</Text>
              </TouchableOpacity>
              <TouchableOpacity style={[styles.btnPrimary, { flex: 1 }]} onPress={() => setIntranetSiteVisible(true)}>
                <Text style={styles.btnPrimaryText}>+ Site Intranet</Text>
              </TouchableOpacity>
            </View>
            <View style={{ flexDirection: 'row', gap: 8, marginBottom: 8 }}>
              <TouchableOpacity
                style={[styles.btnSecondary, { flex: 1 }]}
                onPress={() => navigation.navigate('ClientDocuments', { clientId, nomClient })}
                disabled={!sites.length}
              >
                <Text style={styles.btnSecondaryText}>📄 Documents</Text>
              </TouchableOpacity>
              <TouchableOpacity
                style={[
                  styles.btnSecondary,
                  { flex: 1, flexDirection: 'row', gap: 7, borderColor: accent, backgroundColor: palette.light }
                ]}
            ...
            {sansAdresse > 0 ? (
              ...
              >
                <Text style={{ color: '#7A5700', fontSize: 12, fontWeight: '700' }}>
                  {sansAdresse} site(s) sans adresse complète
                </Text>
              </View>
            ) : null}
            <View style={styles.sectionHeaderRow}>
              ...
              <Text style={{ color: COLORS.muted, fontSize: 12 }}>
                {sitesFiltres.length}/{sites.length}
              </Text>
            </View>
            <TextInput
            ...
            <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 7, marginTop: 1, marginBottom: 12 }}>
              {SITE_SORT_OPTIONS.map((option) => {
                ...
                return (
                  ...
                  >
                    <Text style={{ fontSize: 11, fontWeight: '900', color: active ? COLORS.orange : COLORS.inkSoft }}>
                      {option.label}
                    </Text>
                  </TouchableOpacity>
                );
        ...
        renderItem={({ item }) => (
          <TouchableOpacity
            style={styles.card}
            activeOpacity={0.7}
            onPressIn={() => prewarmSiteLocals(item.id).catch(() => {})}
            onPress={() => ouvrirSite(item)}
          >
            <PatrimoineThumbnail uri={item.image_uri} size={60} radius={10} />
            <View style={{ flex: 1 }}>
              <Text style={styles.cardTitle}>{item.nom_site}</Text>
              {siteGroupLabel(item.id, groupMap) ? (
                <Text style={{ color: COLORS.primary, fontSize: 10.5, fontWeight: '800', marginTop: 2 }}>
                  {siteGroupLabel(item.id, groupMap)}
                </Text>
              ) : null}
              {item.adresse ? (
                <Text style={styles.cardSub}>{item.adresse}</Text>
              ) : (
                <Text style={{ color: '#A26A00', fontSize: 12 }}>Adresse à renseigner</Text>
              )}
              {item.localisation_note ? (
                <Text style={{ color: COLORS.muted, fontSize: 11, marginTop: 4 }}>{item.localisation_note}</Text>
              ) : null}
            </View>
            <TouchableOpacity
              onPress={(e) => ouvrirStructure(e, item)}
              style={{
                minHeight: 38,
                paddingHorizontal: 9,
                borderRadius: 10,
            ...
            >
              <Text style={{ color: COLORS.primary, fontSize: 10.5, fontWeight: '900' }}>Locaux Intranet</Text>
            </TouchableOpacity>
            <View style={[styles.badge, item.statut === 'Actif' ? styles.badgeActif : styles.badgeInactif]}>
              <Text
                style={[styles.badgeText, item.statut === 'Actif' ? styles.badgeTextActif : styles.badgeTextInactif]}
              >
                {item.statut || 'Actif'}
              </Text>
            </View>
            <TouchableOpacity
              onPress={(e) => ouvrirMenuSite(e, item)}
              style={{ minWidth: 46, minHeight: 46, alignItems: 'center', justifyContent: 'center', marginLeft: 4 }}
            >
              <Text style={{ color: COLORS.inkSoft, fontSize: 22, fontWeight: '900' }}>⋯</Text>
            </TouchableOpacity>
          </TouchableOpacity>
        )}
        ListEmptyComponent={
      ...
      <Modal visible={modalVisible} transparent animationType="fade">
        <View style={styles.modalOverlay}>
          <View style={styles.modalSheet}>
            <Text style={styles.modalTitle}>Nouveau site local</Text>
            <Text style={{ color: COLORS.muted, fontSize: 11.5, marginBottom: 10 }}>
              Ce bouton crée uniquement un site dans la base locale METRA. Utilise « Site Intranet » pour créer aussi la
              structure serveur.
            </Text>
            ...
            <View style={{ flexDirection: 'row', gap: 8, marginTop: 10 }}>
              <TextInput
                ...
                placeholder="N°"
                value={nouvelleAdresse.numero}
                keyboardType="numbers-and-punctuation"
                onChangeText={(v) => patchNouvelleAdresse({ numero: v })}
              />
              <TextInput
                style={[styles.input, { flex: 1 }]}
                placeholder="Rue / avenue / voie"
                value={nouvelleAdresse.voie}
                onChangeText={(v) => patchNouvelleAdresse({ voie: v })}
              />
            </View>
            <TextInput
              style={[styles.input, { marginTop: 8 }]}
              placeholder="Complément : bâtiment, entrée…"
              value={nouvelleAdresse.complement}
              onChangeText={(v) => patchNouvelleAdresse({ complement: v })}
            />
            <View style={{ flexDirection: 'row', gap: 8, marginTop: 8 }}>
              <TextInput
                style={[styles.input, { width: 120 }]}
                placeholder="Code postal"
                value={nouvelleAdresse.codePostal}
                keyboardType="number-pad"
                maxLength={5}
                onChangeText={(v) => patchNouvelleAdresse({ codePostal: v.replace(/\D/g, '').slice(0, 5) })}
              />
              <TextInput
                style={[styles.input, { flex: 1 }]}
                placeholder="Ville"
                value={nouvelleAdresse.ville}
                onChangeText={(v) => patchNouvelleAdresse({ ville: v })}
              />
            </View>
      ...
      <IntranetSiteCreationModal
        visible={intranetSiteVisible}
        clientId={clientId}
        onClose={() => setIntranetSiteVisible(false)}
        onCreated={charger}
      />
      ...
      <Modal visible={!!renameSite} transparent animationType="fade" onRequestClose={() => setRenameSite(null)}>
        <View style={styles.modalOverlay}>
          <View style={styles.modalSheet}>
            <Text style={styles.modalTitle}>Renommer le site</Text>
            <TextInput
              autoFocus
              style={styles.input}
              value={renameValue}
            ...
            />
            <View style={styles.modalActions}>
              <TouchableOpacity style={styles.btnSecondary} onPress={() => setRenameSite(null)}>
                <Text style={styles.btnSecondaryText}>Annuler</Text>
              </TouchableOpacity>
              <TouchableOpacity style={styles.btnPrimary} onPress={enregistrerRenommage}>
      ...
      <SiteGroupsManager
        visible={groupesVisible}
        clientId={clientId}
        sites={sites}
        onClose={() => {
          setGroupesVisible(false);
          charger().catch(() => {});
        }}
      />
