...
import {
  exporterSauvegardeBase,
  exporterSauvegardeComplete,
  choisirEtRestaurerSauvegardeComplete
...
function ParametresScreen() {
  ...
  useEffect(() => {
    ...
    ensureEquipmentCatalogReady()
      .then(() => {
        if (actif) setCataloguePret(true);
      })
      .catch((e) => {
        if (actif) setCatalogueErreur(String(e.message || e));
      });
    return () => {
      actif = false;
    };
      ...
      {catalogueErreur ? (
        <>
          ...
          <TouchableOpacity
            ...
            onPress={() => {
              ...
              setOnglet('reserves');
              setTimeout(() => setOnglet('equipements'), 0);
            }}
          >
        ...
        <>
          ...
          <Text style={{ marginTop: 5, color: COLORS.muted, textAlign: 'center' }}>
            Cette étape est surtout visible au premier lancement. Les ouvertures suivantes utilisent la base déjà
            enrichie.
          </Text>
        </>
      )}
  ...
  return (
    <View style={{ flex: 1, backgroundColor: COLORS.bg }}>
      <View style={styles.paramTabs}>
        <TouchableOpacity
          style={[styles.paramTab, onglet === 'reserves' && styles.paramTabActive]}
          onPress={() => setOnglet('reserves')}
        >
          <Text style={[styles.paramTabText, onglet === 'reserves' && styles.paramTabTextActive]}>Réserves</Text>
        </TouchableOpacity>
        <TouchableOpacity
          style={[styles.paramTab, onglet === 'equipements' && styles.paramTabActive]}
          onPress={() => setOnglet('equipements')}
        >
          <Text style={[styles.paramTabText, onglet === 'equipements' && styles.paramTabTextActive]}>Équipements</Text>
        </TouchableOpacity>
        <TouchableOpacity
          style={[styles.paramTab, onglet === 'donnees' && styles.paramTabActive]}
          onPress={() => setOnglet('donnees')}
        >
          <Text style={[styles.paramTabText, onglet === 'donnees' && styles.paramTabTextActive]}>Données</Text>
        </TouchableOpacity>
      </View>
      {onglet === 'reserves' ? (
        <BibliothequeReserves />
      ) : onglet === 'equipements' ? (
        contenuEquipements
      ) : (
        <GestionDonnees />
  ...
  return (
    <TouchableOpacity
      style={[
        ...
        danger && { backgroundColor: '#FFF1F0', borderWidth: 1, borderColor: '#F5B7B1' }
      ]}
      disabled={disabled}
      onPress={onPress}
    >
...
function GestionDonnees() {
  ...
  const sauvegarderComplet = () =>
    executer('complete', async () => {
      try {
        const r = await exporterSauvegardeComplete();
        if (r)
          Alert.alert(
            'Sauvegarde créée',
            `Archive complète créée avec ${r.manifeste?.counts?.visites || 0} visite(s) et ${r.manifeste?.counts?.photos || 0} photo(s).`
          );
      } catch (e) {
        Alert.alert('Sauvegarde complète impossible', String(e.message || e));
      }
    });

  const lancerRestauration = () =>
    executer('restore', async () => {
      try {
  ...
  const diagnostiquer = () =>
    executer('diagnostic', async () => {
      try {
        const d = await diagnostiquerStockageLocal();
        setDiagnostic(d);
      } catch (e) {
        Alert.alert('Diagnostic impossible', String(e.message || e));
      }
    });
  ...
  const deverrouillerSupport = () => {
    ...
    Alert.alert(
      'Support METRA activé',
      'Le bouton de DUMP technique est disponible pour cette session dans Paramètres > Données.'
    );
  };
  ...
  const lancerDump = () =>
    executer('dump', async () => {
      try {
        ...
        Alert.alert('DUMP impossible', String(e.message || e));
      }
    });
    ...
    <ScrollView contentContainerStyle={styles.content}>
      ...
      <View style={[styles.card, { alignItems: 'flex-start' }]}>
        <View style={{ flex: 1 }}>
          ...
          <Text style={[styles.cardSub, { marginTop: 5 }]}>
            Archive ZIP recommandée pour le terrain : base SQLite, toutes les visites et photos gérées par
            l’application, plus un manifeste de version.
          </Text>
          <Text style={[styles.cardSub, { marginTop: 6 }]}>
            L’archive peut être enregistrée dans Drive, OneDrive, un dossier réseau ou envoyée par mail.
          </Text>
        </View>
        <BoutonDonnees
          label={action === 'complete' ? 'Création de l’archive…' : 'Exporter base + photos'}
          disabled={occupe}
          onPress={sauvegarderComplet}
        />
        <BoutonDonnees
          label={action === 'base' ? 'Export…' : 'Exporter la base seule (.db)'}
          disabled={occupe}
          secondaire
          onPress={sauvegarderBase}
        />
      </View>
      ...
      <View style={[styles.card, { alignItems: 'flex-start' }]}>
        <View style={{ flex: 1 }}>
          ...
          <Text style={[styles.cardSub, { marginTop: 5 }]}>
            Restaure une archive complète créée par l’application. La version est vérifiée avant remplacement, les
            chemins des photos sont automatiquement adaptés à la nouvelle tablette et l’intégrité SQLite est contrôlée
            après restauration.
          </Text>
        </View>
        <BoutonDonnees
          label={action === 'restore' ? 'Restauration…' : 'Restaurer une sauvegarde ZIP'}
          disabled={occupe}
          danger
          onPress={demanderRestauration}
        />
      </View>
      ...
      <View style={[styles.card, { alignItems: 'flex-start' }]}>
        <View style={{ flex: 1 }}>
          ...
          <Text style={[styles.cardSub, { marginTop: 5 }]}>
            Vérifie l’intégrité SQLite, les relations de base, la version du schéma et la présence physique des photos.
          </Text>
          {diagnostic && (
            <View
              style={{
                ...
                padding: 12,
                borderRadius: 10,
                backgroundColor: diagnostic.ok ? '#EDF8F0' : '#FFF4E5',
                alignSelf: 'stretch'
              }}
            >
              <Text style={{ fontWeight: '900', color: diagnostic.ok ? '#246B38' : '#8A5400' }}>
                {diagnostic.ok ? '✓ Données saines' : '⚠ Vérification nécessaire'}
              </Text>
              <Text style={[styles.cardSub, { marginTop: 6 }]}>
                SQLite : {diagnostic.integrityOk ? 'OK' : 'Erreur'} · Relations :{' '}
                {diagnostic.foreignKeysOk ? 'OK' : 'Erreur'} · Schéma : v{diagnostic.versionSchema}/
                {diagnostic.versionAttendue}
              </Text>
              <Text style={[styles.cardSub, { marginTop: 3 }]}>
                {diagnostic.clients} clients · {diagnostic.sites} sites · {diagnostic.visites} visites ·{' '}
                {diagnostic.remarques} réserves
              </Text>
              <Text style={[styles.cardSub, { marginTop: 3 }]}>
                Photos : {diagnostic.photosTotal} référencées · {diagnostic.photosManquantes} manquante(s)
              </Text>
            </View>
          )}
        </View>
        <BoutonDonnees
          label={action === 'diagnostic' ? 'Diagnostic…' : 'Lancer le diagnostic'}
          disabled={occupe}
          secondaire
          onPress={diagnostiquer}
        />
      </View>
        ...
        <>
          ...
          <View style={[styles.card, { alignItems: 'flex-start' }]}>
            <View style={{ flex: 1 }}>
              ...
              <Text style={[styles.cardSub, { marginTop: 5 }]}>
                Génère un JSON lisible par le support avec les références Intranet reçues, les
                catégories/sous-catégories/critères, les liaisons locales et les dernières visites concernées. Les
                secrets d’authentification sont exclus ou masqués.
              </Text>
            </View>
            <BoutonDonnees
              label={action === 'dump' ? 'Création du DUMP…' : 'Exporter le DUMP support'}
              disabled={occupe}
              secondaire
              onPress={demanderDump}
            />
          </View>
        </>
      )}
...
function BibliothequeReserves() {
  const [reserves, setReserves] = useState([]),
    ...
    [prix, setPrix] = useState(''),
    [poste, setPoste] = useState(''),
    [delai, setDelai] = useState(''),
  ...
  const ouvrirEdition = (r) => {
    ...
    setPrix(r.prix ? String(r.prix) : '');
    setPoste(r.poste || '');
    setDelai(r.delai ? String(r.delai) : '');
    setModalVisible(true);
  };
  const enregistrer = async () => {
    if (!nom.trim()) {
      Alert.alert('Nom requis', 'Merci de donner un nom à cette réserve.');
      return;
    }
    const data = {
      nom: nom.trim(),
  ...
  const supprimer = (r) =>
    Alert.alert('Supprimer', `Supprimer « ${r.nom} » de la bibliothèque ?`, [
      { text: 'Annuler', style: 'cancel' },
      {
        ...
        style: 'destructive',
        onPress: async () => {
          await supprimerReserveBiblio(r.id);
          charger();
        }
      }
    ]);
  const visibles = useMemo(() => {
    ...
    return reserves.filter((r) =>
      `${r.nom || ''} ${r.description || ''} ${r.poste || ''}`.toLocaleLowerCase('fr').includes(q)
    );
  }, [reserves, recherche]);
      ...
      <FlatList
        ...
        ListHeaderComponent={
          <View>
            <View style={styles.sectionHeaderRow}>
              <View style={{ flex: 1, paddingRight: 12 }}>
                ...
                <Text style={[styles.cardSub, { marginTop: 4 }]}>
                  {reserves.length} réserve(s) disponible(s) · toucher une ligne pour l’ouvrir entièrement
                </Text>
              </View>
            ...
            <TextInput
              style={[styles.input, { marginBottom: 12, backgroundColor: COLORS.white }]}
              value={recherche}
              onChangeText={setRecherche}
              placeholder="Rechercher une réserve, un poste, une prestation…"
        ...
        ListEmptyComponent={
          <View style={styles.empty}>
            <Text style={styles.emptyText}>{recherche ? 'Aucune réserve trouvée' : 'Aucune réserve enregistrée'}</Text>
            <Text style={styles.emptySub}>
              {recherche
                ? 'Essaie un autre mot-clé.'
                : 'Ajoute une réserve pour la retrouver ensuite pendant les visites.'}
            </Text>
          </View>
        }
        renderItem={({ item }) => {
          const meta = [
            item.poste,
            item.delai ? `${item.delai} mois` : null,
            item.prix != null ? `${item.prix} € HT` : null
          ]
            .filter(Boolean)
            .join(' · ');
          return (
            ...
            >
              <View style={{ flex: 1, minWidth: 0 }}>
                ...
                {item.description ? (
                  <Text style={[styles.cardSub, { marginTop: 4, lineHeight: 17 }]}>{item.description}</Text>
                ) : null}
                {meta ? (
                  <Text style={{ fontSize: 10.5, color: COLORS.inkFaint, marginTop: 6, fontWeight: '700' }}>
                    {meta}
                  </Text>
                ) : null}
                <Text style={{ fontSize: 9.5, color: COLORS.orangeDark, marginTop: 6, fontWeight: '800' }}>
                  Ouvrir / modifier
                </Text>
              </View>
              <TouchableOpacity onPress={() => supprimer(item)} hitSlop={{ top: 10, bottom: 10, left: 10, right: 10 }}>
      ...
      <Modal visible={modalVisible} transparent animationType="fade" onRequestClose={fermerModal}>
        <View style={[styles.modalOverlay, { paddingVertical: 24, paddingHorizontal: 18 }]}>
          <View
            style={[
              styles.modalSheet,
              { width: '94%', maxWidth: 760, maxHeight: '92%', paddingBottom: 12, overflow: 'hidden' }
            ]}
          >
            <ScrollView
              style={{ flexGrow: 0 }}
            ...
            >
              ...
              {!editId ? (
                <View
                  style={{ marginBottom: 14, paddingBottom: 14, borderBottomWidth: 1, borderBottomColor: COLORS.line }}
                >
              ...
              <TextInput
                style={[styles.input, { marginTop: 10, minHeight: 90, textAlignVertical: 'top' }]}
            ...
            <View style={[styles.modalActions, { paddingTop: 10, borderTopWidth: 1, borderTopColor: COLORS.line }]}>
              <TouchableOpacity style={styles.btnSecondary} onPress={fermerModal}>
