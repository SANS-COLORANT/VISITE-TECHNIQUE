...
const BUILTIN_FORMULAS = Object.freeze([
  { key: 'hydraulic_power', family: 'CVC', label: 'Puissance hydraulique eau', formula: 'flow_m3h * deltaT_K * 1.163', unit: 'kW', inputs: [
    ['flow_m3h', 'Débit', 'm³/h'], ['deltaT_K', 'ΔT', 'K'],
  ] },
  { key: 'hydraulic_flow', family: 'CVC', label: 'Débit hydraulique depuis puissance', formula: 'power_kW / (1.163 * deltaT_K)', unit: 'm³/h', inputs: [
    ['power_kW', 'Puissance', 'kW'], ['deltaT_K', 'ΔT', 'K'],
  ] },
  { key: 'air_power', family: 'Ventilation', label: 'Puissance sensible air', formula: '0.34 * flow_m3h * deltaT_K', unit: 'W', inputs: [
    ['flow_m3h', 'Débit d’air', 'm³/h'], ['deltaT_K', 'ΔT air', 'K'],
  ] },
  { key: 'deviation_pct', family: 'Mesures', label: 'Écart mesuré / référence', formula: '((measured - reference) / reference) * 100', unit: '%', inputs: [
    ['measured', 'Mesuré', ''], ['reference', 'Référence', ''],
  ] },
  { key: 'cop', family: 'Clim/PAC', label: 'COP instantané', formula: 'thermal_power_kW / electrical_power_kW', unit: '', inputs: [
    ['thermal_power_kW', 'Puissance thermique', 'kW'], ['electrical_power_kW', 'Puissance électrique', 'kW'],
  ] },
  { key: 'eer', family: 'Clim/PAC', label: 'EER instantané', formula: 'cooling_power_kW / electrical_power_kW', unit: '', inputs: [
    ['cooling_power_kW', 'Puissance froid', 'kW'], ['electrical_power_kW', 'Puissance électrique', 'kW'],
  ] },
  { key: 'specific_energy_area', family: 'Énergie', label: 'Consommation spécifique surfacique', formula: 'energy_kWh / area_m2', unit: 'kWh/m²', inputs: [
    ['energy_kWh', 'Énergie', 'kWh'], ['area_m2', 'Surface', 'm²'],
  ] },
  { key: 'specific_energy_dwelling', family: 'Énergie', label: 'Consommation par logement', formula: 'energy_kWh / dwellings', unit: 'kWh/logt', inputs: [
    ['energy_kWh', 'Énergie', 'kWh'], ['dwellings', 'Logements', ''],
  ] },
  { key: 'dju_corrected', family: 'Énergie', label: 'Consommation corrigée DJU', formula: 'consumption_kWh * reference_DJU / actual_DJU', unit: 'kWh', inputs: [
    ['consumption_kWh', 'Consommation', 'kWh'], ['reference_DJU', 'DJU de référence', ''], ['actual_DJU', 'DJU période', ''],
  ] },
  { key: 'payback', family: 'Énergie', label: 'Temps de retour simple', formula: 'investment_EUR / annual_saving_EUR', unit: 'ans', inputs: [
    ['investment_EUR', 'Investissement', '€'], ['annual_saving_EUR', 'Économie annuelle', '€/an'],
  ] },
  { key: 'air_velocity', family: 'Ventilation', label: 'Vitesse d’air depuis débit et section', formula: 'flow_m3h / (area_m2 * 3600)', unit: 'm/s', inputs: [
    ['flow_m3h', 'Débit', 'm³/h'], ['area_m2', 'Section', 'm²'],
  ] },
  { key: 'heat_recovery_efficiency', family: 'Ventilation', label: 'Rendement récupération température', formula: '((supply_after_C - outdoor_C) / (extract_C - outdoor_C)) * 100', unit: '%', inputs: [
    ['supply_after_C', 'Soufflage après échangeur', '°C'], ['outdoor_C', 'Air extérieur', '°C'], ['extract_C', 'Air extrait', '°C'],
  ] },
  { key: 'ecs_delta_t', family: 'ECS', label: 'ΔT départ / retour ECS', formula: 'supply_C - return_C', unit: 'K', inputs: [
    ['supply_C', 'Départ ECS', '°C'], ['return_C', 'Retour ECS', '°C'],
  ] },
...
export async function garantirBibliothequeFormulesMission() {
  ...
  for (const item of BUILTIN_FORMULAS) {
    ...
    await db.runAsync(
      ...
      [id, null, 'global', item.family, item.label, item.formula, item.unit, JSON.stringify(item.inputs.map(([key, label, unit]) => ({ key, label, unit })))]
    );
  }
}
...
export async function creerFormuleMission({ missionId = null, scope = 'mission', family = null, missionType = null, label, formula, unit = null, inputs = [], assumptions = null } = {}) {
  const db = await getDb();
  ...
  await db.runAsync(
    ...
    [id, missionId, scope, clean(family), clean(missionType), clean(label) || 'Formule', clean(formula) || '0', clean(unit), JSON.stringify(inputs || []), assumptions ? JSON.stringify(assumptions) : null]
  );
  return id;
}

function validateFormula(formula, keys) {
  const allowed = String(formula || '').replace(/[A-Za-z_][A-Za-z0-9_]*/g, (name) => keys.includes(name) || ['Math','abs','min','max','sqrt','pow','round'].includes(name) ? '' : 'INVALID');
...
export function calculerFormuleMission(formulaRow, inputValues = {}) {
  const schema = (() => { try { return JSON.parse(formulaRow?.input_schema_json || '[]'); } catch { return []; } })();
  ...
  const body = '"use strict"; const abs=Math.abs,min=Math.min,max=Math.max,sqrt=Math.sqrt,pow=Math.pow,round=Math.round; return (' + formulaRow.formula + ');';
...
export async function enregistrerCalculDepuisFormule({
  ...
  await db.runAsync(
    `INSERT INTO mission_calculations(id,mission_id,site_id,equipment_id,subject_id,label,formula,unit,result_number,inputs_json,assumptions_json,status,source_type)
     VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)`,
    [
      id, missionId, clean(siteId), clean(equipmentId), clean(subjectId), formulaRow.label, formulaRow.formula, clean(formulaRow.unit),
      result, JSON.stringify(inputValues), assumptions ? JSON.stringify(assumptions) : formulaRow.assumptions_json, 'active', 'formula_library',
    ]
  );
