  ...
  return (
    <View style={{ flexDirection: 'row', alignItems: 'center', paddingVertical: 5, paddingLeft: 12 }}>
      ...
      <View style={{ flex: 1 }}>
        ...
        <Text style={{ color: COLORS.inkFaint, fontSize: 8.3 }}>
          {[equipment.brand, equipment.model, equipment.state].filter(Boolean).join(' · ') || 'À compléter'}
        </Text>
      </View>
    </View>
  );
...
function LocationNode({ node, onAddChild, onEdit, onDelete }) {
  return (
    <View style={{ marginLeft: Math.min(38, Number(node.depth || 0) * 14), marginTop: 6 }}>
        ...
        <View style={{ flexDirection: 'row', alignItems: 'center' }}>
          ...
          <View style={{ flex: 1 }}>
            ...
            <Text style={{ color: COLORS.inkFaint, fontSize: 8.3, marginTop: 2 }}>
              {KIND_LABEL[node.kind] || node.kind || 'Localisation'} · {node.equipment?.length || 0} équipement(s)
              direct(s)
            </Text>
          </View>
      ...
      {(node.children || []).map((child) => (
        <LocationNode key={child.id} node={child} onAddChild={onAddChild} onEdit={onEdit} onDelete={onDelete} />
      ))}
    </View>
  );
}
    ...
    <View style={{ flex: 1, backgroundColor: MISSION_COLORS.bg }}>
      <ScrollView contentContainerStyle={{ padding: 16, paddingBottom: 110 }}>
        ...
        <Text style={{ color: COLORS.inkSoft, fontSize: 10.5, lineHeight: 15 }}>
          Construis une hiérarchie simple : Site → Bâtiment → Niveau → Local / local technique → Équipement. Elle sert
          ensuite aux plans, photos, mesures et constats sans ressaisie.
        </Text>
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false}>
          {tree.map((site) => (
            <Chip
              ...
              onPress={() => setExpandedSite(site.id)}
            />
          ))}
        </ScrollView>
        ...
        {selectedSite ? (
          <>
            <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 8, marginTop: 10 }}>
              <TouchableOpacity
                ...
                onPress={() => openNewRoot(selectedSite)}
              >
              ...
              <TouchableOpacity
                ...
                onPress={() => navigation.navigate('MissionEquipment', { missionId, siteId: selectedSite.id })}
              >
            ...
            {(selectedSite.locations || []).length ? (
              selectedSite.locations.map((node) => (
                <LocationNode key={node.id} node={node} onAddChild={openChild} onEdit={openEdit} onDelete={remove} />
              ))
            ) : (
              <View style={[missionStyles.card, { padding: 13 }]}>
                <Text style={{ color: COLORS.inkSoft, fontSize: 9.8 }}>
                  Aucune structure créée. Commence par un bâtiment ou une zone.
                </Text>
              </View>
            )}

            {(selectedSite.unlocatedEquipment || []).length ? (
              <View style={{ marginTop: 16 }}>
                <Text style={[styles.sectionLabel, missionStyles.sectionLabel]}>Équipements sans localisation</Text>
                <View style={[missionStyles.card, { padding: 10 }]}>
                  {selectedSite.unlocatedEquipment.map((eq) => (
                    <EquipmentRow key={eq.id} equipment={eq} />
                  ))}
                </View>
          ...
          <Text style={{ color: COLORS.inkFaint, fontSize: 10, marginTop: 14 }}>
            Aucun Site rattaché à cette Mission.
          </Text>
        )}
      </ScrollView>
        ...
        <View style={styles.modalOverlay}>
          <View style={[styles.modalSheet, missionStyles.modalSheet]}>
            <Text style={[styles.modalTitle, missionStyles.title]}>
              {editingId ? 'Modifier la localisation' : 'Ajouter une localisation'}
            </Text>
            <TextInput
              ...
              autoFocus
            />
            <Text style={{ color: COLORS.inkFaint, fontSize: 8.6, fontWeight: '800', marginTop: 10, marginBottom: 5 }}>
              TYPE
            </Text>
            <ScrollView style={{ maxHeight: 250 }}>
              <View style={{ flexDirection: 'row', flexWrap: 'wrap' }}>
                {MISSION_LOCATION_KINDS.map(([key, label]) => (
                  <Chip
                    key={key}
                    label={label}
                    selected={draft.kind === key}
                    onPress={() => setDraft((p) => ({ ...p, kind: key }))}
                  />
                ))}
              </View>
            </ScrollView>
