...
function localAddress(adresse, codePostal, ville) {
  return (
    [nullable(adresse), [nullable(codePostal), nullable(ville)].filter(Boolean).join(' ') || null]
      .filter(Boolean)
      .join('\n') || null
  );
}
...
export function subscribeStructureOutbox(listener) {
  listeners.add(listener);
  return () => listeners.delete(listener);
}
export function getStructureOutboxRevision() {
  return revision;
}

function validateSitePayload(payload, referential = null) {
  const violations = [];
  if (!/^[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(clean(payload?.creationId)))
    violations.push('creationId UUID v4 invalide.');
  const lotId = positiveInteger(payload?.lotId);
  if (!lotId) violations.push('lotId invalide.');
  const nom = clean(payload?.nom);
  if (nom.length < 2 || nom.length > 100) violations.push('Le nom du site doit contenir 2 à 100 caractères.');
  if (!['Oui', 'Non'].includes(payload?.sitePrincipal)) violations.push('sitePrincipal doit être Oui ou Non.');
  if (payload?.adresse != null && String(payload.adresse).length > 255)
    violations.push('Adresse limitée à 255 caractères.');
  if (payload?.codePostal != null && !/^.{5}$/.test(String(payload.codePostal)))
    violations.push('Le code postal doit contenir exactement 5 caractères.');
  if (payload?.ville != null && String(payload.ville).length > 50) violations.push('Ville limitée à 50 caractères.');
  if (!clean(payload?.energie) || clean(payload.energie).length > 255)
    violations.push('Énergie requise (255 caractères maximum).');
  if (!clean(payload?.typeBatiment) || clean(payload.typeBatiment).length > 255)
    violations.push('Type de bâtiment requis (255 caractères maximum).');
  if (referential && lotId && !(referential?.lots || []).some((lot) => Number(lot?.id) === lotId))
    violations.push('Le lot choisi n’appartient pas au référentiel de ce client.');
  if (violations.length)
    throw structureError(violations.join('\n'), 'local_validation', {
      violations: violations.map((message) => ({ path: 'local', message }))
    });
}
...
function validateLocalPayload(payload, referential = null) {
  ...
  if (designation.length < 1 || designation.length > 100)
    violations.push('Désignation requise (100 caractères maximum).');
  const type = clean(payload?.type);
  if (type.length < 1 || type.length > 255) violations.push('Type de local requis (255 caractères maximum).');
  const situation = clean(payload?.situation);
  if (situation.length < 2 || situation.length > 50) violations.push('Situation requise (2 à 50 caractères).');
  const trameId = positiveInteger(payload?.trameId);
  if (!trameId) violations.push('Trame Intranet obligatoire.');
  if (payload?.periodiciteVisite != null && !positiveInteger(payload.periodiciteVisite))
    violations.push('La périodicité doit être un nombre entier de mois strictement positif.');
  if (referential && trameId && !(referential?.trames || []).some((trame) => Number(trame?.id) === trameId))
    violations.push('La trame choisie n’existe plus dans le référentiel de ce client.');
  if (violations.length)
    throw structureError(violations.join('\n'), 'local_validation', {
      violations: violations.map((message) => ({ path: 'local', message }))
    });
...
export async function cacheStructureReferential(remoteClientId, payload) {
  ...
  await db.runAsync(
    `INSERT INTO api_structure_referential(remote_client_id,payload_json,synced_at)
...
export async function getCachedStructureReferential(remoteClientId) {
  const row = await (
    await getDb()
  ).getFirstAsync(`SELECT * FROM api_structure_referential WHERE remote_client_id=?`, [clean(remoteClientId)]);
...
async function linkedRemoteClientsForLocalClient(localClientId) {
  return (await getDb()).getAllAsync(
    `SELECT remote_client_id,nom FROM api_client_links
    WHERE local_client_id=? AND autorise=1 ORDER BY remote_client_id`,
...
export async function getStructureContextForLocalClient(localClientId) {
  ...
  if (rows.length > 1)
    return { linked: true, ambiguous: true, remoteClientId: null, referential: null, remoteClients: rows };
  const remoteClientId = clean(rows[0].remote_client_id);
  return {
    linked: true,
    ambiguous: false,
    remoteClientId,
    referential: await getCachedStructureReferential(remoteClientId),
    remoteClient: rows[0]
  };
}
...
export async function getSiteStructureContext(localSiteId) {
  ...
  const site = await db.getFirstAsync(
    `SELECT s.*,c.id AS local_client_id,c.nom AS client_nom FROM sites s JOIN clients c ON c.id=s.client_id WHERE s.id=?`,
    [String(localSiteId)]
  );
  if (!site) throw new Error('Site METRA introuvable.');
  const pendingSite = await db.getFirstAsync(
    `SELECT * FROM api_structure_outbox WHERE resource_type='site' AND local_site_id=? ORDER BY queued_at DESC LIMIT 1`,
    [String(localSiteId)]
  );
  ...
  return {
    ...
    ambiguous,
    remoteClientId,
    remoteSiteId: remoteSite?.remote_site_id
      ? clean(remoteSite.remote_site_id)
      : pendingSite?.status === 'synced'
        ? clean(pendingSite.remote_id)
        : null,
...
export async function listStructureOutbox({ remoteClientId = null, localSiteId = null, includeSynced = false } = {}) {
  ...
  const params = [];
  if (remoteClientId) {
    where.push('remote_client_id=?');
    params.push(clean(remoteClientId));
  }
  if (localSiteId) {
    where.push('local_site_id=?');
    params.push(String(localSiteId));
  }
  if (!includeSynced) where.push("status<>'synced'");
  return db.getAllAsync(
    `SELECT * FROM api_structure_outbox ${where.length ? `WHERE ${where.join(' AND ')}` : ''} ORDER BY queued_at`,
    params
  );
}

export async function queueMetraSiteCreation({
  localClientId,
  remoteClientId,
  lotId,
  nom,
  ...
  if (!referential)
    throw structureError(
      'Le référentiel de création de ce client n’est pas encore disponible hors connexion. Actualise-le une première fois avec Internet.',
      'structure_referential_required'
    );
  ...
  if (payloadBytes > 64 * 1024)
    throw structureError('La création du site dépasse la limite serveur de 64 Kio.', 'request_too_large');
  ...
  await db.withTransactionAsync(async () => {
    await db.runAsync(`INSERT INTO sites(id,client_id,nom_site,adresse,statut) VALUES(?,?,?,?, 'Actif')`, [
      ...
      String(localClientId),
      payload.nom,
      localAddress(payload.adresse, payload.codePostal, payload.ville)
    ]);
    await db.runAsync(
      `INSERT INTO api_structure_outbox(operation_id,resource_type,creation_id,remote_client_id,local_site_id,payload_json,payload_bytes,status)
      VALUES(?,'site',?,?,?,?,?,'pending')`,
      [operationId, creationId, clientId, localSiteId, serialized, payloadBytes]
    );
  });
  notify();
...
export async function queueMetraLocalCreation({
  localSiteId,
  remoteClientId,
  designation,
  type,
  ...
  if (!referential)
    throw structureError(
      'Le référentiel de création de ce client n’est pas disponible. Actualise-le avant de créer un local.',
      'structure_referential_required'
    );
  const creationId = await createIntranetUploadId();
  const payload = {
    ...
    periodiciteVisite:
      periodiciteVisite == null || clean(periodiciteVisite) === '' ? null : positiveInteger(periodiciteVisite)
  };
  validateLocalPayload(payload, referential);
  const serialized = JSON.stringify(payload);
  const payloadBytes = utf8ByteLength(serialized);
  if (payloadBytes > 64 * 1024)
    throw structureError('La création du local dépasse la limite serveur de 64 Kio.', 'request_too_large');
  ...
  const siteOperation = await db.getFirstAsync(
    `SELECT * FROM api_structure_outbox WHERE resource_type='site' AND local_site_id=? AND remote_client_id=? ORDER BY queued_at DESC LIMIT 1`,
    [String(localSiteId), clientId]
  );
  if (!remoteSite && !siteOperation)
    throw structureError(
      'Ce site METRA n’est pas encore relié à un site Intranet et aucune création de site n’est en attente.',
      'remote_site_required'
    );
  if (!remoteSite && siteOperation && TERMINAL_STATUSES.has(siteOperation.status))
    throw structureError(
      'La création Intranet du site est à corriger avant de pouvoir créer un local dessus.',
      'site_creation_blocked'
    );
  ...
  await db.withTransactionAsync(async () => {
    ...
    await db.runAsync(
      `INSERT INTO api_structure_outbox(operation_id,resource_type,creation_id,remote_client_id,local_site_id,local_installation_id,depends_on_id,payload_json,payload_bytes,status)
      ...
      [
        ...
        String(localSiteId),
        localInstallationId,
        remoteSite ? null : siteOperation.operation_id,
        serialized,
        payloadBytes
      ]
    );
...
export async function listSiteStructureLocals(localSiteId) {
  ...
  const localRows = await db.getAllAsync(
    `SELECT i.id AS local_installation_id,i.nom AS installation_nom,i.description AS installation_description,i.type_code,
    ...
    ORDER BY i.cree_le,i.nom`,
    [String(localSiteId)]
  );
  const seenRemote = new Set(localRows.map((row) => clean(row.remote_local_id || row.remote_id)).filter(Boolean));
  const remoteRows = context.remoteSiteId
    ? await db.getAllAsync(
        `SELECT * FROM api_local_links WHERE remote_site_id=? AND remote_present=1 ORDER BY designation,remote_local_id`,
        [String(context.remoteSiteId)]
      )
    : [];
  const result = localRows.map((row) => {
    ...
    return {
      ...
      remoteTrameNom:
        row.remote_trame_nom ||
        (context.referential?.trames || []).find((t) => Number(t?.id) === Number(payload.trameId))?.nom ||
        null,
...
async function resolveRemoteSiteIdForOperation(db, row) {
  ...
  if (row.depends_on_id) {
    const parent = await db.getFirstAsync(`SELECT status,remote_id FROM api_structure_outbox WHERE operation_id=?`, [
      row.depends_on_id
    ]);
...
async function markRetry(db, row, error, delay) {
  await db.runAsync(
    `UPDATE api_structure_outbox SET status='retry',next_attempt_at=?,http_status=?,error_code=?,error_message=?,violations_json=?,updated_at=datetime('now') WHERE operation_id=?`,
    [
      isoAfter(delay),
      error?.status || null,
      error?.code || null,
...
async function markTerminal(db, row, status, error) {
  await db.runAsync(
    `UPDATE api_structure_outbox SET status=?,next_attempt_at=NULL,http_status=?,error_code=?,error_message=?,violations_json=?,updated_at=datetime('now') WHERE operation_id=?`,
    [
      status,
      error?.status || null,
      error?.code || null,
      String(error?.message || 'Création refusée'),
      violationsJson(error),
      row.operation_id
    ]
  );
...
async function materializeSiteAck(db, row, response) {
  ...
  const payload = parseJson(row.payload_json) || {};
  await db.runAsync(
    `INSERT INTO api_site_links(remote_site_id,remote_client_id,local_site_id,nom,cree_localement,payload_json,synced_at,remote_present)
    ...
    [
      remoteSiteId,
      row.remote_client_id,
      row.local_site_id,
      remoteSite.nom || payload.nom || `Site ${remoteSiteId}`,
      json(remoteSite)
    ]
  );
  await db.runAsync(
...
async function materializeLocalAck(db, row, response, remoteSiteId) {
  ...
  await db.runAsync(
    `INSERT INTO api_local_links(remote_local_id,remote_site_id,designation,remote_trame_id,remote_trame_nom,derniere_visite_id,derniere_visite_date,derniere_visite_statut,reference_json,synced_at,local_installation_id,remote_present,criteria_count,historical_criteria_count,remark_count,material_count)
    ...
    [
      remoteLocalId,
      remoteSiteId,
      remoteLocal.designation || payload.designation,
      clean(remoteLocal.trameId || payload.trameId),
      remoteTrameName,
      json(reference),
  ...
  if (row.local_installation_id) {
    await db.runAsync(
      `UPDATE visites SET api_remote_client_id=COALESCE(api_remote_client_id,?),api_remote_local_id=COALESCE(api_remote_local_id,?),api_remote_trame_id=COALESCE(api_remote_trame_id,?),modifie_le=datetime('now')
...
async function sendRow(db, row) {
  if (row.depends_on_id) {
    const parent = await db.getFirstAsync(
      `SELECT status,remote_id,error_code,error_message FROM api_structure_outbox WHERE operation_id=?`,
      [row.depends_on_id]
    );
    if (!parent || parent.status !== 'synced') {
      if (parent && TERMINAL_STATUSES.has(parent.status)) {
        const error = structureError(
          'La création du local est bloquée car la création de son site parent a échoué.',
          'dependency_failed'
        );
  ...
  await db.runAsync(
    `UPDATE api_structure_outbox SET status='sending',attempt_count=attempt_count+1,last_attempt_at=datetime('now'),error_code=NULL,error_message=NULL,violations_json=NULL,updated_at=datetime('now') WHERE operation_id=?`,
    [row.operation_id]
  );
  notify();
  try {
    ...
    const response = await protectedRequest('POST', path, {
      body: row.payload_json,
      headers: { 'Content-Type': 'application/json' }
    });
    if (String(response?.creationId || '') !== String(row.creation_id))
      throw structureError('Accusé Intranet incohérent : creationId différent.', 'invalid_ack');
    if (typeof response?.rejoue !== 'boolean')
      throw structureError('Accusé Intranet incomplet : indicateur rejoue absent.', 'invalid_ack');
    const resource = row.resource_type === 'site' ? response?.site : response?.local;
    if (!positiveInteger(resource?.id))
      throw structureError(`Accusé Intranet incomplet : identifiant ${row.resource_type} absent.`, 'invalid_ack');
    const remoteId =
      row.resource_type === 'site'
        ? await materializeSiteAck(db, row, response)
        : await materializeLocalAck(db, row, response, remoteSiteId);
    await db.runAsync(
      `UPDATE api_structure_outbox SET status='synced',next_attempt_at=NULL,http_status=?,error_code=NULL,error_message=NULL,violations_json=NULL,remote_id=?,replayed=?,synced_at=datetime('now'),updated_at=datetime('now') WHERE operation_id=?`,
    ...
    else if (!status || RETRYABLE_HTTP.has(status))
...
export async function recoverInterruptedStructureUploads() {
  ...
  const result = await db.runAsync(
    `UPDATE api_structure_outbox SET status='retry',next_attempt_at=datetime('now'),error_code='interrupted',error_message='Création interrompue avant confirmation : reprise idempotente avec le même creationId.',updated_at=datetime('now') WHERE status='sending'`
  );
...
export async function processStructureOutbox({ limit = 4 } = {}) {
  ...
  processorPromise = (async () => {
    ...
    const rows = await db.getAllAsync(
      `SELECT * FROM api_structure_outbox
      WHERE status IN ('pending','retry') AND (next_attempt_at IS NULL OR datetime(next_attempt_at)<=datetime('now'))
      ORDER BY CASE resource_type WHEN 'site' THEN 0 ELSE 1 END,queued_at LIMIT ?`,
      [Math.max(1, Math.min(8, Number(limit || 4)))]
    );
    const results = [];
    for (const row of rows) {
      ...
      if (result?.status === 'error' && (auth || !http || http === 401 || http === 429 || RETRYABLE_HTTP.has(http)))
        break;
    }
    return results;
  })().finally(() => {
...
export async function retryStructureOperation(operationId) {
  ...
  await db.runAsync(
    `UPDATE api_structure_outbox SET status='pending',next_attempt_at=NULL,error_code=NULL,error_message=NULL,violations_json=NULL,updated_at=datetime('now') WHERE operation_id=?`,
    [String(operationId)]
  );
  notify();
