  ...
  return (
    <Modal visible transparent animationType="fade" onRequestClose={onClose}>
      <View style={{ flex: 1 }}>
        <Pressable
          onPress={onClose}
          style={{ position: 'absolute', top: 0, right: 0, bottom: 0, left: 0, backgroundColor: '#0004' }}
        />
        <View
          pointerEvents="box-none"
          style={{ position: 'absolute', left: centerX - HALF, top: centerY - HALF, width: SIZE, height: SIZE }}
        >
          <View
            pointerEvents="none"
            style={{
          ...
          {actions.map((action) => {
            ...
            return (
              <TouchableOpacity
                key={action.key}
                onPress={() => onAction?.(action.key, menu.site)}
                style={{
                  position: 'absolute',
                  left,
                  top,
                  ...
                  borderWidth: 1.5,
                  borderColor: action.danger ? '#FF7A80' : '#477186',
                  backgroundColor: action.danger ? '#53252B' : '#143143'
                }}
              >
                <Text style={{ color: action.danger ? '#FFB0B4' : '#F5FBFF', fontWeight: '900', fontSize: 19 }}>
                  {action.icon}
                </Text>
                <Text
                  numberOfLines={1}
                  style={{
                    color: action.danger ? '#FFB0B4' : '#D8EEF8',
                    fontSize: 8.5,
                    fontWeight: '800',
                    marginTop: 1
                  }}
          ...
          >
            <Text
              numberOfLines={2}
              style={{ color: '#F5FBFF', fontWeight: '900', fontSize: 9.5, textAlign: 'center', maxWidth: 52 }}
            >
              {String(menu.site?.nom_site || 'Actions').slice(0, 18)}
            </Text>
