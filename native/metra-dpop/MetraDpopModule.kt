...
class MetraDpopModule(context: ReactApplicationContext) : ReactContextBaseJavaModule(context) {
  companion object { private const val KEY_ALIAS = "metra_dpop_p256_v1" }
  ...
  private fun unsigned32(value: BigInteger): ByteArray {
    val raw = value.toByteArray(); val out = ByteArray(32)
  ...
  private fun ensureKey() {
    val store = keyStore(); if (store.containsAlias(KEY_ALIAS)) return
  ...
  private fun publicKey(): ECPublicKey { ensureKey(); return keyStore().getCertificate(KEY_ALIAS).publicKey as ECPublicKey }
  private fun joseSignature(der: ByteArray): ByteArray {
    ...
    if ((seqLen and 0x80) != 0) { val n = seqLen and 0x7f; seqLen = 0; repeat(n) { seqLen = (seqLen shl 8) or (der[p++].toInt() and 0xff) } }
    fun readInteger(): ByteArray {
      ...
      if ((len and 0x80) != 0) { val n = len and 0x7f; len = 0; repeat(n) { len = (len shl 8) or (der[p++].toInt() and 0xff) } }
      var value = der.copyOfRange(p, p + len); p += len
    ...
    val r = readInteger(); val s = readInteger(); return r + s
  }

  @ReactMethod
  fun createProof(method: String, htu: String, accessToken: String?, promise: Promise) {
    try {
      ensureKey(); val pub = publicKey()
      val jwk = JSONObject().put("kty", "EC").put("crv", "P-256").put("x", b64(unsigned32(pub.w.affineX))).put("y", b64(unsigned32(pub.w.affineY)))
      val header = JSONObject().put("typ", "dpop+jwt").put("alg", "ES256").put("jwk", jwk)
      val payload = JSONObject().put("jti", UUID.randomUUID().toString()).put("htm", method.uppercase()).put("htu", htu).put("iat", System.currentTimeMillis() / 1000L)
      if (!accessToken.isNullOrBlank()) payload.put("ath", b64(MessageDigest.getInstance("SHA-256").digest(accessToken.toByteArray(StandardCharsets.US_ASCII))))
      ...
      val signer = Signature.getInstance("SHA256withECDSA"); signer.initSign(entry.privateKey); signer.update(signingInput.toByteArray(StandardCharsets.US_ASCII))
      promise.resolve(signingInput + "." + b64(joseSignature(signer.sign())))
    } catch (e: Exception) { promise.reject("METRA_DPOP_ERROR", e.message, e) }
  }
}
