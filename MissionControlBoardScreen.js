...
import { creerActionMission, creerReferenceMission, enregistrerDetailsPointMission } from './missionDomainDb.js';
    ...
    try {
      ...
      await db.withTransactionAsync(async () => {
        await db.runAsync(
          ...
          [qualification, status, now, ['closed', 'no_follow_up'].includes(status) ? now : null, row.id, missionId]
        );
        await db.runAsync(
    ...
    <View style={{ flex: 1, backgroundColor: MISSION_COLORS.bg }}>
      <ScrollView contentContainerStyle={{ padding: 16, paddingBottom: 120 }}>
        ...
        <Text style={{ color: COLORS.inkSoft, fontSize: 10.5, lineHeight: 15 }}>
          Référence → contrôle → preuve → écart éventuel → action → recontrôle. METRA aide à structurer le constat sans
          se substituer à un organisme de contrôle ou à une certification réglementaire.
        </Text>
        ...
        {visible.map((row) => (
          <View key={row.id} style={[missionStyles.card, { padding: 11, marginBottom: 8 }]}>
            <View style={{ flexDirection: 'row', alignItems: 'flex-start' }}>
              <View style={{ flex: 1 }}>
                ...
                <Text style={{ color: COLORS.inkFaint, fontSize: 8.3, marginTop: 3 }}>
                  {[
                    ...
                    .filter(Boolean)
                    .join(' · ') || 'Contexte Mission'}
                </Text>
                {row.description ? (
                  <Text style={{ color: COLORS.inkSoft, fontSize: 8.9, lineHeight: 13, marginTop: 4 }}>
                    {row.description}
                  </Text>
                ) : null}
              </View>
              <Text
                style={{
                  color: ['conforme', 'non_applicable'].includes(row.qualification)
            ...
            row.reference_source ? (
              <View style={{ borderRadius: 9, backgroundColor: MISSION_COLORS.accentSoft, padding: 8, marginTop: 7 }}>
                ...
                <Text style={{ color: COLORS.ink, fontSize: 8.9, lineHeight: 13, marginTop: 2 }}>
                  {row.reference_number !== null && row.reference_number !== undefined
                    ? String(row.reference_number) + (row.reference_unit ? ' ' + row.reference_unit : '')
                    : row.reference_text || ''}
                </Text>
                {row.reference_source ? (
            ...
            <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 7, marginTop: 2 }}>
              <TouchableOpacity
                ...
                onPress={() => addProof(row)}
              >
                <Text style={[styles.btnSecondaryText, missionStyles.secondaryButtonText]}>
                  📷 Preuve ({row.photo_count || 0})
                </Text>
              </TouchableOpacity>
              {Number(row.open_action_count || 0) ? (
                <TouchableOpacity
                  style={[styles.btnSecondary, missionStyles.secondaryButton]}
                  onPress={() => navigation.navigate('MissionActions', { missionId, siteId: row.site_id })}
                >
                  <Text style={[styles.btnSecondaryText, missionStyles.secondaryButtonText]}>
                    {row.open_action_count} action(s) ouverte(s)
                  </Text>
                </TouchableOpacity>
              ) : null}
            </View>
      ...
      <Modal visible={createVisible} transparent animationType="fade" onRequestClose={() => setCreateVisible(false)}>
        <View style={styles.modalOverlay}>
          ...
          >
            ...
            <Field
              label="Point contrôlé"
              value={draft.label}
              onChangeText={(v) => setDraft((d) => ({ ...d, label: v }))}
              placeholder="Ex. Repérage organe, dispositif de sécurité, document attendu…"
            />
            <Field
              label="Objet / méthode / constat attendu"
              value={draft.description}
              onChangeText={(v) => setDraft((d) => ({ ...d, description: v }))}
              multiline
            />
            <Field
              label="Référence / exigence"
              value={draft.reference}
              onChangeText={(v) => setDraft((d) => ({ ...d, reference: v }))}
              multiline
              placeholder="Texte, valeur ou exigence à vérifier."
            />
            <Field
              label="Source de la référence"
              value={draft.source}
              onChangeText={(v) => setDraft((d) => ({ ...d, source: v }))}
              placeholder="CCTP, document, texte applicable, consigne…"
            />
            <Field
              label="Action attendue en cas d’écart"
              value={draft.requestedAction}
              onChangeText={(v) => setDraft((d) => ({ ...d, requestedAction: v }))}
            />
            <Text style={{ color: COLORS.inkFaint, fontSize: 8.2, fontWeight: '900', marginBottom: 5 }}>
              SITE (FACULTATIF)
            </Text>
            <View style={{ flexDirection: 'row', flexWrap: 'wrap' }}>
              {sites.map((site) => (
                <Chip
                  ...
                  onPress={() => setDraft((d) => ({ ...d, siteId: d.siteId === site.id ? '' : site.id }))}
                />
              ))}
            </View>
