...
function Chip({ label, selected, onPress }) {
  return (
    ...
    >
      <Text
        style={{ color: selected ? MISSION_COLORS.accentStrong : COLORS.inkSoft, fontSize: 8.6, fontWeight: '900' }}
      >
        {label}
      </Text>
    </TouchableOpacity>
...
function Stat({ value, label }) {
  return (
    <View style={[missionStyles.statBox, { minWidth: '30%', flexGrow: 1, padding: 10, borderRadius: 11 }]}>
...
export function MissionReceptionBoardScreen({ navigation, route }) {
  ...
  const load = useCallback(async () => {
    ...
    const [m, e, t, a, p, d] = await Promise.all([
      ...
      db.getAllAsync('SELECT * FROM mission_actions WHERE mission_id=? ORDER BY created_at DESC', [missionId]),
      db.getAllAsync('SELECT * FROM mission_points WHERE mission_id=? ORDER BY created_at DESC', [missionId]),
      db.getAllAsync('SELECT * FROM mission_expected_documents WHERE mission_id=? ORDER BY created_at DESC', [
        missionId
      ])
    ]);
    setMission(m || null);
  ...
  const summary = useMemo(() => {
    const openReserves = points.filter(
      (row) => row.type === 'reserve' && !['closed', 'no_follow_up', 'cancelled'].includes(row.status)
    );
    ...
    const expectedPending = documents.filter(
      (row) => !['received', 'validated', 'up_to_date', 'not_existing'].includes(row.status)
    );
    return {
      equipment: equipment.length,
      checked: equipment.filter((row) => ['controle', 'receptionne', 'mis_en_service'].includes(row.lifecycle_status))
        .length,
      ...
      inventoryDifferences: equipment.filter((row) =>
        ['different', 'non_retrouve', 'a_verifier'].includes(row.verification_status)
      ).length
    };
  ...
  const setLifecycle = async (row, status) => {
    ...
    setBusyId(row.id);
    try {
      await modifierEquipementMission(row.id, {
        missionId,
        lifecycleStatus: status,
        changeComment: 'Mise à jour depuis tableau réception / passation'
      });
  ...
  const preparePassationChecks = async () => {
    ...
    try {
      const existing = new Set(
        passationChecks.map((row) =>
          String(row.label || '')
            .replace(/^PV Passation · /, '')
            .trim()
            .toLowerCase()
        )
      );
    ...
    try {
      ...
      await db.withTransactionAsync(async () => {
        ...
        await db.runAsync(
          ...
          [createId('mph'), row.id, before, status, 'PV de passation · ' + qualification, 'mission_passation_pv']
        );
      });
      await load();
    } finally {
  ...
  const filteredEquipment = useMemo(() => {
    if (equipmentFilter === 'issues')
      return equipment.filter((row) => ['different', 'non_retrouve', 'a_verifier'].includes(row.verification_status));
    if (equipmentFilter === 'reserved') return equipment.filter((row) => row.lifecycle_status === 'avec_reserve');
    if (equipmentFilter === 'pending')
      return equipment.filter((row) => !['controle', 'receptionne', 'mis_en_service'].includes(row.lifecycle_status));
    return equipment;
    ...
    <View style={{ flex: 1, backgroundColor: MISSION_COLORS.bg }}>
      <ScrollView contentContainerStyle={{ padding: 16, paddingBottom: 120 }}>
        ...
        <Text style={{ color: COLORS.inkSoft, fontSize: 10.5, lineHeight: 15 }}>
          Tableau terrain synthétique : l’état de chaque ouvrage, les essais, les réserves, les actions et les documents
          restent reliés à la même Mission. Aucun inventaire n’est recréé pour cette phase.
        </Text>
        {mission?.type === 'opr_reception' ? (
          <View style={[missionStyles.card, { padding: 11, marginTop: 12 }]}>
            <Text style={{ color: COLORS.inkFaint, fontSize: 8.2, fontWeight: '900', letterSpacing: 0.45 }}>
              MODES OPR · CUMULABLES
            </Text>
            <Text style={{ color: COLORS.inkSoft, fontSize: 8.8, lineHeight: 12, marginTop: 3 }}>
              Une même OPR peut combiner contrôle statique, essais dynamiques et recontrôle de réserves. Aucun dossier
              séparé n’est créé.
            </Text>
            <View style={{ flexDirection: 'row', flexWrap: 'wrap', marginTop: 8 }}>
              ...
              <Chip
                label="Levée / recontrôle"
                selected={activeModes.clearance}
                onPress={() => toggleMode('clearance')}
              />
            </View>
          </View>
        ) : null}
        {mission?.type === 'passation_travaux_exploitant' ? (
          <View style={[missionStyles.card, { padding: 11, marginTop: 12 }]}>
            <Text style={{ color: COLORS.inkFaint, fontSize: 8.2, fontWeight: '900', letterSpacing: 0.45 }}>
              PV DE PASSATION · CONTRADICTOIRE
            </Text>
            <Text style={{ color: COLORS.inkSoft, fontSize: 8.8, lineHeight: 12, marginTop: 3 }}>
              Le PV organise la remise de l’exploitation : documents, stocks/index, moyens d’accès, inventaire, état des
              installations, essais, réserves et annexes.
            </Text>
            <Text style={{ color: COLORS.inkFaint, fontSize: 8.1, lineHeight: 11, marginTop: 5 }}>
              Ce PV ne vaut pas contrôle réglementaire de conformité. Les contrôles réglementaires restent du ressort
              des organismes habilités lorsqu’ils sont requis.
            </Text>
            <TouchableOpacity
            ...
            >
              <Text style={[styles.btnSecondaryText, missionStyles.secondaryButtonText]}>
                {preparingPassation
                  ? 'Préparation…'
                  : passationChecks.length
                    ? 'Compléter / vérifier le PV'
                    : 'Préparer le PV de passation'}
              </Text>
            </TouchableOpacity>
          </View>
        ) : null}
        ...
        <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 7, marginTop: 14 }}>
          {activeModes.dynamic || mission?.type !== 'opr_reception' ? (
            <TouchableOpacity
              ...
              onPress={() => navigation.navigate('MissionTests', { missionId })}
            >
          ...
          {mission?.type === 'opr_reception' && activeModes.clearance && summary.openReserves ? (
            <TouchableOpacity
              style={[styles.btnSecondary, missionStyles.secondaryButton]}
              onPress={() => navigation.navigate('MissionReserveClearance', { missionId })}
            >
          ...
          <TouchableOpacity
            style={[styles.btnSecondary, missionStyles.secondaryButton]}
            onPress={() => navigation.navigate('MissionSignature', { missionId })}
          >
        ...
        {activeModes.static || mission?.type !== 'opr_reception' ? (
          <>
            ...
            <View style={{ flexDirection: 'row', flexWrap: 'wrap', marginBottom: 7 }}>
              <Chip
                label={'Tous · ' + equipment.length}
                selected={equipmentFilter === 'all'}
                onPress={() => {
              ...
              />
              <Chip
                label={'À contrôler · ' + summary.inventoryDifferences}
                selected={equipmentFilter === 'issues'}
                onPress={() => {
                  setEquipmentFilter('issues');
                  setDisplayLimit(120);
                }}
              />
              <Chip
                label={'Avec réserve · ' + summary.reserved}
                selected={equipmentFilter === 'reserved'}
                onPress={() => {
                  setEquipmentFilter('reserved');
                  setDisplayLimit(120);
                }}
              />
              <Chip
                label="Restant à valider"
                selected={equipmentFilter === 'pending'}
                onPress={() => {
                  setEquipmentFilter('pending');
                  setDisplayLimit(120);
                }}
              />
            </View>
            {displayedEquipment.map((row) => (
              <View key={row.id} style={[missionStyles.card, { padding: 11, marginBottom: 8 }]}>
                ...
                >
                  ...
                  <Text style={{ color: COLORS.inkSoft, fontSize: 8.9, marginTop: 3 }}>
                    {[row.brand, row.model].filter(Boolean).join(' · ') || 'Caractéristiques à compléter'}
                  </Text>
                  <Text style={{ color: COLORS.inkFaint, fontSize: 8.3, marginTop: 3 }}>
                    {[row.site_name, row.location_label].filter(Boolean).join(' · ')}
                  </Text>
                </TouchableOpacity>
                <View style={{ flexDirection: 'row', flexWrap: 'wrap', marginTop: 8 }}>
            ...
            {filteredEquipment.length > displayedEquipment.length ? (
              <TouchableOpacity
            ...
            {!equipment.length ? (
              <View style={[missionStyles.card, { padding: 14 }]}>
                <Text style={{ color: COLORS.inkSoft, fontSize: 9.5 }}>
                  Aucun équipement dans cette Mission. L’inventaire peut être préparé sur PC ou créé rapidement sur le
                  terrain.
                </Text>
              </View>
            ) : null}
          ...
          <>
            ...
            <View style={[missionStyles.card, { padding: 11 }]}>
              ...
              <Text style={{ color: COLORS.inkFaint, fontSize: 8.6, lineHeight: 12, marginTop: 3 }}>
                Les protocoles conservent attendu, observé, valeur de référence, mesure, résultat et preuve. Plusieurs
                passages restent possibles sur le même essai.
              </Text>
              <TouchableOpacity
                style={[styles.btnSecondary, missionStyles.secondaryButton, { alignSelf: 'flex-start', marginTop: 8 }]}
                onPress={() => navigation.navigate('MissionTests', { missionId })}
        ...
        {mission?.type === 'opr_reception' && activeModes.clearance ? (
          <>
            ...
            <View style={[missionStyles.card, { padding: 11 }]}>
              <Text style={{ color: COLORS.ink, fontSize: 10, fontWeight: '900' }}>
                {summary.openReserves} réserve(s) ouverte(s)
              </Text>
              <Text style={{ color: COLORS.inkFaint, fontSize: 8.6, lineHeight: 12, marginTop: 3 }}>
                Reprendre la réserve initiale, contrôler, photographier après intervention puis qualifier : levée,
                maintenue, partielle, inaccessible ou non vérifiable.
              </Text>
              <TouchableOpacity
                style={[styles.btnSecondary, missionStyles.secondaryButton, { alignSelf: 'flex-start', marginTop: 8 }]}
                onPress={() => navigation.navigate('MissionReserveClearance', { missionId })}
        ...
        {documents.length ? (
          <>
            ...
            {documents.slice(0, 16).map((row) => (
              <View key={row.id} style={[missionStyles.card, { padding: 10, marginBottom: 7 }]}>
                <Text style={{ color: COLORS.ink, fontSize: 10, fontWeight: '900' }}>
                  {row.label || 'Document attendu'}
                </Text>
            ...
            {documents.length > 16 ? (
              <TouchableOpacity
                style={[styles.btnSecondary, missionStyles.secondaryButton, { alignSelf: 'flex-start' }]}
                onPress={() => navigation.navigate('MissionDocuments', { missionId })}
              >
                <Text style={[styles.btnSecondaryText, missionStyles.secondaryButtonText]}>
