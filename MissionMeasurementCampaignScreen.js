...
export function MissionMeasurementCampaignScreen({ route }) {
  ...
  const comparisonById = useMemo(
    () => new Map((comparison.rows || []).map((row) => [row.id, row])),
    [comparison]
  );

  useEffect(() => {
    setQuickValue(currentPoint?.measured_value !== null && currentPoint?.measured_value !== undefined
  ...
  const importExcel = async () => {
    ...
    try {
      ...
      if (result) {
        await refreshAll();
        Alert.alert('Liste importée', String(result.added || 0) + ' point(s) ajouté(s) depuis ' + (result.sourceName || 'Excel') + '.');
      }
    } catch (e) {
  ...
  return <View style={{ flex: 1, backgroundColor: MISSION_COLORS.bg }}>
    <ScrollView contentContainerStyle={{ padding: 16, paddingBottom: 110 }} keyboardShouldPersistTaps="handled">
      ...
      <Text style={{ color: COLORS.inkSoft, fontSize: 10.5, lineHeight: 15 }}>
        Prépare la liste au bureau, puis sur le terrain : valeur → suivant. Les absents, refus, inaccessibles et points à replanifier restent tracés sans ralentir la campagne.
      </Text>
      ...
      <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 8, marginTop: 12 }}>
        ...
        {selectedCampaign ? <TouchableOpacity style={[styles.btnSecondary, missionStyles.secondaryButton]} onPress={duplicateCampaign}>
          <Text style={[styles.btnSecondaryText, missionStyles.secondaryButtonText]}>Dupliquer · avant / après / J+30</Text>
      ...
      <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{ maxHeight: 105 }}>
        {campaigns.map((campaign) => <TouchableOpacity
          key={campaign.id}
          onPress={() => { setSelectedCampaignId(campaign.id); setCurrentIndex(0); }}
          style={{
            width: 200,
            minHeight: 88,
            marginRight: 8,
      ...
      {selectedCampaign ? <>
        <View style={[missionStyles.card, { padding: 12, marginTop: 14 }]}>
          <View style={{ flexDirection: 'row', alignItems: 'flex-start' }}>
            <View style={{ flex: 1 }}>
              ...
              <Text style={{ color: COLORS.inkSoft, fontSize: 9, marginTop: 4 }}>
                {selectedCampaign.measured_count || 0} mesuré(s) · {selectedCampaign.exception_count || 0} statut(s) terrain · {selectedCampaign.point_count || 0} point(s)
              </Text>
            </View>
          ...
          <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 7, marginTop: 10 }}>
            <TouchableOpacity style={[styles.btnSecondary, missionStyles.secondaryButton]} disabled={busy} onPress={() => addFromStructure('locations')}><Text style={[styles.btnSecondaryText, missionStyles.secondaryButtonText]}>＋ Locaux</Text></TouchableOpacity>
            <TouchableOpacity style={[styles.btnSecondary, missionStyles.secondaryButton]} disabled={busy} onPress={() => addFromStructure('equipment')}><Text style={[styles.btnSecondaryText, missionStyles.secondaryButtonText]}>＋ Équipements</Text></TouchableOpacity>
            <TouchableOpacity style={[styles.btnSecondary, missionStyles.secondaryButton]} disabled={busy} onPress={importExcel}><Text style={[styles.btnSecondaryText, missionStyles.secondaryButtonText]}>⇧ Liste Excel</Text></TouchableOpacity>
            <TouchableOpacity style={[styles.btnSecondary, missionStyles.secondaryButton]} onPress={() => setManualPointVisible(true)}><Text style={[styles.btnSecondaryText, missionStyles.secondaryButtonText]}>＋ Point libre</Text></TouchableOpacity>
          </View>
        </View>
        ...
        {currentPoint ? <View style={[missionStyles.card, { padding: 14, marginTop: 12, borderWidth: 2, borderColor: MISSION_COLORS.accentLineStrong }]}>
          ...
          <Text style={[styles.sectionLabel, missionStyles.sectionLabel, { marginTop: 11 }]}>Ou statut terrain</Text>
          <View style={{ flexDirection: 'row', flexWrap: 'wrap' }}>
            {CAMPAIGN_POINT_STATUSES.filter(([key]) => !['planned','measured'].includes(key)).map(([key,label]) => <Chip key={key} label={label} selected={currentPoint.status === key} onPress={() => recordCurrent(key)} compact />)}
          </View>
          ...
          <Text style={{ color: COLORS.inkSoft, fontSize: 9.7 }}>Aucun point préparé. Ajoute les locaux, équipements, une liste Excel ou un point libre.</Text>
        </View>}

        {points.length ? <>
          <Text style={[styles.sectionLabel, missionStyles.sectionLabel, { marginTop: 16 }]}>Liste de campagne</Text>
          {points.map((point,index) => {
            ...
            return <TouchableOpacity
              ...
              style={[missionStyles.card, { padding: 10, marginBottom: 6, borderColor: index === currentIndex ? MISSION_COLORS.accent : MISSION_COLORS.accentLine }]}
            >
          ...
          <Text style={{ color: COLORS.inkFaint, fontSize: 8.5, lineHeight: 12, marginTop: 2 }}>Appui long sur un point pour le retirer de la campagne.</Text>
        </> : null}
      </> : null}
    </ScrollView>
    ...
    <Modal visible={createVisible} transparent animationType="fade" onRequestClose={() => setCreateVisible(false)}>
      <View style={styles.modalOverlay}><ScrollView style={[styles.modalSheet, missionStyles.modalSheet]} contentContainerStyle={{ paddingBottom: 16 }}>
        ...
        <TextInput style={[styles.input, missionStyles.input]} value={draft.label} onChangeText={(v) => setDraft((p) => ({ ...p, label: v }))} placeholder="Ex. Températures ECS · campagne initiale" />
        <TextInput style={[styles.input, missionStyles.input, { marginTop: 8 }]} value={draft.measureType} onChangeText={(v) => setDraft((p) => ({ ...p, measureType: v }))} placeholder="Type de mesure" />
        <TextInput style={[styles.input, missionStyles.input, { marginTop: 8 }]} value={draft.unit} onChangeText={(v) => setDraft((p) => ({ ...p, unit: v }))} placeholder="Unité" />
        <View style={{ flexDirection: 'row', gap: 8, marginTop: 8 }}>
          <TextInput style={[styles.input, missionStyles.input, { flex: 1 }]} keyboardType="decimal-pad" value={draft.expectedValue} onChangeText={(v) => setDraft((p) => ({ ...p, expectedValue: v }))} placeholder="Valeur attendue" />
          <TextInput style={[styles.input, missionStyles.input, { flex: 1 }]} value={draft.expectedText} onChangeText={(v) => setDraft((p) => ({ ...p, expectedText: v }))} placeholder="Attendu texte" />
        </View>
        <TextInput style={[styles.input, missionStyles.input, { marginTop: 8 }]} value={draft.referenceSourceLabel} onChangeText={(v) => setDraft((p) => ({ ...p, referenceSourceLabel: v }))} placeholder="Source référence : CCTP, consigne exploitant…" />
        <View style={{ flexDirection: 'row', gap: 8, marginTop: 8 }}>
          <TextInput style={[styles.input, missionStyles.input, { flex: 1 }]} keyboardType="decimal-pad" value={draft.toleranceAbs} onChangeText={(v) => setDraft((p) => ({ ...p, toleranceAbs: v }))} placeholder="Tolérance abs." />
          <TextInput style={[styles.input, missionStyles.input, { flex: 1 }]} keyboardType="decimal-pad" value={draft.tolerancePct} onChangeText={(v) => setDraft((p) => ({ ...p, tolerancePct: v }))} placeholder="Tolérance %" />
        </View>
    ...
    <Modal visible={manualPointVisible} transparent animationType="fade" onRequestClose={() => setManualPointVisible(false)}>
      <View style={styles.modalOverlay}><View style={[styles.modalSheet, missionStyles.modalSheet]}>
        ...
        <TextInput style={[styles.input, missionStyles.input]} value={manualPoint.label} onChangeText={(v) => setManualPoint((p) => ({ ...p, label: v }))} placeholder="Ex. Logement 24 · cuisine" autoFocus />
        <Text style={[styles.sectionLabel, missionStyles.sectionLabel, { marginTop: 10 }]}>Type de point</Text>
        <View style={{ flexDirection: 'row', flexWrap: 'wrap' }}>
          {POINT_TYPES.map((type) => <Chip key={type} label={type} selected={manualPoint.pointType === type} onPress={() => setManualPoint((p) => ({ ...p, pointType: type }))} compact />)}
        </View>
        <View style={{ flexDirection: 'row', gap: 8, marginTop: 7 }}>
          <TextInput style={[styles.input, missionStyles.input, { flex: 1 }]} keyboardType="decimal-pad" value={manualPoint.expectedValue} onChangeText={(v) => setManualPoint((p) => ({ ...p, expectedValue: v }))} placeholder="Attendu numérique" />
          <TextInput style={[styles.input, missionStyles.input, { flex: 1 }]} value={manualPoint.expectedText} onChangeText={(v) => setManualPoint((p) => ({ ...p, expectedText: v }))} placeholder="Attendu texte" />
        </View>
