...
function ModuleTile({ item, onPress, accent, light }) {
  ...
  return <TouchableOpacity onPress={() => onPress(item)} style={[card, { width: '100%', minHeight: 116, padding: 13, justifyContent: 'space-between' }]}>
    ...
    <View><Text style={{ color: COLORS.ink, fontFamily: FONTS.black, fontSize: 14 }}>{item.label}</Text><Text style={{ marginTop: 2, color: accent, fontSize: 20, fontFamily: FONTS.black }}>{count}</Text></View>
  </TouchableOpacity>;
}
...
function PhotoPhoneScreen({ onExit }) {
  ...
  if (module && snapshot?.visit) {
    ...
    return <View style={{ flex: 1, backgroundColor: 'transparent' }}><Header title={module.label} subtitle={targets.length + ' élément' + (targets.length > 1 ? 's' : '')} icon={module.icon} onBack={() => { setTargetId(null); setModuleId(null); }} accent={accent} light={light} /><Status text={status} accent={accent} light={light} />
      ...
      {module.id === 'photos' ? <View style={{ padding: 14 }}><TouchableOpacity disabled={Boolean(busy)} onPressIn={() => prewarmCameraRuntime().catch(() => {})} onPress={() => capture({ currentModule: module, label: 'Photo générale' })} style={[card, { minHeight: 150, backgroundColor: light, borderColor: accent, alignItems: 'center', justifyContent: 'center', gap: 9 }]}>{busy ? <ActivityIndicator color={accent} /> : <CvcIcon name="camera" size={48} color={accent} />}<Text style={{ color: accent, fontFamily: FONTS.black }}>Capture libre</Text></TouchableOpacity></View> : <FlatList data={targets} keyExtractor={(item) => String(item.id)} contentContainerStyle={{ padding: 14, paddingBottom: 34 }} renderItem={({ item }) => <TargetRow item={item} module={module} onOpen={(row) => setTargetId(row.id)} onCapture={(row) => capture({ currentModule: module, currentTarget: row })} busy={busy === module.id + ':' + item.id + ':shot'} accent={accent} light={light} />} ListEmptyComponent={module.id === 'remarks' ? null : <View style={{ marginTop: 60, alignItems: 'center' }}><CvcIcon name={module.icon} size={50} color={accent} /><Text style={{ marginTop: 12, color: COLORS.ink, fontFamily: FONTS.black }}>Aucun élément</Text></View>} />}
    </View>;
  }

  if (snapshot?.visit) return <View style={{ flex: 1, backgroundColor: 'transparent' }}><Header title="Mode Photo" subtitle={[snapshot.visit.site, snapshot.visit.date].filter(Boolean).join(' · ')} onBack={() => { setSnapshot(null); setStatus(''); }} onExit={onExit} accent={accent} light={light} /><Status text={status} accent={accent} light={light} /><ScrollView contentContainerStyle={{ padding: 14, paddingBottom: 38 }}><View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 9 }}>{modules.map((m, i) => <FadeUp key={m.id} delay={i * 40} style={{ width: '48.5%' }}><ModuleTile item={m} onPress={(row) => { setModuleId(row.id); setTargetId(null); setStatus(''); }} accent={accent} light={light} /></FadeUp>)}</View></ScrollView></View>;
