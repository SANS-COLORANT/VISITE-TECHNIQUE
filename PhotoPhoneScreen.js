...
function Status({ text, accent, light }) {
  if (!text) return null;
  return <View style={{ alignSelf: 'center', marginTop: 8, maxWidth: '92%', paddingHorizontal: 11, paddingVertical: 6, borderRadius: 999, backgroundColor: light }}><Text style={{ color: accent, fontSize: 11, fontWeight: '900', textAlign: 'center' }}>{text}</Text></View>;
}
...
function Field({ field, onSave, saving, accent, light }) {
  ...
  if (Array.isArray(field?.options) && field.options.length) return <View style={{ marginBottom: 12 }}><Text style={{ marginBottom: 6, color: COLORS.inkSoft, fontSize: 11.5, fontWeight: '900' }}>{field.label}</Text><View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 7 }}>{field.options.map((option) => { const selected = String(field.value ?? '') === String(option); return <TouchableOpacity key={String(option)} disabled={saving} onPress={() => onSave(field, String(option))} style={{ minWidth: 44, minHeight: 40, paddingHorizontal: 9, borderRadius: 11, borderWidth: 1, borderColor: selected ? accent : COLORS.line, backgroundColor: selected ? light : COLORS.white, alignItems: 'center', justifyContent: 'center' }}><Text style={{ color: selected ? accent : COLORS.ink, fontWeight: '900' }}>{option}</Text></TouchableOpacity>; })}</View></View>;
  const save = () => { if (value !== String(field.value ?? '')) onSave(field, value); };
  return <View style={{ marginBottom: 12 }}><Text style={{ marginBottom: 6, color: COLORS.inkSoft, fontSize: 11.5, fontWeight: '900' }}>{field.label}{field.unit ? ' · ' + field.unit : ''}</Text><TextInput value={value} onChangeText={setValue} onBlur={save} onSubmitEditing={() => { if (!field.multiline) save(); }} keyboardType={field.input === 'numeric' ? 'decimal-pad' : 'default'} multiline={Boolean(field.multiline)} placeholder="Saisir…" style={{ minHeight: field.multiline ? 78 : 46, borderRadius: 12, borderWidth: 1, borderColor: COLORS.line, paddingHorizontal: 11, paddingVertical: field.multiline ? 9 : 0, textAlignVertical: field.multiline ? 'top' : 'center', color: COLORS.ink }} /></View>;
}
...
function RemarkEditor({ remark, onClose, onChanged, accent, light }) {
  ...
  return <View style={{ flex: 1, backgroundColor: COLORS.bg }}><Header title="Remarque" subtitle="Enregistrement automatique" icon="remark" onBack={onClose} accent={accent} light={light} /><ScrollView contentContainerStyle={{ padding: 14 }} keyboardShouldPersistTaps="handled"><View style={[card, { padding: 14 }]}><View style={{ flexDirection: 'row', gap: 8 }}><TextInput autoFocus multiline value={value} onChangeText={setValue} onBlur={() => save(value)} placeholder="Décrire la remarque…" style={{ flex: 1, minHeight: 112, borderWidth: 1, borderColor: COLORS.line, borderRadius: 12, padding: 10, textAlignVertical: 'top', color: COLORS.ink }} /><TouchableOpacity accessibilityLabel="Dicter" onPress={dictate} style={[iconBox(light, 52), { alignSelf: 'stretch', height: 'auto' }]}>{dictating ? <ActivityIndicator color={accent} /> : <CvcIcon name="microphone" size={27} color={accent} />}</TouchableOpacity></View><Text style={{ marginTop: 17, marginBottom: 7, fontWeight: '900', color: COLORS.ink }}>Criticité</Text><View style={{ flexDirection: 'row', gap: 6 }}>{[1,2,3,4,5].map((n) => <TouchableOpacity key={n} onPress={() => changeSeverity(n)} style={{ flex: 1, minHeight: 42, borderRadius: 11, borderWidth: 1, borderColor: severity === n ? accent : COLORS.line, backgroundColor: severity === n ? light : COLORS.white, alignItems: 'center', justifyContent: 'center' }}><Text style={{ color: severity === n ? accent : COLORS.ink, fontWeight: '900' }}>{n}</Text></TouchableOpacity>)}</View></View></ScrollView></View>;
}
...
function PhotoPhoneScreen({ onExit }) {
  ...
  useEffect(() => { prewarmCameraRuntime().catch(() => {}); loadVisits().catch(() => {}); }, [loadVisits]);
  ...
  const capture = useCallback(async ({ currentModule, currentTarget = null, plaque = false, label = null } = {}) => {
    ...
    try {
      ...
      if ((currentModule?.id === 'meters' || currentModule?.id === 'temperatures') && ocr?.text) {
        ...
        if (found && field?.edit) { setSnapshot(await applyCompanionTargetUpdate({ visiteId: snapshot.visit.id, edit: field.edit, value: found.value })); setStatus('✓ ' + found.value + (field.unit || currentTarget?.unit ? ' ' + (field.unit || currentTarget.unit) : '')); return; }
        setTargetId(currentTarget?.id || null); setStatus('Photo enregistrée · valeur à vérifier'); await refresh(); return;
      }
      if (plaque && ocr?.text && currentTarget) {
        ...
        setSnapshot(next); setStatus(count ? '✓ Plaque lue · ' + count + ' donnée' + (count > 1 ? 's' : '') : 'Plaque enregistrée'); return;
      }
      await refresh(); setStatus('✓ Photo enregistrée');
