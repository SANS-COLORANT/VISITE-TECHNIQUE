...
const SENSITIVE_KEY =
  /(access[_-]?token|refresh[_-]?token|token[_-]?type|password|mot[_-]?de[_-]?passe|secret|authorization|cookie|private[_-]?key|dpop[_-]?(key|private)|client[_-]?secret|jwt)/i;
...
function parseJsonColumn(value) {
  ...
  try {
    return sanitizeDeep(JSON.parse(String(value)));
  } catch {
    return { parseError: true, raw: sanitizeDeep(String(value)) };
  }
}
...
async function safeRows(db, sql, params = []) {
  try {
    return { rows: sanitizeRows(await db.getAllAsync(sql, params)), error: null };
  } catch (error) {
...
async function linkedVisitData(db) {
  const visitsResult = await safeRows(
    db,
    `SELECT v.id,v.site_id,v.installation_id,v.date_visite,v.statut,v.trame_id,
  ...
  const [controls, fields, networks, counters, remarks, materials, provenance] = await Promise.all([
    safeRows(
      db,
      `SELECT visite_id,section_code,cle,avis,commentaire,modifie_le FROM controles_visite WHERE visite_id IN (${placeholders}) ORDER BY visite_id,section_code,cle`,
      ids
    ),
    safeRows(
      db,
      `SELECT visite_id,section_code,cle,valeur FROM champs_visite WHERE visite_id IN (${placeholders}) ORDER BY visite_id,section_code,cle`,
      ids
    ),
    safeRows(
      db,
      `SELECT id,visite_id,ordre,nom_reseau,t_ext_c,t_dep_c,courbe_de_chauffe,tnc,consigne_programme_horaire FROM reseaux WHERE visite_id IN (${placeholders}) ORDER BY visite_id,ordre,id`,
      ids
    ),
    safeRows(
      db,
      `SELECT id,visite_id,label,unite,valeur FROM compteurs WHERE visite_id IN (${placeholders}) ORDER BY visite_id,id`,
      ids
    ),
    safeRows(
      db,
      `SELECT id,visite_id,poste,prestation,delai,estimatif,origine,intranet_date_reserve,intranet_delai,intranet_etat_avancement,cree_le FROM remarques WHERE visite_id IN (${placeholders}) ORDER BY visite_id,cree_le,id`,
      ids
    ),
    safeRows(
      db,
      `SELECT id,visite_id,equipement_id,categorie,nombre,designation,numero_materiel,reseau_desservi,marque,modele,caracteristiques,annee,etat,cree_le FROM materiel WHERE visite_id IN (${placeholders}) ORDER BY visite_id,cree_le,id`,
      ids
    ),
    safeRows(
      db,
      `SELECT id,entite_type,entite_id,origine,reference_externe,details_json,importe_le FROM provenances WHERE origine='api_symfony' AND ((entite_type='visite' AND entite_id IN (${placeholders})) OR entite_type<>'visite') ORDER BY importe_le DESC LIMIT 1000`,
      ids
    )
  ]);
  return {
...
export async function construireSupportDump() {
  ...
  const [integrity, foreignKeys, syncState, clients, clientSites, sites, locals, preparations, outbox, linked] =
    await Promise.all([
      safeRows(db, 'PRAGMA integrity_check'),
      safeRows(db, 'PRAGMA foreign_key_check'),
      safeRows(
        db,
        'SELECT id,base_url,tablette_id,last_clients_sync_at,last_success_at,last_error,modifie_le FROM api_sync_state'
      ),
      safeRows(
        db,
        'SELECT remote_client_id,local_client_id,nom,categorie,code_everwin,ville,agence_id,agence_libelle,autorise,cree_localement,synced_at FROM api_client_links ORDER BY synced_at DESC'
      ),
      safeRows(
        db,
        'SELECT remote_client_id,remote_site_id,local_site_id,remote_present FROM api_client_site_links ORDER BY remote_client_id,remote_site_id'
      ),
      safeRows(
        db,
        'SELECT remote_site_id,remote_client_id,local_site_id,nom,cree_localement,remote_present,synced_at FROM api_site_links ORDER BY synced_at DESC'
      ),
      safeRows(
        db,
        'SELECT remote_local_id,remote_site_id,local_installation_id,designation,remote_trame_id,remote_trame_nom,derniere_visite_id,derniere_visite_date,derniere_visite_statut,remote_present,criteria_count,historical_criteria_count,remark_count,material_count,reference_json,synced_at FROM api_local_links ORDER BY synced_at DESC'
      ),
      safeRows(
        db,
        'SELECT remote_client_id,payload_json,synced_at FROM api_preparation_cache ORDER BY synced_at DESC LIMIT ?',
        [MAX_PREPARATIONS]
      ),
      safeRows(
        db,
        'SELECT envoi_id,visite_id,remote_client_id,payload_json,payload_bytes,status,attempt_count,last_attempt_at,next_attempt_at,http_status,error_code,error_message,violations_json,remote_visit_id,replayed,queued_at,synced_at,updated_at FROM api_visit_outbox ORDER BY updated_at DESC LIMIT ?',
        [MAX_OUTBOX_ROWS]
      ),
      linkedVisitData(db)
    ]);
  ...
  return sanitizeDeep({
    ...
    privacy: {
      ...
      redaction:
        'Les clés et valeurs ressemblant à des jetons, secrets, mots de passe, cookies, clés privées ou en-têtes Authorization sont masquées automatiquement.'
    },
