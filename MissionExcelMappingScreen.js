  ...
  return <View style={{ flex: 1, backgroundColor: 'transparent' }}>
    <ScrollView contentContainerStyle={{ padding: 16, paddingBottom: 110 }}>
      ...
      {sheet ? <View style={[missionStyles.card, { padding: 12, marginTop: 9 }]}>
        <Text style={{ color: MISSION_COLORS.accentStrong, fontSize: 11.5, fontFamily: FONTS.black }}>{sheet.sheetName}</Text>
        ...
        <View style={{ marginTop: 12 }}>
          {(TARGETS[entityType] || []).map(([key, label]) => <View key={key} style={{ flexDirection: 'row', alignItems: 'center', borderBottomWidth: 1, borderBottomColor: MISSION_COLORS.accentLine, paddingVertical: 8 }}>
            <View style={{ flex: 1 }}>
              <Text style={{ color: COLORS.ink, fontSize: 10.2, fontFamily: FONTS.bold }}>{label}</Text>
        ...
        <Text style={{ color: COLORS.inkFaint, fontSize: 8.8, fontFamily: FONTS.bold, marginTop: 12 }}>APERÇU SOURCE</Text>
      ...
      {mappings.length ? <>
        ...
        {mappings.map((m) => <TouchableOpacity key={m.id} onPress={() => reuseMapping(m)} style={[missionStyles.card, { padding: 10, marginBottom: 6 }]}>
          <Text style={{ color: COLORS.ink, fontSize: 10.5, fontFamily: FONTS.black }}>{m.name}</Text>
      ...
      {issues.length ? <>
        ...
        {issues.map((i) => <View key={i.id} style={[missionStyles.card, { padding: 10, marginBottom: 6 }]}>
          <Text style={{ color: i.severity === 'warning' ? '#8A5B14' : MISSION_COLORS.accentStrong, fontSize: 10, fontFamily: FONTS.black }}>{i.message}</Text>
