  ...
  useEffect(() => {
    (async () => {
      ...
      setSheetName((current) =>
        current && s.some((x) => x.sheetName === current) ? current : s?.[0]?.sheetName || null
      );
    })();
  }, [batchId]);
    ...
    <View style={{ flex: 1, backgroundColor: MISSION_COLORS.bg }}>
      <ScrollView contentContainerStyle={{ padding: 16, paddingBottom: 110 }}>
        ...
        <Text style={{ color: COLORS.inkSoft, fontSize: 10.5, lineHeight: 15 }}>
          METRA conserve d’abord le classeur brut, puis transforme les colonnes choisies. Les lignes originales et
          formules ne sont jamais perdues.
        </Text>
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false}>
          {(imports.batches || []).map((b) => (
            <Chip
              key={b.id}
              label={b.source_name || b.source_type || 'Import'}
        ...
        {batch ? (
          <Text style={{ color: COLORS.inkFaint, fontSize: 8.8, marginTop: 4 }}>
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false}>
          {structure.map((s) => (
            <Chip
              key={s.sheetName}
              label={s.sheetName}
              selected={sheetName === s.sheetName}
              onPress={() => setSheetName(s.sheetName)}
            />
          ))}
        </ScrollView>
        ...
        <View style={{ flexDirection: 'row', flexWrap: 'wrap' }}>
          {Object.keys(TARGETS).map((key) => (
            <Chip
              ...
              selected={entityType === key}
              onPress={() => setEntityType(key)}
            />
          ))}
        </View>

        {sheet ? (
          <View style={[missionStyles.card, { padding: 12, marginTop: 9 }]}>
            ...
            <Text style={{ color: COLORS.inkFaint, fontSize: 8.7, marginTop: 2 }}>
              {sheet.columns.length} colonne(s) détectée(s) · aperçu des {sheet.rows.length} première(s) ligne(s)
            </Text>
            ...
            <View style={{ marginTop: 12 }}>
              {(TARGETS[entityType] || []).map(([key, label]) => (
                <View
                  key={key}
                  style={{
                    flexDirection: 'row',
                ...
                >
                  <View style={{ flex: 1 }}>
                    ...
                    <Text
                      style={{
                        color: fieldMap[key] ? MISSION_COLORS.accentDark : COLORS.inkFaint,
                        fontSize: 8.8,
                        marginTop: 2
                      }}
                    >
                      {fieldMap[key] || 'Non associé'}
                    </Text>
                  </View>
                  <TouchableOpacity
                    onPress={() => setColumnPicker({ key, label })}
            ...
            <Text style={{ color: COLORS.inkFaint, fontSize: 8.8, fontWeight: '800', marginTop: 12 }}>
              APERÇU SOURCE
            </Text>
            {(sheet.rows || []).slice(0, 3).map((row) => (
              <View
                key={row.id}
                style={{ backgroundColor: MISSION_COLORS.accentSoft, borderRadius: 9, padding: 8, marginTop: 6 }}
              >
                <Text style={{ color: COLORS.inkFaint, fontSize: 8 }}>Ligne {row.row_index}</Text>
                <Text style={{ color: COLORS.inkSoft, fontSize: 8.8, marginTop: 2 }} numberOfLines={4}>
            ...
            <TouchableOpacity
              style={[styles.btnPrimary, missionStyles.primaryButton, { marginTop: 14, alignItems: 'center' }]}
              disabled={busy}
              onPress={saveAndApply}
            >
              <Text style={[styles.btnPrimaryText, missionStyles.primaryButtonText]}>
                {busy ? 'Application…' : 'Enregistrer le mapping & appliquer'}
              </Text>
            </TouchableOpacity>
          </View>
        ) : null}
        ...
        {mappings.length ? (
          <>
            ...
            {mappings.map((m) => (
              <TouchableOpacity
                key={m.id}
                onPress={() => reuseMapping(m)}
                style={[missionStyles.card, { padding: 10, marginBottom: 6 }]}
              >
                ...
                <Text style={{ color: COLORS.inkFaint, fontSize: 8.5, marginTop: 2 }}>
                  {m.source_signature || 'Mapping Mission'}
                </Text>
              </TouchableOpacity>
            ))}
          </>
        ...
        {issues.length ? (
          <>
            <Text style={[styles.sectionLabel, missionStyles.sectionLabel, { marginTop: 18 }]}>
              Points à vérifier dans l’import
            </Text>
            {issues.map((i) => (
              <View key={i.id} style={[missionStyles.card, { padding: 10, marginBottom: 6 }]}>
                <Text
                  style={{
                    color: i.severity === 'warning' ? '#8A5B14' : MISSION_COLORS.accentStrong,
                    fontSize: 10,
                    fontWeight: '900'
                  }}
                >
                ...
                <Text style={{ color: COLORS.inkFaint, fontSize: 8.5, marginTop: 2 }}>
                  {[i.source_ref, i.suggestion].filter(Boolean).join(' · ')}
                </Text>
              </View>
            ))}
          </>
      ...
      <Modal visible={!!columnPicker} transparent animationType="fade" onRequestClose={() => setColumnPicker(null)}>
        <View style={styles.modalOverlay}>
          <View style={[styles.modalSheet, missionStyles.modalSheet]}>
            ...
            <Text style={{ color: COLORS.inkSoft, fontSize: 9.5, marginBottom: 9 }}>
              Choisis la colonne Excel qui alimente ce champ METRA.
            </Text>
            <ScrollView style={{ maxHeight: 360 }}>
              ...
              {(sheet?.columns || []).map((column) => (
                <TouchableOpacity
                  key={column}
                  onPress={() => {
                    setFieldMap((m) => ({ ...m, [columnPicker.key]: column }));
                    setColumnPicker(null);
                  }}
                  style={{ paddingVertical: 9, borderBottomWidth: 1, borderBottomColor: MISSION_COLORS.accentLine }}
                >
                  <Text
                    style={{
                      color: fieldMap[columnPicker?.key] === column ? MISSION_COLORS.accentStrong : COLORS.ink,
                      fontSize: 10.5,
                      fontWeight: fieldMap[columnPicker?.key] === column ? '900' : '600'
                    }}
                  >
                    {column}
                  </Text>
